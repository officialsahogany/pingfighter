#!/usr/bin/env python3
"""
업데이트된 블루 메탈릭 라운드 점수 전광판 미리보기 생성
- 보스 이름 표시 (스테이지별)
- 20% 감소된 텍스트 사이즈
- 블루 메탈릭 프레임
"""
...
def create_updated_scoreboard_image(player_score, ai_score, stage, filename):
    """업데이트된 블루 메탈릭 점수판 이미지 생성"""
    pygame.init()
    ...
    # 20% 감소된 폰트 사이즈
    font_large = pygame.font.Font(None, 48)   # 60 → 48 (20% 감소)
    font_medium = pygame.font.Font(None, 29)  # 36 → 29 (20% 감소)
    font_small = pygame.font.Font(None, 19)   # 24 → 19 (20% 감소)
    ...
    title = title_font.render(f"Blue Metallic Scoreboard - Stage {stage}: {boss_name} - Score: {player_score} vs {ai_score}", True, (180, 180, 200))
...
def main():
    # 다양한 스테이지와 점수 상황 저장
    test_cases = [
        (1, 2, 1, "updated_scoreboard_stage1.png"),   # 스테이지1: 풍악보이
        (2, 1, 2, "updated_scoreboard_stage2.png"),   # 스테이지2: 악어장군
        (0, 0, 3, "updated_scoreboard_stage3.png"),   # 스테이지3: 멘헤라걸
        (2, 2, 5, "updated_scoreboard_stage5.png"),   # 스테이지5: 네메시스
    ]

    output_dir = "/tmp/scoreboard_previews"
    os.makedirs(output_dir, exist_ok=True)

    for p_score, ai_score, stage, filename in test_cases:
        filepath = os.path.join(output_dir, filename)
        create_updated_scoreboard_image(p_score, ai_score, stage, filepath)
