...
앵커 재정렬) + CB4c-3 랜딩(용융/열 아지랑이 — WritheEmberMaterial 공유
패밀리 프리셋 2종(기본 골드/부작용 적 서지), B3 크레스트 사인 엔벨로프
+B4 초입 잔광, 점등 섀시 아트 자체 왜곡 additive 쉬머) + CB4c-4 랜딩
(§9 전이 SFX 4종 — CHNK 래치/POST 램프/이그니션 THUNK/각성 팡파르,
결정론 합성 원샷·배치=도착점 소리만·팡파르=deployed>0 게이트). 콜드부트
런타임 슬라이스 완결 — 잔여는 §9 밸런스(비트 타이밍 라이브 튜닝·POST
리드아웃 7로케일).
