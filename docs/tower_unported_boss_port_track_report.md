...
## 두더지왕 파리티 수정 결정 (2026-08-19)

- 시간 권한: 같은 Stage 2 형제 보스와 일치하도록 기존 `/60.0` 환산을 유지한다.
  72 Hz 틱 수가 아니라 원본 60 Hz 런타임의 벽시계 체감이 정본이다.
- 오디오 원본 배선: 순차 가시는 저장소에 이미 있는 `odinspirit.wav`, 친구두더지
  등장은 `bonemake.wav`, 피격은 `smallboyhit.wav`를 사용한다.
- 승인된 오디오 이탈: 저장소에 없는 `lurker_attack.wav`는 `cry.wav`,
  `rocking.wav`는 `stonebreak2.wav`, `clue.wav`는 `defense_hit.wav` 대체를
  유지한다. 신규 오디오 파일은 만들지 않는다.
- 결과 화면: 원본 절차형 두더지왕에는 전용 승패 자산이 없으므로 신규 아트 대신
  중립 절차형 표현을 사용하며, 청린귀 결과 자산은 노출하지 않는다.
