...
`docs/viper_wall_leap_raid_slice_plan.md`의 **현재 리비전(rev8)** 이 우선한다.
...
`docs/viper_wall_leap_raid_slice_plan.md` (**rev8**)

**착수 전 전문을 읽어라.** §2.2(강제귀환·관측점) · §4(런타임 계약) · §5(입력) ·
§6(배선표) · §7(씰) · §10(참격 연출) · **§11(폭발 VFX 화약 재작업)** 이 계약이다.
이 문서는 요약이 아니라 진입점이다.
...
## 8. 추가 작업 — 폭발 VFX 화약 계열 재작업 (rev8, 2026-08-01)

rev7 폭발 VFX가 라이브에서 "허접하다" 판정. **정본은
`docs/viper_wall_leap_raid_slice_plan.md` rev8 §11**이다. 전문을 읽고 시작하라.

### 8.1. 핵심 — 새 아트를 생성하지 마라

이 저장소엔 이미 화약 폭발 시스템이 있다. **재사용이 정답이다.**

- `godot/scripts/effects/grenade_explosion_drawer.gd`
  `static func draw_zone(canvas, zone: Dictionary, shake_offset)` — 어디서든 호출 가능
- 기본 레이어가 이미 화약이다: 화염 링 5겹(주황→적→갈색) + 회갈색 연기 퍼프 +
  스파크 + 과압 플래시 + 충격파 아크
- **스타일 = `explosion_style: "jinroe_tan"`** — 진뢰탄(震雷彈) 조선 화약 병기.
  환격전 어휘와 일치하고 `jinroe_tan_impact_stamp_imagegen_v1.png` 실제 아트가 있다
- `airstrike` 스타일은 쓰지 마라 — 버섯구름 + 잔해 청크라 ±50px 스킬엔 과하다

프리웜은 `GrenadeExplosionDrawer.prewarm_assets()` 정적 호출로 대체하고,
wall-leap 전용 프리웜 목록에서 폭발용 항목을 뺀다.

### 8.2. 제거 / 존치

**제거:** `BLAST_RAY_COUNT = 12` 방사 `draw_line` 전량(별표로 읽히는 직접 원인),
흰 코어 `draw_circle`, 시안·자색 이중 쇼크웨이브(검기와 색 충돌).

**존치:** FUSE 텔레그래프(0.7초 예고 기능 + 색 대비), 그리고 드로어 위에 얹는
**얇은 세린 청록 액센트 1겹**(낮은 알파). 액센트가 주역이 되면 "수류탄과 구분 안 됨"과
"별표 재발" 사이에서 다시 실패한다.
