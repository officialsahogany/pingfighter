...
`docs/viper_wall_leap_raid_slice_plan.md`의 **현재 리비전(rev10)** 이 우선한다.
...
`docs/viper_wall_leap_raid_slice_plan.md` (**rev10**)

**착수 전 전문을 읽어라.** §2.2(강제귀환·관측점) · §4(런타임 계약) · §5(입력) ·
§6(배선표) · §7(씰) · §10(참격 연출) · §12(폭발 VFX) ·
**§13(도화선 가독성 마무리)** 이 계약이다.
이 문서는 요약이 아니라 진입점이다.
...
| §8 | 폭발 VFX 세린 각색판(rev9) | 완료 `df5e6aef6` — 폭발 본체 **라이브 합격** |
| **§9** | **도화선 가독성 마무리(rev10)** | **착수 대상** |
...
## 9. 추가 작업 — 도화선 가독성 마무리 (rev10, 2026-08-01)

**정본은 `docs/viper_wall_leap_raid_slice_plan.md` rev10 §13**이다. 전문을 읽고 시작하라.

rev9 폭발 본체는 **라이브 캡처로 합격**했다. **건드리지 마라.** 이번 범위는 셋뿐이다.

### 9.1. 범위

| # | 항목 | 요지 |
|---|---|---|
| ① | 4번째 피스 = **화약통 텍스처** | 밀조 화약 주머니 / 비대칭 대나무 화통, 먹빛 흑청 + 주황 봉인, **MIX** |
| ② | **심지 + 불똥 가독성** | 남은 길이=남은 시간 계약 유지, 발광 강화, 불똥 확대 |
| ③ | **후반 자주 잔불** | 칼날 파편 수명을 폭연보다 길게 |

폭연·파편·링 합성은 **유지**한다.

### 9.2. ① 화약통

`serin_blast_powder_charge_imagegen_v1.png`, 기존 3피스와 같은 디렉터리.
프롬프트 전문은 §13.3. 생성 배경은 기존과 동일하게 초록 단색 → 크로마키·디스필.

⚠️**MIX 합성이다** — 빛이 아니라 실체다. ADD로 깔면 먹빛 몸체가 사라진다.
⚠️금지: 둥근 만화 폭탄, 빈 외곽선, 방사 대칭.

### 9.3. ② 심지 + 불똥

- 심지는 절차적 유지. **"남은 길이 = 남은 시간" 계약을 깨지 마라** — 정보 전달은
  이미 옳다. 외곽선·내부 발광만 굵고 밝게
- 불똥 = **백열 코어 + 주황 글로우 + 후행 불티 2~4개**
- 불똥이 **심지의 연소 선단을 정확히 추적**해야 한다. 어긋나면 "타들어간다"가 깨진다

### 9.4. ③ 후반 자주 잔불

파편 자산엔 자주 불티가 이미 있는데 초반 주황 화염에 씻겨 안 보인다.
**강도를 올리지 말고 수명으로 풀어라** — 칼날 파편층이 폭연보다 오래 남아
화염이 죽은 후반부에 자주 불티와 금속 파편이 읽히게 한다.

### 9.5. 배선 · 산출물

- 전용 자산 **프리웜 · 플레이필드 클립 · 라운드 종료 정리** (기존 FX 호스트 경로 재사용)
- **실제 창 렌더 캡처** 포함

### 9.6. 씰 — 3항목 각각 + 단일 계약 토글 RED

| 검사 | 단언 |
|---|---|
| 화약통 텍스처 누락 | 조용한 빈 화면이 되지 않음(가시 degradation 또는 명시 실패) |
| 불똥 가독성 | 불똥 영역 밝은 픽셀 카운트 ≥ 임계 |
| 후반 자주 픽셀 | 폭발 **후반 프레임**에서 자주 계열 픽셀 > 0 |

⚠️**강한 임계로 판정하라** — 약한 임계는 버그와 정상이 같은 값을 내 변별력 0이다.
⚠️**`SubViewport` 기본 클리어색은 검정이 아니다** — 명시 설정하지 않으면 대조군
오탐이 대량 발생한다(풍운천선무 570,000px 사례).

§2~§4 규율과 완료 게이트는 그대로 적용된다. 기존 씰 15종 전부 GREEN 유지.
