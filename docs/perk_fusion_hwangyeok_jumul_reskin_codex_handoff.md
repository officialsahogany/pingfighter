...
⚠️ **목업 원본 파일 선행 조건**: 목업은 채팅으로 전달되어 Codex가 직접 볼 수
없다. 착수 전 `images/perk_fusion_hwangyeok_jumul_reskin/reference/
fusion_confirm_mockup_v1.png`로 저장해 둘 것. 아래 §11.2/§11.3 표는 코드 실측
+ 목업 육안 추정이므로, **원본 파일이 확보되면 목업 측 수치를 재실측해
§11.3 목표값을 갱신**한다(코드 산출 As-built 표는 정확값이라 갱신 불필요).
...
1. **패널이 사실상 정사각 고정.** `PerkFusionModalLayout.PANEL_MAX_SIZE =
   ...
   744x714 이상이면 패널은 **항상 정확히 720x690** (비율 1.043). 목업 비율
   ≈1.04 → **오차 0.3%**. 따라서 배경 아트는 9-slice 불필요, `panel_rect`에
   `draw_texture_rect` 1콜 스트레이트 매핑으로 왜곡 없이 들어간다.
...
| `pair_height` | `min(166, gy*0.54)` | `min(170, gy*0.55)` | 170 |
...
| 티어 라벨 y | 380 | 0.551 | ~0.565 ✓ |
| 메달리온 중심 y / 지름 | 443 / 110 | 0.642 / 0.153(w) | ~0.635 ✓ |
| 퍼센트 y | 528 | 0.765 | ~0.71 |
...
텍스처→퍼센트)** 으로. 메달리온은 열 중심 x(0.187/0.500/0.813)에 정사각
110px 드로.
...
1. **`fusion_modal_backdrop.png`** — 1440x1380 소스(런타임 720x690 x2).
   ...
   **⚠️ 텍스트·숫자 일체 금지** — 제목/부제/퍽 이름/퍼센트/버튼 라벨은 전부
   런타임 드로다. 세로 한자 컬럼은 §11.6-1 규율 하에서만.
