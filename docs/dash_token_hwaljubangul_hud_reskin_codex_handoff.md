...
- 2026-07-28 실게임 후속 검토에서 기력 v3 링은 중앙 가로선 두께가 약
  `35px`, 활주 v3는 약 `21px`로 기력이 67%가량 두껍게 읽혔다. 따라서
  `GAUGE_ORB_FRAME_TEXTURE_PATH`도 `DASH_TOKEN_FRAME_TEXTURE_PATH`를 가리켜
  두 HUD가 동일한 개수 중립 링을 공유한다.
- 공유 링의 중앙 홀은 `154x157px`이므로 기력의 배경·액체·유리 반경도
  활주와 같은 `1.15x` 콘텐츠 반경으로 확장한다. 기준 반경 `55px`에서는
  콘텐츠 반경 `63.25px`가 되어 구슬-테두리 빈 공간 없이 같은 두께로 맞는다.
  기력/활주의 구분은 각각 상단 청옥과 방울, 그리고 내부 액체 팔레트가 맡는다.
- 검증 씰 `gauge_orb_hwangyeok_frame_smoke.gd`는 경로·임포트·알파 홀·bbox,
  불투명 마젠타 및 고정 청람 옥 부재, 공유 링·콘텐츠 반경, 실전 씬 컨텍스트
  전달, 코어 프리웜 스펙, 프레임 최종 전경 드로우 순서를 고정한다.
...
- 실전 소유 체인에서는 `stage1_pillar_hud_scene_drawer.gd`가 반드시
  `gauge_orb_ki_jade_ornament_texture`를 `ui_context`에 복사해야 한다. 이
  중간 전달이 없으면 리소스 로드와 직접 렌더 스모크가 성공해도 실게임에서는
  장식이 `null`로 누락된다.

## 12. 초식 HUD 황역전 통일 (2026-07-28)

- 초식 소켓은 `skill_orb_frame_imagegen_v2.png`를 사용한다. 기준 런타임
  소켓은 `64x64px`, 소스는 `256x256`, `alpha > 16` 외곽 bbox
  `223x228`, 중앙 투명 홀 `173x176`이다. 옻칠 흑갈·놋쇠·적색/심청색
  단청 링에 12시 방향 매화 봉인 하나만 두며 방울·청옥·고정 슬롯 노드는
  사용하지 않는다.
- 5초식 합성 프레임은 기존 좌표를 그대로 보존한
  `player_skill_gauge_full_frame_165_33_5_hwangyeok_v2.png`와
  `player_skill_gauge_full_frame_155_32_5_hwangyeok_v2.png`를 사용한다.
  두 PNG는 `250x650` 소스와 기력 중심 `(141, 559)` 계약을 유지한다.
- v2 소켓의 불투명 외곽이 기존 프레임보다 넓으므로 기력구슬과 초식 소켓의
  중심 간 여백은 `38px`를 사용한다. 기준 반경에서 약 `8px`의 실제 투명
  간격을 남기며, 5슬롯 합성 PNG와 6슬롯 런타임 폴백에 같은 값을 적용한다.
- 소켓 그림자는 프레임 중심에서 우하단 `(1, 2)px`, 명목 프레임보다 반경
  `1px` 작게, 알파 `0.26`으로 그린다. 구형의 우하단 `(2, 3)px`, 반경
  `+3px`, 알파 `0.38` 원판은 사용하지 않는다.
- 기존 합성 PNG에 구워져 있던 구형 대형 기력 링은 제거한다. 합성 PNG는
  다섯 초식 소켓과 단청 연결선만 소유하고, 기력 링·액체·청옥은 기존
  기력구슬 렌더러가 별도로 그린다. 새 기력 링 아래에 구형 프레임을 섞지
  않는다.
- 천상의 망토 6슬롯과 합성 프레임을 쓰지 않는 캐릭터는
  `smasher_skill_orb_underlay_renderer.gd`의 런타임 폴백을 사용한다. 이
  폴백도 같은 v2 개별 링과 옻칠·놋쇠·심청 단청 연결선을 사용하므로 5/6슬롯
  사이 재질 언어가 바뀌지 않는다.
- 초식 아이콘, 쿨다운 웨지, 준비/활성 상태, 툴팁, 슬롯 순서와 5/6슬롯
  각도는 비스코프다. 경로 상수 세 개만 교체해 기존 리소스 스펙과 PSO
  프리웜이 새 PNG를 자동 추종하게 한다.
