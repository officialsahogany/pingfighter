...
## 10. 기력구슬 짝 프레임 후속 확정 (2026-07-28)

- 사용자 후속 요청으로 기력구슬도 청동 무령 다이얼과 같은 HUD 세트로
  리스킨한다. 방울은 활주방울 HUD 전용 장식이므로 기력구슬에는 넣지 않는다.
- 신규 프레임은 `gauge_orb_frame_imagegen_v2.png`. 기존 v1의 8방위 구조와
  중앙 홀 계약을 유지하면서 옻칠 흑갈·놋쇠·단청 문양을 적용하고, 기력
  구분용 청람 옥 장식 8개를 둔다.
- 런타임 PNG는 `240x240`, `alpha > 16` 외곽 bbox `197x198`, 중앙 투명
  홀 가로 `129px` / 세로 `130px`. 기존 v1의 `198x198`, `130x131px` 대비
  1px 이내라 구슬 반경 보정 없이 `GAUGE_ORB_FRAME_TEXTURE_PATH`만 v2로
  교체한다. 기존 리소스 스펙과 PSO 프리웜은 이 상수를 자동 추종한다.
- imagegen 편집 기준: v1은 정확한 실루엣/홀 기준, 활주방울 v3는 재질 기준.
  마젠타 `#ff00ff` 키를 제거한 뒤 외곽 최대축을 `198px`로 정규화했다.
- 검증 씰 `gauge_orb_hwangyeok_frame_smoke.gd`는 경로·임포트·알파 홀·bbox,
  불투명 마젠타 부재, 컨텍스트 전달, 코어 프리웜 스펙, 프레임 최종 전경
  드로우 순서를 고정한다.
