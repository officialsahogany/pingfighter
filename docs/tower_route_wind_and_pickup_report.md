...
## 개정 (2026-08-23) — 아래 판정은 사용자 피드백으로 번복됨

피드백2.hwpx 6항: "노드 선택 공발사시 바람이 실질적으로 공에 영향을 주지
않고 있음"이 문제로 제기되어, 2026-08-21의 비행-중-힘-없음 판정을 번복한다.
경로 바람은 이제 비행 중 공에 세기별 횡가속을 실제로 가한다
(`TowerAscentTuning.TEMP_ROUTE_WIND_FLIGHT_FORCE_PER_FRAME`, 세기 1~3 배율,
날씨 바람 상수와 분리 소유). 조준 범위 편향은 유지되어 화살표는 여전히
실제 발사각을 가리키고, 역풍 보정 각도도 도달 가능하다. 씰의 궤적 격리
레그는 드리프트 양성 레그(횡전용·방향 대칭)로 개정되었다
(`tower_ascent_route_serve_smoke.gd`의
`_verify_wind_bias_reachability_and_flight_drift`).

## 판정 (2026-08-21 — 번복됨, 기록 보존)
