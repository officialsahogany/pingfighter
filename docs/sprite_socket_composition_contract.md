...
## 파일럿 배선 3 — 레인 C (오니마루 수호령 탑승, 목말 구도)

`lingpet_mount_state.gd` + `lingpet_egg_runtime` 통합. 커밋 995b8854c
(코어) + b2ee82c09(목말 드로우 순서) + 831d1f5e9(라이드 모션).

- 인터랙션: 오니마루 근접(78px) + **맨 우클릭** 엣지 토글. S+우클릭은
  스매셔 오버드라이브 예약 — 절대 가로채지 않음. 모듈-로컬 엣지 폴링
  (공유 리더 엣지 소모 금지).
- 탑승 중: 지상펫 lane-Y 유지 + X 지수 관성 추종(11/s), 바디히트/수비
  명시 억제(parked≠disabled), 스킬 출격·펫 교체·라운드 리셋 강제 하차.
- 목말 구도: 탑승 중 lingpet body hook을 플레이어 스프라이트 **뒤로
  지연**(오니마루 머리·올린 손이 라이더 하반신을 가림). 플레이어-숨김
  얼리리턴 경로에서도 지연 호출 보존.
- 라이드 모션(전부 프로시저럴): 호프 온 아크(0.28s+오버슛)·하차 드롭
  (0.16s)·주행 걸음 바운스(3.2px)·정지 호흡 스웨이(1.5px).
- 라이더 리프트 체인: egg_runtime 접근자 →
  scene context(`get_cached_instance` 비인스턴스화 피크, lazy-init 트랩
  준수) → actor context → stage1 플레이어 y-offset. 발광·코인 등 소켓
  스택 전체가 함께 승강 — "탑승 × 임의 퍽" 자동 성립.
- M+N 표준: 수호령 M벌 + 캐릭터 탑승 포즈 N벌만 제작, 조합 0벌.
  안장(캐리) 포즈는 AutoSprite 생성, 표준 탑승 포즈 규격에서 벗어나는
  수호령별 맞춤 포즈는 금지(허용 시 M×N 회귀).

씰: `lingpet_mount_state_smoke.gd` 13레그(CI/pre-push 락스텝 등재).
반증검증: lane-Y 파괴 토글 → 해당 레그 RED (2026-07-21).

## 다음 슬라이스

- 오니마루 캐리 포즈 시트(AutoSprite, 뒷모습 양팔 올림) 수락 시:
  탑승 중 컴패니언 시트를 캐리 포즈로 스왑하는 렌더 분기 + 시트 소켓
  재저작.
- 환격전 리스타일 시트 랜딩 시: 저작 도구 재실행으로 소켓 데이터만 갱신
  (계약·배선 무변경).
- 라이브 QA: 신속 발광·행운 코인·오니마루 탑승(호프/바운스/관성) 통합
  확인, `player_socket_debug_overlay_enabled` 토글로 소켓 마커 확인.
