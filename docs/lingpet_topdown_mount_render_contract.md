...
§6-b는 씰 해석 고정, §8은 결정 이력(전 항목 확정), §9는 착수 순서다.
...
## 6-b. 씰 해석 고정 (rev7, 리뷰 승인 시 확정)

레그 문구가 같아도 해석이 갈리면 공허 GREEN이 된다. 세 가지를 못박는다.

| # | 레그 | 확정된 해석 | 금지되는 해석 |
|---|---|---|---|
| 1 | **P16 객체 동일성** | 준비도 resolver가 읽은 N `Texture2D`를 **실제 actor context까지 전달**한 뒤 그 객체가 renderer가 받은 것과 동일함을 단언 | 게이트와 렌더러에서 **각각 재조회**해 둘을 비교하는 것 — 같은 캐시를 두 번 읽으면 항상 통과해 정본 분리 결함을 못 잡는다 |
| 2 | **P15 "draw 1회"** | **각 레이어의 진입/헬퍼 호출 1회**(L1 `_draw_soft_aura`, L2 gauge 버스트, L3 skill, L4 hit) | `CanvasItem` **primitive 수** 세기 — 오라·버스트는 내부적으로 여러 primitive를 그리므로 숫자가 의미를 잃는다 |
| 3 | **P17 전환 공존 불가** | **실제 `switch_lingpet_slot()` 관통**. 현행 순서가 `_switch_transition_state.begin()`(`:1870`) → `_set_current_pet_id()`(`:1871`, 내부에서 mount reset)이라 **렌더 시점엔 이미 비탑승**이 성립한다 | `_mount_state.reset()` **직접 호출** — 전이 순서를 우회해 실제 계약을 증명하지 못한다 |
...
### 9-b. 커밋 경계 (rev7 확정, 리뷰)

커밋은 **rev7 문서 → 슬라이스 A → 슬라이스 B** 세 개로 분리한다. 각 커밋의 소유권과
"아직 안 닫힌 계약"이 흐려지지 않게 아래 둘을 경계에서 유지한다.

| # | 경계 규칙 |
|---|---|
| 1 | 슬라이스 A의 `mount_presentation_model`은 **필드 해석·검증 인프라만**이다. **백린 실제 엔트리에는 계속 미등재**(8-10) — A가 끝나도 shipped 카탈로그는 그대로다 |
| 2 | **P16은 A에서 완결로 표시하지 않는다.** A는 "로더 호출 0회"만 부분 봉인하고, **B에서 같은 N 객체가 actor context와 renderer까지 도달**해야 비로소 완결이다 |

슬라이스별 씰 범위:
- **A(데이터·게이트)**: P10 · P14 · **P16①(로더 0회, 부분)** · P17
- **B(렌더)**: P1 · P3 · P8 · P9 · P11 · P12 · P13 · P15 · **P16②(객체 동일성, 완결)**
