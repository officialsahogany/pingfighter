...
| D5 | 소식 체질의 부화 풀 편입 | **확정(2026-07-04, 사용자)**: **부화 공용 풀 편입(원안)** — 카탈로그 추가만으로 부화 랜덤 추첨 + 해금 후보에 자동 편입. 전투 패시브 확률 1/5→1/6 희석과 "소식만 뽑힌 펫" 꽝 체감은 **의도적으로 수용**(권장안이었던 2차 해금 전용을 기각) |
...
### 포만도 경제 슬라이스 지시 (소식 체질 + 먹이 4종 — 착수 2026-07-04, c6785c55b 후)

> ✅ **완료(2026-07-04, Codex 배선 + Claude 적대 리뷰 APPROVE, 미커밋)** —
> `lingpet_light_eater` 부화 풀 편입 + 양슬롯 드레인 순회(반증검증: 슬롯0-only
> 토글 RED) + 사과/멜론 ADD + 6언어. 구조 노트: 감면 테이블이 카탈로그
> by_level + affinity 상수 **이중**(카탈로그→affinity preload 순환이라 불가피)
> — 리뷰어가 dual-table 드리프트 가드를 satiety 스모크에 추가(반증검증: 99.0
> 토글 RED). nit(비차단): 신규 아이콘 3장이 32px 단순 도트로 기존 귤 64px 대비
> 하향 — item-generation 스킬 리젠 후보.

원안 잔여분 2건을 한 슬라이스로. 결정은 전부 잠김(D5 편입/D6 이름·수치),
카탈로그 ADD 중심이라 원자 커밋 1개 목표.

**Part A — '소식 체질' 패시브 (D5 확정: 부화 공용 풀 편입)**
- `LingpetCatalog.COMMON_PASSIVE_SKILL_POOL`에 엔트리 추가: 표시명 **소식 체질**
  (4자 시적 한자어 톤), 효과 `satiety_drain_reduction_pct_by_level := [10, 17,
  24, 31, 38]` (%). 카탈로그 추가만으로 부화 랜덤 추첨(`_roll_hatch_passive_id`)
  + 해금 후보에 자동 편입됨(D5가 그 자동 편입을 의도로 확정). 기존
  `lingpet_hatch_randomization_smoke`는 풀 크기 적응형이라 깨지지 않아야 함 — 확인.
- 소비 훅은 이미 배선됨: `egg_runtime._get_satiety_drain_multiplier()`가 플랫 키
  `satiety_drain_reduction_pct`를 읽고 `_apply_skill_level_values`가 `*_by_level`
  배열을 레벨 해석해 플랫 키로 풀어줌(검증 완료). 감면 상한은 기존 클램프
  (1-pct/100 → [0.4, 1.0]) 유지.
- **Slice 1 이월 의무 2건 해소(§4.6)**: (a) `_get_satiety_drain_multiplier`가
  슬롯 0 패시브만 읽음 → `get_passive_skills` **양슬롯 순회**로 교체(감면 합산
  후 동일 클램프; 2번째 패시브 슬롯에 소식 체질이 앉는 케이스가 D5 편입으로
  실재하게 됨) + 슬롯 2 배치 스모크. (b) `lingpet_affinity_state.gd`의
  `SATIETY_DRAIN_REDUCTION_PCT_BY_LEVEL` 상수+`get_satiety_drain_reduction_pct_for_level`
  헬퍼(소비자 0) 제거 — 카탈로그 배열이 단일 소스.
- 설명문 카피 규율(D1): "포만도 소모 속도가 느려집니다" — '허기 증가/감소' 표현
  금지. 다국어 6벌(EN/ZH/JA/ES/PT_BR/RU) 동기.
- 스모크: 드레인 감면 OUTCOME(레벨별 실측 드레인, 상수 파생) / 슬롯 2 배치 /
  부화 풀 자동 편입 확인. 반증검증: 양슬롯 순회를 슬롯 0-only로 임시 되돌리면
  슬롯 2 케이스 RED.

**Part B — 먹이 4종 확장 (D6 잠김: 순수 ADD)**
- 신규 2종: **사과(포만도 +30)**, **멜론(포만도 +50)** — 기존 귤(+40)/특제
  사료(+100) 사이 삽입. 치즈 3종 선례 = 공유 effect id `lingpet_feed` +
  `feed_amount` 파라미터화 빌더 (Slice 4의 귤/특제 패턴 그대로).
- 획득: 필드 스폰 가중치(귤보다 낮게) + 플라자 샵 판매(가격 곡선은 귤~특제
  사이 보간). reward_only 아님(특제만 reward_only 유지).
- 아이콘 2장(item-generation 스킬 경유, 과일 크기=티어 위계 가독), 다국어 6벌,
  `active_item_catalog_korean_names_smoke`/`plaza_shop_stock_smoke`/
  `item_field_spawn_pool_smoke`/`localization_coverage_smoke` 갱신,
  item_runtime_checklist 감사.
- 만복 차단/배틀 2회 캡/no_global_cooldown/KO 급식 즉시 기상은 effect 공유라
  자동 상속 — 스모크에서 사과/멜론 중 1종으로 상속 확인 1케이스.
