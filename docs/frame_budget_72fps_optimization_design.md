...
## 0.2 클린 런 판정 (2026-06-11 오전, 전 수정 반영 후 — 디버그/스크린샷/에러 0)

soldier, stage1~2 각 ~1.5분, 72캡:

| | 더블링(≥27.78ms) | worst | draw avg | 베이스라인 |
|---|---|---|---|---|
| stage1 | 25% (11/44) | 111ms | 3.84ms | 11~20% |
| stage2 | 32% (10/31) | 55.6ms | 4.25ms | 33~72% |

- 스크립트 비용 역대 최저: draw 3.8~4.3ms + 물리 1.9ms = 메인스레드 합
  6~7ms (예산 13.89ms의 절반). fps 대부분 72 고정.
- **체감 게이트 통과: 사용자 보고 "프레임드랍 없었음"** — 원래 문제(스테이지2+
  연속 끊김)는 체감에서 해소.
- 잔여 더블링의 성격: 더블링 윈도우 내부 스크립트 전부 ≤8.3ms, delta만
  41.7ms(정확 3틱) 양자화 단발 스킵. 메인스레드 과부하 아님. 유력 해석:
  단발 1~2프레임 스킵은 72fps에서 지각 한계 이하 + BattlePerf 로깅(매 2초
  수 KB 출력, 에디터 디버거 파이프 경유)의 관찰자 효과. 로깅을 끄면 측정
  불가(캐치-22)이므로 수치 추적은 여기서 종료, 체감 우선 원칙으로 판정.
- 남은 판정: 스테이지 3~4 클린 체감 런 → 통과 시 §3 승급 진행.
