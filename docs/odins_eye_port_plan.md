...
- 세부 실행 계약: `docs/odins_eye_dark_swamp_slice_plan.md`.
- 3A 런타임 코어: 게이지 100 소모, 2초 쿨, 부활 finalize 이후 활성화,
  12개 럴커 가시 스폰/쿨다운/경계 cleanup.
- 3B 충돌: 볼 반사, 보스 넉백 25 + 스턴 60프레임, 1회 히트/분해 처리.
- 3C presentation: 오브 HUD + 툴팁(이펙트 미리보기 포함, §7.5) + 사운드 + VFX.
- **분량을 가장 크게 가르는 슬라이스.** 3A부터 별도 패치로 시작 권장.
