...
## 23. 리플레이 중 필러 HUD 이중 표시 — 진단 우선 (2026-08-16)

### 23.0 증상 (9차 인게임 QA)

리플레이 재생 중 **필러 HUD 요소(스킬카드·활주구슬·기력구슬·초상화·오브)가
가로로 어긋난 두 벌**로 보인다(오프셋 ~85 표시픽셀). §22 항목 자체(프레임 제거·
제목 페이드·풀캔버스)는 전부 정상.

**관측 근거**:
- 우측 크롭에서 **두 벌 모두 리플레이 테두리 바깥(레터박스)**에 있다 →
  캡처 오염이 아니라 라이브 측 이중 드로잉이 유력.
- 재생 호스트는 `clip_contents` 캔버스 클립(`_build_host`) → 호스트 내부
  드로우는 레터박스로 나갈 수 없다 → 배틀 씬 자체의 필러 렌더 경로가 용의선.
- **이중인 것** = 캔버스 가장자리 앵커 요소(카드·오브·초상화) /
  **단일인 것** = 원거리 레터박스 크롬(골드·상단 미니 전광판·벽 아트) —
  레이아웃(offset) 값이 다른 두 패스가 같은 요소를 그리는 서명.

### 23.1 진단 프로토콜 (수정 전 필수 — 원격 추정 확정 금지, §16.9 전례)

1. **귀속 축소**: ①상태 밴드 폴백 재생에서도 이중인가(§20 프레임 레인 귀속
   여부) ②일반 플레이·전리품 페이즈에서는 단일인가(리플레이 활성 전용인지).
2. **소유자 특정**: 리플레이 활성 중 1프레임에 대해 필러 드로어 호출을 로그
   (패스명 · `game_offset` · side_rects) — 같은 요소가 두 번 그려지는지,
   두 패스의 offset 차이가 관측 ~85px(표시)과 일치하는지.
3. **클립 우회 점검**: `_host` 직속(클립 밖) 자식 유무, overlay 브리지의
   부모·범위(overlay가 클립 밖이면 그 드로우 목록 전수).
4. 소유자 특정 후에만 수정. 수정 계약은 특정 결과에 따라 이 절에 추가.

### 23.2 씰 (수정과 함께)

- **리플레이 활성 중 레터박스 픽셀 비교**: 일반 프레임 vs 리플레이 프레임에서
  필러 요소의 위치·개수가 동일함을 단언(이중이면 RED). 좌·우 필러 각각.
- 회귀: 전리품 페이즈·일반 플레이의 필러 표시 무변.
- **10차 인게임 판정**.
