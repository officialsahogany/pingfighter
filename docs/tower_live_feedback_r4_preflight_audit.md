...
> **후속 정정(2026-08-19)**: 아래 1·2는 착수 당시의 원인 기록이다.
> 현재 본 트리에서는 보상 픽·노드 모달의 스크린 공간 승격과 신화 획득
> REVEAL 입력 선점 복구가 모두 완료됐다. 현행 결함으로 다시 읽지 않는다.

아래 두 가지는 당시 감사 결론의 뼈대였으며 코드로 확인했다. `[역사적 확인]`.

1. **보상 픽과 노드 모달은 플레이필드 변환 패스에서 그려졌다** `[수정 완료]`.
...
2. **절세무공 REVEAL 클릭이 보상 픽에 먹혔다** `[수정 완료]`.
