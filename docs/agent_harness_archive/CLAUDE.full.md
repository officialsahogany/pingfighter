# Exact Pre-Compaction CLAUDE Snapshot

> The payload beginning at the next `# CLAUDE.md` heading is the exact
> 190,711-byte pre-compaction snapshot (SHA-256
> `dcb8dbac686e791d11c92ddd0c2147e9a800428709d4c070432a38c00089be53`).
> It is inactive historical provenance. Its links remain byte-exact and
> root-relative, so use the active root registry and owner documents for live
> navigation and instructions.

# CLAUDE.md

This repository now develops the Godot project **환격전** only. The English
product title is not decided; do not invent or infer one. The original
Python/Pygame PingFighter codebase is frozen and is kept as a porting reference.

Legacy `pingfighter`, `DiskHearts`, `Ringpia` / `Lingpia`, package, CLI, env,
save, resource-path, and export identifiers are compatibility IDs, not current
branding. Do not rename them without an explicit migration. In particular,
keep `godot/project.godot`'s `config/name="pingfighter"` until the `user://`
save-data migration is designed and verified.
...
[GRT-001](docs/godot_runtime_traps.md#grt-001)
...
[GRT-002](docs/godot_runtime_traps.md#grt-002)
...
[GRT-003](docs/godot_runtime_traps.md#grt-003)
...
[GRT-004](docs/godot_runtime_traps.md#grt-004)
...
[GRT-005](docs/godot_runtime_traps.md#grt-005)
...
[GRT-006](docs/godot_runtime_traps.md#grt-006)
...
[GRT-007](docs/godot_runtime_traps.md#grt-007)
...
[GRT-008](docs/godot_runtime_traps.md#grt-008)
...
child z (plaza z=1200 case). ⭐분리 호스트는 **상태**라 immediate 와 달리 "안
그리면 사라진다"가 없다 — 소유 렌더러의 본체-숨김 얼리리턴(홀로그램/빙의/깜빡임)
과 `draw()` 자체를 건너뛰는 빈-컨텍스트·스테이지전환 경로를 상속 못 해 유령이
남는다. 정본=`draw()` 최상단 무조건 `set_active(false)` + 말미 sync 재점등(같은
`_draw()` 안 false→true 는 안 보임) + `clear_transient_canvas_items()` 에도 내리기
(호스트 참조 캐시 필요). ⭐단일 Node2D 라 **플레이어와 보스 "사이" z 슬롯은 없다**
— 뒤 레이어에 일부러 가려지는 draw 는 `overlay_safe` 로 제외하되 **생산자 2종을
전수 조사**하라(그리는 쪽이 아는 베놈 엣지 + 호출 렌더러가 아는 목말 탑승 지연
드로우; 순서 결정과 제외 판정은 공용 술어 1개). 소비자 씰만으론 생산 배선이
안 잠긴다(실 분기 draw 관통 레그 필수). ⭐알파
마스크 계약은 **불투명 픽스처로 검증 안 된다**(투명 모서리 실루엣 + 실 Vulkan
캡처 필수). This section also owns the authored-facing
...
[GRT-009](docs/godot_runtime_traps.md#grt-009)
...
`frames−1`. 씰은 `step_motion` 관통 + **변위 px 결과** 단언. 동반 트랩: 턴관성
리졸버가 **도착 데드존 없이 부호만** 보면 서브픽셀 오차에서 가속↔역전제동
한계 순환(제자리 진동 pp 1.83px) — 데드존 `accel×fps_scale²`, 정착 씰 픽스처는
목표를 중심과 어긋나게. 동반 트랩②: 리그 **오차폭 < 미스 임계**면 표시 실수율이
실효 0%다(극한 80 vs 72.8 / 초월 20 vs **76.8** = 구조적 무돌파) — 패들 배율을
건드리면 오차폭을 같이 재검토하고, `_roll_approach_decision`은 상승 시작 1회뿐이라
**이미 상승 중인 공을 다시 때리는 연계**(쉐도우→마샬→더블마샬)는 판정을 못 받는다.
해법=전역 실수율이 아니라 **타격 1회 이벤트**(발행은 teardown보다 위·되감지 않는
단조 serial+만료 pending 폐기(**조기 return 전부보다 위**)·연계도 별도 serial·이미
이긴 래치는 유지하되 **매 프레임 재검증**·확률 0 리그는 완전 no-op). ⭐**목표만
어긋나게 잡으면 미스가 보장되지 않는다** — 도달 게이트는 근사 적분이 아니라 실제
리졸버로 live 위치·**속도**에서 돌리고 **이동 후처리(감속 배율)까지 같은 static을
공유**해야 하며, 씰은 **실제 충돌 왕복**을 (y·속도·보스 초기속도·**감속 축**까지)
관통해야 한다. Full rule: `docs/godot_runtime_traps.md`.

## Godot 회복 램프 플레이어 재개입 트랩 (해제는 한 프레임 늦다)

[GRT-010](docs/godot_runtime_traps.md#grt-010)
...
[GRT-011](docs/godot_runtime_traps.md#grt-011)
...
[GRT-012](docs/godot_runtime_traps.md#grt-012)
...
[GRT-013](docs/godot_runtime_traps.md#grt-013)
...
[GRT-014](docs/godot_runtime_traps.md#grt-014)
...
[GRT-015](docs/godot_runtime_traps.md#grt-015)
...
[GRT-016](docs/godot_runtime_traps.md#grt-016)
...
(바나나슬라이스 2연속 밟기 = 보스 벽 고착). 게이트는 호스트의 생존 술어
`needs_runtime_update_for_skill(skill_id)`도 함께 본다 — 새 스킬은 **거기에** 등록하라.
⚠️`is_launch_blocked()` 재사용 **금지**(재시전 정책 질문이라 nest-allowed 스킬에서
의도적으로 false를 반환 → 등재해도 게이트가 그대로 끊는다). `update()`를 직접 부르는
유닛 스모크는 공허-GREEN, 씰은 실제 게이트를 관통해야 한다.
Full rule: `docs/godot_runtime_traps.md`.

## Godot Owner-Field Schema Trap (runtime stat → character-info panel)

[GRT-017](docs/godot_runtime_traps.md#grt-017)
...
[GRT-018](docs/godot_runtime_traps.md#grt-018)
...
[GRT-019](docs/godot_runtime_traps.md#grt-019)
...
[GRT-020](docs/godot_runtime_traps.md#grt-020)
...
[GRT-021](docs/godot_runtime_traps.md#grt-021)
...
## Godot 공유 레이아웃 빌더 요소-추가 트랩 (그린 자리 ≠ 클릭 자리)

[GRT-022](docs/godot_runtime_traps.md#grt-022)
...
[GRT-023](docs/godot_runtime_traps.md#grt-023)
...
[GRT-024](docs/godot_runtime_traps.md#grt-024)
...
directions. ⚠️같은 트랩이 HUD 밖에서도 반복된다(수호령 교체 비교 카드):
슬롯 0만 퍼블리시하면 해금된 2번째 패시브가 사라져 "교체하면 스킬을 잃는다"로
읽히고, **표시 레벨을 저장 로드아웃 레벨로 뽑으면 획득 당시 롤(Lv.3)이 실효
설명·쿨타임(Lv.5) 옆에 찍힌다** — 정본은 해석된 스킬 dict의 `"level"`.
⚠️부화 롤 0 = **그 슬롯 없음**(정상 ~25%)이므로 빈 슬롯을 풀 폴백·기본 로드아웃으로
메우지 마라(`get_loadout`이 이미 기본값 폴백이라 추가 가드는 실 로드아웃만 날린다).
스킬 **이름도** `translate_text()` 대상(설명만 번역하면 반쪽 한국어).
Full rules: `docs/godot_runtime_traps.md`.

## Godot Boss-Paddle-Scripting Skill Trap (drag / grab / displace the boss)

[GRT-025](docs/godot_runtime_traps.md#grt-025)
...
[GRT-026](docs/godot_runtime_traps.md#grt-026)
...
[GRT-027](docs/godot_runtime_traps.md#grt-027)
...
[GRT-028](docs/godot_runtime_traps.md#grt-028)
...
[GRT-029](docs/godot_runtime_traps.md#grt-029)
...
[GRT-030](docs/godot_runtime_traps.md#grt-030)
...
[GRT-031](docs/godot_runtime_traps.md#grt-031)
...
⚠️거울상 변종: 콘텐츠 계열을 통째로 끄는 플래그(패시브→퍽 전환)는 그 계열을
스케일하던 소비자를 **조용히 0으로** 만든다(`x if sum>0 else 0` 빈-그룹 가드) —
grep은 플래그가 아니라 **제거되는 그룹 키**로, 씰은 **flag ON 레그**로.
Full rule: `docs/godot_runtime_traps.md`.

## Godot Per-Frame Catalog Lookup Trap (miss-case full scan)

[GRT-032](docs/godot_runtime_traps.md#grt-032)
...
[GRT-033](docs/godot_runtime_traps.md#grt-033)
...
[GRT-034](docs/godot_runtime_traps.md#grt-034)
...
[GRT-035](docs/godot_runtime_traps.md#grt-035)

공유 큐/스펙 목록에 항목을 **맨 뒤에** 더해도, 그 목록을 순회하는 소비자
(setup·SFX버스·프리웜)의 산출 순서에 의존하는 **형제 씰이 절대 인덱스**를 굳혀
뒀으면 그 씰이 RED가 된다(UI 큐 4→5 → `PaddleHitSfx` 4→5). 표준 러너는 첫
실패에서 `throw` 하므로 **알파벳순 뒤쪽 수백 개가 미실행**이고, 정작 새 기능의
씰이 거기 있으면 "통과"가 거짓이 된다 — 출력 마지막 `All Godot smoke tests
passed.` 없으면 통과로 세지 마라. 앵커는 실계산(`get_cue_ids().size()`)으로
바꾸되 **순서를 뒤집어 RED 반증검증**할 것. 동반: 공유 오버레이 **개폐 부수효과**
(열기 큐·BGM 먹먹함)는 호스트 라우터가 아니라 오버레이 `open()` 정본 소유 —
입력 종류를 가리는 **닫기** 큐만 입력 핸들러 소유이며, 그 무음 계약은 ESC·
하위모달 취소 **부정 레그** 없이는 공허-GREEN. Full rule:
`docs/godot_runtime_traps.md`.

## Godot 벤더 WAV 컨테이너 결함 트랩 (임포트는 되는데 매 로드마다 경고 → 러너 RED)

[GRT-036](docs/godot_runtime_traps.md#grt-036)
...
[GRT-037](docs/godot_runtime_traps.md#grt-037)
...
[GRT-038](docs/godot_runtime_traps.md#grt-038)
...
[GRT-039](docs/godot_runtime_traps.md#grt-039)
...
[GRT-040](docs/godot_runtime_traps.md#grt-040)
...
[GRT-041](docs/godot_runtime_traps.md#grt-041)
...
[GRT-042](docs/godot_runtime_traps.md#grt-042)
...
[GRT-043](docs/godot_runtime_traps.md#grt-043)
...
[GRT-044](docs/godot_runtime_traps.md#grt-044)
...
[GRT-045](docs/godot_runtime_traps.md#grt-045)
...
[GRT-046](docs/godot_runtime_traps.md#grt-046)
...
+ 배율!=1에서(`content_scale_size` 절반으로 강제; ⚠️스모크 `_init()`의
`get_root().size=`는 조용히 무시된다). 하드 discard는 하드-에지 아트를 면도날로
자르니 안쪽 feather 밴드로 램프. ⭐재발: `screen_px`와 비교되는 uniform은 클립
rect뿐 아니라 **페더·웨이브·램프·span 전부**다 — 기본 창은 배율 1.0이라 무증상,
창을 키우거나 전체화면일 때만 발현(로딩 먹연 종단선 66%↔52%). 배율은 전체화면
여부가 아니라 **창 크기**의 함수라 "전체화면 한정 오프셋" 보정은 오답.
Full rule: `docs/godot_runtime_traps.md`.

## Godot VFX 리브랜드 발광 예산 트랩 (ADD × 어두운 아트 = 빛 0)

[GRT-047](docs/godot_runtime_traps.md#grt-047)
...
[GRT-048](docs/godot_runtime_traps.md#grt-048)
...
[GRT-049](docs/godot_runtime_traps.md#grt-049)
...
[GRT-050](docs/godot_runtime_traps.md#grt-050)

커맨드 버퍼 갱신이 `_can_activate` 조기 return **위**에 있으면, 게이트가 닫힌
동안 시퀀스가 살아남아 **게이트 해제 프레임에 지연 발동**한다 — 해제 조건이
곧 발동 타이밍이 된다(흡인장이 천선무를 막아 "공이 패들에 맞은 뒤" 발동한 사례).
게이트를 걸 땐 버릴지/미룰지 명시하고, **콤보 시동기를 후속기 게이트에 올리지
마라**. 원본 파리티는 정합성 근거가 아니다. 겹치기를 허용하면 볼 파이프라인
호출 순서(흡인장 :115 < 천선무 충돌 :171)가 결과를 정하므로 **인계 지점**이
필수 — `consume_ball_hit` 정본 1곳에서 `consume_release_hit_speed_cap()` →
`deactivate()` 순서로(consume 누락 = pending 45 캡 전역 누수). 씰은 플래그 말고
**속도 결과** + 되끄는 대조군 레그까지. Full rule: `docs/godot_runtime_traps.md`.

## Typed 배열에 조건식 리터럴을 대입하는 draw-time 런타임 트랩

[GRT-051](docs/godot_runtime_traps.md#grt-051)
...
[GRT-052](docs/godot_runtime_traps.md#grt-052)
...
[GRT-053](docs/godot_runtime_traps.md#grt-053)
...
상한/렌더 컷오프**를 같이 안 올리면 같은 버스트가 잘린다. ⚠️역방향 함정: 항등원
(×1.0)으로 평탄화된 상수는 **기록된 설계 결정**일 수 있다 — 복원 전 체크리스트의
"recorded design decision"부터 대조하고, felt-gap 리포트는 상향/하향 방향 결정이
아니다(천뢰격 무콤보 페널티 복원 반려 사례). 씰은 미발동 경로 + 버프 상태를 각각
레그로. Full rule: `docs/godot_runtime_traps.md`.

## Godot 룰 상수 파생-임계값 리터럴 트랩 (승리 점수를 올리면 보상이 전멸)

[GRT-054](docs/godot_runtime_traps.md#grt-054)

승리 점수에서 **파생된** 임계값이 리터럴이면 기준 룰이 움직일 때 판정이 통째로
뒤집힌다 — 보상 듀스 판정 `if winning_score > 5`가 7점제에서 **모든 정규 승리를
듀스 취급**해 전 클리어 상자 1개로 접었다(무에러·기존 씰 GREEN). 파생값은
`MatchScoreState.WIN_GOAL/DEUCE_TRIGGER`를 읽고, 듀스 사다리는 정본 static
`resolve_deuce_goal` 한 벌만(폴백 경로도 사본 금지). ⚠️보스 각성 임계값 9개는
**독립 튜닝값**이라 자동 파생 말고 비례 재보정을 명시 결정 + "매치 대비 %" 주석
(S2 고압=듀스 전용이라 WIN_GOAL과 동일 유지). 씰 픽스처를 개편 전 절대 점수로
굳히지 말고 최소 한 레그는 룰 상수로 생성한 전 구간 불변식으로. ⚠️리터럴은
**소스 텍스트 단언**(`contains("... := 5")`)·**같은 폴백의 2번째 렌더 사본**·
**디버그 치트 경로**·**소비자 0인 죽은 중복 상수**에 숨는다. 러너는 첫 실패에서
멈추니 **배치가 죽으면 뒤 항목을 통과로 세지 마라.**
Full rule: `docs/godot_runtime_traps.md`.

## Godot `draw_line`에는 라인 캡이 없다 (pygame ellipse 실루엣 포팅)

[GRT-055](docs/godot_runtime_traps.md#grt-055)
...
포팅이 렌더하는 경우. ⚠️`pygame.draw.*`는 **알파 블렌딩을 안 하고 덮어쓴다** —
원 3겹(a / a/2 / a/3)을 Godot에 그대로 겹치면 source-over로 중심이 0.854까지
차올라 속 빈 기포가 밝은 구체로 뒤집힌다. 해법=알파 밴드 프리베이크 텍스처 1장.
씰=레이어 계약(기대값은 원본 리터럴) + 알파 프로파일·캐시 소비 + 픽셀 캡처.
Full rule: `docs/godot_runtime_traps.md`.

## Godot 바닥밀착 패들 지면 VFX 트랩 (회전한 쿼드는 클램프를 뚫는다)

[GRT-056](docs/godot_runtime_traps.md#grt-056)

플레이어 패들 밑면은 750 바닥에 딱 붙어 "발밑"에 그릴 지면이 없다 — 지면 계열
VFX는 넓고 납작하게(aspect ~0.30) 눕히고 중심을 `min(anchor, FIELD_BOTTOM -
h/2)`로 들어 올려라. ⭐`h`에는 **회전 반영 세로 폭**(`|w·sinθ|+|h·cosθ|`)을 넣을
것 — 축정렬 `size.y`로 재면 46° 기운 235x70 타원이 세로 217로 부풀어 클램프를
통째로 뚫는다(코드상 정상, **라이브 픽셀로만 잡힘**). 지면에 누운 진은 쿼드가
아니라 **UV만 회전**. 씰=최하단 행 lit==0(불티는 면제), 흙먼지는 jade 카운터에
안 잡히니 `r≈g and b<g`로 별도 계수. ⭐동반 발견: **immediate `_draw()` 안의
`canvas.material` 스왑은 no-op**(머터리얼=CanvasItem 단위 속성) — ADD↔MIX를
뒤집어도 픽셀이 자릿수까지 동일했다. 동작하는 사례는 자식 `Sprite2D`라 되는
것이지 스왑이 먹어서가 아니다. status dict에 `blend_mode:"add"`를 실으면 스모크
GREEN·화면 MIX인 **거짓 계약**. Full rule: `docs/godot_runtime_traps.md`.

## Godot 페인티드 크롬 뒤 평면 헤일로 rect 트랩 (그림틀 여백이 상자를 드러낸다)

[GRT-057](docs/godot_runtime_traps.md#grt-057)

불투명 패널 시절엔 카드에 덮여 안 보이던 `draw_rect(rect.grow(N), glow α0.10)`
선택 헤일로는, 그 자리를 **투명 여백을 가진 페인티드 9-patch**로 교체하는 순간
하드엣지 사각 상자로 드러난다(9-patch 압축이 여백 비율을 더 키움). 크롬 아트
교체는 콜사이트마다 배경 드로를 따로 남기므로 **형제 전수 조사 필수** — 실제로
로스터 카드·확정 CTA·리그 탭 3곳이 동시 발현했다(여백 없는 정사각 소켓은 무해).
규칙=평면 헤일로는 **텍스처 없는 폴백 분기 안으로** 옮기고, 상태는 아트 스왑·
실루엣 따르는 modulate 리프트·아트 안쪽 악센트로. 드로 전용이라 상태 스모크는
전부 GREEN — **비헤드리스 캡처의 마진 vs 바깥 대조 픽셀 델타**로만 잡힌다.
Full rule: `docs/godot_runtime_traps.md`.

## Godot 물리차단 모달 개폐 계약 트랩 (새 진입점은 형제 훅을 상속 못 한다)

[GRT-058](docs/godot_runtime_traps.md#grt-058)

모달 활성 플래그를 `is_choice_active()`에 OR로 다는 것은 **물리차단·입력
라우팅·드로만** 상속시키고 **개폐 부수효과는 상속시키지 않는다**. 새 진입점은
형제(퍽 선택·천사의 축복)의 쌍을 전수 대조하라 — 열 때
`_pause_skill_cooldowns_for_choice`(액티브 아이템 쿨다운까지 정지; 기준선이
**벽시계**라 안 멈추면 모달 연 실시간만큼 공짜 쿨다운), 닫을 때
`_resume_skill_cooldowns_for_choice` + `_try_arm_resume_safety`(퍽 모달과 달리
아이템은 **랠리 임의 프레임**에 열려 회피 불가 실점이 난다). `has_method` 뒤
동적 호출은 인자수 대조 필수(2/0/2). 씰은 호출 카운트 말고 **실물 안전장치
결과**로 단언(`ball_vel==ZERO`+실점차단), 실경로 pause 는 레지스트리에
`battle_scene_skill_tooltip_driver` 페이크가 있어야 걸린다(없으면 공허-GREEN).
⚠️동반: **쿨다운만 멈추면 부족하다** — 벽시계 **발동창/입력창/예약 시각**도 밀어야
한다(풍운천선무 1.2초 창이 퍽 고르는 사이 만료). 팬아웃은 드라이버
`_pause/_resume_runtime_perk_modal_time` 한 곳이고 등재는 **양쪽**(드라이버 키 목록
+ 상태의 pause/resume) — 하나만 하면 조용히 no-op. 시각만 밀고 **길이 금지**,
시드(잔영 `start_msec`)·읽기가-쓰는 질의(화랑킥 창)·델타 앵커(`last_update_msec`)
주의, 조회는 **peek 전용**. 커맨드 버퍼도 미는 것이 **기록된 설계 결정**. 씰은
리프 직접호출만으론 최상위 배선이 무방비 — 실 진입점 + 실경로 레그 필수.
Full rule: `docs/godot_runtime_traps.md`.

## Godot 스프라이트 셀 여백 × 패들 배율 트랩 (캐릭터가 벽에 못 닿는다)

[GRT-059](docs/godot_runtime_traps.md#grt-059)

"패들이 커질수록 캐릭터가 벽 끝까지 못 간다"는 물리 클램프 버그가 아니라 **그림**
문제다: 160 셀과 155 패들이 같은 `player_paddle_scale`로 커지므로 셀 안 투명 여백도
같이 커져 `gap = (셀 여백 − 2.5) × 배율`이 벌어진다(코만도 유휴 기본 49.5px → 2배
패들 99px). 해법=시각 전용 벽 슬라이드(`player_sprite_wall_slide.gd`), 물리 패들·공
판정 불변. ⚠️클램프 완화·히트박스 축소는 밸런스 변경이라 사용자 승인 필요.
여백은 오프라인 표의 **시트당 1개 앵커 = raw bbox 최소**(`tools/measure_player_
sprite_body_insets.py`) — 더 느슨하면 필러 배경 **위에** 그려진다(플레이필드는 클립
없음, 드로우 순서 필러→플레이필드→필러HUD). 프레임별로는 더 좁아도 되고 그게 전
프레임 접촉의 유일한 방법이나, 가장자리가 고정되고 **몸이 움직인다** → 인접 프레임
이동량 ≤6px 시트에만 적용(76장 중 33장, 벽 관련 14장 중 11장이 잔여 0).
⚠️**클립은 이 잔여를 못 줄인다**(잔여=프레임별 편차) — 남는 3장은 아트 재크롭 건.
⚠️회전(투척 30°/코어플립)은 rect 중심 기준이라 축정렬 인셋으로 못 막는다 — 세로 여백
까지 써서 회전 AABB로 클램프. ⚠️시간 기반 프레임 시트는 앵커 조회와 draw 가 다른
프레임을 골라 새므로 `player_sprite_sample_msec` 로 프레임당 고정.
측정은 가시성 임계 없이 **alpha>0**. ⚠️여백 0 시트도 셀 오버행 2.5px×배율 때문에
클램프 대상이라 "미등재"와 "유효한 0"을 값으로 구분해야 한다.
**새 시트를 추가/교체하면 재생성**(표에 없으면 조용히 보정 0). 슬라이드 뒤에 얹히는
타격 런지·글리치 흔들림은 최종 rect 클램프로 막는다. 씰은 최종 draw rect를 단언하고,
앵커는 실제 픽셀로 독립 재측정하며, SKIPPED는 자동 면책이 아닌 사람이 적은
허용목록이어야 한다. Full rule: `docs/godot_runtime_traps.md`.
...
### Character-select pristine-master gold reference

For fixed-crop anime-style character-select Live2D, the Han Miryang pristine
rebuild from 2026-07-29 is the quality floor for source cleanliness, matte
edges, fixed framing, rigid-prop stability, alpha-aware Real-ESRGAN delivery,
and actual-menu QA. Final motion frames remain AutoSprite-derived even when
image generation is used to rebuild the clean still master. Do not promote a
branch that only looks clean on white, repairs a broken prior sheet, recenters
frames independently, or skips the real dark character-select render.

The authoritative pipeline, reference asset paths, and exact acceptance gates
live in `.claude/skills/sprite-generation/SKILL.md` section 2.4.1 and
`checklists.md` section 0.4d. The reference sets a technical/visual quality
floor; it does not force other characters to copy Han Miryang's pose or motion.
Its user-approved default cadence is 49 frames at `0.042` sec/frame
(approximately 23.81 fps), with `2.10` sec click/confirm minimum duration,
0.10 sec entry blend, 0.12 sec return blend, and idle restart semantics.
Only an actual-menu or voice-sync review may justify a documented exception.
