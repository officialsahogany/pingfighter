# Exact Pre-Compaction AGENTS Snapshot

> The payload beginning at the next `# Repository Guidelines` heading is the
> exact 116,246-byte pre-compaction snapshot (SHA-256
> `a2d60dd460fb30a2edc9496c9bb3cd6c28da8f5a294a13013b5a13bdfd1092d5`).
> It is inactive historical provenance and preserves the then-current brand
> wording. Current instructions live in root `AGENTS.md` and its owner docs.
...
Treat `pingfighter`, `DiskHearts`, `Ringpia` / `Lingpia`, package names, CLI
flags, environment variables, save keys, resource paths, and export filenames
as compatibility identifiers unless a migration task explicitly owns them.
In particular, keep `godot/project.godot`'s `config/name="pingfighter"` until
save-data migration is designed and verified; changing it moves `user://`.
...
- When porting or tuning a physics / gameplay SCALE constant (per-hit rally
  acceleration, knockback, boost, decay, speed cap), verify the Godot value
  against the Python NORMAL-path effective value, not just whether it "feels
  fine" in isolation. A Godot-only multiplier where `pingfighter.py` returns
  1.0 on the normal (non-arena / non-weather / champion) branch is a silent
  regression: `DEFAULT_RALLY_SPEED_MULT` sat at `0.1008` (~10x too gentle vs
  Python's `1.0`) from May–Jul 2026 because a multi-perspective "fun" review
  compared the port to itself, not to the original — arena/weather ratios
  looked right, so nobody diffed the base. Grep the Python constant / normal
  return value before shipping or retuning a ported scale.
...
- Player-facing terminology is `수호령` (`Guardian Spirit`). `Lingpet` /
  `Ringpet` remains internal compatibility vocabulary only: keep existing
  `lingpet_*` IDs, paths, save keys, script/class names, and asset filenames
  stable unless an explicit migration is requested. New Korean UI copy must
  say `수호령`; localized UI must use the equivalent guardian-spirit term.
...
- The validation wrappers (`run_smoke_tests.ps1`, `run_headless_load_check.ps1`,
  `run_warning_scan.ps1`) refuse to start while this project is being played
  interactively (shared guard `assert_no_interactive_godot_game.ps1`; editor
  and `--headless` processes are exempt). The "blocked because this project is
  running interactively" throw is the guard working, NOT a RED test — stop play
  mode and rerun instead of diagnosing it as a smoke failure. The local
  pre-push gate inherits this block. Background validation during live play
  measurably degrades frame pacing (2026-08-03 triage), so do not bypass it,
  and avoid other heavy repo-wide work (searches, git sweeps) during play too.
