...
### H-2. 에셋 · 연출 결정 범위 *(2026-08-04 상태 갱신 — rev8의 "㉱ 1건만 미결"은 구식)*

㉮㉯는 확정·종결. **㉱(정상 백린 후면 몸체)는 2026-08-04 해결** — idle 승격
`6620aca1f`(정적 폴백 별칭 계약 포함, rev12 §S1). **실제 차단점은 정면 계열이다**:
① 정면 동적 애니메이션(cutin/click) = 생성 경로 전 기각(character R1~R3 ·
자산 pro · 자산 turbo — 아래 2026-08-04 기록 3건)으로 **후속 슬라이스 이월**
② 카드 ㉰ = 사양 확정·산출물 미생성 ③ **S1에 승인 원화 기반 정적 정면 폴백을
넣을지 = 별도 기획 결정 대기**(폴백 채택 시에만 카탈로그 A5 활성 가능).
...
**turbo 단일 변수 모델 대조 (2026-08-04, 사용자 계약 — 정면 동적 축 최종 이월)**:
앵커 v2 바이트 동일본을 새 자산 `cmsec5rs70050ku8wypypgc62`로 복제, 프롬프트 동일,
turbo(Seedance 1.0 Pro loop)·2s·loop·512·32f·removeBg ultra(미승격) 1회 생성(5크레딧,
잔여 303). raw 640×640·24fps·49f → `floor(i·49/32)` 픽 + keyed_v2 동일 물리 키잉
(시트 SHA `4638A7B4…`). 생성 후 보정 전면 미적용(계약). **결과**: ① dom_bg 궤적
220→217 **사실상 평탄**·배경 마젠타 순수(G=0) — pro의 203→169 시간 드리프트는
**모델 의존** ② 그러나 **피사체 팔레트는 f1부터 균일 이탈**: body 5.43~5.71 /
gold 4.88~5.94 → **게이트(1.5/2.5) 32/32 FAIL** — 피사체 색 이탈은 **패밀리 고유**
(pro는 같은 이탈이 더 크고(8.8~9.7) + 시간 드리프트 추가; turbo는 작지만 게이트의
~4배) ③ 구조·모션은 최상: 사슬 금 32/32(치환 0)·마젠타 에지 0px·중심 스팬
2.57/5.22px·인접 최대 0.39/0.73px·눈감김 1회(프롬프트 일치)·L 진폭 2.00(<pro 2.68,
펌핑 없음) ④ jade 최악 33.4는 눈감김 f22~24의 마스크 퇴화 아티팩트(비감김 6.1~8.5로
pro 대비 개선) — 눈감김 프레임 jade 계측은 무의미함을 기록. **판정 = 팔레트 FAIL →
계약대로 Turbo→Max→Ultra 모델 룰렛 금지, 정면 동적 자산(cutin_anim·click 계열)은
후속 슬라이스로 이월.** S1 정적 정면 폴백(승인 원화 기반) 채택 여부 = 별도 기획
결정으로 복귀(H-2 상태 참조). 과학적 결론: 이 비디오 생성 패밀리는 구조(강체·모션)는
계약 수준으로 보존하나 피사체 팔레트 충실도가 게이트에 4~6배 미달 — 재개하려면
팔레트 충실 생성원 또는 게이트 재정의가 선행돼야 한다.

## ✅ S1 전체 종결 (2026-08-05 사용자 승인)

**백린/묵린변신 S1은 코드·에셋·회귀·라이브 렌더·실부화 경로까지 전부 닫혔다.**
커밋 사슬: S1a `a1e6f8838` → S1b-a `261d3a39c` → idle 승격 `6620aca1f` →
static 인프라 `7ff3e4d39` → cutin_art `8cc62ec1b` → 카드 `c6bbc022e` → A5 활성
`fbd77062a` → 픽셀 QA `bea4bc3c9` → P1·P2 보강 `72420492b` → 실 부화 씰
`5f24a852b`. 최종 검증(검토자): 실 경로 = egg.update() 충돌 판정 → 셸브레이크 →
advance_hatch_break() → _commit_pending_hatch() → _finish_regular_hatch() 관통,
독립 재실행 9회째 추첨 도달·소유/슬롯/companion/획득 컷인 전부 확인. 라이브 =
정적 획득 컷인·점진 퇴장 페이드·idle·묵린변신 5초 왕복·경계 팝 없음 확인.
**후속 범위(계획대로)**: 정면 동적 계열(cutin_anim/click 98f — 98f 제작 시 97f
원본 확장 규칙 봉인 포함) · 실제 locomotion(R 네이티브→L per-cell 미러) ·
strike 5×5·25f(impact 22 정렬) · 백린의안장 S2~S7 · 스킬 아이콘 · 스탯 밸런스
튜닝(현재 코요라 미러 보수 기준선).

**S1b-b 종결 — 정적 정면 폴백 채택 이행 (2026-08-04, 카탈로그 활성 완료)**:
사용자 판정(정적 폴백 채택·별칭 위장 반려)에 따라 `front_presentation_model`
계약으로 랜딩. 커밋 사슬: **A `7ff3e4d39`** static 모델 인프라(카탈로그 검증
분기 — 동적 3키 면제 + 정의 금지 fail-closed; 호스트 dismiss 외래 폴백 차단 +
정적 원화 페이드 분기; 씰 15레그+반증) → **B `8cc62ec1b`** cutin_art 승격(a07
경계 dom 역산 unmix, 마젠타 0px·semi 2.96%) → **C `c6bbc022e`** 묵린변신 카드
㉰(Gemini 21:9→1720×541, 먹빛 변신 서사) → **D `fbd77062a`** A5 백린 엔트리
활성(정적 모델·별칭 5키+전 키 1×1 메타·puppet_control 6×2·12f+delta 6.0·묵린변신
액티브 쿨40/5s/mokrin_ink/카드·**transform_duration 페이로드 화이트리스트 등재**
— 빌더 침묵 미전달 함정) + 활성화 씰 33레그(A5 파일실재 검증·디스패처·페이로드·
레이아웃·Y1 미등재·dismiss 로드게이트 마리보 대조·클릭 false+무음·D15 왕복) +
반증 2종. 퇴장 픽셀 증명 = 비헤드리스 `lingpet_static_front_dismiss_visual_qa.gd`
전 레그 PASS(중반 12,006px·에너지 감쇠 4554<11,980·종단 0·대조군 0 —
`bea4bc3c9`에서 개수→에너지 계측 정정). ⚠️스탯은 코요라 미러 보수 기준선(기획
수치 부재 명시) — 밸런스 튜닝 후속. ⚠️debug_picker 선재 RED(풀슬롯 교체)와
egg_runtime_smoke 선재 RED 2건은 백린 토글 A/B로 비기인 증명(외래 WIP 도메인).
잔여 = 라이브 인게임 QA(부화→획득 컷인 등장/퇴장→전투 idle→묵린변신 실주행·
normal→f1/f12→normal 심) · 후속 슬라이스(정면 동적 계열·locomotion·strike
25f·백린의안장 S2~S7) · 스킬 아이콘(icon_texture_path, 선택 항목).
