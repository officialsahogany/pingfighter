...
**변종 사고 (2026-08-05, 천뢰격 무콤보 페널티 항등원 평탄화).** 이번엔 "수식은
살아 있는데 상수가 무력화"된 형태다. 원본 파워스매싱의 무콤보 페널티
(`actual_boost *= 0.75`, 원본 주석 "콤보 쌓아야 본래 성능 도달")가 포트 리라이트
체크포인트(0958adef1)에서 `POWER_SMASH_NO_COMBO_BOOST_MULT := 1.0`으로 평탄화됐다
— 상수 이름과 곱셈 코드가 전부 남아 있어 grep/구조 검토로는 "포팅됨"으로 읽히지만
값이 항등원이라 no-op다. 결과: 순항(target_speed)이 콤보 유무와 완전 동일해지고,
남은 콤보 보너스(+6.6% 캡)는 발사 캡(2.14x/2.40x)에 콤보/무콤보 **양쪽이 함께
포화**해 0.5초 버스트의 캡 비율 차(+12%)만 남았다 — 유저 리포트 "콤보를 모아
쏘나 그냥 쏘나 공속이 같다". 콤보증폭칩 적대리뷰 R2의 "고정 캡이 amp를 마스킹
(Lv0==Lv5)"과 같은 계열로, 같은 파일에서 캡 마스킹 사고만 두 번째다.
...
- 파리티 대상 상수가 ×1.0 / +0 같은 **항등원으로 평탄화**돼 있으면 "포팅됨"으로
  세지 마라 — 코드 구조가 아니라 원본 값과의 수치 대조로 판정한다. 특히
  차이-계약(콤보/레벨/버프 유무로 갈라져야 하는 값)이 걸린 상수는 관련 **캡과 한
  세트**로 본다: 양쪽 레그가 같은 캡에 포화하면 차이 계약이 무에러로 지워진다.
- 차이-계약 씰은 플래그나 발사 순간값이 아니라 **순항(감쇄 이후) OUTCOME**을
  양쪽 레그로 단언한다. 버스트 크기만 재면 캡 비율 차이가 씰을 통과시켜 준다.
- 씰은 "미발동 경로가 정말 미발동인지"(하프대쉬 생성 0)와 "버프 상태의 값"을
  각각 레그로 둔다. 기본 상태 1레그만 두면 셋 다 통과시킨다.

현행 씰: `godot/tests/smasher_dash_spirit_laser_geometry_smoke.gd`
(하프대쉬 미생성 / 비천보 Lv.1·Lv.5 길이 / 파티클 수량·상한·3겹 원 계약),
`godot/tests/smasher_power_smash_combo_speed_split_smoke.gd`
(천뢰격 콤보 vs 무콤보 순항 target·감쇄 후 실측 순항 분리 + 과너프 대조군).
