...
- 파이썬 이펙트를 포팅할 때 `PLAYER.centery` / `PLAYER.bottom` / `PLAYER.top`
  ...
  계산하고, Godot 상수는 그 절대값이 재현되도록 24px(= 40 − 리그보너스 16)을
  흡수해서 정한다. 잔영호법은 `30.0 → 6.0`
  (원본 `756 - H/2` == Godot `750 + 6 - H/2` — 벌크업 등 패들 높이 변동까지 동일).
- 리그 보너스가 다른 난이도(주니어 18 / 신화 20 / 리미트·기본 0)에서는 원본도
  ±2~16px 흔들리므로, 파리티 기준은 **챔피언(16)**으로 잡고 그 사실을 씰
  메시지에 남긴다.
- 씰은 상수 비교가 아니라 **결과**를 단언한다: 실제 스폰 경로
  (`try_spawn_from_dash`)로 띄운 뒤 `y + 최외곽 글로우 반높이 <= HEIGHT`.
