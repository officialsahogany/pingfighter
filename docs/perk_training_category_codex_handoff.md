...
### S5 — 수련 교체형 전환 (v3, 2026-08-05 사용자 결정 — S1~S4 랜딩 후 추가 지시)

S1~S4의 append 구현을 **교체형**으로 전환한다(정본 §2.1-5·6 근거). 화면 장수는
기본 3장(천안결 4장)으로 고정되고, 수련 카드가 일반 선택지 1장을 대체한다.
주사위는 append 유지(비대칭은 의도된 결정).

1. `physique_training_offer_planner.plan_offer`: append → **replaceable 교체**.
   - 교체 후보 술어는 합일 플래너의 `is_choice_replaceable`를 **재사용**(복사
     금지 — offer_protected 존중이 단일 소스로 유지되어야 천안결 보너스·예약
     레인 잠식 사고가 구조적으로 막힌다).
   - 교체 인덱스 선택용 roll_unit 추가 — RNG 무소비 계약 유지: 부적격(교체
     후보 0장·합일 교체 발생 화면·수련 상한 소진) 경로는 어떤 롤도 소비 금지.
   - result 계약: `append_index` → `replacement_index`.
2. `runtime_perk_state._try_inject_physique_training_offer`: 합일 주입 결과
   (appeared)를 인자로 받아 **합일 교체 화면에서는 수련 판정 자체를 스킵**하고
   적격 계측에도 미집계. `runtime_perk_choice_open_flow`에서 fusion_result 전달.
3. 씰 락스텝: physique 씰의 append/배타 단언 → 교체 단언으로 재작성 — 장수
   불변 레그, 교체 대상이 replaceable였음 레그, 보호 레인(천안결 보너스) 비잠식
   레그, 합일 화면 스킵 레그, 후보 0 no-op 레그. dowsing 씰에 수련 교체 비잠식
   레그. 반증: 보호 무시 토글 → RED 확인.
4. 캡처 재정의: "수련 포함 3장"이 표준 케이스, 5장 픽스처는 천안결+주사위
   케이스로 라벨 변경.
5. 카드 카피·확률·상한·다국어 등 나머지 계약 전부 불변.
