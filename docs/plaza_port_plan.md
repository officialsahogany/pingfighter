...
## 0.4 피벗 — 건물·NPC 링피아 통일 (2026-07-02 사용자 확정, ⚠ 계획 대체)

**건물 7종 + 인테리어 NPC 7종은 스테이지별 테마가 아니라 하나의 중립 "링피아"
아트로 통일한다.** 바닥·패럴랙스만 스테이지별 테마를 유지한다.

- **이 피벗의 단일 소스 = `docs/plaza_lingpia_unify_slice_plan.md`** (결정·아트
  디렉션 브리프·rename 슬라이스·매니페스트 계약·스모크·수락 게이트·분담).
- **폐기**: §0.5의 "건물 테마별 재생성" 및 §0.7의 "건물 7동은 S1 한옥 그대로 …
  건물 테마별 재생성은 별도 대형 슬라이스(6테마×7동)" 계획은 **취소**한다.
  건물/NPC는 런타임에 스테이지별로 갈린 적이 없으므로(로더 상수 하드코딩) 이
  통일은 큰 리팩터가 아니라 신규 중립 아트 생산 + 상수 repoint + 구 에셋 제거다.
- **아트 방식**: 새 중립 링피아 아트 재생성(현 cyber_joseon 유지 아님). VR 허브
  인프라 톤, 실세계 문화 모티프 금지, 팔레트 안정으로 어떤 바닥 위에도 어울림.
- **보존**: 7종 엠블럼(§5.1) + NPC 이름/역할/인사(§0.9.2)는 그대로, 아트만 리스킨.
- 아래 §0.5~§0.9의 나머지 계약(횡스크롤·패럴랙스·경제·워프·인테리어 배선)은 유효.
