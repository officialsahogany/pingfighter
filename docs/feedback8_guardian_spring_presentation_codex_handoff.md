...
- **★선행 차단 조건 2건 (둘 다 만족 전 착수 금지)**
  1. **W5 `77efc52f9`(상점 좌우분할) 통합.** W5가
     `tower_ascent_flow_renderer.gd`(+223),
     `tower_ascent_node_modal_state.gd`(+194),
     `tower_ascent_flow_node_progress.gd`(+35),
     `runtime_perk_overlay_renderer.gd`(+122)를 건드리는데 **이 지시문이
     반드시 편집할 4개 파일 전부**다.
  2. **X1(샘터 기능 결함) 착지.** X1이 교체 UI와 자동 진행 골격을 세운다.
     이 지시문은 그 위에 연출을 얹는다. 순서를 뒤집으면 재작업이다.
- 관제탑이 두 선행이 끝나면 기준선 커밋 해시를 다시 알린다.
