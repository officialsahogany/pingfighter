    ...
    if is_waiting_for_serve:
        ...
        def fake_motion():
            ...
            if style == 1:
                ...
                return math.sin(time_now / SCREEN_CENTER_X) * 4
...
def apply_red_overlay(surface, intensity):
    ...
    if surface.get_width() > SCREEN_CENTER_X or surface.get_height() > SCREEN_CENTER_X:
        return
...
def draw_pachinko():
    ...
    slot_area_height = SCREEN_CENTER_X
    # 슬롯 배경
...
def show_character_selection():
    ...
    while True:
        ...
        glitch_timer += 1
        # 랜덤 글리치 효과 활성화
        if random.randint(0, SCREEN_CENTER_X) == 0:
            glitch_active = True
        if glitch_timer > 10:
            glitch_active = False
            glitch_timer = 0
    ...
    for alpha in range(0, 256, 8):
        ...
        if decoration_alpha > 0:
            ...
            if decoration_alpha < 255:
                line_surface = pygame.Surface((SCREEN_CENTER_X, 3), pygame.SRCALPHA)
...
def handle_mega_smashing_trajectory():
    ...
    if hasattr(handle_mega_smashing_trajectory, 'original_y'):
        ...
        center_y = handle_mega_smashing_trajectory.original_y - SCREEN_CENTER_X
    else:
    ...
    if boss_hitbox_expanded.colliderect(BALL) and boss_collision_cooldown <= 0 and not is_waiting_for_serve:
        ...
        if not new_boss_mode_active and current_stage == 1:
            ...
            if not stage4_magnetic_active:
                ...
                if boss_special_gauge_stage4 >= SCREEN_CENTER_X:
                    boss_special_gauge_stage4 = SCREEN_CENTER_X
...
def show_result(won):
    ...
    if won:
        ...
        SCREEN.fill(BLACK)
        pygame.display.flip()
        pygame.time.delay(SCREEN_CENTER_X)
        # 🆕 뽑기 시작
        # 사용 가능한 아이템들 생성
        available_items = []
