...
class PaddleRenderer:
    ...
    def _draw_boss_paddle_body(self, rect: pygame.Rect, color: Tuple[int, int, int],
        ...
        if boss_stage >= 4:
            ...
            for i in range(3):
                ...
                alpha = 100 - i * 30
                surf = _get_pooled_surface(expanded_rect.width, expanded_rect.height)
    ...
    def _draw_charge_effect(self, rect: pygame.Rect, charge_level: float):
        ...
        size = charge_radius * 2
        charge_surf = _get_pooled_surface(size, size)
    ...
    def _draw_dash_effect(self, rect: pygame.Rect, color: Tuple[int, int, int]):
        """대시 효과 그리기 - gfxdraw 직접 렌더링"""
        # 잔상 효과 - 반투명 사각형을 직접 그리기
        for i in range(3):
            offset = i * 5
            alpha = 150 - i * 40
            dash_surf = _get_pooled_surface(rect.width, rect.height)
    ...
    def _draw_shield_effect(self, rect: pygame.Rect):
        """실드 효과 그리기 - Surface 풀 사용"""
        shield_rect = rect.inflate(20, 10)

        # 실드 버블 - Surface 풀 사용
        shield_surf = _get_pooled_surface(shield_rect.width, shield_rect.height)
    ...
    def _draw_wide_paddle_effect(self, rect: pygame.Rect):
        """와이드 패들 효과 그리기 - Surface 풀 사용"""
        # 확장 표시 - Surface 풀 사용
        extend_surf = _get_pooled_surface(20, rect.height)

        # 왼쪽 확장
        pygame.draw.rect(extend_surf, (100, 255, 100, 50), extend_surf.get_rect())
        self.screen.blit(extend_surf, (rect.left - 20, rect.top))

        # 오른쪽 확장 - 같은 내용이므로 다시 풀에서 가져와 그리기
        extend_surf2 = _get_pooled_surface(20, rect.height)
        pygame.draw.rect(extend_surf2, (100, 255, 100, 50), extend_surf2.get_rect())
        self.screen.blit(extend_surf2, (rect.right, rect.top))
    ...
    def _draw_rage_mode_effect(self, rect: pygame.Rect):
        """광폭화 모드 효과 그리기 - Surface 풀 사용"""
        # 붉은 오라
        for i in range(3):
            rage_rect = rect.inflate(i * 10, i * 5)
            alpha = 100 - i * 30
            rage_surf = _get_pooled_surface(rage_rect.width, rage_rect.height)
    ...
    def _draw_special_attack_effect(self, rect: pygame.Rect, attack_type: str):
        """특수 공격 효과 그리기 - gfxdraw/Surface 풀 사용"""
        if attack_type == "laser":
            ...
            for i in range(3):
                ...
                alpha = 150 - i * 40
                size = radius * 2
                laser_surf = _get_pooled_surface(size, size)
    ...
    def _draw_boss_aura(self, rect: pygame.Rect, boss_stage: int):
        ...
        if aura_radius > 0:
            size = aura_radius * 2
            aura_surf = _get_pooled_surface(size, size)
    ...
    def _draw_hit_animation(self, rect: pygame.Rect):
        """히트 애니메이션 그리기 - gfxdraw 직접 렌더링"""
        # 충격파 효과
        wave_radius = int(30 - self.hit_animation_timer)
        wave_alpha = int(self.hit_animation_timer * 8)

        if wave_alpha > 0 and wave_radius > 0:
            cx = rect.centerx
            cy = rect.centery
            # 링(테두리 원)을 gfxdraw로 직접 그리기
            pygame.gfxdraw.aacircle(self.screen, cx, cy, wave_radius,
                                    (255, 255, 255, wave_alpha))
    ...
    def _draw_trail(self):
        """트레일 그리기 - Surface 풀 사용"""
        for point in self.trail_points:
            if point['alpha'] > 10:
                w = int(point['width'])
                if w > 0:
                    trail_surf = _get_pooled_surface(w, 20)
