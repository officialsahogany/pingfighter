...
def draw_gradient_rect(rect, color1, color2, vertical=True):
    ...
    if vertical:
        for y in range(rect.height):
            ...
            draw.line(color, (rect.x, rect.y + y), (rect.x + rect.width, rect.y + y))
    else:
        for x in range(rect.width):
            ratio = x / rect.width
            color = blend_colors(color1, color2, ratio)
            draw.line(color, (rect.x + x, rect.y), (rect.x + x, rect.y + rect.height))
...
def activate_flare():
    ...
    # 투척 모션 시작
    flare_throwing = True
    flare_throw_timer = HALF_SECOND_FRAMES  # 0.5초 (60fps * 0.5)
...
def activate_grenade():
    ...
    grenade_throw_timer = HALF_SECOND_FRAMES  # 0.5초 (60fps * 0.5)
    # 목표 지점 미리 계산 (투척 모션 중에 사용)
...
def activate_molotov():
    ...
    molotov_throw_timer = HALF_SECOND_FRAMES  # 0.5초 (60fps * 0.5)
    # 목표 지점 미리 계산 (투척 모션 중에 사용)
...
def draw_objects():
    ...
    if mega_smashing_active:
        ...
        # 검은 테두리 효과
        pygame.draw.circle(overlay, (DEFAULT_RADIUS, 0, 40, 150),
...
def show_start_screen():
    ...
    for _ in range(15):  # 파티클 수 대폭 감소
        neon_particles.append({
            ...
            "color": random.choice([CYAN, (255, 0, 128), (128, 255, 0)]),  # 색상 수 감소
    ...
    while True:
        ...
        # 은하수 중심선
        for x in range(WIDTH):
            galaxy_y = HEIGHT // 3 + int(LARGE_SIZE * math.sin(x * 0.01 + animation_timer * 0.02))
        ...
        for i in range(5):
            nebula_x = 100 + i * 100
            nebula_y = HEIGHT // 3 + int(HALF_SECOND_FRAMES * math.sin(i + animation_timer * 0.015))
        ...
        # 도시 불빛 (자전하며 나타났다 사라짐)
        for angle in range(0, FULL_ROTATION, 15):
            ...
            if visibility > 0:
                ...
                city_color = random.choice([YELLOW, (0, 255, 255), (255, 0, 255)])
                alpha = int(visibility * 255)
                pygame.draw.circle(planet3_surf, (*city_color, alpha), 
                                 (int(city_x), int(city_y)), 2)
        # 대기층 글로우
        glow_surf = pygame.Surface((planet3_radius * 2 + DEFAULT_RADIUS, planet3_radius * 2 + DEFAULT_RADIUS), pygame.SRCALPHA)
        ...
        for i in range(moon_radius, 0, -1):
            ...
            alpha = min(255, max(0, int(DEFAULT_ALPHA * ratio)))
        ...
        for i in range(12):
            debris_angle = math.radians(i * HALF_SECOND_FRAMES + animation_timer * 0.5)
        ...
        title_text = font_title.render("메인 메뉴", True, WHITE)
        ...
        for i, (option, display_option) in enumerate(zip(all_options, display_options)):
            ...
            if i == selected:
                # 선택된 항목 배경
                option_width = 300
                option_height = LARGE_SIZE
                option_x = WIDTH // 2 - option_width // 2
                option_y = 220 + i * 60
                # 심플한 선택 표시
                option_bg = pygame.Surface((option_width, option_height), pygame.SRCALPHA)
                pygame.draw.rect(option_bg, (DEFAULT_RADIUS, 30, 40, 100), (0, 0, option_width, option_height), border_radius=10)
    ...
    if elapsed_time >= 2.0:
        ...
        if not hasattr(handle_mega_smashing_trajectory, 'boosted'):
            ...
            if boss_x < WIDTH // 2:
                ...
                teleport_x = WIDTH // 2 + int((WIDTH // 2 - LARGE_SIZE) * 0.8)  # 중앙에서 끝까지 거리의 80%
            else:
            ...
            # 폭발 시 속도를 2.8~4배 랜덤으로 증가
            speed_multiplier = random.uniform(2.8, 4.0)
...
def handle_ball():
    ...
    if boss_hitbox_expanded.colliderect(BALL) and boss_collision_cooldown <= 0 and not is_waiting_for_serve:
        ...
        # 🐊 스피드디펜스 상태에서 보스가 공을 칠 때 속도 조절
        if current_stage == 2 and speed_defense_active:
            current_ball_speed = math.hypot(ball_vel[0], ball_vel[1])
            # 공 속도가 너무 빠르면 (기본 속도의 2배 이상) 속도를 30% 감소
            if current_ball_speed > BALL_BASE_SPEED * 2.0:
                speed_reduction = 0.7  # 30% 감소 (70%로 조정)
                ball_vel[0] *= speed_reduction
                ball_vel[1] *= speed_reduction
                print(f"🐊 스피드디펜스 반격! 빠른 공 속도 30% 감소: {current_ball_speed:.1f} → {current_ball_speed * speed_reduction:.1f}")
...
def main(stage_num, new_boss_mode=False):
    ...
    while running:
        ...
        if screen_shake_offset_x != 0 or screen_shake_offset_y != 0:
            ...
            # UI 요소들은 화면 흔들림과 별도로 그리기 (게이지, 아이템 등)
            if not new_boss_mode_active:
            ...
            if current_stage == 1 and border_flash_active:
                ...
                if border_flash_timer > 0:
                    ...
                    border_flash_active = False
            # UI 요소들 그리기 (화면 흔들림이 없을 때도 그려야 함)
            if not new_boss_mode_active:
                # 장인 아이템이 있으면 쿨타임 10% 감소 (0.8초), 쿨타임 아이템이 있으면 2.4초 추가 감소
                cooldown_reduction = 800 if master_obtained else 0
                cooldown_reduction += 2400 if cooltime_obtained else 0
