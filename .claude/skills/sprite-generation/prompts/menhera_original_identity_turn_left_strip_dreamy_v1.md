# Menhera Original-Identity Turn Left Strip Dreamy V1

Use this when full-sheet turn generation collapsed mid-angles and the
fallback ladder has moved to strip generation. This file covers the LEFT strip
only.
...
LEFT turn strip for real-stage 3 boss Menheragirl (code current_stage == 3).

This is TURN STRIP FALLBACK, LEFT SIDE ONLY.
...
- Produce a clean LEFT strip with true angle separation
...
- Frame order left-to-right: -30, -45, -60, -75, -90
...
- -30 must still be turned, not fake frontal
- -45 must be a true mid-angle, clearly different from -30 and -60
...
- -90 must be clear full left profile
- These five frames must interpolate smoothly in 15-degree steps
- Do NOT return fake-front copies for -30/-45/-60
...
- Reject if -30 / -45 / -60 collapse into fake frontal copies
...
- First generate:
    .tmp/menhera_original_identity_turn_left_strip_dreamy_v1.jpeg
- Then run:
    py .claude/skills/sprite-generation/remove_bg.py \
        .tmp/menhera_original_identity_turn_left_strip_dreamy_v1.jpeg \
        .tmp/menhera_original_identity_turn_left_strip_dreamy_v1.png
