...
**아이템, 스킬, 투사체 등 모든 좌표 작업 시 반드시 이 규격을 따를 것!**
...
│  ┌──────────┬─────────────────────────────────────────────┬──────────┐  │
│  │          │                                             │          │  │
│  │  LEFT    │         GAME PLAY AREA: 600 x 750           │  RIGHT   │  │
│  │  PILLAR  │                                             │  PILLAR  │  │
│  │  80px    │                                             │  80px    │  │
│  │          │                                             │          │  │
│  └──────────┴─────────────────────────────────────────────┴──────────┘  │
...
| 상수명 | 값 | 설명 |
|--------|-----|------|
| `INTERNAL_WIDTH` | **760px** | 전체 내부 해상도 너비 (필러 포함) |
| `INTERNAL_HEIGHT` | **750px** | 전체 내부 해상도 높이 |
| `GAME_PLAY_WIDTH` | **600px** | 실제 게임 플레이 영역 너비 |
| `GAME_PLAY_HEIGHT` | **750px** | 실제 게임 플레이 영역 높이 |
| `PILLAR_UI_WIDTH` | **80px** | 좌우 필러 UI 박스 각각의 너비 |
| `GAME_AREA_OFFSET_X` | **80px** | 게임 영역 시작 X 오프셋 (= 왼쪽 필러 너비) |
...
|------|-------------|------|
| 왼쪽 필러 | `0 ~ 79` | UI 전용 영역 |
| **게임 플레이 영역** | `80 ~ 679` | 실제 게임이 진행되는 영역 |
| 오른쪽 필러 | `680 ~ 759` | UI 전용 영역 |
| 게임 영역 중앙 X | `380` | 80 + (600 / 2) |

### 투사체/아이템 좌표 작업 시 주의사항
```python
# ❌ 잘못된 예 - 구버전 600x750 기준
BOSS_AREA_Y = 250  # 틀림! 보스가 여기에 없음

# ✅ 올바른 예 - 현재 규격 기준
BOSS_AREA_Y = 120  # 보스 진영 하단 경계
PLAYER_AREA_Y = 630  # 플레이어 진영 상단 경계

# 게임 영역 내 좌표 계산
game_center_x = GAME_AREA_OFFSET_X + GAME_PLAY_WIDTH // 2  # = 380
boss_bottom = BOSS_Y + BOSS_HEIGHT  # = 65 (보스 히트박스 하단)
player_top = HEIGHT - 40  # = 710 (플레이어 패들 상단)
```
