...
상한/렌더 컷오프**를 같이 안 올리면 같은 버스트가 잘린다. ⚠️상수가 ×1.0 **항등원으로
평탄화**돼 있으면 코드가 살아 있어도 미포팅이며, 차이-계약 상수는 캡과 한 세트 —
양쪽이 같은 캡에 포화하면 차이가 무에러로 지워진다(천뢰격 무콤보 페널티). 씰은
미발동 경로 + 버프 상태를 각각 레그로, 차이-계약은 **순항 OUTCOME**으로.
Full rule: `docs/godot_runtime_traps.md`.
