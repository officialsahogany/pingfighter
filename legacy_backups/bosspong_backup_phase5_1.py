...
# import input_manager  # 🔧 사용하지 않는 모듈 임포트 제거
...
pygame.display.set_caption("핑파이터")
...
special_gauge_max = get_max_gauge()  # 아카데미 스킬 적용된 최대값
special_ready = False

# 게이지 애니메이션 변수들
displayed_gauge = 0         # 화면에 표시되는 게이지 (부드러운 애니메이션용)
gauge_animation_speed = 0.15  # 게이지 애니메이션 속도 (0.1 = 빠름, 0.2 = 느림)
displayed_boss_gauge = 0    # 멘헤라걸 게이지 표시용 (부드러운 애니메이션)
boss_gauge_animation_speed = 0.15  # 멘헤라걸 게이지 애니메이션 속도

# 홍련폭염 관련 변수
hongryun_hit_count = 0        # 화염탄에 맞은 횟수 (0~4)
hongryun_ready = False        # 홍련폭염 발동 준비 여부
HONGRYUN_MAX_HITS = 3         # 게이지 최대치 (3번 맞으면 홍련폭염 준비)
...
# 퍼펙트 타이밍 감지
perfect_timing_window = 6            # 퍼펙트 타이밍 윈도우 (프레임 수) - 선입력 방지를 위해 감소 (10 → 6)
perfect_timing_active = False        # 현재 퍼펙트 타이밍 감지 중
perfect_timing_frame_count = 0       # 퍼펙트 타이밍 프레임 카운터
perfect_direction = None             # 퍼펙트 샷 방향 (왼쪽: -1, 오른쪽: 1)

# 키 입력 감지
space_just_pressed = False           # 스페이스바가 이번 프레임에 눌렸는지
last_space_state = False             # 이전 프레임 스페이스바 상태
left_just_pressed = False            # 왼쪽 방향키가 이번 프레임에 눌렸는지
last_left_state = False              # 이전 프레임 왼쪽 방향키 상태
right_just_pressed = False           # 오른쪽 방향키가 이번 프레임에 눌렸는지
last_right_state = False             # 이전 프레임 오른쪽 방향키 상태
...
try:
    ...
    # 사이버펑크 스타일 스피드부츠 아이콘
    speedboots_icon = pygame.Surface((32, 32), pygame.SRCALPHA)
    # 부츠 모양
...
try:
    ...
    # 파일이 없으면 기본 아이콘 생성
    gravitybelt_icon = pygame.Surface((32, 32), pygame.SRCALPHA)
...
try:
    ...
    # 파일이 없으면 기본 아이콘 생성
    speedgear_icon = pygame.Surface((32, 32), pygame.SRCALPHA)
...
try:
    ...
    # 파일이 없으면 기본 아이콘 생성
    aipill_icon = pygame.Surface((32, 32), pygame.SRCALPHA)
...
try:
    ...
    # 파일이 없으면 기본 아이콘 생성
    revival_icon = pygame.Surface((32, 32), pygame.SRCALPHA)
...
try:
    ...
    # 파일이 없으면 기본 아이콘 생성
    master_icon = pygame.Surface((32, 32), pygame.SRCALPHA)
...
try:
    ...
    # 파일이 없으면 기본 아이콘 생성
    cooltime_icon = pygame.Surface((32, 32), pygame.SRCALPHA)
...
def show_score(player_score, ai_score):
    ...
    font_score = pygame.font.Font("NanumSquareEB.ttf", 140)
    font_subtitle = pygame.font.Font("NanumSquareR.ttf", 28)
    font_vs = pygame.font.Font("NanumSquareB.ttf", 36)
    ...
    for _ in range(90):  # 1.5초간 표시
        ...
        # 움직이는 네온 글로우 효과 (애니메이션)
        for i in range(4):
            glow_alpha = int(60 * (0.5 + 0.5 * math.sin(animation_timer * 0.1 + i)))
            glow_surface = pygame.Surface((board_width, board_height), pygame.SRCALPHA)
            glow_x = 100 + i * 150 + int(math.sin(animation_timer * 0.2 + i) * 15)
            glow_y = 50 + i * 80 + int(math.sin(animation_timer * 0.3 + i) * 10)
            glow_radius = 30 + int(math.sin(animation_timer * 0.2 + i) * 10)
            pygame.draw.circle(glow_surface, (*accent_color, glow_alpha), (glow_x, glow_y), glow_radius)
            board_surface.blit(glow_surface, (0, 0))
...
def draw_tears():
    if current_stage == 3 and tears_active:
        ...
        for x, y, speed, prev_y in falling_tears:
            if -50 <= x <= WIDTH + 50 and -50 <= y <= HEIGHT + 50:  # 화면 경계 체크
...
def draw_dash_spirit_lasers():
    ...
    for laser in dash_spirit_lasers:
        ...
        if laser_length > 0:
            ...
            # 🌟 광선검 특유의 에너지 파티클 효과
            num_particles = int(laser_length // 15)  # 길이에 비례한 파티클 수
            for i in range(num_particles):
                # 레이저 라인을 따라 무작위 위치에 파티클 생성
                t = random.random()
                particle_x = start_x + t * (end_x - start_x)
                particle_y = start_y + t * (end_y - start_y)
                
                # 파티클 위치에 약간의 랜덤 오프셋 추가
                particle_x += random.uniform(-8, 8)
                particle_y += random.uniform(-8, 8)
                
                # 파티클 크기와 밝기 (랜덤)
                particle_size = random.randint(1, 3)
                particle_alpha = random.randint(laser['alpha']//4, laser['alpha']//2)
                
                # 하늘색 파티클
                pygame.draw.circle(laser_surface, (*DASH_SPIRIT_LASER_COLOR, particle_alpha),
                                 (int(particle_x), int(particle_y)), particle_size)
                
                # 일부 파티클은 흰색으로 (밝은 효과)
                if random.random() < 0.3:
                    pygame.draw.circle(laser_surface, (255, 255, 255, particle_alpha//2),
                                     (int(particle_x), int(particle_y)), particle_size//2)
            
            # 🌟 광선검 끝부분 광선 확산 효과
            if laser_length > 20:  # 충분히 긴 레이저만
                # 끝부분에서 확산되는 광선들
                for i in range(5):
                    spread_angle = random.uniform(-0.3, 0.3)  # 약간의 각도 변화
                    spread_length = random.uniform(10, 25)
                    
                    # 확산 광선의 끝점 계산
                    import math
                    base_angle = math.atan2(end_y - start_y, end_x - start_x)
                    spread_end_x = end_x + math.cos(base_angle + spread_angle) * spread_length
                    spread_end_y = end_y + math.sin(base_angle + spread_angle) * spread_length
                    
                    # 확산 광선 그리기 (더 얇고 투명하게)
                    spread_alpha = laser['alpha'] // 6
                    spread_width = max(1, DASH_SPIRIT_LASER_WIDTH // 4)
                    pygame.draw.line(laser_surface, (*DASH_SPIRIT_LASER_COLOR, spread_alpha),
                                   (int(end_x), int(end_y)), 
                                   (int(spread_end_x), int(spread_end_y)), spread_width)
...
def draw_balloons():
    ...
    for balloon in balloons:
        ...
        pygame.draw.line(SCREEN, (255, 255, 255), (int(x), int(y)), (arrow_x, arrow_y), 2)
...
def draw_medal_score():
    font_small = pygame.font.Font("NanumSquareR.ttf", 24)
    medal_text = font_small.render(f"메달: {medal_score}", True, (255, 255, 0))
    SCREEN.blit(medal_text, (10, 10))
...
def draw_pachinko():
    ...
    pygame.draw.rect(SCREEN, (25, 15, 5), main_container)
    pygame.draw.rect(SCREEN, (255, 215, 0), main_container, 4)
    
    # 내부 컨테이너
    inner_container = pygame.Rect(container_x + 20, container_y + 20, container_width - 40, container_height - 40)
    pygame.draw.rect(SCREEN, (15, 10, 5), inner_container)
    pygame.draw.rect(SCREEN, (255, 200, 0), inner_container, 3)
    
    # 제목 영역
    title_area = pygame.Rect(container_x + 30, container_y + 30, container_width - 60, 80)
    pygame.draw.rect(SCREEN, (30, 20, 10), title_area)
    pygame.draw.rect(SCREEN, (255, 215, 0), title_area, 3)
    ...
    pygame.draw.rect(SCREEN, (20, 15, 10), slot_bg_rect)
    pygame.draw.rect(SCREEN, (255, 215, 0), slot_bg_rect, 3)
    ...
    for reel_idx in range(3):
        ...
        pygame.draw.rect(SCREEN, (10, 8, 5), reel_rect)
        pygame.draw.rect(SCREEN, (255, 200, 0), reel_rect, 2)
        ...
        for i in range(visible_items):
            ...
            if item_y + item_size > reel_y and item_y < reel_y + slot_height:
                ...
                if item_idx < len(slot_reels[reel_idx]):
                    ...
                    pygame.draw.rect(SCREEN, (25, 20, 15), item_rect)
                    pygame.draw.rect(SCREEN, (255, 180, 0), item_rect, 2)
                    ...
                    if "icon" in item and item["icon"]:
                        ...
                        pygame.draw.circle(SCREEN, item["color"], (center_x, center_y), 20)
        ...
        pygame.draw.rect(SCREEN, (255, 215, 0), center_rect, 4)
    ...
    if pachinko_phase == 3 and slot_final_result:
        ...
        for i in range(result_height):
            ...
            g = int(30 + ratio * 80)
            b = int(10 + ratio * 40)
            pygame.draw.line(SCREEN, (r, g, b), (result_x, result_y + i), (result_x + result_width, result_y + i))
        
        pygame.draw.rect(SCREEN, (255, 215, 0), (result_x, result_y, result_width, result_height), 4)
        ...
        # 어두운 배경
        pygame.draw.rect(SCREEN, (30, 20, 10), (result_x, result_y, result_width, result_height))
        pygame.draw.rect(SCREEN, (100, 100, 100), (result_x, result_y, result_width, result_height), 3)
...
def draw_item_obtained_effect():
    ...
    # 메인 원형 배경 - 크기 50% 감소
    pygame.draw.circle(SCREEN, (30, 40, 60, 150), (effect_x, effect_y), 30)  # 더 투명하게
    pygame.draw.circle(SCREEN, (100, 150, 255, 150), (effect_x, effect_y), 30, 2)  # 더 투명하게
...
def draw_player_gauge():
    ...
    pygame.draw.polygon(SCREEN, (80, 90, 100), hex_top)
    pygame.draw.polygon(SCREEN, (180, 190, 200), hex_top, 2)
    ...
    pygame.draw.polygon(SCREEN, (80, 90, 100), hex_bottom)
    pygame.draw.polygon(SCREEN, (180, 190, 200), hex_bottom, 2)
    
    # 🎯 메인 프레임 배경
    # 어두운 메탈릭 배경
    main_rect = pygame.Rect(gauge_x - 2, gauge_y, gauge_width + 4, gauge_height)
    pygame.draw.rect(SCREEN, (25, 28, 35), main_rect)
    
    # 내부 홈 (음각 효과)
    inner_rect = pygame.Rect(gauge_x, gauge_y + 2, gauge_width, gauge_height - 4)
    pygame.draw.rect(SCREEN, (15, 18, 25), inner_rect)
    ...
    if displayed_gauge >= 350:
        ...
        # 상단 코너
        pygame.draw.circle(SCREEN, corner_color, (gauge_x, gauge_y), 3)
        pygame.draw.circle(SCREEN, corner_color, (gauge_x + gauge_width, gauge_y), 3)
        # 하단 코너
        pygame.draw.circle(SCREEN, corner_color, (gauge_x, gauge_y + gauge_height), 3)
        pygame.draw.circle(SCREEN, corner_color, (gauge_x + gauge_width, gauge_y + gauge_height), 3)
    ...
    pygame.draw.rect(SCREEN, (100, 100, 100), text_bg_rect, 1, border_radius=3)
                ...
                if fill_height > 6:  # 최소 높이 확인 (여유 있게)
                    ...
                    if actual_fill_height > 0:
                        ...
                        if actual_fill_top <= scan_y < actual_fill_bottom:
                            ...
                            pygame.draw.line(SCREEN, scan_color,
                                           (gauge_x + 2, scan_y),
    ...
    for i in range(max_tokens):
        ...
        if not is_available and 'charge_progress' in locals() and 'is_charging' in locals() and is_charging and charge_progress > 0:
            # 배경 원 (비어있는 상태)
            pygame.draw.circle(SCREEN, (40, 20, 20), (int(token_x), int(token_y)), token_radius)
            ...
            # 테두리
            pygame.draw.circle(SCREEN, (100, 50, 50), (int(token_x), int(token_y)), token_radius, 1)
            ...
            # 메인 토큰 원 (기본 상태)
            pygame.draw.circle(SCREEN, token_color, (int(token_x), int(token_y)), token_radius)
            
            # 토큰 테두리 (입체감)
            border_color = (200, 200, 200) if is_available else (80, 80, 80)
            pygame.draw.circle(SCREEN, border_color, (int(token_x), int(token_y)), token_radius, 1)
...
def draw_objects():
    ...
    def draw_rect_with_shake(color, rect, width=0):
        ...
        pygame.draw.rect(SCREEN, color, shaken_rect, width)
    
    def draw_circle_with_shake(color, pos, radius, width=0):
        """  화면 흔들림을 적용하여 원 그리기"""
        pygame.draw.circle(SCREEN, color, (int(pos[0] + total_offset_x), int(pos[1] + total_offset_y)), radius, width)
    ...
    if current_stage == 6:
        ...
        # 안테나 기둥
        pygame.draw.rect(SCREEN, (110, 120, 130), 
                        (antenna_left_x - 2, antenna_left_y - 10, 4, 20))
        # 안테나 베이스
        pygame.draw.circle(SCREEN, (140, 150, 160), 
                          (antenna_left_x, antenna_left_y), 5)
        # 안테나 끝 발광체
        antenna_tip_y = antenna_left_y - 15
        pygame.draw.circle(SCREEN, (200, 210, 220), 
    ...
    if DEBUG_SHOW_HITBOX:
        ...
        if current_stage == 6:
            ...
            # 히트박스 테두리 (실선)
            pygame.draw.rect(SCREEN, (255, 0, 0), boss_hitbox_expanded, 2)
        
        # 원본 보스 패들 크기 표시 (파란색) - 모든 스테이지
        pygame.draw.rect(SCREEN, (0, 0, 255), BOSS, 2)
    ...
    if boss_stunned_timer > 0:
        ...
        for i in range(num_stars):
            ...
            pygame.draw.polygon(SCREEN, (255, 200, 0), star_points, 1)  # 얇은 테두리
    ...
    for grenade in grenades:
        ...
        try:
            ...
            # 기본 원형 그리기
            pygame.draw.circle(SCREEN, (80, 100, 80), (int(grenade["x"]), int(grenade["y"])), 12)
        ...
        if smoke_grenade["arrived"]:
            ...
            if "trail" in smoke_grenade:
                for i, pos in enumerate(smoke_grenade["trail"]):
                    ...
                    pygame.draw.circle(SCREEN, (150, 150, 150, alpha), 
    ...
    for flare in flares:
        ...
        if flare.get("arrived", False) and not flare["exploded"]:
            ...
            try:
                ...
                # 기본 원형 그리기
                pygame.draw.circle(SCREEN, (255, 255, 200), (int(flare["x"]), int(flare["y"])), 10)
    ...
    for molotov in molotovs:
        ...
        try:
            ...
            # 기본 원형 그리기
            pygame.draw.circle(SCREEN, (255, 100, 0), (int(molotov["x"]), int(molotov["y"])), 10)
    ...
    for wall in walls:
        ...
        pygame.draw.rect(SCREEN, (255, 255, 255), wall_rect, 2)  # 흰색 테두리
        ...
        if wall_crack_level > 0:
            ...
            for i in range(crack_count):
                ...
                pygame.draw.line(SCREEN, crack_color, (start_x, start_y), (end_x, end_y), crack_thickness)
    ...
    if (power_smashing_parabola_active or power_smashing_freeze_active) and not mega_smashing_active:
        ...
        for i, trail in enumerate(power_smashing_trails):
            ...
            if i < len(power_smashing_trails) - 1:
                ...
                pygame.draw.line(SCREEN, lightning_color[:3], (x, y), (next_x, next_y), max(1, int(3 * color_intensity)))
                ...
                for _ in range(2):
                    ...
                    pygame.draw.line(SCREEN, spark_color[:3], (x, y), 
        ...
        for particle in power_smashing_particles:
            ...
            if particle['life'] > 0:
                ...
                if particle.get('type') == 'spark':
                    ...
                    pygame.draw.line(SCREEN, (*particle['color'], alpha), 
                    ...
                    if len(meteor['trail']) > 1:
                        for i in range(len(meteor['trail']) - 1):
                            ...
                            if trail_width > 0 and trail_alpha > 10:
                                ...
                                # 외곽 글로우 (더 밝은 색)
                                pygame.draw.line(SCREEN, (r, g, b, min(255, trail_alpha // 2)), 
                                               (x1, y1), (x2, y2), max(1, trail_width + 2))
                                # 코어 (황금색 + 무지개)
                                core_r = min(255, (255 + r) // 2)
                                core_g = min(255, (215 + g) // 2)
                                core_b = min(255, b)
                                pygame.draw.line(SCREEN, (core_r, core_g, core_b, min(255, trail_alpha)), 
    ...
    if new_boss_mode_active and lightning_strike_active:
        ...
        for i in range(8):  # 8개의 번개 볼트
            ...
            # 번개 볼트 그리기 (중심에서 외곽으로)
            pygame.draw.line(SCREEN, lightning_color, ball_rect.center, (int(bolt_x), int(bolt_y)), 3)
            
            # 번개 끝에 작은 원 효과
            pygame.draw.circle(SCREEN, lightning_color, (int(bolt_x), int(bolt_y)), 4)
    ...
    if new_boss_mode_active and fire_charge_active:
        ...
        for i in range(12):  # 12개의 화염 파티클
            ...
            pygame.draw.circle(SCREEN, flame_color, (int(flame_x), int(flame_y)), flame_size)
            
            # 내부에 밝은 중심 추가
            inner_size = max(2, flame_size - 2)
            pygame.draw.circle(SCREEN, (255, 255, 200), (int(flame_x), int(flame_y)), inner_size)
    ...
    if flame_trail_active:
        ...
        if len(flame_trail_positions) > 2:
            ...
            if flame_trail_positions:
                ...
                pygame.draw.circle(SCREEN, (eye_glow, eye_glow, 0),
                ...
                pygame.draw.line(SCREEN, horn_color,
    ...
    for p in tear_particles:
        ...
        if alpha > 0:
            ...
            pygame.draw.circle(SCREEN, color, (int(x + size//2), int(y + size//2)), size//2)
    ...
    if current_stage == 6:
        ...
        for missile in turret_missiles:
            ...
            pygame.draw.circle(SCREEN, (200, 50, 50), (int(warhead_x), int(warhead_y)), 2)
            pygame.draw.circle(SCREEN, (255, 100, 100), (int(warhead_x), int(warhead_y)), 1)
            ...
            if beam_time < laser_beam_duration:
                ...
                for spiral_idx in range(num_spirals):
                    ...
                    for i in range(points_per_spiral):
                        ...
                        pygame.draw.circle(SCREEN, wave_color, (int(spiral_x), int(spiral_y)), wave_size)
                        ...
                        if i > 0:
                            ...
                            pygame.draw.line(SCREEN, (*wave_color, wave_alpha//2), 
                ...
                for i in range(5):
                    ...
                    pygame.draw.line(SCREEN, (*glow_color, glow_alpha),
        ...
        if player_stunned:
            if current_time > player_stun_end_time:
                ...
                for _ in range(5):
                    ...
                    pygame.draw.circle(SCREEN, (150, 200, 255), (spark_x, spark_y), spark_size)
                    pygame.draw.circle(SCREEN, (255, 255, 255), (spark_x, spark_y), spark_size - 2)
        ...
        if shield_antenna_active:
            ...
            if shield_elapsed < shield_duration:
                ...
                for _ in range(5):
                    ...
                    pygame.draw.circle(SCREEN, (255, 255, 255), (particle_x, particle_y), 2)
                ...
                if angle_in_range and distance_in_range and ball_vel[1] < 0:
                    ...
                    for _ in range(10):
                        ...
                        pygame.draw.circle(SCREEN, (255, 255, 255), (spark_x, spark_y), random.randint(1, 3))
            else:
                # 쉴드 비활성화
        ...
        if health_percent <= 30:
            ...
            if hangar_door_open:
                ...
                if door_elapsed < 1000:  # 1초간 열림
                    ...
                    pygame.draw.rect(SCREEN, (40, 40, 50), (door_x, door_y, door_width, 15))
                    pygame.draw.rect(SCREEN, (20, 20, 30), (door_x, door_y, door_width, 15), 2)
                    # 내부 빛
                    pygame.draw.rect(SCREEN, (100, 150, 200), (door_x + 2, door_y + 2, door_width - 4, 11))
                else:
                    # 문 완전 열림
                    door_x = BOSS.centerx - 20
                    door_y = BOSS.bottom - 10
                    pygame.draw.rect(SCREEN, (40, 40, 50), (door_x, door_y, 40, 15))
                    pygame.draw.rect(SCREEN, (100, 150, 200), (door_x + 2, door_y + 2, 36, 11))
            ...
            for interceptor in interceptors[:]:
                ...
                pygame.draw.polygon(SCREEN, (30, 30, 40), shadow_points)
                
                # 본체
                pygame.draw.polygon(SCREEN, (100, 120, 140), points)
                pygame.draw.polygon(SCREEN, (150, 170, 190), points, 2)
                
                # 에너지 코어
                pygame.draw.circle(SCREEN, (100 + glow, 150 + glow, 255), 
    ...
    if current_stage == 3:
        ...
        # 달 배경 (금색 테두리)
        pygame.draw.circle(SCREEN, (255, 215, 0), (moon_center_x, moon_center_y), moon_radius + 2)
        pygame.draw.circle(SCREEN, (255, 255, 200), (moon_center_x, moon_center_y), moon_radius)
        
        # 반달 효과 (오른쪽을 가림)
        pygame.draw.circle(SCREEN, (20, 20, 40), (moon_center_x + 5, moon_center_y), moon_radius - 2)
        ...
        # 왼쪽 별
        pygame.draw.circle(SCREEN, star_color, (moon_center_x - star_offset, moon_center_y), star_size)
        # 오른쪽 별
        pygame.draw.circle(SCREEN, star_color, (moon_center_x + star_offset, moon_center_y - 5), star_size - 1)
        # 아래 별
        pygame.draw.circle(SCREEN, star_color, (moon_center_x + 10, moon_center_y + 15), star_size - 2)
        ...
        pygame.draw.polygon(SCREEN, (255, 215, 0), gem_points, 3)
        ...
        pygame.draw.polygon(SCREEN, (40, 20, 40), inner_gem_points)
        ...
        # 리본 매듭
        pygame.draw.circle(SCREEN, (255, 105, 180), (wand_x, ribbon_y + ribbon_height//2), 5)
        ...
        if boss_special_ready:
            if (time_now // 250) % 2 == 0:
                ...
                for radius in range(30, 10, -5):
                    ...
                    pygame.draw.circle(SCREEN, glow_color[:3], (wand_x, gem_y + gem_height//2), radius, 2)
        ...
        pygame.draw.rect(SCREEN, (100, 100, 100), boss_text_bg_rect, 1, border_radius=3)
    ...
    if current_stage == 5:
        ...
        for i in range(HONGRYUN_MAX_HITS):
            ...
            pygame.draw.polygon(SCREEN, dragon_color, dragon_head)
            
            # 용의 눈 (활성화시 빛남) - 크기 조정
            if i < hongryun_hit_count:
                eye_glow = int(abs(math.sin(time_now * 0.005 + i)) * 100 + 155)
                pygame.draw.circle(SCREEN, (eye_glow, eye_glow, 0), (orb_x - 2, start_y - orb_radius - 7), 1)
                pygame.draw.circle(SCREEN, (eye_glow, eye_glow, 0), (orb_x + 2, start_y - orb_radius - 7), 1)
            ...
            if i < hongryun_hit_count:
                ...
                for r in range(3):
                    alpha = 60 - r * 20
                    pygame.draw.circle(SCREEN, (*halo_color, alpha), 
                ...
                for layer in range(5):
                    ...
                    if layer_radius > 0:
                        ...
                        pygame.draw.circle(SCREEN, color, (orb_x, start_y), layer_radius)
                
                # 내부 불꽃 효과 - 붉은 핵
                flame_offset = math.sin(time_now * 0.008 + i) * 2
                pygame.draw.circle(SCREEN, (255, 100, 80), 
                ...
                if random.random() < 0.3:  # 30% 확률로 파티클 생성
                    for _ in range(2):
                        ...
                        pygame.draw.circle(SCREEN, (255, 80, 40), 
                                         (particle_x, particle_y), particle_size)
                
            else:
                # 비활성 구슬 - 어둡고 투명한 느낌
                # 외곽 테두리
                pygame.draw.circle(SCREEN, (50, 50, 50), (orb_x, start_y), orb_radius, 2)
...
def show_victory_screen(stage_cleared, reward):
    ...
    while True:
        ...
        pygame.draw.line(SCREEN, line_color, (WIDTH // 2 - 120, line_y), (WIDTH // 2 + 120, line_y), 2)
        pygame.draw.circle(SCREEN, line_color, (WIDTH // 2 - 120, line_y), 4)
        pygame.draw.circle(SCREEN, line_color, (WIDTH // 2 + 120, line_y), 4)

        # 정보 패널 배경 (중앙으로 이동)
        info_panel_rect = pygame.Rect(WIDTH // 2 - 200, 220, 400, 150)
        pygame.draw.rect(SCREEN, (30, 30, 60, 180), info_panel_rect)
        pygame.draw.rect(SCREEN, (100, 150, 255), info_panel_rect, 2)
        ...
        try:
            ...
            pygame.draw.circle(SCREEN, (255, 215, 0), (WIDTH // 2 - 55, medal_y), 17)
            pygame.draw.circle(SCREEN, (200, 170, 0), (WIDTH // 2 - 55, medal_y), 15, 2)
            ...
            if is_selected:
                ...
                for i in range(rect.height):
                    ...
                    pygame.draw.line(SCREEN, color, 
                                   ...
                                   (rect.right, rect.top + i))
                # 일반 버튼 외곽선
                pygame.draw.rect(SCREEN, (120, 160, 220), rect, 2)
    ...
    while True:
        ...
        if current_char["unlocked"]:
            ...
            for i, (stat_name, stat_value) in enumerate(current_char["stats"].items()):
                ...
                # 홀로그램 배경 바
                pygame.draw.rect(SCREEN, (10, 20, 30), (bar_x, bar_y, bar_width, bar_height), border_radius=3)
                pygame.draw.rect(SCREEN, (*glow_color, 80), (bar_x, bar_y, bar_width, bar_height), 1, border_radius=3)
            ...
            for i in range(3):  # 3개의 스탯
                ...
                # 배경 바만 표시
                pygame.draw.rect(SCREEN, (60, 60, 60), (bar_x, bar_y, bar_width, bar_height), border_radius=4)
        ...
        #             pygame.draw.circle(SCREEN, color, (indicator_x, indicator_y), 6)
        #             pygame.draw.circle(SCREEN, (*color, 150), (indicator_x, indicator_y), 8, 1)
        #         else:
        #             # 잠긴 캐릭터 - 매우 어두운 링
        #             pygame.draw.circle(SCREEN, (50, 50, 50), (indicator_x, indicator_y), 4, 2)
    ...
    while True:
        ...
        for i, difficulty in enumerate(difficulties):
            ...
            if i == selected:
                ...
                # 내부 테두리
                pygame.draw.rect(SCREEN, (*difficulty["color"], 100),
            ...
            if difficulty["id"] == "junior":
                ...
                # 줄기
                pygame.draw.line(SCREEN, (100, 200, 100), (emblem_x, emblem_y + 20), (emblem_x, emblem_y - 5), 3)
                # 잎사귀들 (회전 효과 적용)
                ...
                # 중심 새싹
                pygame.draw.circle(SCREEN, (150, 255, 150), (emblem_x, emblem_y - 10), 8)
                pygame.draw.circle(SCREEN, (100, 255, 100), (emblem_x, emblem_y - 10), 6)
                pygame.draw.circle(SCREEN, (200, 255, 200), (emblem_x - 2, emblem_y - 12), 2)
                # 빛나는 효과
                for r in range(3):
                    alpha = 50 - r * 15
                    pygame.draw.circle(SCREEN, (150, 255, 150, alpha), (emblem_x, emblem_y - 10), 12 + r * 3, 1)
                ...
                for angle in range(0, 360, 60):
                    ...
                    pygame.draw.line(SCREEN, (255, 255, 150), (emblem_x, emblem_y), (spark_x, spark_y), 1)
                # 중심 광원
                pygame.draw.circle(SCREEN, (255, 255, 200), (emblem_x, emblem_y), 5)
                ...
                # 중심 빛
                pygame.draw.circle(SCREEN, (255, 200, 255), (emblem_x, emblem_y), 3)
                # 광채 효과 (회전과 무관하게 유지)
                for angle in range(45, 360, 90):
                    ...
                    pygame.draw.line(SCREEN, (255, 200, 255, 100), (emblem_x, emblem_y), (ray_x, ray_y), 1)
                ...
                if abs(scale_x) > 0.2:
                    pygame.draw.line(SCREEN, (255, 215, 0), crown_base[0], crown_base[1], 3)
                ...
                if abs(scale_x) > 0.2:
                    for j, (px, py, height) in enumerate(peaks):
                        ...
                        pygame.draw.circle(SCREEN, gem_colors[j], (px, py + 5), 3)
                        if scale_x > 0:
                            pygame.draw.circle(SCREEN, (255, 255, 255), (px - 1, py + 4), 1)
                # 중앙 큰 보석
                pygame.draw.circle(SCREEN, (255, 100, 255), (emblem_x, emblem_y), 5)
                pygame.draw.circle(SCREEN, (255, 200, 255), (emblem_x, emblem_y), 3)
                pygame.draw.circle(SCREEN, (255, 255, 255), (emblem_x - 1, emblem_y - 1), 1)
                # 황금빛 후광
                for r in range(3):
                    pygame.draw.circle(SCREEN, (255, 215, 0, 30 - r*10), (emblem_x, emblem_y), 30 + r*5, 1)
        ...
        pygame.draw.line(SCREEN, (0, 255, 255, scan_alpha), (50, scan_y), (WIDTH - 50, scan_y), 1)
...
def start_game_with_difficulty(character_id, difficulty_mode):
    ...
    pygame.draw.rect(SCREEN, (30, 35, 50), (box_x, box_y, box_width, box_height), border_radius=10)
    pygame.draw.rect(SCREEN, (100, 150, 255), (box_x, box_y, box_width, box_height), 3, border_radius=10)
...
def show_item_manager_menu():
    ...
    while True:
        ...
        pygame.draw.rect(SCREEN, active_tab_color, active_tab_rect)
        pygame.draw.rect(SCREEN, WHITE, active_tab_rect, 2)
        ...
        pygame.draw.rect(SCREEN, passive_tab_color, passive_tab_rect)
        pygame.draw.rect(SCREEN, WHITE, passive_tab_rect, 2)
        ...
        for i, item in enumerate(current_items):
            ...
            # 배경 색상
            if is_selected:
                pygame.draw.rect(SCREEN, (255, 255, 0), item_rect)  # 선택된 아이템은 노란색
            elif is_cursor:
                pygame.draw.rect(SCREEN, (100, 150, 255), item_rect)  # 현재 커서는 파란색
            else:
                pygame.draw.rect(SCREEN, (60, 60, 80), item_rect)
            
            # 테두리 색상
            if is_cursor:
                pygame.draw.rect(SCREEN, (255, 255, 255), item_rect, 3)  # 커서는 굵은 흰색 테두리
            else:
...
def show_stage1_intro():
    ...
    for alpha in range(0, 256, 8):
        ...
        if decoration_alpha > 0:
            ...
            if decoration_alpha < 255:
                ...
                pygame.draw.line(SCREEN, line_color[:3], (WIDTH // 2 - 150, HEIGHT // 2 - 80), (WIDTH // 2 + 150, HEIGHT // 2 - 80), 3)
                pygame.draw.line(SCREEN, line_color[:3], (WIDTH // 2 - 150, HEIGHT // 2 - 20), (WIDTH // 2 + 150, HEIGHT // 2 - 20), 3)
    ...
    while waiting:
        ...
        pygame.draw.line(SCREEN, line_color, (WIDTH // 2 - 150, HEIGHT // 2 - 80), (WIDTH // 2 + 150, HEIGHT // 2 - 80), 3)
        pygame.draw.line(SCREEN, line_color, (WIDTH // 2 - 150, HEIGHT // 2 - 20), (WIDTH // 2 + 150, HEIGHT // 2 - 20), 3)
...
def draw_stage2_jungle_border():
    ...
    if current_stage == 2:
        ...
        for i in range(0, WIDTH, 30):
            # 상단 나뭇가지
            pygame.draw.line(SCREEN, jungle_green, (i, 2), (i + 15, 8), 1)
            pygame.draw.line(SCREEN, jungle_green, (i + 15, 2), (i, 8), 1)
            # 하단 나뭇가지
            pygame.draw.line(SCREEN, jungle_green, (i, HEIGHT - 8), (i + 15, HEIGHT - 2), 1)
            pygame.draw.line(SCREEN, jungle_green, (i + 15, HEIGHT - 8), (i, HEIGHT - 2), 1)
        
        for i in range(0, HEIGHT, 30):
            # 좌측 나뭇가지
            pygame.draw.line(SCREEN, jungle_green, (2, i), (8, i + 15), 1)
            pygame.draw.line(SCREEN, jungle_green, (2, i + 15), (8, i), 1)
            # 우측 나뭇가지
            pygame.draw.line(SCREEN, jungle_green, (WIDTH - 8, i), (WIDTH - 2, i + 15), 1)
            pygame.draw.line(SCREEN, jungle_green, (WIDTH - 8, i + 15), (WIDTH - 2, i), 1)
        
        # 코너 장식 (덩굴 매듭)
        corner_radius = 4
        # 좌상단
        pygame.draw.circle(SCREEN, highlight_green, (border_thickness//2, border_thickness//2), corner_radius)
        # 우상단
        pygame.draw.circle(SCREEN, highlight_green, (WIDTH - border_thickness//2, border_thickness//2), corner_radius)
        # 좌하단
        pygame.draw.circle(SCREEN, highlight_green, (border_thickness//2, HEIGHT - border_thickness//2), corner_radius)
        # 우하단
        pygame.draw.circle(SCREEN, highlight_green, (WIDTH - border_thickness//2, HEIGHT - border_thickness//2), corner_radius)
...
def draw_laser_cannon_gauge():
    ...
    pygame.draw.polygon(SCREEN, frame_color, frame_points)
    pygame.draw.polygon(SCREEN, (20, 25, 30), frame_points, 2)
    
    # 내부 배경 (다크 그레이)
    bg_rect = pygame.Rect(gauge_x, gauge_y, gauge_width, gauge_height)
    pygame.draw.rect(SCREEN, (10, 12, 15), bg_rect)
...
def draw_boss_health_bar():
    ...
    if boss_damage_preview_health > boss_current_health:
        ...
        pygame.draw.rect(SCREEN, (100, 30, 30), preview_rect)
    ...
    pygame.draw.rect(SCREEN, (30, 35, 40), text_bg_rect)
    pygame.draw.rect(SCREEN, (60, 65, 70), text_bg_rect, 1)
    ...
    for particle in impact_particles:
        ...
        if alpha > 0:
            ...
            if line_length > 1:
                ...
                # 얇은 스파크 라인
                if size > 1:
                    pygame.draw.line(SCREEN, color, (int(x), int(y)), (int(end_x), int(end_y)), size)
                else:
                    # 1픽셀 점만 그리기
    ...
    if quantum_explosion_active and quantum_balls:
        for qball in quantum_balls:
            ...
            if qball['entangled_with'] is not None:
                ...
                if not entangled['collapsed']:
                    # 얽힌 쌍 사이에 연결선
                    pygame.draw.line(SCREEN, (*quantum_color, alpha // 3),
        ...
        for pattern in quantum_interference_pattern[:]:
            ...
            pygame.draw.circle(SCREEN, (150, 150, 255, alpha),
...
def handle_ball():
    ...
    if BALL.top <= 0:
        ...
        if deuce_mode:
            ...
            show_score(deuce_wins, deuce_losses)
            ...
            round_wins += 1
            show_winner_text("플레이어")
            show_score(round_wins, round_losses)
            # 스테이지 2에서 플레이어가 라운드 이길 때마다 악어 짧은 울상
            if current_stage == 2 and animated_bg_stage2:
                animated_bg_stage2.set_expression('sad')
...
def draw_ability_radar_chart(stats, center_x, center_y, radius=120):
    ...
    for i in range(1, 6):
        ...
        pygame.draw.circle(SCREEN, (50, 50, 50), (center_x, center_y), int(grid_radius), 1)
    ...
    for i, ability_name in enumerate(ability_names):
        ...
        pygame.draw.line(SCREEN, (100, 100, 100), (center_x, center_y), (end_x, end_y), 1)
    ...
    # 능력치 점들을 선으로 연결
    if len(points) >= 2:
        pygame.draw.polygon(SCREEN, (100, 150, 255), points, 2)
...
def main(stage_num, new_boss_mode=False):
    ...
    while running:
        ...
        draw_dash_spirit_lasers()  # 🌟 대쉬 스피릿 레이저 그리기
...
def show_game_info():
    ...
    while True:
        ...
        if current_page == 0:  # 기본정보 페이지
            ...
            # 패널 배경
            pygame.draw.rect(SCREEN, (30, 30, 50), (info_panel_x, info_panel_y, info_panel_width, info_panel_height))
            pygame.draw.rect(SCREEN, (100, 150, 255), (info_panel_x, info_panel_y, info_panel_width, info_panel_height), 3)
            ...
            pygame.draw.rect(SCREEN, (100, 150, 255), next_button_rect)
                ...
                if not items_to_show:
                    ...
                    for i, item in enumerate(items_to_show):
                        ...
                        if i == selected_index and selected_index >= 0:
                            ...
                            pygame.draw.rect(SCREEN, WHITE, highlight_rect, 3)
                        ...
                        if item.get("icon"):
                            ...
                            # 기본 아이콘 (원형)
                            pygame.draw.circle(SCREEN, (100, 100, 100), (icon_x + icon_size // 2, icon_y + icon_size // 2), icon_size // 2)
            ...
            # 설명 패널 배경
            pygame.draw.rect(SCREEN, (20, 20, 40), (desc_panel_x, desc_panel_y, desc_panel_width, desc_panel_height))
            pygame.draw.rect(SCREEN, (80, 120, 200), (desc_panel_x, desc_panel_y, desc_panel_width, desc_panel_height), 2)
...
def show_item_management_menu(item_list, selected_index, item_type):
    ...
    while True:
        ...
        for i, (rect, text) in enumerate(zip(buttons, button_texts)):
            if i == selected:
                # 선택된 버튼
                pygame.draw.rect(SCREEN, (255, 100, 100) if i == 0 else (100, 150, 255), rect)
                pygame.draw.rect(SCREEN, WHITE, rect, 3)
                text_color = BLACK
            else:
                # 선택되지 않은 버튼
