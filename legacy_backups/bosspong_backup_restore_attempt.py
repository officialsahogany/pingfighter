...
# UI 시스템 임포트
from presentation.ui import menus, hud, dialogs, effects as ui_effects
...
def draw_objects():
    ...
    if not new_boss_mode_active:
        ...
        # player gauge drawing - needs gauge_value variable  # 플레이어 게이지바
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key in [pygame.K_LEFT, pygame.K_a, pygame.K_RIGHT, pygame.K_d]:
                    ...
                    if selected == 0:
                        ...
                        menus.show_start_screen(SCREEN, WIDTH, HEIGHT)
                        return
                    else:
                        # 인자 다시 넘겨주기
                        dialogs.show_victory_screen(SCREEN, stage_cleared, 0, {'medals': reward})
                        return
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                result = menus.show_pause_menu(SCREEN, WIDTH, HEIGHT)
                if result == "surrender":
                    # 기권 처리 - 먼저 평가 결과 표시
                    try:
                        if player_analyzer:
        ...
        if screen_shake_offset_x != 0 or screen_shake_offset_y != 0:
            ...
            draw_field()
            draw_objects()
            # boss health bar drawing - needs context-specific variables  # 💪 체력형 보스 체력바 그리기
...
def game_loop():
    while True:
        # 원본처럼 show_start_screen 내부에서 모든 처리
        show_start_screen()
