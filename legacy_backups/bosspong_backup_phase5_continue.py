...
# 패들 설정 - 게임 특정 크기 (config 기본값 오버라이드)
PADDLE_WIDTH, PADDLE_HEIGHT = 155, 50  # 특별 크기 유지
...
# 보스 기본 속도 설정값 - Stage 1 기본값을 기준으로 사용
BOSS_ACCELERATION_DEFAULT = BOSS_CONFIGS[1]["accel"]
BOSS_DECELERATION_DEFAULT = BOSS_CONFIGS[1]["decel"]
BOSS_MAX_SPEED_DEFAULT = BOSS_CONFIGS[1]["max_speed"]
BOSS_INSTANT_STOP_DECELERATION_DEFAULT = BOSS_CONFIGS[1]["instant_stop"]
...
# 체력형 보스 시스템 - config에서 가져옴
boss_health_stages = BOSS_HEALTH_STAGES  # 체력형 보스가 등장하는 스테이지
boss_max_health = BOSS_CONFIGS[6]["max_health"] if 6 in BOSS_CONFIGS else 15  # 보스 최대 체력
                ...
                if event.key in [pygame.K_UP, pygame.K_w]:
                    ...
                    if selected == 0:
                        ...
                        if stage_cleared < 5:  # 스테이지 5까지만 진행
                            ...
                            menus.show_start_screen(SCREEN, WIDTH, HEIGHT)  # 메인 메뉴로 돌아가기
                        return
                    elif selected == 1:
                        # 아카데미 화면 표시
                        import academy
