#!/usr/bin/env python3
"""
LED 깜빡임 애니메이션 프리뷰 생성
여러 프레임을 저장하여 애니메이션 효과 확인
"""
...
def draw_premium_frame(surface, rect, frame_color=(40, 80, 140), thickness=18):
    """프리미엄 블루 메탈릭 프레임"""
    x, y, w, h = rect
...
def create_animated_frame(player_score, ai_score, stage, animation_timer):
    """애니메이션 프레임 하나 생성"""
    ...
    # === 강화된 LED 깜빡임 애니메이션 ===
    # 다중 주파수 조합
    fast_pulse = 0.7 + 0.3 * math.sin(animation_timer * 0.5)
    slow_pulse = 0.85 + 0.15 * math.sin(animation_timer * 0.15)
    flicker = 0.9 + 0.1 * math.sin(animation_timer * 1.2)

    # 강한 깜빡임 (0.5초마다 번쩍)
    strobe_cycle = (animation_timer % 30) / 30.0
    strobe = 1.0 if strobe_cycle < 0.7 else (0.6 + 0.4 * math.sin(strobe_cycle * math.pi * 10))

    combined_pulse = fast_pulse * slow_pulse * flicker * strobe
    combined_pulse = max(0.5, min(1.2, combined_pulse))

    blue_led_base = (100, 200, 255)
    red_led_base = (255, 120, 120)
    ...
    p_intensity = 0.7 + 0.5 * combined_pulse
    ...
    boss_pulse = 0.7 + 0.3 * math.sin(animation_timer * 0.5 + math.pi * 0.3)
    boss_combined = boss_pulse * slow_pulse * flicker * strobe
    boss_combined = max(0.5, min(1.2, boss_combined))
    b_color = (
        int(min(255, red_led_base[0] * boss_combined)),
        int(min(255, red_led_base[1] * boss_combined)),
        int(min(255, red_led_base[2] * boss_combined))
    )
    b_intensity = 0.7 + 0.5 * boss_combined
...
def main():
    ...
    # 60프레임 (1초 분량) 생성
    print("LED 깜빡임 애니메이션 프레임 생성 중...")
    for frame in range(60):
        screen = create_animated_frame(2, 1, 1, frame)
        filename = os.path.join(output_dir, f"led_animation_frame_{frame:03d}.png")
        pygame.image.save(screen, filename)

    print(f"\n60 프레임이 {output_dir}에 저장되었습니다.")

    # 대표 프레임 몇 개 저장 (깜빡임 효과 비교용)
    key_frames = [0, 10, 20, 30, 40, 50]
    for i, frame in enumerate(key_frames):
        screen = create_animated_frame(2, 1, 1, frame)
        filename = os.path.join(output_dir, f"led_blink_sample_{i+1}.png")
        pygame.image.save(screen, filename)
        print(f"샘플 프레임 저장: {filename}")
