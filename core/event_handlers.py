...
class EventHandlers:
    ...
    def handle_game_start(self, data):
        ...
        # 스테이지 설정
        if 'stage' in data:
            self.game_state.current_stage = data['stage']
    ...
    def handle_game_pause(self, data):
        """게임 일시정지 이벤트 처리"""
        self.game_state.game_paused = True
        print("게임 일시정지")
        
    def handle_game_resume(self, data):
        """게임 재개 이벤트 처리"""
        self.game_state.game_paused = False
        print("게임 재개")
    ...
    def handle_round_end(self, data):
        ...
        if winner == 'player':
            self.handle_round_win(data)
        else:
            self.handle_round_lose(data)
            
    def handle_round_win(self, data):
        """라운드 승리 이벤트 처리"""
    ...
    def handle_round_lose(self, data):
        """라운드 패배 이벤트 처리"""
        if self.game_state.deuce_mode:
    ...
    def handle_score_update(self, data):
        ...
        if 'player_score' in data:
            self.game_state.player_score = data['player_score']
        if 'ai_score' in data:
            self.game_state.ai_score = data['ai_score']
            
    def handle_deuce_mode(self, data):
        """듀스 모드 이벤트 처리"""
        self.game_state.deuce_mode = data.get('active', False)
    ...
    def handle_item_collected(self, data):
        ...
        # 메달 보상
        if data.get('medal_reward'):
            self.game_state.medal_score += data['medal_reward']
            
    def handle_item_activated(self, data):
        """아이템 활성화 이벤트 처리"""
        item_name = data.get('item_name')
    ...
    def handle_menu_opened(self, data):
        """메뉴 열림 이벤트 처리"""
        menu_type = data.get('menu_type', 'main')
    ...
    def handle_menu_closed(self, data):
        """메뉴 닫힘 이벤트 처리"""
        # 게임 재개
        if not data.get('keep_paused'):
            self.game_state.game_paused = False
            
    def handle_dialog_shown(self, data):
        """다이얼로그 표시 이벤트 처리"""
        dialog_type = data.get('type')
        title = data.get('title')
        print(f"다이얼로그 표시: {title} ({dialog_type})")
        
    def handle_dialog_hidden(self, data):
        """다이얼로그 숨김 이벤트 처리"""
        pass
        
    def handle_button_clicked(self, data):
        """버튼 클릭 이벤트 처리"""
    ...
    def handle_settings_changed(self, data):
        """설정 변경 이벤트 처리"""
        # 사운드 설정 적용
        if 'sound' in data:
        ...
        # 게임플레이 설정 적용
        if 'gameplay' in data:
            gameplay_settings = data['gameplay']
    ...
    def handle_special_activated(self, data):
        """특수 능력 활성화 이벤트 처리"""
