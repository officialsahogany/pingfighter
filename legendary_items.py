    ...
    def _apply_buff(self, buff: str):
        ...
        if buff == "paddle_size":
            ...
            try:
                ...
                # _GLOBAL_DASH_INST 우선 사용 (실제 게임에서 사용하는 인스턴스)
                dm = dash_manager._GLOBAL_DASH_INST or dash_manager.get_dash_manager()
                print(f"[AngelBlessing][DEBUG] dash_manager._GLOBAL_DASH_INST = {dash_manager._GLOBAL_DASH_INST}")
                print(f"[AngelBlessing][DEBUG] dm = {dm}, id={id(dm) if dm else None}")
                # pingfighter의 dash 변수도 확인
                pf_module = sys.modules.get("pingfighter")
                if pf_module and hasattr(pf_module, "dash"):
                    pf_dash = getattr(pf_module, "dash")
                    print(f"[AngelBlessing][DEBUG] pingfighter.dash = {pf_dash}, id={id(pf_dash) if pf_dash else None}")
                    print(f"[AngelBlessing][DEBUG] 동일 인스턴스? {dm is pf_dash}")
                if dm:
            ...
            try:
                ...
                if dm:
                    ...
                    # pingfighter.dash에도 직접 확인
                    if pf_module and hasattr(pf_module, "dash"):
                        pf_dash = getattr(pf_module, "dash")
                        if pf_dash:
                            pf_cooldown = getattr(pf_dash, 'external_cooldown_multiplier', 1.0)
                            print(f"[AngelBlessing][DEBUG] pingfighter.dash.external_cooldown_multiplier = {pf_cooldown}")
                else:
                    print(f"[AngelBlessing] 대쉬 매니저를 찾을 수 없음")
            except Exception as e:
                import traceback
