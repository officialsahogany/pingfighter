...
def draw_objects():
    ...
    # 👻 고스트샷 발동 시 무서운 영혼의 귀신 효과
    if mega_smashing_active:
        # 시간 기반 애니메이션 (빠른 깜빡임을 위해 속도 증가)
        ghost_time = pygame.time.get_ticks() * 0.01  # 더 빠른 깜빡임
        # 기본 공 이미지를 복사
        ...
        # 무서운 깜빡임 효과 (급격한 밝기 변화)
        flicker = abs(math.sin(ghost_time * 3))  # 빠른 깜빡임
        sudden_flash = 1.0 if random.random() > 0.9 else flicker  # 간헐적 번쩍임
        
        # 공명하는 영혼의 색상 (보라색-검은색-붉은색 혼합)
        ghost_overlay = pygame.Surface((BALL.width, BALL.height), pygame.SRCALPHA)
        
        # 어두운 핵심부 (검은 영혼)
        core_color = (
            int(20 + sudden_flash * 30),  # 거의 검은색
            0,
            int(30 + sudden_flash * 50)   # 깊은 보라색
        )
        pygame.draw.circle(ghost_overlay, (*core_color, 250),
                         (center_x, center_y), max_radius)
        
        # 영혼의 공명 효과 (여러 겹의 진동하는 원)
        resonance_count = 4
        for i in range(resonance_count):
            # 공명 진동 계산
            resonance = math.sin(ghost_time * (5 + i)) * 0.5 + 0.5
            vibration = math.sin(ghost_time * 10 + i * 0.5) * 2  # 떨림 효과
            
            radius = max_radius + i * 3 + vibration
            alpha = int(150 * flicker * (1 - i / resonance_count))
            
            # 무서운 보라색-붉은색 그라데이션
            ghost_color = (
                int(100 + resonance * 100),  # 붉은색 성분
                int(20 * flicker),            # 약간의 녹색
                int(150 + resonance * 105)    # 강한 보라색
            )
            
            # 공명하는 원 그리기
            if radius > 0:
                pygame.draw.circle(ghost_overlay, (*ghost_color, alpha),
                                 (center_x, center_y), int(radius), 2)
        
        # 영혼의 얼굴 효과 (무작위로 나타나는 무서운 눈)
        if random.random() < 0.3 * flicker:  # 간헐적으로 나타남
            # 귀신 눈 효과
            eye_glow = pygame.Surface((BALL.width, BALL.height), pygame.SRCALPHA)
            eye_color = (255, 50, 255, int(200 * sudden_flash))  # 밝은 보라색 눈
            
            # 왼쪽 눈
            pygame.draw.circle(eye_glow, eye_color,
                             (center_x - 5, center_y - 3), 2)
            # 오른쪽 눈
            pygame.draw.circle(eye_glow, eye_color,
                             (center_x + 5, center_y - 3), 2)
            
            ghost_overlay.blit(eye_glow, (0, 0), special_flags=pygame.BLEND_ADD)
        
        # 어두운 오라 (검은 연기 효과)
        dark_aura = pygame.Surface((BALL.width * 2, BALL.height * 2), pygame.SRCALPHA)
        for i in range(3):
            aura_radius = max_radius + 10 + i * 5
            aura_alpha = int(50 * flicker / (i + 1))
            aura_color = (50, 0, 80)  # 어두운 보라색
            pygame.draw.circle(dark_aura, (*aura_color, aura_alpha),
                             (BALL.width, BALL.height), aura_radius)
        
        # 원본 공을 어둡게 만들기
        dark_tint = pygame.Surface((BALL.width, BALL.height), pygame.SRCALPHA)
        dark_tint.fill((30, 0, 50, 200))  # 어두운 보라색 필터
        ghost_ball.blit(dark_tint, (0, 0), special_flags=pygame.BLEND_MULT)
        
        # 모든 효과 합성
        ghost_ball.blit(ghost_overlay, (0, 0), special_flags=pygame.BLEND_ADD)
        
        # 영혼의 불꽃 효과 (무작위 보라색 입자)
        for _ in range(8):
            if random.random() < flicker:
                flame_x = center_x + random.randint(-max_radius, max_radius)
                flame_y = center_y + random.randint(-max_radius, max_radius)
                flame_size = random.randint(1, 3)
                flame_color = (
                    random.randint(150, 255),  # 붉은-보라 스펙트럼
                    0,
                    random.randint(150, 255)
                )
                pygame.draw.circle(ghost_ball, (*flame_color, 150),
                                 (flame_x, flame_y), flame_size)
        
        # 무서운 잔상 효과 (여러 개의 반투명 공)
        ghost_trail = pygame.Surface((BALL.width, BALL.height), pygame.SRCALPHA)
        trail_offset = int(math.sin(ghost_time * 2) * 3)
        pygame.draw.circle(ghost_trail, (100, 0, 150, 50),
                         (center_x + trail_offset, center_y), max_radius - 2)
        pygame.draw.circle(ghost_trail, (150, 0, 200, 30),
                         (center_x - trail_offset, center_y), max_radius - 4)
        ghost_ball.blit(ghost_trail, (0, 0))
        # 최종 이미지로 설정
...
def draw_ability_radar_chart(stats, center_x, center_y, radius=120):
    ...
    # 퍼센트 표시 (격자 옆)
    percent_font = get_font(12)  # 12pt 픽셀 폰트
    for i in range(1, 6):
        percent = i * 20
