...
try:
    ...
    # 코르크 마개
    pygame.draw.rect(life_elixir_icon, (139, 69, 19, MAX_COLOR), (12, 6, 8, 4))
    pygame.draw.rect(life_elixir_icon, (160, 82, 45, MAX_COLOR), (13, 7, 6, 2))
    rainbow_colors = [
        (255, 0, 0),      # 빨강
        (255, 127, 0),    # 주황
        (255, 255, 0),    # 노랑
    ...
    for i, color in enumerate(rainbow_colors):
        ...
        if y_pos < 28:  # 병 안에만 그리기
            ...
            pygame.draw.ellipse(life_elixir_icon, liquid_color, 
                              (10, y_pos, 12, 3))
    # 반짝임 효과
    pygame.draw.circle(life_elixir_icon, (MAX_COLOR, MAX_COLOR, 255, 200), (12, 16), 2)
    pygame.draw.circle(life_elixir_icon, (MAX_COLOR, MAX_COLOR, 255, 150), (19, 20), 1)
    # 병 하이라이트
...
def draw_predicted_trajectory():
            ...
            if segment_length > 0:
                ...
                for dash in range(num_dashes):
                    ...
                    if dash_start < segment_length:
                        ...
                        # 빨간색 점선
                        pygame.draw.line(trajectory_surface, (MAX_COLOR, 0, 0, 200), 
                                       (dash_start_x, dash_start_y), 
                                       (dash_end_x, dash_end_y), 5)
                        # 밝은 중심선 효과
                        pygame.draw.line(trajectory_surface, (MAX_COLOR, 100, 100, 150), 
                                       (dash_start_x, dash_start_y), 
                                       (dash_end_x, dash_end_y), 3)
            if i % 4 == 0:
                # 빨간색 발광 효과
                pygame.draw.circle(trajectory_surface, (MAX_COLOR, 150, 150, DEFAULT_SIZE), start_pos, 6)
                pygame.draw.circle(trajectory_surface, (MAX_COLOR, 50, 50, 200), start_pos, 4)
        # 궤적 Surface를 화면에 블릿
    ...
    if predicted_trajectory:
        ...
        # 외부 원
        pygame.draw.circle(marker_surface, (MAX_COLOR, 0, 0, 200), (40, 40), 30, 3)
        # 십자선
        pygame.draw.line(marker_surface, (MAX_COLOR, 0, 0, MAX_COLOR), (40, 10), (40, 70), 3)
        pygame.draw.line(marker_surface, (MAX_COLOR, 0, 0, MAX_COLOR), (10, 40), (70, 40), 3)
        # 내부 점
        pygame.draw.circle(marker_surface, (MAX_COLOR, 50, 50, MAX_COLOR), (40, 40), 5)
        # 빛나는 효과
        for radius in range(10, 35, 5):
            alpha = 50 - radius
            pygame.draw.circle(marker_surface, (MAX_COLOR, 100, 100, alpha), (40, 40), radius, 1)
...
def draw_speech():
    ...
    if speech_timer > 0:
        ...
        bubble_width = text_surface.get_width() + 20 * 2
        ...
        pygame.draw.rect(bubble_surface, BLACK, main_rect, 3, border_radius=15)
        tail_points = [
        ...
        pygame.draw.rect(highlight_surface, (MAX_COLOR, MAX_COLOR, 255, DEFAULT_SIZE), highlight_surface.get_rect(), border_radius=4)
...
def draw_item_obtained_effect():
    ...
    for i in range(glow_radius):
        ...
        pygame.draw.circle(glow_surface, (MAX_COLOR, MAX_COLOR, 255, alpha), (glow_radius, glow_radius), glow_radius - i)
...
def draw_player_gauge():
    ...
    text_color = (255, 255, 255) if displayed_gauge < special_gauge_max else COLOR_LIGHT_YELLOW  # 만렙일 때는 노란색
    try:
    ...
    for i in range(max_tokens):
        ...
        if not is_available and 'charge_progress' in locals() and 'is_charging' in locals() and is_charging and charge_progress > 0:
            ...
            if charge_progress > 0.5:
                ...
                pygame.draw.circle(pulse_surface, (MAX_COLOR, 100, 100, pulse_alpha),
...
def draw_aircraft_carrier_boss(boss_speed=0, boss_x=0):
    ...
    for i in range(8):  # 더 많은 패널
        ...
        if panel_x < carrier_width - 35:
            ...
            # 중앙 에너지 코어
            core_x = panel_x + 10
            pygame.draw.circle(carrier_surface, panel_glow, (core_x, 35), 2)
            if boss_current_health > 8:
                pygame.draw.circle(carrier_surface, (180, 210, MAX_COLOR), (core_x, core_y), 1)
    # 구조적 리벳 라인들
    ...
    for hx, hy in hardpoint_positions:
        if hx < carrier_width - 25:
            # 하드포인트 베이스
            pygame.draw.rect(carrier_surface, (75, 85, DEFAULT_SIZE), (hx-3, hy-3, 6, 6))
    ...
    for idx, (tx, ty) in enumerate(turret_positions):
        if tx < carrier_width - 25:
            ...
            for barrel_offset in [-2, 2]:  # 두 개의 포신
                ...
                pygame.draw.line(carrier_surface, (80, 90, DEFAULT_SIZE), 
    ...
    pygame.draw.rect(carrier_surface, (75, 85, DEFAULT_SIZE), (bridge_x, 15, 50, 30))
    ...
    radar_x = bridge_x + 25
    # 레이더 베이스
    pygame.draw.circle(carrier_surface, (80, 90, DEFAULT_SIZE), (radar_x, 8), 7)
    ...
    pulse = abs(math.sin(time_now * 0.005))
    core_color = (int(180 + pulse * 75), int(200 + pulse * 55), MAX_COLOR)
    ...
    if shield_antenna_active:
        # 쉴드 활성화 시 밝게 빛남
        pygame.draw.circle(carrier_surface, (200, 220, MAX_COLOR), 
                          (shield_antenna_x, shield_antenna_y - 8), 4)
        pygame.draw.circle(carrier_surface, (MAX_COLOR, MAX_COLOR, MAX_COLOR), 
                          (shield_antenna_x, shield_antenna_y - 8), 2)
    else:
        # 비활성화 시 약하게 빛남
        glow_value = min(MAX_COLOR, int(150 + tip_glow))
        glow_color = (glow_value, min(MAX_COLOR, int(170 + tip_glow)), min(MAX_COLOR, int(200 + tip_glow)))
    ...
    if laser_charging or laser_cannon_active:
        ...
        if laser_charging:
            ...
            for _ in range(int(5 * charge_progress)):
                ...
                pygame.draw.circle(carrier_surface, (150, 200, MAX_COLOR), (spark_x, spark_y), 1)
    else:
        pygame.draw.line(carrier_surface, (50, 60, 70), 
                        (laser_cannon_x, laser_cannon_y), 
    ...
    if laser_charging or laser_cannon_active:
        # 충전/발사 중 밝게
        pygame.draw.circle(carrier_surface, (150, 200, MAX_COLOR), (laser_cannon_x, laser_cannon_y), 2)
    ...
    for idx, (ex, ey) in enumerate(engine_positions):
        ...
        pygame.draw.rect(carrier_surface, (80, 90, DEFAULT_SIZE), (ex+2, ey+1, 12, 6))
    ...
    for idx, (dx, dy) in enumerate(defense_positions):
        if dx < carrier_width - 15:
            ...
            # 아래를 향하는 포신 그리기
            barrel_end_x = dx + int(math.cos(angle_rad) * 10)
    ...
    if damage_ratio > 0.5:
        ...
        for _ in range(int(damage_ratio * 7)):
            ...
            if random.random() < 0.6:
                pygame.draw.circle(carrier_surface, (MAX_COLOR, 100, 0), (leak_x, leak_y), 3)
                pygame.draw.circle(carrier_surface, (MAX_COLOR, 150, 0), (leak_x, leak_y), 2)
                pygame.draw.circle(carrier_surface, (MAX_COLOR, 200, 50), (leak_x, leak_y), 1)
            else:
                pygame.draw.circle(carrier_surface, COLOR_LIGHT_BLUE, (leak_x, leak_y), 2)
                pygame.draw.circle(carrier_surface, (150, 200, MAX_COLOR), (leak_x, leak_y), 1)
            # 연기 효과
    ...
    if health_percent <= 30:
        ...
        for fire_x, fire_y in fire_points:
            ...
            pygame.draw.circle(carrier_surface, (MAX_COLOR, MAX_COLOR, 200), (fire_x, fire_y), core_size)
    ...
    if health_percent <= 10:
        ...
        if random.random() < 0.2:  # 20% 확률로 폭발
            ...
            pygame.draw.circle(carrier_surface, (MAX_COLOR, MAX_COLOR, MAX_COLOR), (explosion_x, explosion_y), flash_size)
            pygame.draw.circle(carrier_surface, (MAX_COLOR, 200, DEFAULT_SIZE), (explosion_x, explosion_y), flash_size - 3)
            pygame.draw.circle(carrier_surface, (MAX_COLOR, 100, 0), (explosion_x, explosion_y), flash_size - 6)
            # 파편 효과
            for _ in range(8):
    ...
    # 캐논 베이스 그리기
    pygame.draw.circle(carrier_surface, (80, 90, DEFAULT_SIZE), (cannon_x, cannon_y), 12)
    ...
    if molotov_throwing or grenade_throwing or flare_throwing:
        ...
        if molotov_throwing:
            ...
            try:
                ...
                pygame.draw.circle(item_icon, (MAX_COLOR, 100, 0), (20, 20), 15)
        elif grenade_throwing:
            ...
            try:
                ...
                # 수류탄 기본 아이콘
                item_icon = pygame.Surface((40, 40), pygame.SRCALPHA)
                pygame.draw.circle(item_icon, (100, 150, DEFAULT_SIZE), (20, 20), 15)
        elif flare_throwing:
            ...
            try:
                ...
                # 조명탄 기본 아이콘
                item_icon = pygame.Surface((40, 40), pygame.SRCALPHA)
                pygame.draw.circle(item_icon, (MAX_COLOR, MAX_COLOR, 200), (20, 20), 15)
    ...
    for zone in explosion_zones:
        if zone["active"]:
            ...
            if shockwave_radius < zone["radius"] * 2.5:
                pygame.draw.circle(explosion_surface, (MAX_COLOR, MAX_COLOR, 255, 30), 
            ...
            for i in range(zone["radius"], 0, -3):
                ...
                if zone["duration"] > 10:  # 초기: 흰색-노란색
                    g = int(255 - (1 - 255atio) * DEFAULT_SIZE)
                    b = int(200 - (1 - ratio) * 150)
                elif zone["duration"] > 5:  # 중반: 주황색-빨간색
                    g = int(150 - (1 - 255atio) * DEFAULT_SIZE)
                    b = int(50 - (1 - ratio) * 40)
                else:  # 후반: 빨간색-검은색
            ...
            if zone["duration"] > 10:
                for k in range(8):
                    ...
                    pygame.draw.line(explosion_surface, (MAX_COLOR, MAX_COLOR, 200, 150), 
...
def draw_ball():
        ...
        for particle in power_smashing_particles:
            ...
            if particle['life'] > 0:
                ...
                if particle.get('type') == 'spark':
                    ...
                    pygame.draw.circle(particle_surface, (MAX_COLOR, MAX_COLOR, 255, core_alpha), 
                                     (center, center), size)
            ...
            for meteor in mega_smashing_meteor_trail:
                ...
                if meteor['life'] > 0:
                    ...
                    if head_size > 0:
                        ...
                        pygame.draw.circle(glow_surface, (0, MAX_COLOR, 255, min(MAX_COLOR, glow_alpha // 2)),
                                         (head_size * 2, head_size * 2), head_size * 2)
                        pygame.draw.circle(glow_surface, (MAX_COLOR, 215, 0, min(MAX_COLOR, glow_alpha)),
    ...
    if mega_smashing_active:
        ...
        for _ in range(5):
            ...
            pygame.draw.circle(sparkle_surface, (200, 150, MAX_COLOR, 200),
                             (sparkle_x, sparkle_y), sparkle_size)
        ghost_ball.blit(sparkle_surface, (0, 0))
        # 크리스탈 하이라이트 효과
        highlight_offset = int(math.sin(ghost_time * 3) * 2)
        pygame.draw.circle(ghost_ball, (220, 180, MAX_COLOR, 200),
        ...
        pygame.draw.circle(highlight, (MAX_COLOR, MAX_COLOR, 255, DEFAULT_SIZE),
                         (BALL.width//2 - 3, BALL.height//2 - 3), 6)
        pygame.draw.circle(highlight, (MAX_COLOR, MAX_COLOR, 255, 50),
    ...
    if new_boss_mode_active and lightning_strike_active:
        ...
        pygame.draw.circle(glow_surface, (MAX_COLOR, MAX_COLOR, 150, glow_alpha), 
    ...
    if new_boss_mode_active and fire_charge_active:
        ...
        for i in range(5):
            ...
            if tail_alpha > 0:
                ...
                pygame.draw.circle(tail_surface, (MAX_COLOR, 150, 0, tail_alpha), (10, 10), 10 - i)
...
def draw_objects():
    ...
    if is_waiting_for_serve:
        ...
        if special_ready:
            ...
            pygame.draw.circle(timing_surface, (MAX_COLOR, 80, 80, timing_alpha), (timing_radius, timing_radius), timing_radius, 3)
        else:
            # 일반 드라이브 상태일 때 - 노란색 원
            pygame.draw.circle(timing_surface, (MAX_COLOR, MAX_COLOR, 0, timing_alpha), (timing_radius, timing_radius), timing_radius, 3)
    ...
    if DEBUG_SHOW_HITBOX:
        ...
        draw.rect((0, 0, MAX_COLOR), BOSS, 2)
    if boss_stunned_timer > 0:
        ...
        for i in range(num_stars):
            ...
            pygame.draw.circle(glow_surface, (MAX_COLOR, MAX_COLOR, 100, 60), 
    ...
    if current_stage == 4 and stage4_magnetic_active:
        ...
        # 기본 자기장 원
        pygame.draw.circle(magnetic_surface, (100, 200, MAX_COLOR, 80), (130, 130), 130, width=6)
        pygame.draw.circle(magnetic_surface, (180, 220, MAX_COLOR, 50), (130, 130), 80, width=0)
        # 전기 이펙트 - 번개 모양의 선들
        ...
        if pulse > 0.8:  # 주기적으로 강한 번쩍임
            ...
            pygame.draw.circle(flash_surface, (200, 230, MAX_COLOR, int(40 * pulse)), (130, 130), 130, width=0)
            ...
            if shield_elapsed < shield_duration:
                ...
                for arc_num in range(3):
                    ...
                    for angle_deg in range(start_angle, end_angle):
                        ...
                        pygame.draw.line(shield_surface, (220, 240, MAX_COLOR, arc_alpha), 
                ...
                for i in range(2):
                    ...
                    for angle_deg in range(start_angle, end_angle, 3):
                        ...
                        pygame.draw.circle(shield_surface, (200, 220, MAX_COLOR, glow_alpha), (x, y), 2)
        ...
        if health_percent <= 30:
            ...
            for interceptor in interceptors[:]:
                ...
                if interceptor['state'] == 'launching':
                    ...
                    interceptor['x'] = interceptor['patrol_center_x'] + math.cos(interceptor['angle']) * 30
    ...
    if current_stage == 3:
        ...
        boss_text_color = (255, 255, 255) if displayed_boss_gauge < 500 else COLOR_LIGHT_YELLOW  # 만렙일 때는 노란색
        try:
...
def show_victory_screen(stage_cleared, reward):
    ...
    button_width = 280
    next_stage_rect = pygame.Rect(WIDTH // 2 - button_width // 2, 420, button_width, 70)
...
def show_start_screen():
    ...
    while True:
        ...
        pygame.draw.circle(glow_surf, (0, 200, MAX_COLOR, 30), 
        ...
        pygame.draw.rect(medal_panel, COLOR_GOLD, (0, 0, 150, 50), 2, border_radius=8)
        ...
        for i, (option, display_option) in enumerate(zip(all_options, display_options)):
            if i == selected:
                ...
                pygame.draw.rect(option_bg, (20, 30, 40, DEFAULT_SIZE), (0, 0, option_width, option_height), border_radius=10)
        ...
        if locked_message_timer > 0:
            ...
            for j in range(10, 0, -2):
                ...
                pygame.draw.rect(glow_surface, (MAX_COLOR, 100, 100, alpha), (0, 0, message_width + j*2, message_height + j*2), border_radius=20)
                SCREEN.blit(glow_surface, (message_x - j, message_y - j))
            # 메인 배경
            message_bg = pygame.Surface((message_width, message_height), pygame.SRCALPHA)
            pygame.draw.rect(message_bg, (MAX_COLOR, 100, 100, 40), (0, 0, message_width, message_height), border_radius=20)
            pygame.draw.rect(message_bg, (MAX_COLOR, 100, 100, 120), (0, 0, message_width, message_height), width=2, border_radius=20)
...
def show_character_selection():
    ...
    while True:
        ...
        if animation_timer % 60 < 30:  # 0.5초마다 깜박임
            ...
            pygame.draw.rect(cursor_surface, (0, MAX_COLOR, 255, 150), (0, 0, 200, 3))
        ...
        def draw_card_front(surface, character, is_selected, w, h):
            ...
            inner_border = 4
            pygame.draw.rect(surface, (*border_color, DEFAULT_SIZE), 
            ...
            try:
                ...
                pygame.draw.circle(surface, (MAX_COLOR, MAX_COLOR, MAX_COLOR), (w//2, 30 + radius), radius - 5, 3)
        ...
        def draw_card_pattern(surface, pattern, width, height):
            ...
            if pattern_type == "circuit":
                ...
                # 육각형 허니컴 패턴
                for x in range(margin, margin + pattern_width, 12 * 2):
...
def show_difficulty_selection():
    ...
    while True:
        ...
        if animation_timer % 60 < 30:
            ...
            pygame.draw.rect(cursor_surface, (0, MAX_COLOR, 255, 150), (0, 0, 150, 2))
...
def show_item_manager_menu():
    ...
    grid_rows = 4
    item_size = 60
    grid_start_x = (WIDTH - (grid_cols * item_size + (grid_cols - 1) * 20)) // 2
    grid_start_y = 200
    # 모든 아이템 목록
    all_items = [
        # 엑티브 아이템들
...
def get_item_icon(item_name):
    ...
    if item_name == "molotov":
        try:
            ...
            pygame.draw.circle(molotov_icon, (MAX_COLOR, 100, 0), (16, 16), 12)
            return molotov_icon
    # 수류탄 아이콘 동적 로드
    ...
    if item_name == "flare":
        ...
        pygame.draw.rect(flare_icon, COLOR_LIGHT_GRAY2, (13, 9, 6, 16))
        pygame.draw.circle(flare_icon, (MAX_COLOR, MAX_COLOR, 200), (16, 8), 5)
        pygame.draw.circle(flare_icon, (MAX_COLOR, MAX_COLOR, 150), (16, 8), 3)
        pygame.draw.circle(flare_icon, (MAX_COLOR, MAX_COLOR, MAX_COLOR), (16, 8), 2)
        for angle in range(0, 360, 45):
            rad = math.radians(angle)
            ...
            pygame.draw.line(flare_icon, (MAX_COLOR, MAX_COLOR, 100, 150), (x1, y1), (x2, y2), 1)
        # 하단 안전핀
        pygame.draw.rect(flare_icon, COLOR_DARK_GRAY2, (14, 24, 4, 3))
        return flare_icon
    # 연막탄 아이콘 동적 로드
    if item_name == "smoke_grenade":
        try:
            ...
            pygame.draw.circle(smoke_grenade_icon, COLOR_MID_GRAY, (16, 16), 12)
            return smoke_grenade_icon
...
def draw_boss_health_bar():
    ...
    if boss_current_health <= 3:
        ...
        pygame.draw.rect(warning_surface, (MAX_COLOR, 0, 0, warning_alpha), 
...
def show_result(won):
    ...
    if won:
        ...
        try:
            ...
            flare_icon = pygame.Surface((32, 32), pygame.SRCALPHA)
            pygame.draw.circle(flare_icon, (MAX_COLOR, MAX_COLOR, 200), (16, 16), 12)
        ...
        try:
            ...
            predictor_icon = pygame.Surface((32, 32), pygame.SRCALPHA)
            pygame.draw.circle(predictor_icon, (100, 200, MAX_COLOR), (16, 16), 12)
...
def draw_modern_panel(surface, rect, title="", title_color=COLOR_GOLD):
    ...
    # 내부 테두리
    pygame.draw.rect(surface, (80, 80, DEFAULT_SIZE), rect, 2)
    # 상단 헤더 영역
    if title:
