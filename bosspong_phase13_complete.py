...
COLOR_4 = (0, 255, 0)  # 초록색
COLOR_5 = (0, 0, 255)  # 파란색
COLOR_6 = (255, 255, 0)  # 노란색
COLOR_7 = (255, 0, 255)  # 마젠타
COLOR_8 = (0, 255, 255)  # 시안
COLOR_9 = (128, 128, 128)  # 회색
COLOR_10 = (255, 128, 0)  # 주황색
COLOR_11 = (128, 0, 255)  # 보라색
COLOR_12 = (0, 128, 255)  # 하늘색
COLOR_13 = (255, 128, 128)  # 연한 빨간색
COLOR_14 = (128, 255, 128)  # 연한 초록색
COLOR_15 = (128, 128, 255)  # 연한 파란색
COLOR_16 = (192, 192, 192)  # 밝은 회색
COLOR_17 = (64, 64, 64)  # 어두운 회색
COLOR_18 = (255, 192, 203)  # 분홍색
COLOR_19 = (165, 42, 42)  # 갈색
COLOR_20 = (0, 128, 0)  # 짙은 초록색
...
# Color Values
HALF_COLOR_VALUE = 128
MAX_COLOR_VALUE = 255
# === End Phase 12 Constants ===
...
def get_angle_components(angle):
    ...
    return get_angle_components(angle)
...
def get_random_boss_x():
    ...
    return get_random_boss_x()

def check_ball_player_collision():
    """Check if ball collides with player"""
    return check_ball_player_collision()

# === End Phase 13 Helper Functions ===


COLOR_GOLD = (MAX_COLOR, 215, 0)
COLOR_LIGHT_BLUE = (100, 150, MAX_COLOR)
COLOR_LIGHT_GRAY2 = (200, 200, 200)
COLOR_MID_GRAY = (150, 150, 150)
COLOR_DARK_GRAY2 = (100, 100, DEFAULT_SIZE)
COLOR_LIGHT_RED = (MAX_COLOR, 100, DEFAULT_SIZE)
COLOR_LIGHT_YELLOW = (MAX_COLOR, MAX_COLOR, DEFAULT_SIZE)
...
def lighten_color(color, factor=1.3):
    """색상을 밝게 만들기"""
    return tuple(min(MAX_COLOR, int(c * factor)) for c in color[:3])
...
def draw_text_centered(text, pos, font_size=20, color=COLOR_1):
    """중앙 정렬된 텍스트 그리기"""
...
class DrawHelper:
    ...
    def rect(self, color, rect, width=0, border_radius=0):
        """사각형 그리기 간소화"""
        # 디버그: 빨간색/파란색 테두리 감지
        if width == 2 and (color == (255, 0, 0) or color == (0, 0, 255) or color == (0, 0, MAX_COLOR)):
            print(f"🔴🔵 디버그 테두리 감지! color={color}, rect={rect}, width={width}")
            return  # 그리지 않음
        if border_radius > 0:
...
special_gauge = 0
rainbow_colors = [(MAX_COLOR,0,0), (MAX_COLOR,165,0), (MAX_COLOR,255,0), (0,255,0), (0,127,255), (0,0,255), (139,0,255)]
rainbow_index = 0
ball_trail = []
# 🤖 보스 AI & 움직임 시스템
# 보스 기본 설정
...
laser_cooldown = FIVE_SECONDS_MS  # 레이저 쿨타임 (5~9초 랜덤)
...
shield_duration = THREE_SECONDS_MS  # 쉴드 지속 시간 (3초)
...
fireball_cooldown = FIVE_SECONDS_MS      # ms 단위 쿨타임
...
draw.circle( (255, 80, 0), (20, 20), 20)  # 화염탄 예비 이미지
# 아이템 아이콘 로드
...
try:
    ...
    # 병 목
    draw.rect( (60, 60, 80, 200), (13, 8, 6, 6))
    draw.rect( (80, 80, 100, 180), (14, 9, 4, 5))
    # 코르크 마개
    draw.rect( (139, 69, 19, MAX_COLOR), (12, 6, 8, 4))
    draw.rect( (160, 82, 45, MAX_COLOR), (13, 7, 6, 2))
    rainbow_colors = [
        (255, 0, 0),      # 빨강
        (255, 127, 0),    # 주황
        COLOR_2,    # 노랑
    ...
    for i, color in enumerate(rainbow_colors):
        ...
        if y_pos < 28:  # 병 안에만 그리기
            ...
            pygame.draw.ellipse(life_elixir_icon, liquid_color, 
                              (DEFAULT_TIMER, y_pos, 12, 3))
    # 반짝임 효과
    draw.circle( (MAX_COLOR, MAX_COLOR, 255, 200), (12, 16), 2)
    draw.circle( (MAX_COLOR, MAX_COLOR, 255, 150), (19, 20), 1)
    # 병 하이라이트
    pygame.draw.arc(life_elixir_icon, (MAX_COLOR, MAX_COLOR, 255, DEFAULT_SIZE), 
...
try:
    ...
    draw.circle( COLOR_3, (16, 16), 12)  # 초록색 외곽
    draw.circle( COLOR_1, (16, 16), 8)  # 흰색 내부
    draw.circle( COLOR_3, (16, 16), 4)   # 초록색 중심
# 스파이크부츠 아이콘 로드
try:
    ...
    draw.circle( (255, 100, 255), (16, 16), 12)  # 마젠타색 외곽
    draw.circle( COLOR_1, (16, 16), 8)  # 흰색 내부
    draw.circle( (255, 100, 255), (16, 16), 4)   # 마젠타색 중심
# 대쉬기어 아이콘 로드
try:
    ...
    draw.circle( (100, 100, 255), (16, 16), 12)  # 파란색 외곽
    draw.circle( COLOR_1, (16, 16), 8)  # 흰색 내부
    draw.circle( (100, 100, 255), (16, 16), 4)   # 파란색 중심
# 벌크업 아이콘 로드
try:
    ...
    draw.circle( (255, 100, 100), (16, 16), 12)  # 빨간색 외곽
    draw.circle( COLOR_1, (16, 16), 8)  # 흰색 내부
    draw.circle( (255, 100, 100), (16, 16), 4)   # 빨간색 중심
# 감지센서 아이콘 로드
try:
    ...
    draw.circle( (150, 150, 255), (16, 16), 12)  # 연파란색 외곽
    draw.circle( COLOR_1, (16, 16), 8)  # 흰색 내부
    draw.circle( (150, 150, 255), (16, 16), 4)   # 연파란색 중심
# 대쉬홀더 아이콘 로드
try:
    ...
    draw.circle( (255, 150, 100), (16, 16), 12)  # 주황색 외곽
    draw.circle( COLOR_1, (16, 16), 8)  # 흰색 내부
    draw.circle( (255, 150, 100), (16, 16), 4)   # 주황색 중심
# ITEM_TYPES에 아이콘 할당
for item in items.ITEM_TYPES:
    icon_map = {
    ...
    if item["name"] in icon_map:
        ...
        try:
            ...
            draw.circle( (255, 100, 0), (16, 16), 12)
        ...
        try:
            ...
            # 수류탄 기본 아이콘
            item["icon"] = pygame.Surface((32, 32), pygame.SRCALPHA)
            draw.circle( (80, 100, 80), (16, 16), 12)
            draw.rect( (60, 60, 60), (14, 8, 4, 6))
        ...
        try:
            ...
            # 레이저스코프 기본 아이콘
            item["icon"] = pygame.Surface((32, 32), pygame.SRCALPHA)
            draw.circle( COLOR_8, (16, 16), 12, 2)
            draw.line( (255, 0, 0), (16, 4), (16, 28), 1)
            draw.line( (255, 0, 0), (4, 16), (28, 16), 1)
...
DASH_SPIRIT_ELECTRIC_COLOR = COLOR_1  # 전기 효과 (흰색)
...
player_hit_animation_active = False
player_hit_animation_timer = 0
balloon_particles = []
water_particles = []
flame_trail_phase = 0
...
try:
    ...
    BOSS_IMG_STAGE1.fill((MAX_COLOR, MAX_COLOR, MAX_COLOR))
try:
    ...
    BOSS_IMG_STAGE2.fill((0, MAX_COLOR, 0))
try:
...
try:
    ...
    BOSS_IMG_STAGE5.fill((MAX_COLOR, 80, 0))
try:
...
try:
    ...
    draw.circle( (0, 200, 255), (12, 12), 12)  # 예비용 원형 눈물
...
HIT_ANIMATION_DURATION = 15  # 프레임 수 (약 0.25초)
...
def draw_jupiter_ring(surface, paddle_rect):
    ...
    for i in range(jupiter_ring_segments):
        ...
        # 세그먼트 그리기
        draw.circle( color, (int(x), int(y)), int(segment_size))
        glow_color = COLOR_LIGHT_BLUE
        draw.circle( glow_color, (int(x), int(y)), int(segment_size // 2))
...
def show_winner_text(winner_name):
    ...
    if "Player" in winner_name or "플레이어" in winner_name:
        main_color = (MAX_COLOR, MAX_COLOR, MAX_COLOR)
        accent_color = (0, 200, DEFAULT_SIZE)
        result_text = "VICTORY"
    ...
    for alpha in range(0, 256, 25):
        ...
        top_line_surface.fill((*accent_color, min(MAX_COLOR, alpha + 50)))
        ...
        if show_winner_name:
            ...
            bottom_line_surface.fill((*accent_color, min(MAX_COLOR, alpha + 50)))
    ...
    while pygame.time.get_ticks() - hold_start < 1000:
        draw_field()
        draw_objects()
        # 반투명 오버레이
        overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        ...
        pygame.display.flip()
        pygame.time.delay(16)
    for alpha in range(MAX_COLOR, -1, -35):
        draw_field()
        draw_objects()
        # 반투명 오버레이
        overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
...
def go_to_next_round():
    ...
    if current_stage == 3:
        # 게이지 100 감소, 최소 0 제한
        boss_special_gauge = max(0, boss_special_gauge - DEFAULT_SIZE)
...
def activate_tears_of_pain():
    ...
    falling_tears = []
    for _ in range(DEFAULT_TIMER):  # 눈물 개수
    ...
    if tears_active and falling_tears:
        for tear in falling_tears:
            ...
            if TEAR_IMG:
                ...
                draw.circle((0, 150, MAX_COLOR), (x, y), 12)
                draw.circle((100, 200, MAX_COLOR), (x - 3, y - 3), 5)
    
    # 눈물 파티클 효과 그리기
    if tear_particles:
        effect_renderer.draw_particles(SCREEN, tear_particles, (0, 100, MAX_COLOR))
def check_tear_collisions():
    global falling_tears, player_slow_timer
    new_tears = []
    for tear in falling_tears:
        x, y, speed, prev_y = tear
        tear_rect = pygame.Rect(x, y, DEFAULT_TIMER, DEFAULT_TIMER)
        if (
...
def apply_effect(effect_name):
    ...
    if effect_name not in ["battery", "chargebag", "spikeboots", "dashgear"]:
        play_sound_safe(SOUND_ACTIVE_ITEM)
    if effect_name == "long_paddle":
        ...
        for _ in range(20):
            ...
            rainbow_color = random.choice([
                (MAX_COLOR, 0, 0), (MAX_COLOR, 127, 0), (MAX_COLOR, MAX_COLOR, 0),
                (0, MAX_COLOR, 0), (0, 127, MAX_COLOR), (0, 0, MAX_COLOR), (127, 0, MAX_COLOR)
            ])
            # 파티클 효과는 나중에 추가 가능
...
def draw_predicted_trajectory():
            ...
            if segment_length > 0:
                ...
                for dash in range(num_dashes):
                    ...
                    if dash_start < segment_length:
                        ...
                        # 빨간색 점선
                        draw.line( (MAX_COLOR, 0, 0, 200), 
                                       (dash_start_x, dash_start_y), 
                                       (dash_end_x, dash_end_y), 5)
                        # 밝은 중심선 효과
                        draw.line( (MAX_COLOR, 100, 100, 150), 
                                       (dash_start_x, dash_start_y), 
                                       (dash_end_x, dash_end_y), 3)
            if i % 4 == 0:
                # 빨간색 발광 효과
                draw.circle( (MAX_COLOR, 150, 150, DEFAULT_SIZE), start_pos, 6)
                draw.circle( (MAX_COLOR, 50, 50, 200), start_pos, 4)
        # 궤적 Surface를 화면에 블릿
    ...
    if predicted_trajectory:
        ...
        # 외부 원
        draw.circle( (MAX_COLOR, 0, 0, 200), (40, 40), 30, 3)
        # 십자선
        draw.line( (MAX_COLOR, 0, 0, MAX_COLOR), (40, DEFAULT_TIMER), (40, 70), 3)
        draw.line( (MAX_COLOR, 0, 0, MAX_COLOR), (DEFAULT_TIMER, 40), (70, 40), 3)
        # 내부 점
        draw.circle( (MAX_COLOR, 50, 50, MAX_COLOR), (40, 40), 5)
        # 빛나는 효과
        for radius in range(DEFAULT_TIMER, 35, 5):
            alpha = 50 - radius
            draw.circle( (MAX_COLOR, 100, 100, alpha), (40, 40), radius, 1)
...
def draw_speech():
    ...
    if speech_timer > 0:
        ...
        bubble_surface = pygame.Surface((bubble_width + DEFAULT_TIMER, bubble_height + 20), pygame.SRCALPHA)
        # 1. 그림자 효과
        shadow_rect = pygame.Rect(4, 4, bubble_width, bubble_height)
        draw.rect( (0, 0, 0, 80), shadow_rect, border_radius=15)
        main_rect = pygame.Rect(0, 0, bubble_width, bubble_height)
        draw.rect( WHITE, main_rect, border_radius=15)
        draw.rect( BLACK, main_rect, 3, border_radius=15)
        tail_points = [
        ...
        highlight_rect = pygame.Rect(DEFAULT_TIMER, 5, bubble_width - 20, 8)
        highlight_surface = pygame.Surface((highlight_rect.width, highlight_rect.height), pygame.SRCALPHA)
        draw.rect( (MAX_COLOR, MAX_COLOR, 255, DEFAULT_SIZE), highlight_surface.get_rect(), border_radius=4)
        ...
        if speech_timer % 30 < 15:  # 깜빡임 효과
            ...
            sparkle_y = 10
            # 십자 반짝임
            pygame.draw.line(bubble_surface, (MAX_COLOR, MAX_COLOR, 200), 
            ...
            # 대각선 반짝임
            pygame.draw.line(bubble_surface, (MAX_COLOR, MAX_COLOR, 150), 
            ...
            pygame.draw.line(bubble_surface, (MAX_COLOR, MAX_COLOR, 150), 
                           ...
                           (sparkle_x + sparkle_size//2, sparkle_y - sparkle_size//2), 1)
        # 텍스트 그림자
        text_shadow = font.render(speech_text, True, COLOR_4)
...
def handle_lightning_master_as_top():
    ...
    if is_waiting_for_serve:
        ...
        def fake_motion():
            ...
            if style == 1:
                return math.sin(time_now / DEFAULT_SIZE) * 2.5
            elif style == 2 and random.random() < 0.02:
                return random_direction() * random.randint(20, 30)
            elif style == 3:
                return math.sin(time_now / 300) * 4
            elif style == 4 and random.random() < 0.015:
                return random_direction() * 10
            return 0
        # --- 보스 서브 차례 ---
...
def handle_player(keys):
    global special_gauge, special_ready, special_active, recent_dash_time, recent_dash_success_window, mega_smashing_bonus_applied
    ...
    global player_hit_animation_active, player_hit_animation_timer  # 플레이어 패들 애니메이션
        ...
        if rolling_active:
            ...
            if max_charges > 1 and rolling_charges > 0:
                ...
                if keys[pygame.K_LEFT] and down_pressed and special_gauge >= required_gauge and rolling_charges > 0:
                    ...
                    battery_bonus = academy.get_skill_bonus("dash_battery_pack")
                    final_gauge_cost = max(DEFAULT_TIMER, int(discounted_cost * (1 - battery_bonus)))  # 최소 10은 소모
    ...
    if check_ball_player_collision() and not is_waiting_for_serve:
        # 디버그: 충돌 위치 정보
        ...
        # 플레이어 패들 애니메이션 활성화
        player_hit_animation_active = True
        player_hit_animation_timer = HIT_ANIMATION_DURATION
...
border_flash_color = COLOR_2  # 깜빡임 색상 (노란색) - 더 눈에 띄게
...
def handle_emotional_overdrive():
    ...
    overdrive_trails = [(x, y, a - DEFAULT_TIMER) for x, y, a in overdrive_trails if a > 10][:15]
    # 4. 보스 반짝임 타이머
    ...
    for grenade in grenades[:]:
        ...
        if abs(grenade["x"] - grenade["target_x"]) < 30 and abs(grenade["y"] - grenade["target_y"]) < 30:
            ...
            for _ in range(50):  # 파편 수 증가
                ...
                vy = math.sin(math.radians(angle)) * speed
                tear_particles.append([
                    grenade["x"] + random.uniform(-DEFAULT_TIMER, DEFAULT_TIMER),
                    grenade["y"] + random.uniform(-DEFAULT_TIMER, DEFAULT_TIMER),
                    vx, vy,
                    255,  # alpha
            ...
            for _ in range(20):
                ...
                tear_particles.append([
                    ...
                    vx, vy,
                    255,  # alpha
                    random.randint(DEFAULT_TIMER, 15)  # 섬광 크기
                ])
            explosion_zones.append(explosion_zone)
            grenades.remove(grenade)
            # 폭발 효과음
            try:
                play_sound_safe(SOUND_WALL)
            except:
                pass
    ...
    for smoke_grenade in smoke_grenades[:]:
        if not smoke_grenade["arrived"]:
            ...
            if smoke_grenade["timer"] >= 60:
                ...
                for i in range(30):
                    angle = random_angle()
                    dist = random.uniform(0, DEFAULT_TIMER)
                    particle = {
        ...
        if molotov["y"] <= molotov["target_y"]:
            ...
            for i in range(15):
                flame = {
                    ...
                    "y": molotov["target_y"] + random.uniform(-DEFAULT_TIMER, DEFAULT_TIMER),
        ...
        if fire_zone["spread_timer"] % 5 == 0 and len(fire_zone["flames"]) < 30:
            for i in range(3):
                flame = {
                    ...
                    "size": random.uniform(DEFAULT_TIMER, 25),
...
def draw_balloons():
    ...
    if balloon_active and balloons:
        for balloon in balloons:
            ...
            draw.circle((MAX_COLOR, MAX_COLOR, 255, 128), 
                             (highlight_x, highlight_y), 
                             highlight_radius)
    
    if balloon_particles:
        effect_renderer.draw_particles(SCREEN, balloon_particles, (MAX_COLOR, 200, DEFAULT_SIZE))
...
def apply_red_overlay(surface, intensity):
    ...
    # 빨간색 오버레이 색상 계산
    red_factor = min(MAX_COLOR, 255 + int(intensity * 0.8))
...
def apply_blue_overlay(surface, intensity):
    ...
    overlay = pygame.Surface(surface.get_size(), pygame.SRCALPHA)
    # 파란색 오버레이 색상 계산
    blue_factor = min(MAX_COLOR, int(intensity * 0.5))
...
def draw_water_trail():
    ...
    if water_particles:
        effect_renderer.draw_particles(SCREEN, water_particles, COLOR_LIGHT_BLUE)
def draw_pachinko():
    ...
    draw.rect(COLOR_GOLD, main_container, 4)
    # 내부 컨테이너
    inner_container = pygame.Rect(container_x + 20, container_y + 20, container_width - 40, container_height - 40)
    draw.rect((15, DEFAULT_TIMER, 5), inner_container)
    draw.rect((MAX_COLOR, 200, 0), inner_container, 3)
    # 제목 영역
    title_area = pygame.Rect(container_x + 30, container_y + 30, container_width - 60, 80)
    draw.rect((30, 20, DEFAULT_TIMER), title_area)
    draw.rect(COLOR_GOLD, title_area, 3)
    # 제목 텍스트
    title_font = pygame.font.Font("NanumSquareB.ttf", 40)
    title_text = title_font.render("🎰 슬롯머신", True, COLOR_GOLD)
    ...
    # 슬롯 배경
    slot_bg_rect = pygame.Rect(slot_area_x - DEFAULT_TIMER, slot_area_y - DEFAULT_TIMER, slot_area_width + 20, slot_area_height + 20)
    draw.rect((20, 15, DEFAULT_TIMER), slot_bg_rect)
    draw.rect(COLOR_GOLD, slot_bg_rect, 3)
    # 3개 슬롯 그리기
    ...
    for reel_idx in range(3):
        ...
        draw.rect((DEFAULT_TIMER, 8, 5), reel_rect)
        draw.rect((MAX_COLOR, 200, 0), reel_rect, 2)
        item_size = 60
        ...
        for i in range(visible_items):
            ...
            if item_y + item_size > reel_y and item_y < reel_y + slot_height:
                ...
                if item_idx < len(slot_reels[reel_idx]):
                    ...
                    draw.rect((MAX_COLOR, 180, 0), item_rect, 2)
                    # 아이템 아이콘
        ...
        center_rect = pygame.Rect(reel_x - 5, center_y - 35, slot_width + DEFAULT_TIMER, 70)
        # 글로우 효과
        for i in range(3):
            glow_alpha = 100 - i * 30
        ...
        draw.rect(COLOR_GOLD, center_rect, 4)
    # 결과 표시
    if pachinko_phase == 3 and slot_final_result:
        ...
        for i in range(result_height):
            ...
            r = int(50 + ratio * DEFAULT_SIZE)
            g = int(30 + ratio * 80)
            b = int(10 + ratio * 40)
            draw.line((r, g, b), (result_x, result_y + i), (result_x + result_width, result_y + i))
        draw.rect(COLOR_GOLD, (result_x, result_y, result_width, result_height), 4)
        # 당첨 아이템 표시
        ...
        text = result_font.render(result_text, True, (MAX_COLOR, MAX_COLOR, MAX_COLOR))
        ...
        text_small = continue_font.render(continue_text, True, (MAX_COLOR, MAX_COLOR, MAX_COLOR))
        text_small_rect = text_small.get_rect(center=(result_x + result_width // 2, result_y + DEFAULT_SIZE))
        ...
        result_y = container_y + container_height + 20
        # 어두운 배경
        draw.rect((30, 20, DEFAULT_TIMER), (result_x, result_y, result_width, result_height))
        draw.rect(COLOR_DARK_GRAY2, (result_x, result_y, result_width, result_height), 3)
        # 꽝 메시지
        result_font = pygame.font.Font("NanumSquareB.ttf", 32)
        result_text = "💔 아쉽게도 꽝입니다... 💔"
        text = result_font.render(result_text, True, COLOR_LIGHT_GRAY2)
        text_rect = text.get_rect(center=(result_x + result_width // 2, result_y + 60))
        SCREEN.blit(text, text_rect)
        # 계속하기 안내
        continue_font = pygame.font.Font("NanumSquareR.ttf", 20)
        continue_text = "✨ SPACE를 눌러 계속... ✨"
        text_small = continue_font.render(continue_text, True, COLOR_LIGHT_GRAY2)
        text_small_rect = text_small.get_rect(center=(result_x + result_width // 2, result_y + DEFAULT_SIZE))
        SCREEN.blit(text_small, text_small_rect)
    # 안내 텍스트
    if pachinko_phase == 0:
        ...
        text = guide_font.render(guide_text, True, COLOR_GOLD)
...
def show_item_obtained_effect(item_data, item_x=None, item_y=None):
    ...
    target_x = 100  # 플레이어 쪽
    target_y = HEIGHT // 2
    item_color = item_data.get("color", COLOR_1)
...
def draw_item_obtained_effect():
    ...
    for i in range(glow_radius):
        ...
        draw.circle( (MAX_COLOR, MAX_COLOR, 255, alpha), (glow_radius, glow_radius), glow_radius - i)
    ...
    obtain_text = obtain_font.render("획득!", True, COLOR_GOLD)
    ...
    for x in range(0, surface.get_width(), step):
        for y in range(0, surface.get_height(), step):
            ...
            if pixel.a != 0:  # 기체 부분에만 적용
                glow_r = min(MAX_COLOR, max(0, pixel.r + intensity))
                glow_g = min(MAX_COLOR, max(0, pixel.g + intensity))
                glow_b = min(MAX_COLOR, max(0, pixel.b + intensity))
                try:
                    surface.set_at((x, y), pygame.Color(glow_r, glow_g, glow_b, pixel.a))
                except ValueError:
                    # 색상 값이 잘못된 경우 기본값 사용
                    surface.set_at((x, y), pygame.Color(MAX_COLOR, MAX_COLOR, 255, pixel.a))
def draw_gauge_background(x, y, width, height):
    """게이지 배경 그리기"""
    draw.rect(COLOR_4, (x, y, width, height))
...
def draw_gauge_text(x, y, text, font_size=20):
    """게이지 텍스트 그리기"""
    font = pygame.font.Font(None, font_size)
    text_surface = font.render(text, True, COLOR_1)
...
def draw_player_gauge():
    ...
    if displayed_gauge > 0:
        ...
        if displayed_gauge < 150:
            ...
            # 레벨 2: 전기 청색
            base_color = (100, 180, MAX_COLOR)
            energy_color = (150, 200, MAX_COLOR)
        elif displayed_gauge < 350:
            # 레벨 3: 강렬한 시안
            base_color = (0, 220, MAX_COLOR)
            energy_color = (100, 240, MAX_COLOR)
        else:
            pulse = abs(math.sin(time_now * 0.005))
            base_color = (
                int(100 + pulse * DEFAULT_SIZE),
                int(150 + pulse * 50),
                255
            )
            energy_color = (200, 200, MAX_COLOR)
        # 메인 에너지 채우기
        ...
        for layer in range(3):
            ...
            if layer_width > 0:
                for i in range(fill_height):
                    ...
                    # 색상에 펄스 적용
                    final_color = tuple(min(MAX_COLOR, int(c + pulse_offset * 20)) for c in color)
        ...
        if displayed_gauge >= 350:
            ...
            for _ in range(spark_count):
                ...
                for i in range(3):
                    ...
                    current_y = next_y
                if len(points) > 1:
                    draw.lines((MAX_COLOR, MAX_COLOR, MAX_COLOR), False, points, 1)
        # 상단 에너지 글로우
        if fill_ratio > 0.5:
            for i in range(5):
                alpha = 100 - i * 20
                glow_color = tuple(min(MAX_COLOR, c + 50) for c in energy_color)
    ...
    if displayed_gauge >= 350:
        ...
        if (time_now // 100) % 3 == 0:  # 간헐적으로
            ...
            draw.lines((150, 200, MAX_COLOR), False, 
        ...
        corner_color = (corner_glow, corner_glow, MAX_COLOR)
        # 상단 코너
    ...
    text_color = COLOR_1 if displayed_gauge < special_gauge_max else COLOR_LIGHT_YELLOW  # 만렙일 때는 노란색
    try:
    ...
    draw.rect(COLOR_DARK_GRAY2, text_bg_rect, 1, border_radius=3)
    # 텍스트 렌더링
    ...
    if item_gauge_active:
        ...
        if long_boost_active and long_boost_timer > 0:
            ...
            if remaining_ratio > 0.6:
                base_color = COLOR_GOLD  # 골드
                energy_color = (MAX_COLOR, 235, DEFAULT_SIZE)
            elif remaining_ratio > 0.3:
                base_color = (255, 165, 0)  # 주황색
                energy_color = (MAX_COLOR, 195, 50)
            else:
                # 끝날 때 깜빡임 효과
                pulse = abs(math.sin(time_now * 0.01))
                base_color = (MAX_COLOR, int(100 + pulse * 65), 0)
                energy_color = (MAX_COLOR, int(150 + pulse * 50), 50)
        elif predictor_active and predictor_timer > 0:
            # 레이저스코프 활성화 시
            remaining_ratio = predictor_timer / 600  # 10초 기준 (60fps * 10초 = 600)
            fill_height = int((gauge_height - 4) * remaining_ratio)
            if remaining_ratio > 0.6:
                base_color = COLOR_LIGHT_RED  # 밝은 빨강
                energy_color = (MAX_COLOR, 150, 150)
            elif remaining_ratio > 0.3:
                base_color = (255, 50, 50)  # 진한 빨강
                energy_color = COLOR_LIGHT_RED
            else:
                # 끝날 때 깜빡임
                pulse = abs(math.sin(time_now * 0.01))
                base_color = (MAX_COLOR, int(50 * pulse), int(50 * pulse))
                energy_color = (MAX_COLOR, int(100 * pulse), int(100 * pulse))
        else:
            pass  # 게이지가 없음
        if fill_height > 0:
            fill_y = gauge_y + gauge_height - fill_height - 2
            for layer in range(3):
                ...
                if layer_width > 0:
                    ...
                    # 레이어별 색상 조정
                    layer_color = (
                        min(MAX_COLOR, base_color[0] + layer * 20),
                        min(MAX_COLOR, base_color[1] + layer * 20),
                        min(MAX_COLOR, base_color[2] + layer * 20)
                    )
            ...
            if long_boost_active and remaining_ratio > 0.1:
                ...
                if random.random() < 0.3:  # 30% 확률로 반짝임
                    ...
                    sparkle_color = (MAX_COLOR, MAX_COLOR, 200, 150)
                ...
                if fill_height > 6:  # 최소 높이 확인 (여유 있게)
                    ...
                    if actual_fill_height > 0:
                        scan_offset = (time_now // DEFAULT_TIMER) % actual_fill_height
                        scan_y = actual_fill_top + scan_offset
                        if actual_fill_top <= scan_y < actual_fill_bottom:
                            scan_color = (MAX_COLOR, 200, 200, 180)
    ...
    for i in range(max_tokens):
        ...
        if i < len(token_states) and token_states[i]:
            # 사용 가능한 토큰 - 밝은 빨간색
            token_color = (MAX_COLOR, 80, 80)
            glow_color = (MAX_COLOR, 150, 150)
            is_available = True
        else:
            # 사용된 토큰 - 충전 중
            # 충전 진행률 계산
        ...
        if not is_available and 'charge_progress' in locals() and 'is_charging' in locals() and is_charging and charge_progress > 0:
            ...
            if charge_progress > 0.5:
                ...
                draw.circle( (MAX_COLOR, 100, 100, pulse_alpha),
            ...
            border_color = COLOR_LIGHT_GRAY2 if is_available else (80, 80, 80)
            ...
            if is_available:
                ...
                draw.circle((MAX_COLOR, MAX_COLOR, MAX_COLOR), 
...
def draw_aircraft_carrier_boss(boss_speed=0, boss_x=0):
    ...
    def apply_hit_effect(color):
        if stage6_boss_hit_flash:
            r, g, b = color
            return (min(MAX_COLOR, r + 120), max(0, g - 30), max(0, b - 30))
        return color
    # 최하부 그림자 레이어
    shadow_color = apply_hit_effect((35, 40, 45))
    draw.rect( shadow_color, (6, 48, carrier_width-12, 34))
    hull_base_color = apply_hit_effect((65, 75, 85) if damage_ratio < 0.7 else (55, 60, 70))
    draw.rect( hull_base_color, (8, 45, carrier_width-16, 32))
    # 하부 하이라이트
    draw.line( apply_hit_effect((75, 85, 95)), (8, 46), (carrier_width-8, 46), 1)
    # 중하부 전환 레이어
    trans_color = apply_hit_effect((72, 82, 95) if damage_ratio < 0.6 else (62, 70, 78))
    draw.rect( trans_color, (DEFAULT_TIMER, 40, carrier_width-20, 28))
    hull_mid_color = apply_hit_effect((80, 90, 105) if damage_ratio < 0.5 else (70, 75, 85))
    draw.rect( hull_mid_color, (12, 35, carrier_width-24, 25))
    # 중부 하이라이트
    draw.line( apply_hit_effect((90, 100, 115)), (12, 36), (carrier_width-12, 36), 1)
    hull_top_color = apply_hit_effect((95, 110, 125) if damage_ratio < 0.3 else (85, 95, 110))
    draw.rect( hull_top_color, (16, 25, carrier_width-32, 20))
    # 상부 메탈릭 하이라이트
    draw.line( (115, 130, 145), (16, 26), (carrier_width-16, 26), 1)
    draw.line( (105, 120, 135), (16, 28), (carrier_width-16, 28), 1)
    ...
    for i in range(8):  # 더 많은 패널
        ...
        if panel_x < carrier_width - 35:
            draw.rect( (70, 80, 90), (panel_x-1, 27, 23, 17))
            draw.rect( panel_base, (panel_x, 28, 21, 15))
            draw.rect( panel_glow, (panel_x, 28, 21, 15), 1)
            # 패널 내부 고급 디테일
            ...
            core_x = panel_x + 10
            core_y = 35
            draw.circle( panel_glow, (core_x, core_y), 2)
            if boss_current_health > 8:
                draw.circle( (180, 210, MAX_COLOR), (core_x, core_y), 1)
    # 구조적 리벳 라인들
    ...
    for y_pos in [30, 40, 50, 60]:
        ...
        for x_pos in range(25, carrier_width - 20, 20):
            draw.circle( rivet_color, (x_pos, y_pos), 1)
    # 수직 구조 라인들
    ...
    for hx, hy in hardpoint_positions:
        if hx < carrier_width - 25:
            # 하드포인트 베이스
            draw.rect( (75, 85, 100), (hx-3, hy-3, 6, 6))
            draw.rect( (95, 105, 120), (hx-2, hy-2, 4, 4))
            # 마운트 포인트
            draw.circle( (115, 125, 140), (hx, hy), 2)
            draw.circle( (135, 145, 160), (hx, hy), 1)
    # 배기구/벤트 시스템
    ...
    for vx, vy in vent_positions:
        if vx < carrier_width - 20:
            ...
            # 벤트 프레임
            draw.rect( (80, 90, 105), (vx-1, vy-1, 7, DEFAULT_TIMER), 1)
    # 센서 어레이 시스템
    sensor_positions = [(45, 20), (75, 20), (105, 20), (135, 20)]
    for sx, sy in sensor_positions:
        if sx < carrier_width - 15:
            # 센서 돔
            draw.circle( (90, 110, 130), (sx, sy), 3)
            draw.circle( (110, 130, 150), (sx, sy), 2)
            # 센서 렌즈
            draw.circle( (150, 170, 190), (sx-1, sy-1), 1)
    turret_positions = [(30, 38), (65, 36), (100, 36), (135, 38), (170, 38)]
    for idx, (tx, ty) in enumerate(turret_positions):
        if tx < carrier_width - 25:
            draw.circle( (50, 60, 70), (tx, ty), DEFAULT_TIMER)
            draw.circle( (70, 80, 90), (tx, ty), 8)
            draw.circle( (90, 100, 110), (tx, ty), 6)
            ...
            # 터렛 중앙 에너지 코어
            draw.circle( (150, 180, 210), (tx, ty), 3)
            draw.circle( (180, 210, 240), (tx-1, ty-1), 1)
    bridge_x = carrier_width - 75
    # 브릿지 기초 플랫폼
    draw.rect( (55, 65, 75), (bridge_x-2, 18, 54, 32))
    draw.rect( (75, 85, 100), (bridge_x, 15, 50, 30))
    bridge_mid = (95, 105, 120) if boss_current_health > 5 else (75, 85, 95)
    draw.rect( bridge_mid, (bridge_x + 5, 12, 40, 22))
    # 중간층 디테일
    for i in range(3):
    ...
    draw.rect( bridge_top, (bridge_x + DEFAULT_TIMER, 8, 30, 15))
    # 지휘탑 안테나 타워
    draw.rect( (115, 125, 140), (bridge_x + 23, 3, 4, 8))
    draw.line( (135, 145, 160), (bridge_x + 25, 3), (bridge_x + 25, 0), 2)
    # 파노라마 브릿지 창문 시스템
    for i in range(5):
        window_x = bridge_x + 10 + i * 6
        # 창문 프레임
        draw.rect( (85, 95, 105), (window_x-1, 9, 6, DEFAULT_TIMER), 1)
        glow_intensity = int(150 + 50 * abs(math.sin(time_now * 0.002 + i * 0.3)))
        draw.rect( (glow_intensity, glow_intensity + 30, 200), 
                        (window_x, DEFAULT_TIMER, 4, 8))
        # 창문 반사 및 디테일
        pygame.draw.line(carrier_surface, (200, 220, MAX_COLOR), 
                        (window_x, DEFAULT_TIMER), (window_x + 2, DEFAULT_TIMER), 1)
    radar_x = bridge_x + 25
    # 레이더 베이스
    draw.circle( (80, 90, 100), (radar_x, 8), 7)
    draw.circle( (120, 140, 160), (radar_x, 8), 5)
    draw.circle( (140, 160, 180), (radar_x, 8), 3)
    ...
    for r in range(1, 4):  # 여러 개의 스캔 라인
        ...
        scan_color = (min(MAX_COLOR, 80 + r*15), min(MAX_COLOR, 100 + r*15), min(MAX_COLOR, 120 + r*15))
        pygame.draw.line(carrier_surface, scan_color, 
                        (radar_x, 8), (antenna_end_x, antenna_end_y), 2)  # 두께도 줄임
    pulse = abs(math.sin(time_now * 0.005))
    core_color = (int(130 + pulse * 20), int(135 + pulse * 20), int(140 + pulse * 20))
    draw.circle( core_color, (radar_x, 8), 2)
    # 보조 통신 안테나들
    for i, ant_x in enumerate([bridge_x + DEFAULT_TIMER, bridge_x + 40]):
        ant_height = 5 - i
        pygame.draw.line(carrier_surface, (120, 125, 130), 
                        (ant_x, 15), (ant_x, ant_height), 2)
        # 안테나 팁
        draw.circle( (140, 145, 150), (ant_x, ant_height), 1)
    shield_antenna_x = carrier_width // 2
    shield_antenna_y = 75  # 함선 앞쪽
    draw.circle( (100, 110, 120), (shield_antenna_x, shield_antenna_y), 5)
    draw.circle( (120, 130, 140), (shield_antenna_x, shield_antenna_y), 3)
    # 안테나 기둥
    pygame.draw.line(carrier_surface, (130, 135, 140), 
    ...
    if shield_antenna_active:
        # 쉴드 활성화 시 밝게 빛남
        draw.circle( (180, 185, 190), 
                          (shield_antenna_x, shield_antenna_y - 8), 4)
        draw.circle( (200, 205, 210), 
                          (shield_antenna_x, shield_antenna_y - 8), 2)
    else:
        # 비활성화 시 약하게 빛남
        glow_value = min(MAX_COLOR, int(120 + tip_glow * 0.3))
        glow_color = (glow_value, glow_value, glow_value)
        pygame.draw.circle(carrier_surface, glow_color, 
                          (shield_antenna_x, shield_antenna_y - 8), 3)
        draw.circle( (140, 145, 150), 
                          (shield_antenna_x, shield_antenna_y - 8), 1)
    laser_cannon_x = carrier_width // 2
    laser_cannon_y = 85  # 함선 아래쪽
    draw.circle( (70, 80, 90), (laser_cannon_x, laser_cannon_y), 8)
    draw.circle( (90, 100, 110), (laser_cannon_x, laser_cannon_y), 6)
    if laser_charging or laser_cannon_active:
        ...
        if laser_charging:
            ...
            pygame.draw.circle(carrier_surface, 
                             (min(MAX_COLOR, charge_glow), min(MAX_COLOR, charge_glow + 50), MAX_COLOR), 
            ...
            for _ in range(int(5 * charge_progress)):
                ...
                draw.circle( (150, 200, MAX_COLOR), (spark_x, spark_y), 1)
    else:
        pygame.draw.line(carrier_surface, (50, 60, 70), 
                        (laser_cannon_x, laser_cannon_y), 
    ...
    # 캐논 중심 코어
    draw.circle( (110, 170, 210), (laser_cannon_x, laser_cannon_y), 3)
    if laser_charging or laser_cannon_active:
        # 충전/발사 중 밝게
        draw.circle( (150, 200, MAX_COLOR), (laser_cannon_x, laser_cannon_y), 2)
        ...
        if i < len(fighter_positions):
            ...
            if fx < carrier_width - 15:
                ...
                pygame.draw.polygon(carrier_surface, fighter_body, 
                                   [(fx, fy), (fx+12, fy+1), (fx+DEFAULT_TIMER, fy+3), (fx+2, fy+3)])
                wing_color = (65, 80, 95)
                # 왼쪽 날개
                ...
                # 콕핏 하이라이트
                draw.circle( (120, 140, 160), (fx+6, fy+1), 1)
    # 대형 슬링스트림 엔진 4기
    engine_positions = [(3, 48), (3, 56), (3, 64), (3, 72)]
    for idx, (ex, ey) in enumerate(engine_positions):
        draw.rect( (40, 50, 60), (ex, ey-1, 18, DEFAULT_TIMER))
        draw.rect( (60, 70, 80), (ex, ey, 16, 8))
        draw.rect( (80, 90, DEFAULT_SIZE), (ex+2, ey+1, 12, 6))
        ...
        if boss_velocity > 0.3:  # 더 민감하게 반응
            ...
            for g in range(3):
                pygame.draw.line(carrier_surface, (MAX_COLOR, 80 - g*20, 30), 
                                (ex, ey + 4), (ex - jet_length - g*2, ey + 4), 5 - g)
            for i in range(3):
                offset_y = flame_offset - 1 + i
                flame_alpha = 200 - i * 50
                flame_red = min(MAX_COLOR, jet_intensity + i * DEFAULT_TIMER)
            ...
            mid_color = (MAX_COLOR, min(MAX_COLOR, jet_intensity + 30), 80)
            pygame.draw.line(carrier_surface, mid_color, 
                            (ex, ey + 4), (ex - jet_length*2//3, ey + 4), 3)
            core_color = (MAX_COLOR, MAX_COLOR, min(MAX_COLOR, 180 + boss_velocity * 20))
            ...
            if random.random() < 0.3:
                ...
                pygame.draw.line(carrier_surface, (200, 200, MAX_COLOR),
                                (ex, ey + 4), (bolt_x, bolt_y), 1)
            particle_count = int(3 + boss_velocity * 2)
            for _ in range(particle_count):
                spark_x = ex - random.randint(0, jet_length + DEFAULT_TIMER)
                spark_y = ey + 4 + random.randint(-3, 3)
                spark_size = random.choice([1, 1, 2])
                spark_color = random.choice([
                    (MAX_COLOR, random.randint(180, MAX_COLOR), random.randint(0, DEFAULT_SIZE)),
                    (MAX_COLOR, random.randint(150, 200), 50),
                    (MAX_COLOR, MAX_COLOR, random.randint(150, MAX_COLOR))
                ])
                draw.circle( spark_color, (spark_x, spark_y), spark_size)
    # 자동 방어 미사일 터렛
    ...
    for idx, (dx, dy) in enumerate(defense_positions):
        if dx < carrier_width - 15:
            ...
            if turret_id not in turret_angles:
                turret_angles[turret_id] = random.randint(0, FULL_CIRCLE_DEGREES)
            ...
            # 터렛 베이스
            draw.circle( (65, 75, 85), (dx, dy), 4)
            draw.circle( (85, 95, 105), (dx, dy), 3)
            # 아래를 향하는 포신 그리기
            barrel_length = 10
            barrel_end_x = dx + int(math.cos(angle_rad) * barrel_length)
            barrel_end_y = dy + int(math.sin(angle_rad) * barrel_length)
            draw.line( (90, 100, 110), (dx, dy), (barrel_end_x, barrel_end_y), 3)
            draw.circle( (120, 130, 140), (barrel_end_x, barrel_end_y), 3)
            draw.circle( (100, 200, 255), (barrel_end_x, barrel_end_y), 2)
            ...
            # 터렛 중앙 렌즈
            lens_color = COLOR_DARK_GRAY2  # 회색
            draw.circle( lens_color, (dx, dy), 1)
            draw.circle( COLOR_5, (dx, dy), 2)
    if damage_ratio > 0.2:
        for _ in range(int(damage_ratio * 8)):
            spark_x = random.randint(DEFAULT_TIMER, carrier_width - DEFAULT_TIMER)
    ...
    if damage_ratio > 0.5:
        ...
        for _ in range(int(damage_ratio * 7)):
            ...
            if random.random() < 0.6:
                draw.circle( (MAX_COLOR, 100, 0), (leak_x, leak_y), 3)
                draw.circle( (MAX_COLOR, 150, 0), (leak_x, leak_y), 2)
                draw.circle( (MAX_COLOR, 200, 50), (leak_x, leak_y), 1)
            else:
                draw.circle( COLOR_LIGHT_BLUE, (leak_x, leak_y), 2)
                draw.circle( (150, 200, MAX_COLOR), (leak_x, leak_y), 1)
            # 연기 효과
            if random.random() < 0.5:
                smoke_x = leak_x + random.randint(-5, 5)
                smoke_y = leak_y - random.randint(3, DEFAULT_TIMER)
                draw.circle( COLOR_4, (smoke_x, smoke_y), 2)
    # 파란색 테두리가 나타나지 않도록 주석 처리
    #     # 쉴드 윤곽선
    #     # 쉴드 헥사곤 패턴
    ...
    if health_percent <= 50:
        ...
        for smoke_x, smoke_y in smoke_points:
            ...
            for i in range(3):
                ...
                draw.circle( smoke_color, (smoke_size, smoke_size), smoke_size)
                # 연기 위치 약간 흔들기
        ...
        for fire_x, fire_y in fire_points:
            ...
            if random.random() < 0.3 * fire_intensity:
                for _ in range(int(2 * fire_intensity)):
                    ...
                    spark_y = fire_y + random.randint(-DEFAULT_TIMER, -2)
                    spark_color = (MAX_COLOR, random.randint(100, 200), 0)
                    draw.circle( spark_color, (spark_x, spark_y), random.randint(1, 2))
    # 체력 10% 이하: 폭발 효과 추가
    if health_percent <= 10:
        # 랜덤 폭발 효과
        if random.random() < 0.2:  # 20% 확률로 폭발
            ...
            draw.circle( (MAX_COLOR, MAX_COLOR, MAX_COLOR), (explosion_x, explosion_y), flash_size)
            draw.circle( (MAX_COLOR, 200, DEFAULT_SIZE), (explosion_x, explosion_y), flash_size - 3)
            draw.circle( (MAX_COLOR, 100, 0), (explosion_x, explosion_y), flash_size - 6)
            # 파편 효과
            for _ in range(8):
                debris_x = explosion_x + random.randint(-20, 20)
                debris_y = explosion_y + random.randint(-20, 20)
                debris_color = (random.randint(150, MAX_COLOR), random.randint(50, 150), 0)
                draw.circle( debris_color, (debris_x, debris_y), random.randint(1, 3))
    ...
    # 캐논 베이스 그리기
    draw.circle( (80, 90, DEFAULT_SIZE), (cannon_x, cannon_y), 12)
    draw.circle( COLOR_5, (cannon_x, cannon_y), DEFAULT_TIMER)
    # 회전하는 캐논 포신
    ...
    pygame.draw.line(carrier_surface, COLOR_10, 
                     (cannon_x, cannon_y), (cannon_barrel_end_x, cannon_barrel_end_y), 6)
    draw.circle( (120, 130, 140), 
                      (cannon_barrel_end_x, cannon_barrel_end_y), 6)
    pygame.draw.circle(carrier_surface, COLOR_9, 
    ...
    if laser_charging:
        ...
        for _ in range(int(10 * charge_ratio)):
            ...
            particle_dist = random.uniform(DEFAULT_TIMER, 30 * (1 - charge_ratio))
    ...
    antenna_y = 25
    # 왼쪽 안테나
    pygame.draw.rect(carrier_surface, COLOR_10, 
...
def draw_player():
    """플레이어 이미지 그리기"""
    global BOSS, PLAYER, player_hit_animation_active, player_hit_animation_timer
    global molotov_throwing, grenade_throwing, flare_throwing
    global molotov_throw_timer, grenade_throw_timer, flare_throw_timer
    global new_boss_mode_active, selected_bottom_boss, long_boost_scale
    global HIT_ANIMATION_DURATION, PADDLE_WIDTH, PLAYER_IMG
    if new_boss_mode_active:
        ...
        try:
            if selected_bottom_boss == 1:
                ...
                base_ufo_img = pygame.transform.scale(boss_img, (250, DEFAULT_SIZE))
            elif selected_bottom_boss == 2:
    ...
    tilt_angle = 0
    if player_hit_animation_active:
        progress = HIT_ANIMATION_DURATION - player_hit_animation_timer
        # 더 큰 스윙 애니메이션: -30도에서 시작해서 +15도까지
        tilt_angle = -30 + (progress * 3)
        # 디버그 출력
        if player_hit_animation_timer == HIT_ANIMATION_DURATION:  # 처음 한 번만 출력
            print(f"🎮 플레이어 히트 애니메이션 시작! timer={player_hit_animation_timer}, angle={tilt_angle}, duration={HIT_ANIMATION_DURATION}")
        player_hit_animation_timer -= 1
        if player_hit_animation_timer <= 0:
            player_hit_animation_active = False
    else:
    ...
    if special_ready:
        if (time_now // 250) % 2 == 0:
            ...
            # 미사일 무적 시간이면 반투명 처리
            if is_missile_invulnerable and (time_now // DEFAULT_SIZE) % 2 == 0:
    ...
    if molotov_throwing or grenade_throwing or flare_throwing:
        ...
        if molotov_throwing:
            ...
            try:
                ...
                draw.circle( (MAX_COLOR, 100, 0), (20, 20), 15)
        elif grenade_throwing:
            ...
            try:
                ...
                # 수류탄 기본 아이콘
                item_icon = pygame.Surface((40, 40), pygame.SRCALPHA)
                draw.circle( (100, 150, DEFAULT_SIZE), (20, 20), 15)
        elif flare_throwing:
            ...
            try:
                ...
                # 조명탄 기본 아이콘
                item_icon = pygame.Surface((40, 40), pygame.SRCALPHA)
                draw.circle( (MAX_COLOR, MAX_COLOR, 200), (20, 20), 15)
    ...
    for zone in explosion_zones:
        if zone["active"]:
            ...
            if shockwave_radius < zone["radius"] * 2.5:
                draw.circle( (MAX_COLOR, MAX_COLOR, 255, 30), 
            ...
            for i in range(zone["radius"], 0, -3):
                ...
                if zone["duration"] > 10:  # 초기: 흰색-노란색
                    r = 255
                    g = int(255 - (1 - ratio) * DEFAULT_SIZE)
                    b = int(200 - (1 - ratio) * 150)
                elif zone["duration"] > 5:  # 중반: 주황색-빨간색
                    r = 255
                    g = int(150 - (1 - ratio) * DEFAULT_SIZE)
                    b = int(50 - (1 - ratio) * 40)
                else:  # 후반: 빨간색-검은색
                    r = int(200 - (1 - ratio) * DEFAULT_SIZE)
                    g = int(50 - (1 - ratio) * 40)
                    b = 30
                ...
                color = (r, g, b, alpha)
                draw.circle( color, (int(center), int(center)), i)
    ...
    for smoke_grenade in smoke_grenades:
        if smoke_grenade["arrived"]:
            if smoke_grenade["timer"] < 60:
                ...
                draw.ellipse((0, 0, 0, DEFAULT_SIZE), shadow_rect)
                if smoke_grenade["timer"] % 15 < 8:
                    draw.circle((MAX_COLOR, 0, 0), 
                ...
                try:
                    ...
                    draw.circle(COLOR_DARK_GRAY2, 
                                     (int(smoke_grenade["x"]), int(smoke_grenade["y"] - 5)), DEFAULT_TIMER)
        else:
            # 날아가는 중인 연막탄
            # 궤적 그리기
            ...
            try:
                ...
                # 기본 원형 그리기
                draw.circle(COLOR_MID_GRAY, 
                                 (int(smoke_grenade["x"]), int(smoke_grenade["y"])), DEFAULT_TIMER)
    # 연막 지역 그리기
    for smoke_zone in smoke_zones:
    ...
    for flare in flares:
        ...
        if flare.get("arrived", False) and not flare["exploded"]:
            if flare["timer"] % 10 < 5:
                draw.circle(COLOR_LIGHT_YELLOW, 
                                 (int(flare["x"]), int(flare["y"])), 12)
            draw.circle((MAX_COLOR, 200, 0), 
            ...
            try:
                ...
                # 기본 원형 그리기
                draw.circle((255, 255, 200), (int(flare["x"]), int(flare["y"])), DEFAULT_TIMER)
    for zone in flare_zones:
        if zone.get("flash", False):
            # 섬광 효과: 매우 밝고 강렬한 빛
            if zone["duration"] > 0:
                ...
                flash_surface.fill((MAX_COLOR, MAX_COLOR, 230, flash_alpha))
                ...
                for i in range(5):  # 더 많은 레이어로 강렬함 표현
                    ...
                    if alpha > 0:
                        ...
                        # 흰색에 가까운 밝은 빛
                        color = (MAX_COLOR, MAX_COLOR, 240, min(MAX_COLOR, alpha))
                ...
                if zone["intensity"] > 0.7:
                    ...
                    cross_width = 5
                    draw.line((MAX_COLOR, MAX_COLOR, MAX_COLOR), 
            ...
            for i in range(3):
                ...
                if alpha > 0:
                    light_surface = pygame.Surface((int(radius * 2), int(radius * 2)), pygame.SRCALPHA)
                    color = (MAX_COLOR, MAX_COLOR, 200, alpha)
                    pygame.draw.circle(light_surface, color, 
                                     (int(radius), int(radius)), int(radius))
                    SCREEN.blit(light_surface, (zone["x"] - radius, zone["y"] - radius))
    if boss_confused_timer > 0:
        ...
        for i in range(num_questions):
            ...
            if math.sin(current_time * 0.01 + i) > 0:
                color = COLOR_LIGHT_YELLOW  # 밝은 노란색
            else:
    ...
    for molotov in molotovs:
        ...
        try:
            ...
            # 기본 원형 그리기
            draw.circle((255, 100, 0), (int(molotov["x"]), int(molotov["y"])), DEFAULT_TIMER)
    # 화염 지대 그리기 - 불길이 번지는 효과
    for fire_zone in fire_zones:
        oil_surface = pygame.Surface((fire_zone["width"], fire_zone["height"]), pygame.SRCALPHA)
        pygame.draw.ellipse(oil_surface, (30, 20, DEFAULT_TIMER, 50), 
    ...
    for wall in walls:
        ...
        draw.rect(COLOR_1, wall_rect, 2)  # 흰색 테두리
        # 균열 그리기
        if wall_crack_level > 0:
            crack_color = (MAX_COLOR, 0, 0) if wall_crack_level >= 3 else (MAX_COLOR, MAX_COLOR, 0)
            # 충돌횟수에 비례해서 균열 개수 증가
...
def draw_ball():
    """공 그리기 (화면 흔들림 포함)"""
    global current_stage, BALL, ball_fire_mode, fire_ball_size
    global screen_shake_offset_x, screen_shake_offset_y, inverted_controls
    global power_smashing_particles, power_smashing_parabola_active, power_smashing_freeze_active
    global mega_smashing_active, power_smashing_trails, quake_offset_y
    global ball_trail, special_active
    ball_rect = BALL.copy()
    ...
    if (power_smashing_parabola_active or power_smashing_freeze_active) and not mega_smashing_active:
        if len(power_smashing_trails) == 0 or \
           (len(power_smashing_trails) > 0 and 
            ...
            power_smashing_trails.append((BALL.centerx, BALL.centery, MAX_COLOR, BALL.width))
        # 오래된 잔상 제거 및 알파값 감소
        new_trails = []
        ...
        for i, trail in enumerate(power_smashing_trails):
            ...
            if i < len(power_smashing_trails) - 1:
                ...
                # 메인 번개
                lightning_color = (150, 200, MAX_COLOR, int(alpha * 0.8))
                draw.line(lightning_color[:3], (x, y), (next_x, next_y), max(1, int(3 * color_intensity)))
                # 주변 전기 스파크
                for _ in range(2):
                    spark_offset_x = random.randint(-DEFAULT_TIMER, DEFAULT_TIMER)
                    spark_offset_y = random.randint(-DEFAULT_TIMER, DEFAULT_TIMER)
                    spark_color = (200, 220, MAX_COLOR, int(alpha * 0.4))
        ...
        for particle in power_smashing_particles:
            ...
            if particle['life'] > 0:
                ...
                if particle.get('type') == 'spark':
                    ...
                    draw.circle( (MAX_COLOR, MAX_COLOR, 255, core_alpha), 
                                     (center, center), size)
            ...
            for meteor in mega_smashing_meteor_trail:
                ...
                if meteor['life'] > 0:
                        ...
                        for i in range(len(meteor['trail']) - 1):
                            ...
                            if trail_width > 0 and trail_alpha > 10:
                                ...
                                if h < 1:
                                    ...
                                    r, g, b = c, 0, x_val
                                # 네온 효과를 위한 밝기 증가
                                r = min(MAX_COLOR, int(r * 255 * 1.5))
                                g = min(MAX_COLOR, int(g * 255 * 1.5))
                                b = min(MAX_COLOR, int(b * 255 * 1.5))
                                draw.line((r, g, b, min(MAX_COLOR, trail_alpha // 2)), 
                                               (x1, y1), (x2, y2), max(1, trail_width + 2))
                                core_r = min(MAX_COLOR, (255 + r) // 2)
                                core_g = min(MAX_COLOR, (215 + g) // 2)
                                core_b = min(MAX_COLOR, b)
                                draw.line((core_r, core_g, core_b, min(MAX_COLOR, trail_alpha)), 
                    ...
                    if head_size > 0:
                        ...
                        draw.circle( (0, MAX_COLOR, 255, min(MAX_COLOR, glow_alpha // 2)),
                                         (head_size * 2, head_size * 2), head_size * 2)
                        draw.circle( (MAX_COLOR, 215, 0, min(MAX_COLOR, glow_alpha)),
        ...
        if power_smashing_parabola_active and not mega_smashing_active and random.random() < 0.7:
            ...
            for _ in range(1):
                ...
                power_smashing_particles.append({
                    ...
                    'vy': math.sin(angle) * speed,
                    'life': 25,
                    'color': random.choice([COLOR_LIGHT_BLUE, (150, 200, 255), (200, 220, 255)]),
                    'type': 'energy'
                })
            # 전기 스파크
            if random.random() < 0.3:
    ...
    if mega_smashing_active:
        ...
        for i in range(3):
            ...
            color = (
                ...
                20 + int(pulse * 20) - i * DEFAULT_TIMER,
        ...
        color_idx = int((prism_time / DEFAULT_TIMER) % len(rainbow_colors))
        ...
        draw.circle( (MAX_COLOR, MAX_COLOR, 255, DEFAULT_SIZE),
                         (BALL.width//2 - 3, BALL.height//2 - 3), 6)
        draw.circle( (MAX_COLOR, MAX_COLOR, 255, 50),
                         (BALL.width//2, BALL.height//2), DEFAULT_TIMER)
    ...
    if new_boss_mode_active and lightning_strike_active:
        ...
        for i in range(8):  # 8개의 번개 볼트
            ...
            lightning_color = (flash_intensity, flash_intensity, DEFAULT_SIZE)
        ...
        draw.circle( (MAX_COLOR, MAX_COLOR, 150, glow_alpha), 
                         ((BALL.width + 30) // 2, (BALL.height + 30) // 2), BALL.width // 2 + DEFAULT_TIMER)
    ...
    if new_boss_mode_active and fire_charge_active:
        ...
        for i in range(12):  # 12개의 화염 파티클
            ...
            draw.circle((MAX_COLOR, MAX_COLOR, 200), (int(flame_x), int(flame_y)), inner_size)
        # 패들 중심에 화염 글로우 효과
        glow_alpha = int(80 + 40 * math.sin(flame_time * 6))
        glow_surface = pygame.Surface((PADDLE_WIDTH + 40, PADDLE_HEIGHT + 40), pygame.SRCALPHA)
        pygame.draw.ellipse(glow_surface, (MAX_COLOR, 100, 0, glow_alpha), 
        ...
        for i in range(5):
            ...
            if tail_alpha > 0:
                ...
                draw.circle( (MAX_COLOR, 150, 0, tail_alpha), (DEFAULT_TIMER, DEFAULT_TIMER), 10 - i)
                SCREEN.blit(tail_surface, (tail_x - DEFAULT_TIMER, tail_y - DEFAULT_TIMER))
    # 🔹 공 꼬리
    if special_active:
        ball_trail.append((BALL.centerx, BALL.centery, 200))
    ball_trail = [(x, y, a - DEFAULT_TIMER) for x, y, a in ball_trail if a > 10][:10]
...
def draw_stage5_fire_dragon():
    """Stage 5 화염 용 이펙트 그리기"""
    global current_stage, BALL, fire_dragon_segments
    global screen_shake_offset_x, screen_shake_offset_y
    global flame_trail_active, flame_trail_positions, tear_particles
    global turret_missiles, player_missile_stunned_timer, player_missile_knockback_vel
    global player_missile_invulnerable_time, PLAYER
        ...
        if len(flame_trail_positions) > 2:
            ...
            for i in range(len(flame_trail_positions) - 1):
                if i < len(flame_trail_positions) - 1:
                    ...
                    g = int(20 + progress * DEFAULT_SIZE)
                    b = int(10 + progress * 20)
                    body_color = (min(MAX_COLOR, r), min(MAX_COLOR, g), min(MAX_COLOR, b))
                    # 용의 몸체 중심선
                    ...
                    if i % 3 == 0:
                        scale_color = (min(MAX_COLOR, r + 50), min(MAX_COLOR, g + 30), b)
            ...
            if flame_trail_positions:
                ...
                draw.line(horn_color,
                               (int(head_x - 8), int(head_y - DEFAULT_TIMER)),
                ...
                if random.random() < 0.6:  # 60% 확률로 화염 분출
                    for _ in range(5):
                        ...
                        breath_dist = random.randint(DEFAULT_TIMER, 25)
                        ...
                        draw.circle((MAX_COLOR, random.randint(100, 200), 0),
    ...
    if current_stage == 6:
        ...
        for missile in turret_missiles:
            ...
            if (0 <= missile['x'] <= WIDTH and 0 <= missile['y'] <= HEIGHT and 
                ...
                if missile_rect.colliderect(PLAYER) and current_time > player_missile_invulnerable_time and not is_player_in_smoke():
                    ...
                    if missile_speed > 0:
                        knockback_direction = missile['vx'] / abs(missile['vx']) if missile['vx'] != 0 else random_direction()
                    ...
                    # 충돌 효과 표시
                    effects_manager.create_impact_effect(missile['x'], missile['y'], DEFAULT_TIMER, is_player=False)
                    # 화면 흔들림 효과 추가
                    for _ in range(DEFAULT_TIMER):
        ...
        for missile in turret_missiles:
            ...
            draw.circle(COLOR_LIGHT_RED, (int(warhead_x), int(warhead_y)), 1)
            ...
            for i in range(4):
                ...
                flame_size = 4 - i
                if i < 2:
                    flame_color = (MAX_COLOR, 200 - i*50, 100 - i*30)
                else:
...
def draw_plasma_cannon():
    """플라즈마 레이저 캐논 시스템 그리기"""
    global current_stage, plasma_cannon_active, plasma_cannon_charging
    global plasma_cannon_charge, plasma_cannon_beam_active, plasma_cannon_beam_timer
    global plasma_cannon_position, plasma_cannon_target, BOSS
    global screen_shake_offset_x, screen_shake_offset_y
    global laser_cannon_angle, boss_current_health, boss_max_health, boss_confused_timer
    if current_stage == 6:
        ...
        if laser_cannon_active:
            ...
            if beam_time < laser_beam_duration:
                ...
                for _ in range(DEFAULT_TIMER):
                    ...
                    spark_length = random.randint(DEFAULT_TIMER, 30)
                ...
                if line_len > 0:
                    ...
                    if distance < beam_width/2 + PADDLE_WIDTH/2 and not player_stunned and not is_player_in_smoke():
                        ...
                        # 감전 효과
                        play_sound_safe(SOUND_WALL)  # 전기 소리 대용
            else:
                laser_cannon_active = False
                # 레이저 빔 사운드 정지
                SOUND_STAGE6_BEAM.stop()
                # 다음 레이저 쿨타임을 5~9초 랜덤으로 설정
                laser_cooldown = random.randint(FIVE_SECONDS_MS, 9000)
        # 플레이어 감전 상태 체크
        if player_stunned:
            if current_time > player_stun_end_time:
                ...
                for _ in range(5):
                    ...
                    draw.circle((150, 200, MAX_COLOR), (spark_x, spark_y), spark_size)
                    draw.circle((MAX_COLOR, MAX_COLOR, MAX_COLOR), (spark_x, spark_y), spark_size - 2)
def draw_perfect_timing_indicator():
    """퍼펙트 타이밍 인디케이터 그리기"""
    global perfect_timing_active, perfect_timing_frame_count, perfect_timing_window
    global special_gauge, special_ready
    
    if not (perfect_timing_active and special_gauge >= 200):
        return
    ...
    if special_ready:
        ...
        draw.circle( (MAX_COLOR, 80, 80, timing_alpha), (timing_radius, timing_radius), timing_radius, 3)
    else:
        # 일반 드라이브 상태일 때 - 노란색 원
        draw.circle( (MAX_COLOR, MAX_COLOR, 0, timing_alpha), (timing_radius, timing_radius), timing_radius, 3)
    ...
    if special_ready:
        ...
        timing_text = timing_font.render("Drive!", True, COLOR_2)
    text_rect = timing_text.get_rect(center=(BALL.centerx, BALL.centery - 50))
    SCREEN.blit(timing_text, text_rect)

def draw_serve_waiting_screen():
    """서브 대기 화면 그리기"""
    if not is_waiting_for_serve:
        return
    ...
    if is_player_serve:
        ...
        # 간단한 텍스트만 표시
        serve_text = serve_font.render("Player Serve", True, COLOR_1)
        serve_rect = serve_text.get_rect(center=(WIDTH // 2, HEIGHT - DEFAULT_SIZE))
        ...
        line_surface.fill((0, 200, DEFAULT_SIZE))
        SCREEN.blit(line_surface, ((WIDTH - line_width) // 2, line_y))
        
        # 3초 후 자동 서브 카운트다운
        if wait_time >= 1000:
            remaining = max(0, THREE_SECONDS_MS - wait_time) // 1000
            if remaining > 0:
                countdown_text = info_font.render(f"SPACE ({remaining}s)", True, COLOR_LIGHT_GRAY2)
            else:
                countdown_text = info_font.render("Auto serve", True, COLOR_LIGHT_YELLOW)
        else:
            countdown_text = info_font.render("Press SPACE", True, COLOR_LIGHT_GRAY2)
        ...
        serve_text = serve_font.render(f"{boss_name} Serve", True, COLOR_1)
        ...
        if boss_fake_move and wait_delay > 0:
            ...
            if remaining > 0:
                status_text = info_font.render("Preparing...", True, COLOR_LIGHT_GRAY2)
            else:
                status_text = info_font.render("Ready", True, COLOR_LIGHT_YELLOW)
        else:
            status_text = info_font.render("Waiting...", True, COLOR_LIGHT_GRAY2)
...
def draw_objects():
    ...
    # 서브 대기 화면 그리기
    draw_serve_waiting_screen()
    
    # 퍼펙트 타이밍 인디케이터 그리기
    draw_perfect_timing_indicator()
    # 화면 흔들림 효과 계산
    ...
    if current_stage in stage_boss_config:
        ...
        boss_img.fill((MAX_COLOR, MAX_COLOR, MAX_COLOR))
        boss_w, boss_h = BOSS_IMG_WIDTH, BOSS_IMG_HEIGHT
    ...
    if current_stage == 5 and boss_throwing:
        ...
        tilt_angle_boss = -15 if (pygame.time.get_ticks() // DEFAULT_SIZE) % 2 == 0 else 15
    #     # 평상시 좌우 기울기
    # 보스 패들 충돌 애니메이션 처리
    ...
    if current_stage == 2 and speed_defense_active:
        boss_trail.append((BOSS.centerx, BOSS.centery, MAX_COLOR))
    ...
    if current_stage == 6:
        ...
        # 안테나 기둥
        draw.rect(COLOR_10, 
                        (antenna_left_x - 2, antenna_left_y - DEFAULT_TIMER, 4, 20))
        # 안테나 베이스
        draw.circle(COLOR_9, 
        ...
        if current_stage == 6:
            ...
            # draw.rect((MAX_COLOR, 0, 0), boss_hitbox_expanded, 2)
        # draw.rect((0, 0, MAX_COLOR), BOSS, 2)  # 디버그용 보스 테두리 (비활성화)
    if boss_stunned_timer > 0:
        ...
        for i in range(num_stars):
            ...
            draw.circle( (MAX_COLOR, MAX_COLOR, 100, 60), 
    ...
    if current_stage == 4 and stage4_magnetic_active:
        ...
        # 기본 자기장 원
        draw.circle( (100, 200, MAX_COLOR, 80), (130, 130), 130, width=6)
        draw.circle( (180, 220, MAX_COLOR, 50), (130, 130), 80, width=0)
        # 전기 이펙트 - 번개 모양의 선들
        ...
        for i in range(num_bolts):
            ...
            inner_radius = 60 + random.randint(-DEFAULT_TIMER, DEFAULT_TIMER)
            outer_radius = 120 + random.randint(-DEFAULT_TIMER, DEFAULT_TIMER)
            # 시작점과 끝점 계산
            ...
            electric_color = (150 + random.randint(0, 105), 200 + random.randint(0, 55), MAX_COLOR, electric_alpha)
            points = [(start_x, start_y)] + mid_points + [(end_x, end_y)]
            for k in range(len(points) - 1):
                draw.line( electric_color, points[k], points[k+1], 2)
            glow_color = (100, 150, MAX_COLOR, electric_alpha // 3)
            for k in range(len(points) - 1):
                draw.line( glow_color, points[k], points[k+1], 4)
        ...
        if pulse > 0.8:  # 주기적으로 강한 번쩍임
            ...
            draw.circle( (200, 230, MAX_COLOR, int(40 * pulse)), (130, 130), 130, width=0)
            ...
            if shield_elapsed < shield_duration:
                ...
                for arc_num in range(3):
                    ...
                    for angle_deg in range(start_angle, end_angle):
                        ...
                        draw.line( (220, 240, MAX_COLOR, arc_alpha), 
                ...
                for i in range(2):
                    ...
                    for angle_deg in range(start_angle, end_angle, 3):
                        ...
                        draw.circle( (200, 220, MAX_COLOR, glow_alpha), (x, y), 2)
                ...
                for _ in range(5):
                    ...
                    draw.circle((MAX_COLOR, MAX_COLOR, MAX_COLOR), (particle_x, particle_y), 2)
                # 공과 쉴드 충돌 체크
                ...
                distance_in_range = (shield_radius - DEFAULT_TIMER) < distance_to_shield < (shield_radius + 20)
                # 공이 쉴드에 닿으면 튕겨냄
                if angle_in_range and distance_in_range and ball_vel[1] < 0:
                    ...
                    for _ in range(DEFAULT_TIMER):
                        ...
                        draw.circle((MAX_COLOR, MAX_COLOR, MAX_COLOR), (spark_x, spark_y), random.randint(1, 3))
            else:
                # 쉴드 비활성화
    ...
    if current_stage == 6:
        ...
        if health_percent <= 30:
            ...
            if interceptor_launch_queue:
                ...
                for interceptor_data in interceptor_launch_queue:
                    if current_time >= interceptor_data['launch_time']:
                        ...
                        play_sound_safe(SOUND_WALL)  # 출격 사운드
                # 출격한 인터셉터 제거
            ...
            for interceptor in interceptors[:]:
                ...
                # 에너지 코어
                draw.circle((100 + glow, 150 + glow, MAX_COLOR), 
    ...
    if not new_boss_mode_active:
        ...
        items.draw_active_item(SCREEN, active_item_slot, (28, 28), selected_item_index, 8000 - cooldown_reduction)  # 쿨타임, 원본 아이콘 크기 (28x28) 사용
    ...
    if current_stage == 3:
        ...
        draw.circle(COLOR_GOLD, (moon_center_x, moon_center_y), moon_radius + 2)
        draw.circle((MAX_COLOR, MAX_COLOR, 200), (moon_center_x, moon_center_y), moon_radius)
        draw.circle((20, 20, 40), (moon_center_x + 5, moon_center_y), moon_radius - 2)
        star_offset = 20
        star_size = 4
        # 작은 별들 애니메이션
        twinkle = abs(math.sin(time_now * 0.003)) * 0.5 + 0.5
        star_color = (MAX_COLOR, int(255 * twinkle), int(255 * twinkle))
        # 왼쪽 별
        ...
        # 아래 별
        draw.circle(star_color, (moon_center_x + DEFAULT_TIMER, moon_center_y + 15), star_size - 2)
        gem_y = wand_y + 25
        gem_height = 60
        gem_width = 24
        gem_points = [
        ...
        draw.polygon(COLOR_GOLD, gem_points, 3)
        inner_gem_points = [
            (wand_x, gem_y + 3),
        ...
        if displayed_boss_gauge > 0:
            ...
            for i in range(segments):
                ...
                if displayed_boss_gauge <= 170:
                    ...
                    segment_color = (color_intensity, color_intensity, MAX_COLOR)
                elif displayed_boss_gauge <= 350:
                    # 연분홍 → 진분홍
                    color_intensity = 255 - (55 * i // segments)
                    segment_color = (MAX_COLOR, color_intensity, MAX_COLOR)
                elif displayed_boss_gauge <= 499:
                    # 진보라색
                    color_intensity = 200 - (50 * i // segments)
                    segment_color = (200, color_intensity, MAX_COLOR)
                else:
                    # 무지개 효과
        ...
        if displayed_boss_gauge >= 350:  # 파워 레벨 이상일 때
            ...
            for i in range(sparkle_count):
                ...
                draw.circle((MAX_COLOR, MAX_COLOR, MAX_COLOR), (sparkle_x, sparkle_y), sparkle_size)
        # 🎀 하단 리본 장식
        ribbon_y = gem_y + gem_height + 5
        ribbon_width = 30
        ribbon_height = 8
        # 리본 중앙
        draw.ellipse((MAX_COLOR, 20, 147), 
                          (wand_x - ribbon_width//2, ribbon_y, ribbon_width, ribbon_height))
        # 리본 매듭
        draw.circle((MAX_COLOR, 105, 180), (wand_x, ribbon_y + ribbon_height//2), 5)
        if boss_special_ready:
            if (time_now // 250) % 2 == 0:
                # 후광 효과
                for radius in range(30, DEFAULT_TIMER, -5):
                    alpha = 255 - (radius - DEFAULT_TIMER) * 8
                    glow_color = (MAX_COLOR, 200, MAX_COLOR, alpha)
                    draw.circle(glow_color[:3], (wand_x, gem_y + gem_height//2), radius, 2)
        boss_gauge_text = f"{int(displayed_boss_gauge)}/500"
        boss_text_color = COLOR_1 if displayed_boss_gauge < 500 else COLOR_LIGHT_YELLOW  # 만렙일 때는 노란색
        try:
        ...
        draw.rect(COLOR_DARK_GRAY2, boss_text_bg_rect, 1, border_radius=3)
        # 텍스트 렌더링
        SCREEN.blit(boss_gauge_text_surface, (boss_text_x, boss_text_y))
    if current_stage == 5:
        # 구슬 크기와 간격
        orb_radius = 10  # 30% 작게 조정 (14 -> DEFAULT_TIMER)
        spacing = 30  # 간격도 조정
        ...
        for i in range(HONGRYUN_MAX_HITS):
            ...
            if i < hongryun_hit_count:
                ...
                halo_color = (MAX_COLOR, 50 + int(pulse * 30), 30)
                for r in range(3):
                    alpha = 60 - r * 20
                ...
                for layer in range(5):
                    ...
                    if layer_radius > 0:
                        ...
                        if layer == 0:  # 외곽 - 진한 붉은색
                            ...
                            color = (MAX_COLOR, 50, 40)
                        elif layer == 3:
                            color = (MAX_COLOR, 80, 60)
                        else:  # 중심 - 밝은 붉은색
                            color = (MAX_COLOR, 120, DEFAULT_SIZE)
                        draw.circle(color, (orb_x, start_y), layer_radius)
                # 내부 불꽃 효과 - 붉은 핵
                flame_offset = math.sin(time_now * 0.008 + i) * 2
                draw.circle((MAX_COLOR, 100, 80), 
                                 (orb_x + int(flame_offset), start_y), 2)
                draw.circle((MAX_COLOR, 200, 200), 
                                 (orb_x - 3, start_y - 3), 2)
                draw.circle((MAX_COLOR, 240, 240), 
                                 (orb_x - 2, start_y - 2), 1)
                # 신비로운 불꽃 파티클 - 붉은 불씨
                if random.random() < 0.3:  # 30% 확률로 파티클 생성
                    for _ in range(2):
                        ...
                        draw.circle((MAX_COLOR, 80, 40), 
                                         (particle_x, particle_y), particle_size)
            else:
                # 비활성 구슬 - 어둡고 투명한 느낌
                # 외곽 테두리
                draw.circle(COLOR_4, (orb_x, start_y), orb_radius, 2)
                for layer in range(3):
                    layer_radius = orb_radius - layer * 3
                    if layer_radius > 0:
                        color = (40 + layer * DEFAULT_TIMER, 40 + layer * DEFAULT_TIMER, 40 + layer * DEFAULT_TIMER)
                        draw.circle(color, (orb_x, start_y), layer_radius)
                # 희미한 하이라이트
            ...
            draw.lines(dragon_color, False, jaw_points, 2)
            if i < hongryun_hit_count:
                for tooth_x in [-4, -1, 1, 4]:
                    draw.line((MAX_COLOR, MAX_COLOR, 200), 
...
def show_victory_screen(stage_cleared, reward):
    ...
    while True:
        ...
        draw.rect(COLOR_LIGHT_BLUE, info_panel_rect, 2)
        ...
        try:
            ...
            draw.circle(COLOR_GOLD, (WIDTH // 2 - 55, medal_y), 17)
            draw.circle((200, 170, 0), (WIDTH // 2 - 55, medal_y), 15, 2)
        # 메달 개수 텍스트
        medal_count_text = font_info.render(f"{total_medals}", True, COLOR_1)
        medal_count_x = WIDTH // 2 - 25  # 메달 아이콘 옆
        SCREEN.blit(medal_count_text, (medal_count_x, medal_y - DEFAULT_TIMER))
        reward_text = font_info.render(f"(+ {reward})", True, COLOR_3)
        reward_x = WIDTH // 2 + 15  # 메달 개수 옆
        SCREEN.blit(reward_text, (reward_x, medal_y - DEFAULT_TIMER))
        ...
        for rect, text, idx in buttons:
            ...
            if is_selected:
                ...
                # 선택된 버튼 외곽선
                draw.rect((MAX_COLOR, MAX_COLOR, MAX_COLOR), rect, 3)
                text_color = (30, 30, 30)
            else:
                # 일반 버튼: 블루 그라데이션
                for i in range(rect.height):
                ...
                text_color = (MAX_COLOR, MAX_COLOR, MAX_COLOR)
            # 버튼 텍스트
...
def confirm_rest(stage_cleared, reward):
    ...
    while True:
        ...
        yes_color = (MAX_COLOR, MAX_COLOR, 0) if selected == 0 else (100, 180, DEFAULT_SIZE)
        no_color = (MAX_COLOR, MAX_COLOR, 0) if selected == 1 else (180, 100, DEFAULT_SIZE)
...
def draw_start_menu_buttons(buttons, selected_idx):
    ...
    for i, button in enumerate(buttons):
        color = COLOR_1 if i == selected_idx else (150, 150, 150)
        # 버튼 그리기 로직
...
def show_start_screen():
    ...
    for _ in range(15):  # 파티클 수 대폭 감소
        neon_particles.append({
            ...
            "color": random.choice([COLOR_7, (255, 0, 128), (128, 255, 0)]),  # 색상 수 감소
    ...
    while True:
        ...
        for x in range(0, WIDTH, 30):
            draw.line( grid_color, (x, 0), (x, HEIGHT))
        for y in range(0, HEIGHT, 30):
            draw.line( grid_color, (0, y), (WIDTH, y))
        ...
        for x in range(WIDTH):
            ...
            for dy in range(-30, 30):
                ...
                if alpha > 0:
                    ...
                    draw.circle( (r, g, b, alpha), (x, galaxy_y + dy), 1)
        # 성운 구름 효과
        for i in range(5):
            nebula_x = 100 + i * 100
            nebula_y = HEIGHT // 3 + int(30 * math.sin(i + animation_timer * 0.015))
            nebula_color = [(MAX_COLOR, 100, 150, 20), (100, 150, MAX_COLOR, 20), (150, MAX_COLOR, 100, 20)][i % 3]
            for r in range(60, 0, -5):
                alpha = nebula_color[3] * (r / 60)
                nebula_circle = pygame.Surface((r * 2, r * 2), pygame.SRCALPHA)
                draw.circle( (*nebula_color[:3], int(alpha)), (r, r), r)
        ...
        for i in range(planet1_radius, 0, -1):
            ...
            color = (
                min(MAX_COLOR, int(200 + 55 * (1 - ratio) + rotation_offset)),
                min(MAX_COLOR, int(50 + 30 * (1 - ratio))),
                min(MAX_COLOR, int(30 + 50 * (1 - ratio)))
            )
            draw.circle( (*color, int(180 * ratio)), (planet1_radius, planet1_radius), i)
        # 자전 띠 패턴
        for j in range(5):
            band_y = planet1_radius - 30 + j * 15
            band_offset = int(20 * math.sin(rotation1 + j))
            pygame.draw.arc(planet1_surf, (MAX_COLOR, 150, 100, 50), 
                          (band_offset, band_y - 5, planet1_radius * 2 - band_offset * 2, DEFAULT_TIMER),
                          0, math.pi, 2)
        ...
        for ring_num in range(3):
            ...
            for angle in range(0, 360, 5):
                particle_angle = math.radians(angle + ring_rotation * DEFAULT_SIZE)
                particle_x = ring_radius + math.cos(particle_angle) * (ring_radius - DEFAULT_TIMER)
                particle_y = ring_radius // 4 + math.sin(particle_angle) * 20
                particle_color = (MAX_COLOR, 200 - ring_num * 50, 100, 80 - ring_num * 20)
                draw.circle( particle_color, (int(particle_x), int(particle_y)), 2)
            SCREEN.blit(ring_surf, (planet1_x - ring_radius, planet1_y - DEFAULT_TIMER))
        ...
        for i in range(planet2_radius, 0, -1):
            ...
            color = (
                min(MAX_COLOR, int(150 + 100 * (1 - ratio) + storm_offset * 0.5)),
                min(MAX_COLOR, int(50 + 150 * (1 - ratio))),
                min(MAX_COLOR, int(200 + 55 * (1 - ratio) + storm_offset))
            )
            draw.circle( (*color, int(150 * ratio)), (planet2_radius, planet2_radius), i)
        storm_x = planet2_radius + int(30 * math.cos(rotation2))
        storm_y = planet2_radius
        pygame.draw.ellipse(planet2_surf, (MAX_COLOR, 100, 150, DEFAULT_SIZE), 
                          (storm_x - 20, storm_y - DEFAULT_TIMER, 40, 20))
        for j in range(6):
            band_y = planet2_radius - 40 + j * 15
            flow_offset = int(10 * math.sin(rotation2 * 2 + j))
            band_color = (200 - j * 20, 100 + j * 10, MAX_COLOR, 60)
        ...
        for i in range(planet3_radius, 0, -1):
            ...
            draw.circle( (*color, int(180 * ratio)), (planet3_radius, planet3_radius), i)
        for angle in range(0, 360, 15):
            city_angle = math.radians(angle + rotation3 * DEFAULT_SIZE)
            ...
            if visibility > 0:
                ...
                city_color = random.choice([(MAX_COLOR, MAX_COLOR, 0), (0, MAX_COLOR, MAX_COLOR), (MAX_COLOR, 0, MAX_COLOR)])
        ...
        draw.circle( (0, 200, MAX_COLOR, 30), 
                         (planet3_radius + DEFAULT_TIMER, planet3_radius + DEFAULT_TIMER), planet3_radius + DEFAULT_TIMER)
        SCREEN.blit(glow_surf, (planet3_x - planet3_radius - DEFAULT_TIMER, planet3_y - planet3_radius - DEFAULT_TIMER))
        ...
        for i in range(moon_radius, 0, -1):
            ...
            gray = min(MAX_COLOR, max(0, int(180 + 70 * (1 - ratio) + crystal_shift)))
            moon_surf = pygame.Surface((i * 2, i * 2), pygame.SRCALPHA)
            alpha = min(MAX_COLOR, max(0, int(150 * ratio)))
            color = (gray, gray, min(MAX_COLOR, gray + 30), alpha)
            draw.circle( color, (i, i), i)
        ...
        for angle in range(0, 360, 45):
            ...
            draw.circle((200, 220, MAX_COLOR, DEFAULT_SIZE), 
        ...
        for i in range(12):
            ...
            size = 5 + i % 3 * 3
            # 네온 테두리를 가진 조각
            debris_color = [COLOR_LIGHT_RED, (100, MAX_COLOR, DEFAULT_SIZE), (100, 100, MAX_COLOR)][i % 3]
            points = []
            for j in range(5):
                angle = math.radians(j * 72 + piece_rotation * DEFAULT_SIZE)
                px = dx + math.cos(angle) * size
                py = dy + math.sin(angle) * size
                points.append((px, py))
            draw.polygon((*debris_color, 150), points)
            draw.polygon((0, MAX_COLOR, MAX_COLOR), points, 2)
        # 홀로그램 별 효과
        ...
        for star in show_start_screen.stars:
            ...
            star_alpha = int(twinkle * DEFAULT_SIZE)
            star_color = (MAX_COLOR, MAX_COLOR, 255, star_alpha)
        ...
        for scan_line in scan_lines:
            ...
            for i in range(3):
                ...
                if scan_alpha > 0:
                    ...
                    scan_surf.fill((0, MAX_COLOR, 255, scan_alpha))
                    SCREEN.blit(scan_surf, (0, scan_line["y"] + i))
        # 홀로그램 메달 표시
        medal_panel = pygame.Surface((150, 50), pygame.SRCALPHA)
        medal_panel.fill((DEFAULT_TIMER, 15, 25, 180))
        draw.rect( COLOR_GOLD, (0, 0, 150, 50), 2, border_radius=8)
        SCREEN.blit(medal_panel, (WIDTH - 170, DEFAULT_TIMER))
        # 메달 아이콘과 텍스트
        try:
        ...
        global medal_score
        medal_text = font_medal.render(f"🏅 {medal_score}", True, COLOR_GOLD)
        ...
        title_text = font_title.render("메인 메뉴", True, COLOR_1)
        ...
        for i, (option, display_option) in enumerate(zip(all_options, display_options)):
            if i == selected:
                ...
                draw.rect( (20, 30, 40, DEFAULT_SIZE), (0, 0, option_width, option_height), border_radius=DEFAULT_TIMER)
                draw.rect( (0, 150, 200, 200), (0, 0, option_width, option_height), width=2, border_radius=DEFAULT_TIMER)
                SCREEN.blit(option_bg, (option_x, option_y))
            # 텍스트 색상 결정
            if i == selected:
                color = COLOR_2  # 선택된 항목은 노란색
                try:
        ...
        if locked_message_timer > 0:
            ...
            message_y = 580
            # 배경 글로우
            for j in range(DEFAULT_TIMER, 0, -2):
                alpha = int(60 * (1 - j / DEFAULT_TIMER))
                glow_surface = pygame.Surface((message_width + j*2, message_height + j*2), pygame.SRCALPHA)
                draw.rect( (MAX_COLOR, 100, 100, alpha), (0, 0, message_width + j*2, message_height + j*2), border_radius=20)
                SCREEN.blit(glow_surface, (message_x - j, message_y - j))
            # 메인 배경
            message_bg = pygame.Surface((message_width, message_height), pygame.SRCALPHA)
            draw.rect( (MAX_COLOR, 100, 100, 40), (0, 0, message_width, message_height), border_radius=20)
            draw.rect( (MAX_COLOR, 100, 100, 120), (0, 0, message_width, message_height), width=2, border_radius=20)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_1:
                    play_sound_safe(SOUND_BUTTON_CLICK)  # 🎵 클릭 사운드
                ...
                if event.key in [pygame.K_DOWN, pygame.K_s]:
                    ...
                    play_sound_safe(SOUND_BUTTON_HOVER)  # 🎵 호버 사운드
...
def show_character_selection():
    ...
    characters = [
        ...
        },
        {
            ...
            "unlocked": False,
            "card_color": COLOR_2,  # 네온 옐로우
            "glow_color": COLOR_LIGHT_YELLOW,
            "card_suit": "◆"
        }
    ]
    selected = 0
    ...
    center_y = HEIGHT - 100
    card_back_patterns = [
        {"color": (0, 20, 40), "pattern": "circuit", "accent": COLOR_7},
        ...
        {"color": (40, 40, 0), "pattern": "scan", "accent": COLOR_2}
    ]
    # 사이버펑크 배경 효과용 변수들
    neon_particles = []
    for _ in range(50):  # 더 많은 네온 파티클
        neon_particles.append({
            ...
            "color": random.choice([COLOR_7, (255, 0, 255), COLOR_2]),
    ...
    while True:
        ...
        SCREEN.blit(grid_surface, (0, 0))
        try:
            font_matrix = pygame.font.Font(None, DEFAULT_TIMER)
        except:
            font_matrix = pygame.font.Font(None, DEFAULT_TIMER)
        ...
        # 네온 글로우 레이어들
        for i, (offset, color, alpha) in enumerate([(4, (0, MAX_COLOR, MAX_COLOR), 60), (2, (MAX_COLOR, 0, 128), 120), (0, (MAX_COLOR, MAX_COLOR, MAX_COLOR), MAX_COLOR)]):
        ...
        # 서브 제목
        sub_text = font_subtitle.render(title_sub, True, (0, MAX_COLOR, MAX_COLOR))
        ...
        if animation_timer % 60 < 30:  # 0.5초마다 깜박임
            ...
            draw.rect( (0, MAX_COLOR, 255, 150), (0, 0, 200, 3))
        ...
        def draw_card_front(surface, character, is_selected, w, h):
            """홀로그램 스타일 카드 앞면 그리기"""
            border_color = character["card_color"] if is_selected else COLOR_MID_GRAY
            ...
            if is_selected:
                for glow_layer in range(5, 0, -1):
                    ...
                    draw.rect( (*glow_color, glow_intensity // glow_layer), 
                                   (0, 0, w + glow_size, h + glow_size), border_radius=20)
                    surface.blit(glow_surface, (-glow_size//2, -glow_size//2))
            card_bg_color = (DEFAULT_TIMER, 15, 25, 220)  # 매우 어두운 반투명
            card_surface = pygame.Surface((w, h), pygame.SRCALPHA)
            draw.rect( card_bg_color, (0, 0, w, h), border_radius=15)
            surface.blit(card_surface, (0, 0))
            # 네온 테두리
            draw.rect( border_color, (0, 0, w, h), 2, border_radius=15)
            inner_border = 4
            draw.rect( (*border_color, DEFAULT_SIZE), 
            ...
            try:
                ...
                surface.blit(char_image, (image_x, 30))
            except:
                # 이미지 로드 실패 시 대체 그래픽
                radius = image_size // 2
                draw.circle( border_color, (w//2, 30 + radius), radius)
                draw.circle( (MAX_COLOR, MAX_COLOR, MAX_COLOR), (w//2, 30 + radius), radius - 5, 3)
        ...
        def draw_card_back(surface, card_index, is_selected, w, h):
            ...
            if is_selected:
                # 선택된 카드는 더 밝게
                base_color = tuple(min(MAX_COLOR, c + 30) for c in base_color)
                accent_color = tuple(min(MAX_COLOR, c + 50) for c in accent_color)
                # 글로우 효과
                glow_intensity = int(abs(math.sin(animation_timer * 0.15)) * 30) + 40
                glow_surface = pygame.Surface((w + DEFAULT_TIMER, h + DEFAULT_TIMER), pygame.SRCALPHA)
                draw.rect( (*accent_color, glow_intensity), 
                               (0, 0, w + DEFAULT_TIMER, h + DEFAULT_TIMER), border_radius=15)
            ...
            # 카드 배경
            draw.rect( base_color, (0, 0, w, h), border_radius=DEFAULT_TIMER)
            draw.rect( accent_color, (0, 0, w, h), 3, border_radius=DEFAULT_TIMER)
            # 패턴 그리기
        ...
        def draw_card_pattern(surface, pattern, width, height):
            ...
            if pattern_type == "circuit":
                ...
                for y in range(margin, margin + pattern_height, 20):
                    alpha = int(abs(math.sin(animation_timer * 0.1 + y * 0.05)) * DEFAULT_SIZE) + 50
                    line_color = (*color, alpha)
                    line_surface = pygame.Surface((pattern_width, 2), pygame.SRCALPHA)
                    draw.rect( line_color, (0, 0, pattern_width, 2))
                ...
                for x in range(margin, margin + pattern_width, grid_size):
                    for y in range(margin, margin + pattern_height, grid_size):
                        ...
                        draw.rect( (*color, alpha), (x, y, grid_size-1, grid_size-1), 1)
            elif pattern_type == "wave":
                # 사인 웨이브 패턴
                for y in range(margin, margin + pattern_height, DEFAULT_TIMER):
                    points = []
                    ...
                    if len(points) > 1:
                        ...
                        for i in range(len(points) - 1):
                            draw.line( (*color, alpha), points[i], points[i+1])
                ...
                for x in range(margin, margin + pattern_width, hex_size * 2):
                    for y in range(margin, margin + pattern_height, hex_size * 2):
                        alpha = int(abs(math.sin(animation_timer * 0.12 + x * 0.05 + y * 0.05)) * DEFAULT_SIZE) + 50
                ...
                for i in range(5):
                    ...
                    if alpha > 0:
                        ...
                        draw.rect( (*color, alpha), (0, 0, pattern_width, 3))
                ...
                for i in range(cols):
                    for j in range(rows):
                        ...
                        draw.circle( color, (int(x), int(y)), int(radius))
                ...
                for i in range(cols):
                    for j in range(rows):
                        ...
                        draw.rect( color, (int(x-size), int(y-size/3), int(size*2), int(size*2/3)))
                        draw.rect( color, (int(x-size/3), int(y-size), int(size*2/3), int(size*2)))
        ...
        # 홀로그램 상세 정보 패널 배경
        border_color = current_char["card_color"] if current_char["unlocked"] else COLOR_DARK_GRAY2
        ...
        detail_surface.fill((5, DEFAULT_TIMER, 20, 240))  # 매우 어두운 반투명
        # 네온 테두리
        draw.rect( border_color, (0, 0, detail_card_width, detail_card_height), 2, border_radius=15)
        # 내부 전자 회로 패턴
        for x in range(0, detail_card_width, 40):
            for y in range(0, detail_card_height, 30):
                if random.randint(0, 5) == 0:  # 랜덤 전자 회로 노드
                    ...
                    draw.circle( (*glow_color, node_alpha), (x, y), 1)
        ...
        if current_char["unlocked"]:
            ...
            for glow_offset in [(2, 2), (1, 1), (0, 0)]:
                ...
                name_color = (*glow_color, alpha) if glow_offset != (0, 0) else (MAX_COLOR, MAX_COLOR, MAX_COLOR)
            ...
            for i, (stat_name, stat_value) in enumerate(current_char["stats"].items()):
                ...
                # 홀로그램 배경 바
                draw.rect((DEFAULT_TIMER, 20, 30), (bar_x, bar_y, bar_width, bar_height), border_radius=3)
            ...
            # 해금되지 않은 캐릭터 - ???? 표시
            # 캐릭터 이름
            name_text = font_subtitle.render("????", True, COLOR_MID_GRAY)
            ...
            for i in range(3):  # 3개의 스탯
                stat_x = stats_start_x + (i * stat_width)
                # 스탯명
                stat_name_text = font_small.render("???", True, COLOR_MID_GRAY)
                stat_name_rect = stat_name_text.get_rect(center=(stat_x, stats_start_y))
                SCREEN.blit(stat_name_text, stat_name_rect)
                bar_width = 60
                bar_height = 8
                ...
                # 물음표 표시
                value_text = font_small.render("?", True, COLOR_MID_GRAY)
    ...
    while True:
        ...
        for i, (offset, color, alpha) in enumerate([(4, (0, MAX_COLOR, MAX_COLOR), 60), (2, (MAX_COLOR, 0, 128), 120), (0, (MAX_COLOR, MAX_COLOR, MAX_COLOR), MAX_COLOR)]):
            ...
            if alpha < 255:  # 글로우 레이어
                ...
                SCREEN.blit(glow_text, glow_rect)
        # 서브 제목
        sub_text = font_desc.render(title_sub, True, (0, MAX_COLOR, MAX_COLOR))
        ...
        if animation_timer % 60 < 30:
            ...
            draw.rect( (0, MAX_COLOR, 255, 150), (0, 0, 150, 2))
        ...
        for i, difficulty in enumerate(difficulties):
            ...
            if i == selected:
                ...
                for glow_layer in range(5, 0, -1):
                    ...
                    draw.rect( (*layer_color, glow_intensity // glow_layer),
            ...
            # 어두운 배경과 회로 패턴
            if i == selected:
                card_surface.fill((DEFAULT_TIMER, 15, 25, 250))  # 더 진한 배경
                # 전자 회로 패턴 추가
                for _ in range(8):
                    node_x = random.randint(DEFAULT_TIMER, card_width - DEFAULT_TIMER)
                    node_y = random.randint(DEFAULT_TIMER, card_height - DEFAULT_TIMER)
                    draw.circle( (*difficulty["color"], 30), (node_x, node_y), 1)
            else:
                card_surface.fill((15, 20, 30, 200))
            # 네온 테두리
            draw.rect( (*difficulty["color"], 150), (0, 0, card_width, card_height), 2, border_radius=12)
            ...
            if i == selected:
                ...
                for j in range(2, 0, -1):
                    ...
                    SCREEN.blit(name_glow, (name_rect.x + j, name_rect.y + j))
                name_text = font_subtitle.render(difficulty['name'], True, COLOR_1)
            ...
            if difficulty["id"] == "junior":
                ...
                # 줄기
                draw.line((100, 200, DEFAULT_SIZE), (emblem_x, emblem_y + 20), (emblem_x, emblem_y - 5), 3)
                ...
                # 중심 새싹
                draw.circle((150, MAX_COLOR, 150), (emblem_x, emblem_y - DEFAULT_TIMER), 8)
                draw.circle((100, MAX_COLOR, DEFAULT_SIZE), (emblem_x, emblem_y - DEFAULT_TIMER), 6)
                draw.circle((200, MAX_COLOR, 200), (emblem_x - 2, emblem_y - 12), 2)
                # 빛나는 효과
                for r in range(3):
                    alpha = 50 - r * 15
                    draw.circle((150, MAX_COLOR, 150, alpha), (emblem_x, emblem_y - DEFAULT_TIMER), 12 + r * 3, 1)
                ...
                if abs(scale_x) > 0.1:
                    ...
                    draw.ellipse((MAX_COLOR, 200, DEFAULT_SIZE), ellipse_rect, 2)
                lightning_points = [
                ...
                if abs(scale_x) > 0.2:
                    draw.lines(COLOR_LIGHT_YELLOW, False, lightning_points, 4)
                    draw.lines((MAX_COLOR, 200, 0), False, lightning_points, 2)
                for angle in range(0, 360, 60):
                    ...
                    draw.line((MAX_COLOR, MAX_COLOR, 150), (emblem_x, emblem_y), (spark_x, spark_y), 1)
                # 중심 광원
                draw.circle((MAX_COLOR, MAX_COLOR, 200), (emblem_x, emblem_y), 5)
                ...
                # 그라데이션 효과를 위한 여러 층
                colors = [(MAX_COLOR, 150, MAX_COLOR), (MAX_COLOR, 100, MAX_COLOR), (200, 50, 200)]
                ...
                # 반짝임 효과
                if abs(scale_x) > 0.2:
                    draw.lines((MAX_COLOR, MAX_COLOR, MAX_COLOR), True, diamond_points, 2)
                # 중심 빛
                draw.circle((MAX_COLOR, 200, MAX_COLOR), (emblem_x, emblem_y), 3)
                for angle in range(45, 360, 90):
                    ...
                    draw.line((MAX_COLOR, 200, MAX_COLOR, DEFAULT_SIZE), (emblem_x, emblem_y), (ray_x, ray_y), 1)
            elif difficulty["id"] == "mythic":
                # 👑 신화리그 - 왕관 엠블럼
                # 회전 효과를 위한 스케일 계산
                scale_x = math.cos(math.radians(rotation_angle)) if i == selected else 1.0
                crown_base = [(emblem_x - 25 * scale_x, emblem_y + DEFAULT_TIMER), (emblem_x + 25 * scale_x, emblem_y + DEFAULT_TIMER)]
                if abs(scale_x) > 0.2:
                    draw.line(COLOR_GOLD, crown_base[0], crown_base[1], 3)
                peaks = [
                    (emblem_x - 20 * scale_x, emblem_y - 15, 12),  # 왼쪽
                    (emblem_x - 10 * scale_x, emblem_y - DEFAULT_TIMER, DEFAULT_TIMER),
                ...
                if abs(scale_x) > 0.2:
                    for j, (px, py, height) in enumerate(peaks):
                        # 봉우리 그리기
                        draw.lines(COLOR_GOLD, False, 
                                        [(px - 5 * abs(scale_x), emblem_y + DEFAULT_TIMER), (px, py), (px + 5 * abs(scale_x), emblem_y + DEFAULT_TIMER)], 3)
                        # 보석
                        gem_colors = [(MAX_COLOR, 50, 50), (50, MAX_COLOR, 50), (MAX_COLOR, MAX_COLOR, MAX_COLOR), (50, 50, MAX_COLOR), (MAX_COLOR, 50, 50)]
                        draw.circle(gem_colors[j], (px, py + 5), 3)
                        if scale_x > 0:
                            draw.circle((MAX_COLOR, MAX_COLOR, MAX_COLOR), (px - 1, py + 4), 1)
                # 중앙 큰 보석
                draw.circle((MAX_COLOR, 100, MAX_COLOR), (emblem_x, emblem_y), 5)
                draw.circle((MAX_COLOR, 200, MAX_COLOR), (emblem_x, emblem_y), 3)
                draw.circle((MAX_COLOR, MAX_COLOR, MAX_COLOR), (emblem_x - 1, emblem_y - 1), 1)
                # 황금빛 후광
                for r in range(3):
                    draw.circle((MAX_COLOR, 215, 0, 30 - r*DEFAULT_TIMER), (emblem_x, emblem_y), 30 + r*5, 1)
        # 선택된 난이도 상세 정보
        if selected < len(difficulties):
            ...
            for j in range(3, 0, -1):
                glow_surface = pygame.Surface((detail_width + j*DEFAULT_TIMER, detail_height + j*DEFAULT_TIMER), pygame.SRCALPHA)
                glow_alpha = 30 - j*8
                draw.rect( (*selected_diff["color"], glow_alpha), 
                               (0, 0, detail_width + j*DEFAULT_TIMER, detail_height + j*DEFAULT_TIMER), 
                               border_radius=15)
            ...
            detail_surface.fill((DEFAULT_TIMER, 15, 25, 220))
            pygame.draw.rect(detail_surface, selected_diff["color"], 
                           (0, 0, detail_width, detail_height), 2, border_radius=12)
            # 내부 회로 패턴
            for j in range(5):
                node_x = random.randint(DEFAULT_TIMER, detail_width - DEFAULT_TIMER)
                node_y = random.randint(DEFAULT_TIMER, detail_height - DEFAULT_TIMER)
                draw.circle( (*selected_diff["color"], 40), (node_x, node_y), 2)
            SCREEN.blit(detail_surface, (detail_x, detail_y))
            if selected_diff['id'] == 'champion':
                champion_text1 = font_subtitle.render(f"{selected_diff['icon']} {selected_diff['name']} - 상세 정보", True, COLOR_1)
                ...
                champion_text2 = font_detail.render("빠른 공 속도", True, COLOR_LIGHT_GRAY2)
                ...
                champion_text3 = font_detail.render("AI 소수 실수 (8%)", True, COLOR_LIGHT_GRAY2)
                ...
                # 제목
                title_text = font_subtitle.render(f"{selected_diff['icon']} {selected_diff['name']} - 상세 정보", True, COLOR_1)
                ...
                for i, desc_line in enumerate(desc_lines[:2]):
                    desc_text = font_detail.render(desc_line, True, COLOR_LIGHT_GRAY2)
        ...
        # 네온 텍스트 효과
        control_surface = font_desc.render(control_text, True, (0, MAX_COLOR, MAX_COLOR))
        control_rect = control_surface.get_rect(center=(WIDTH // 2, controls_y))
        SCREEN.blit(control_surface, control_rect)
        # 하단 스캔라인
        scan_y = HEIGHT - 20
        scan_alpha = int(abs(math.sin(animation_timer * 0.1)) * DEFAULT_SIZE) + 50
        draw.line((0, MAX_COLOR, 255, scan_alpha), (50, scan_y), (WIDTH - 50, scan_y), 1)
        pygame.display.flip()
        clock.tick(60)
def start_game_with_difficulty(character_id, difficulty_mode):
    ...
    draw.rect((30, 35, 50), (box_x, box_y, box_width, box_height), border_radius=DEFAULT_TIMER)
    draw.rect(COLOR_LIGHT_BLUE, (box_x, box_y, box_width, box_height), 3, border_radius=DEFAULT_TIMER)
    # 제목
    title_text = font.render("🎮 게임 시작!", True, COLOR_1)
    title_rect = title_text.get_rect(center=(WIDTH // 2, box_y + 40))
    SCREEN.blit(title_text, title_rect)
    # 난이도 표시
    diff_text = font.render(message, True, (100, MAX_COLOR, DEFAULT_SIZE))
    diff_rect = diff_text.get_rect(center=(WIDTH // 2, box_y + 80))
    SCREEN.blit(diff_text, diff_rect)
    # 안내 메시지
    help_font = pygame.font.Font(None, 24)
    help_text = help_font.render("준비되면 아무 키나 누르세요...", True, COLOR_LIGHT_GRAY2)
...
def show_developer_stage_select():
    ...
    while True:
        SCREEN.fill((DEFAULT_TIMER, DEFAULT_TIMER, 40))
        ...
        for idx, (rect, num) in enumerate(stage_buttons):
            color = (MAX_COLOR, MAX_COLOR, 0) if idx == selected_index else (180, 220, MAX_COLOR)
...
def show_item_manager_menu():
    ...
    while True:
        ...
        tab_y = 120
        # 엑티브 탭
        active_tab_rect = pygame.Rect(WIDTH // 2 - tab_width - DEFAULT_TIMER, tab_y, tab_width, tab_height)
        active_tab_color = COLOR_LIGHT_BLUE if selected_category == 0 else (60, 60, 80)
        ...
        # 패시브 탭
        passive_tab_rect = pygame.Rect(WIDTH // 2 + DEFAULT_TIMER, tab_y, tab_width, tab_height)
        passive_tab_color = COLOR_LIGHT_BLUE if selected_category == 1 else (60, 60, 80)
        ...
        for i, item in enumerate(current_items):
            ...
            # 배경 색상
            if is_selected:
                draw.rect(COLOR_2, item_rect)  # 선택된 아이템은 노란색
            elif is_cursor:
                draw.rect(COLOR_LIGHT_BLUE, item_rect)  # 현재 커서는 파란색
            else:
                draw.rect((60, 60, 80), item_rect)
            # 테두리 색상
            if is_cursor:
                draw.rect(COLOR_1, item_rect, 3)  # 커서는 굵은 흰색 테두리
            else:
                draw.rect(WHITE, item_rect, 2)
            # 아이템 아이콘
            if item["icon"]:
                icon_surface = pygame.transform.scale(item["icon"], (item_size - DEFAULT_TIMER, item_size - DEFAULT_TIMER))
        ...
        text_color = COLOR_LIGHT_GRAY2  # 기본 색상
        ...
        # 조작법 안내
        controls_text = font_small.render("방향키: 아이템 선택, SPACE: 아이템 선택/해제, TAB: 탭 전환, ENTER: 관리자 모드로 이동", True, COLOR_MID_GRAY)
...
def get_item_icon(item_name):
    ...
    if item_name == "molotov":
        try:
            ...
            draw.circle( (MAX_COLOR, 100, 0), (16, 16), 12)
            return molotov_icon
    # 수류탄 아이콘 동적 로드
    ...
    if item_name == "predictor":
        try:
            ...
            draw.circle( (200, 50, 50), (16, 16), 12)
            return predictor_icon
    # 조명탄 아이콘 생성
    if item_name == "flare":
        flare_icon = pygame.Surface((32, 32), pygame.SRCALPHA)
        draw.rect( (180, 180, 180), (12, 8, 8, 18))
        draw.rect( COLOR_LIGHT_GRAY2, (13, 9, 6, 16))
        draw.circle( (MAX_COLOR, MAX_COLOR, 200), (16, 8), 5)
        draw.circle( (MAX_COLOR, MAX_COLOR, 150), (16, 8), 3)
        draw.circle( (MAX_COLOR, MAX_COLOR, MAX_COLOR), (16, 8), 2)
        for angle in range(0, 360, 45):
            rad = math.radians(angle)
            ...
            draw.line( (MAX_COLOR, MAX_COLOR, 100, 150), (x1, y1), (x2, y2), 1)
        # 하단 안전핀
        draw.rect( COLOR_DARK_GRAY2, (14, 24, 4, 3))
        return flare_icon
    # 연막탄 아이콘 동적 로드
    if item_name == "smoke_grenade":
        try:
            ...
            draw.circle( COLOR_MID_GRAY, (16, 16), 12)
            return smoke_grenade_icon
...
def show_stage1_intro():
    ...
    for alpha in range(0, 256, 8):
        ...
        red_component = min(MAX_COLOR, 120 + int(fade_pulse * 1.5))
        ...
        # 장식 요소 추가
        decoration_alpha = min(MAX_COLOR, alpha + 50)
    ...
    while waiting:
        ...
        stage_color = (MAX_COLOR, min(MAX_COLOR, 220 + int(pulse)), min(MAX_COLOR, 220 + int(pulse)))
        # 파랑에서 빨강으로 변하는 맥동 효과
        red_component = min(MAX_COLOR, 120 + int(pulse * 1.5))
        ...
        line_color = (min(MAX_COLOR, line_intensity + 60), line_intensity, line_intensity)
        ...
        if frame_count % 120 < 60:
            ...
            hint_text = hint_font.render("Press SPACE to continue", True, COLOR_MID_GRAY)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                waiting = False
    for alpha in range(MAX_COLOR, -1, -8):
        boss_img.set_alpha(alpha)
        SCREEN.fill(BLACK)
        SCREEN.blit(boss_img, (0, 0))
        pygame.display.flip()
...
def show_stage2_intro():
    ...
    while waiting:
        ...
        SCREEN.blit(boss_img, (0, 0))
        pulse = abs(math.sin(frame_count * 0.03)) * 20
        stage_color = (min(MAX_COLOR, 240 + int(pulse)), min(MAX_COLOR, 220 + int(pulse)), min(MAX_COLOR, 180 + int(pulse)))
        # 파랑에서 빨강으로 변하는 맥동 효과
        red_component = min(MAX_COLOR, 120 + int(pulse * 1.5))
        blue_component = max(80, 200 - int(pulse))
        boss_color = (red_component, 100, blue_component)
        ...
        line_color = (min(MAX_COLOR, line_intensity), max(0, line_intensity - 20), max(0, line_intensity - 60))
        draw.line(line_color, (WIDTH // 2 - 150, HEIGHT // 2 - 80), (WIDTH // 2 + 150, HEIGHT // 2 - 80), 3)
        draw.line(line_color, (WIDTH // 2 - 150, HEIGHT // 2 - 20), (WIDTH // 2 + 150, HEIGHT // 2 - 20), 3)
...
def show_stage3_intro():
    try:
        ...
        boss_img = pygame.Surface((WIDTH, HEIGHT))
        boss_img.fill((MAX_COLOR, 0, MAX_COLOR))
    for alpha in range(0, 256, 8):
        boss_img.set_alpha(alpha)
        SCREEN.fill(BLACK)
        SCREEN.blit(boss_img, (0, 0))
    ...
    while waiting:
        ...
        SCREEN.blit(boss_img, (0, 0))
        pulse = abs(math.sin(frame_count * 0.03)) * 20
        stage_color = (MAX_COLOR, min(MAX_COLOR, 180 + int(pulse)), MAX_COLOR)
        # 파랑에서 빨강으로 변하는 맥동 효과
        red_component = min(MAX_COLOR, 120 + int(pulse * 1.5))
        blue_component = max(80, 200 - int(pulse))
        boss_color = (red_component, 100, blue_component)
        ...
        line_intensity = 160 + int(pulse)
        line_color = (min(MAX_COLOR, line_intensity), 100, min(MAX_COLOR, line_intensity))
        draw.line(line_color, (WIDTH // 2 - 150, HEIGHT // 2 - 80), (WIDTH // 2 + 150, HEIGHT // 2 - 80), 3)
        draw.line(line_color, (WIDTH // 2 - 150, HEIGHT // 2 - 20), (WIDTH // 2 + 150, HEIGHT // 2 - 20), 3)
        # 부드러운 Space 키 안내
        if frame_count % 120 < 60:
...
def show_stage4_intro():
    ...
    while waiting:
        ...
        SCREEN.blit(boss_img, (0, 0))
        pulse = abs(math.sin(frame_count * 0.03)) * 20
        stage_color = (MAX_COLOR, min(MAX_COLOR, 240 + int(pulse)), min(MAX_COLOR, 200 + int(pulse)))
        # 파랑에서 빨강으로 변하는 맥동 효과
        red_component = min(MAX_COLOR, 120 + int(pulse * 1.5))
        blue_component = max(80, 200 - int(pulse))
        boss_color = (red_component, 100, blue_component)
        ...
        line_intensity = 160 + int(pulse)
        line_color = (min(MAX_COLOR, line_intensity + 30), min(MAX_COLOR, line_intensity), max(100, line_intensity - 30))
        draw.line(line_color, (WIDTH // 2 - 150, HEIGHT // 2 - 80), (WIDTH // 2 + 150, HEIGHT // 2 - 80), 3)
        draw.line(line_color, (WIDTH // 2 - 150, HEIGHT // 2 - 20), (WIDTH // 2 + 150, HEIGHT // 2 - 20), 3)
        # 부드러운 Space 키 안내
        if frame_count % 120 < 60:
...
def show_stage5_intro():
    ...
    while waiting:
        ...
        SCREEN.blit(boss_img, (0, 0))
        pulse = abs(math.sin(frame_count * 0.03)) * 20
        stage_color = (MAX_COLOR, min(MAX_COLOR, 150 + int(pulse)), min(MAX_COLOR, 100 + int(pulse)))
        # 파랑에서 빨강으로 변하는 맥동 효과
        red_component = min(MAX_COLOR, 120 + int(pulse * 1.5))
        blue_component = max(80, 200 - int(pulse))
        boss_color = (red_component, 100, blue_component)
        ...
        line_intensity = 160 + int(pulse)
        line_color = (min(MAX_COLOR, line_intensity + 50), max(50, line_intensity - 50), 50)
        draw.line(line_color, (WIDTH // 2 - 150, HEIGHT // 2 - 80), (WIDTH // 2 + 150, HEIGHT // 2 - 80), 3)
        draw.line(line_color, (WIDTH // 2 - 150, HEIGHT // 2 - 20), (WIDTH // 2 + 150, HEIGHT // 2 - 20), 3)
        # 부드러운 Space 키 안내
        if frame_count % 120 < 60:
...
def draw_stage2_jungle_border():
    ...
    if current_stage == 2:
        ...
        draw.rect(vine_green, (border_thickness - 2, border_thickness - 2, 
...
def draw_field():
    ...
    if current_stage == 3 and emotional_overdrive_active:
        ...
        if psycho_bg_timer % 35 < 15:
            # 밝은 보라색 계열
            SCREEN.fill((MAX_COLOR, 100, MAX_COLOR))
        else:
            # 어두운 남보라 계열
    ...
    if current_stage in boss_health_stages:
        barrier_height = 8
        barrier_y = 0
        barrier_color = (50, 150, MAX_COLOR)
        time_now = pygame.time.get_ticks()
        ...
        for i in range(0, WIDTH, 20):
            ...
            if random.random() < 0.1:
                ...
                draw.circle((MAX_COLOR, MAX_COLOR, MAX_COLOR), (spark_x, spark_y), 1)
        try:
            font_barrier = pygame.font.Font("NanumSquareR.ttf", 12)
        except:
            font_barrier = pygame.font.Font(None, 12)
        barrier_text = font_barrier.render("⚡ ENERGY BARRIER ⚡", True, COLOR_1)
        text_rect = barrier_text.get_rect(center=(WIDTH // 2, barrier_height + 15))
        # 텍스트 그림자
        shadow_text = font_barrier.render("⚡ ENERGY BARRIER ⚡", True, COLOR_6)
...
def draw_laser_cannon_gauge():
    ...
    draw.rect((DEFAULT_TIMER, 12, 15), bg_rect)
    # 그리드 패턴
    ...
    if laser_charging:
        ...
        warning_text = warning_font.render("CHARGING", True, COLOR_LIGHT_RED)
        ...
        fire_text = fire_font.render("FIRING", True, COLOR_8)
        ...
        cooldown_progress = min(1.0, (current_time - last_laser_time) / FIVE_SECONDS_MS)
...
def draw_boss_health_bar():
    ...
    if boss_displayed_health > 0:
        ...
        if display_ratio > 0.6:
            ...
            glow_color = (0, MAX_COLOR, 220)
        elif display_ratio > 0.3:
            # 노란색 경고
            health_color = (200, 180, 0)
            glow_color = (MAX_COLOR, 220, 0)
        else:
            # 빨간색 위험
            health_color = (200, 50, 50)
            glow_color = COLOR_LIGHT_RED
        # 메인 체력 그라데이션
    ...
    font_label = pygame.font.SysFont("Courier", DEFAULT_TIMER, bold=True)
    ...
    hp_surface = font_digital.render(hp_text, True, (0, MAX_COLOR, 200))
    SCREEN.blit(hp_surface, (text_bg_x + 3, health_bar_y))
    if boss_current_health <= 3:
        warning_alpha = int(abs(math.sin(pygame.time.get_ticks() * 0.01)) * DEFAULT_SIZE)
        warning_surface = pygame.Surface((boss_health_bar_width + DEFAULT_TIMER, boss_health_bar_height + DEFAULT_TIMER), pygame.SRCALPHA)
        draw.rect( (MAX_COLOR, 0, 0, warning_alpha), 
                        (0, 0, boss_health_bar_width + DEFAULT_TIMER, boss_health_bar_height + DEFAULT_TIMER), 2)
...
def show_fade_text(message):
    ...
    if is_serve_message:
        ...
        font_size = 56
        main_color = (MAX_COLOR, MAX_COLOR, MAX_COLOR)
        accent_color = (MAX_COLOR, 200, 0)
        display_text = "POWER SMASHING"
    elif is_deuce:
        font_size = 52
        main_color = (MAX_COLOR, MAX_COLOR, MAX_COLOR)
        accent_color = (180, 100, MAX_COLOR)
        display_text = message  # 듀스 메시지 그대로
    elif is_special_skill:
        font_size = 44
        main_color = (MAX_COLOR, MAX_COLOR, MAX_COLOR)
        accent_color = (100, 200, MAX_COLOR)
        display_text = message  # 스킬 메시지 그대로
    else:
        font_size = 42
        main_color = (MAX_COLOR, MAX_COLOR, MAX_COLOR)
        accent_color = COLOR_MID_GRAY
    ...
    for alpha in range(0, 256, 32):
        ...
        # 액센트 라인
        line_alpha = min(MAX_COLOR, alpha + 50)
    ...
    while pygame.time.get_ticks() - start_time < hold_duration:
        ...
        pygame.display.flip()
        pygame.time.delay(16)
    for alpha in range(MAX_COLOR, -1, -42):
        draw_field()
        draw_objects()
        # 반투명 오버레이
        overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        ...
        SCREEN.blit(text_surface, text_rect)
        pygame.display.flip()
        pygame.time.delay(DEFAULT_TIMER)
    # 🎵 파워스매싱 두 번째 효과음
    if is_power_smashing:
        try:
            ...
            if not hasattr(handle_mega_smashing_trajectory, 'shadow_burst'):
                ...
                for i in range(20):  # 20개의 검은 그림자
                    ...
                    handle_mega_smashing_trajectory.shadow_particles.append({
                        ...
                        'size': random.randint(DEFAULT_TIMER, 30),
                        'life': 60,  # 1초간 지속
                        'alpha': 255
                    })
        if hasattr(handle_mega_smashing_trajectory, 'base_angle'):
...
def render_mega_smashing_effects():
    ...
    for ghost in mega_smashing_ghosts:
        ...
        draw.circle( (*color, min(255, ghost['alpha'])), 
        ...
        if not mega_smashing_ghost_scatter:
            eye_offset = 5
            draw.circle( (255, 0, 0, min(255, ghost['alpha'] * 2)),
    ...
    if not mega_smashing_ghost_scatter:
        ...
        for i in range(5):
            alpha = 30 - i * 5
            draw.circle( (50, 0, 70, alpha), (75, 75), 30 + i * DEFAULT_TIMER)
    ...
    if hasattr(handle_mega_smashing_trajectory, 'shadow_particles'):
        ...
        for particle in handle_mega_smashing_trajectory.shadow_particles:
            ...
            if particle['life'] > 0:
                ...
                color = (20, 0, 30)
                draw.circle( (*color, min(255, particle['alpha'])), 
...
def draw_fireball_explosion_particles():
    ...
    if fireball_explosion_particles:
        ...
        if converted_particles:
            effect_renderer.draw_particles(SCREEN, converted_particles, (MAX_COLOR, 100, 0))
def check_boss_collision():
    """보스와 공의 충돌 체크"""
    global ball_speed_y, BALL, BOSS, combo_count
    ...
    if BALL.colliderect(BOSS):
        ...
        boss_hit_animation_timer = 10
        return True
    return False
def check_player_collision():
    """플레이어와 공의 충돌 체크"""
    global ball_speed_y, BALL, PLAYER, combo_count
    global player_hit_animation_active, player_hit_animation_timer
    
    if check_ball_player_collision():
        ball_speed_y = -abs(ball_speed_y)
        combo_count += 1
        player_hit_animation_active = True
        player_hit_animation_timer = HIT_ANIMATION_DURATION
        return True
    return False
def handle_stage5_fireball():
    """Stage 5 화염탄 처리"""
    global fireball_last_cast, fireball_cooldown, round_start_time
    global current_stage, fireballs, BOSS, ball_speed_x, ball_speed_y
...
def handle_stage4_magnetic():
    ...
    global ball_vel, player_last_shot_speed, stage4_magnetic_radius
...
def handle_stage5_fireballs():
    ...
    if (now - fireball_last_cast > fireball_cooldown and 
        ...
        fireball_cooldown = random.randint(3500, FIVE_SECONDS_MS)
...
def handle_ball():
    ...
    update_ball_timers()
    # Stage 4 특수 효과
    if handle_stage4_magnetic():
        return
    if handle_stage4_meditation():
        return
    # ✅ 서브 대기
    if is_waiting_for_serve:
        if is_player_serve:
    ...
    if flame_trail_active:
        ...
        if check_ball_player_collision():
            ...
            for _ in range(80):
                ...
                flame_particles.append([
                    explosion_x + random.randint(-DEFAULT_TIMER, DEFAULT_TIMER),
            ...
            for _ in range(3):
                ...
                flame_particles.append([
                    ...
                    explosion_y + random.randint(-DEFAULT_TIMER, DEFAULT_TIMER),
            ...
            flash_surface.fill((MAX_COLOR, 200, DEFAULT_SIZE))
    ...
    if abs(ball_vel[1]) < 0.5:
        ball_vel[1] += random_direction() * 1.5
    # --- 위험감지센서 자동 대쉬 로직 ---
    # 디버깅: 센서 상태 확인
    ...
    if BALL.top <= 0:
        ...
        if current_stage in boss_health_stages:
            ...
            effects_manager.spawn_star_particles(BALL.centerx, DEFAULT_TIMER, count=8)
            return
        if deuce_mode:
            deuce_wins += 1
    ...
    if current_speed < BALL_BASE_SPEED * 0.85 and not special_active:  # 🚀 파워스매싱 중에는 보정 무시
        ...
        slow_ball_timer = 0
    # tear_shower_active가 정의되지 않았으므로 비활성화
    if False and check_ball_player_collision():  # tear_shower_active 관련 기능 비활성화
        player_stunned_timer = int(0.3 * FPS)
    ...
    # 서브 대기 중에는 충돌 체크하지 않음
    if check_ball_player_collision() and player_collision_cooldown <= 0 and not player_collision_handled and not is_waiting_for_serve:
        # 디버그: 충돌 위치 정보
        collision_x = BALL.centerx - PLAYER.centerx
        collision_side = "LEFT" if collision_x < 0 else "RIGHT"
        edge_distance = abs(collision_x) - (PADDLE_WIDTH / 2)
        ...
        player_last_shot_speed = get_ball_speed()
        create_impact_effect(BALL.centerx, BALL.centery, ball_vel, is_player=True)
        # 플레이어 패들 애니메이션 활성화
        player_hit_animation_active = True
        player_hit_animation_timer = HIT_ANIMATION_DURATION
    ...
    if boss_hitbox_expanded.colliderect(BALL) and boss_collision_cooldown <= 0 and not is_waiting_for_serve:
        ...
        if drive_ball_active and not drive_hit_boss:
            ...
            current_speed_after = get_ball_speed()
            ...
            if ball_vel[1] > 0:  # 플레이어 방향으로 향하는 공
                player_direction_speed = get_ball_speed()
...
def handle_boss_pro():
    ...
    # 간단한 예측 로직
    predict_frame = max(DEFAULT_TIMER, min(30, int(60 / max(1, abs(ball_vel[0])))))
    # 조명탄 혼란 효과 체크
    if boss_confused_timer > 0:
        if not hasattr(handle_boss, 'confusion_target'):
            handle_boss.confusion_target = get_random_boss_x()
...
def handle_boss_champion():
    ...
    if boss_confused_timer > 0:
        if not hasattr(handle_boss_champion, 'confusion_target'):
            handle_boss_champion.confusion_target = get_random_boss_x()
...
def handle_boss_mythic():
    ...
    if boss_confused_timer > 0:
        if not hasattr(handle_boss_mythic, 'confusion_target'):
            handle_boss_mythic.confusion_target = get_random_boss_x()
    ...
    if is_waiting_for_serve:
        ...
        def fake_motion():
            ...
            if style == 1:
                ...
                return random_direction() * random.randint(DEFAULT_TIMER, 20)  # 더 작은 움직임
            return 0
        # --- 보스 서브 차례 ---
        if not is_player_serve:
            if boss_fake_move and time_now - boss_fake_start_time < wait_delay:
...
def draw_ai_visualization():
    ...
    league_colors = {
        ...
        "mythic": COLOR_GOLD       # 황금색
    }
    mode_color = league_colors.get(ai_mode, COLOR_LIGHT_GRAY2)
    mode_text = f"AI: {ai_mode.upper()}"
    mode_surface = font_small.render(mode_text, True, mode_color)
    SCREEN.blit(mode_surface, (DEFAULT_TIMER, HEIGHT - DEFAULT_SIZE))
    # AI 결정 시각화
    ...
    if ai_performance_stats["confidence_history"]:
        ...
        confidence_surface = font_small.render(confidence_text, True, (MAX_COLOR, MAX_COLOR, MAX_COLOR))
        SCREEN.blit(confidence_surface, (DEFAULT_TIMER, HEIGHT - 80))
        # 성공률 표시
        ...
        if total_results > 0:
            ...
            success_color = (100, MAX_COLOR, DEFAULT_SIZE) if success_rate > 0.7 else (COLOR_LIGHT_YELLOW if success_rate > 0.5 else COLOR_LIGHT_RED)
            success_surface = font_small.render(success_text, True, success_color)
            SCREEN.blit(success_surface, (DEFAULT_TIMER, HEIGHT - 60))
    if ai_mode == "mythic":
        try:
            mythic_text = "MYTHIC LEAGUE ACTIVE"
            learning_surface = font_small.render(mythic_text, True, COLOR_GOLD)
            SCREEN.blit(learning_surface, (DEFAULT_TIMER, HEIGHT - 40))
        except:
            pass  # 에러 무시
    help_text = "N: Toggle AI Mode | ESC: Menu"
    help_surface = font_small.render(help_text, True, COLOR_MID_GRAY)
    SCREEN.blit(help_surface, (WIDTH - help_surface.get_width() - DEFAULT_TIMER, HEIGHT - 30))
...
def score_to_grade(score):
    """점수를 등급으로 변환"""
    if score == 0:
        return "플레이 필요", COLOR_DARK_GRAY2  # 회색 - 아직 플레이 안함
    elif score >= 90:
        return "S", COLOR_GOLD  # 골드
    elif score >= 80:
        return "A", COLOR_LIGHT_RED  # 레드
    elif score >= 70:
        return "B", COLOR_3  # 그린
    elif score >= 60:
        return "C", COLOR_LIGHT_BLUE  # 블루
    elif score >= 50:
        return "D", COLOR_LIGHT_YELLOW  # 옐로우
    elif score >= 30:
        return "E", (255, 150, 100)  # 오렌지
    else:
        return "F", COLOR_MID_GRAY  # 그레이
def draw_ability_radar_chart(stats, center_x, center_y, radius=120):
    ...
    for i in range(1, 6):
        ...
        draw.circle(COLOR_4, (center_x, center_y), int(grid_radius), 1)
    # 축 선 그리기
    ...
    for i, ability_name in enumerate(ability_names):
        ...
        draw.line(COLOR_DARK_GRAY2, (center_x, center_y), (end_x, end_y), 1)
        # 능력치 값에 따른 점 위치 계산
        value = abilities[ability_name]
        value_radius = radius * (value / DEFAULT_SIZE)
    ...
    if len(points) >= 3:
        ...
        pygame.draw.polygon(overlay, (100, 150, MAX_COLOR, 80), relative_points)
        SCREEN.blit(overlay, (center_x - radius, center_y - radius))
    # 능력치 점들을 선으로 연결
    if len(points) >= 2:
        draw.polygon(COLOR_LIGHT_BLUE, points, 2)
    # 능력치 점 표시
    for point in points:
        draw.circle((MAX_COLOR, MAX_COLOR, MAX_COLOR), (int(point[0]), int(point[1])), 4)
        draw.circle(COLOR_LIGHT_BLUE, (int(point[0]), int(point[1])), 3)
    # 중심점 표시
    draw.circle(COLOR_LIGHT_GRAY2, (center_x, center_y), 3)
    percent_font = pygame.font.Font("NanumSquareR.ttf", 12)
    for i in range(1, 6):
        percent = i * 20
        percent_text = percent_font.render(f"{percent}%", True, COLOR_MID_GRAY)
...
def handle_boss():
    ...
    if boss_confused_timer > 0:
        if not hasattr(handle_boss, 'confusion_target_ai'):
            handle_boss.confusion_target_ai = get_random_boss_x()
...
def show_death_evaluation():
    ...
    while True:
        ...
        try:
            ...
            detail_text = font_tiny.render(f"점수 구성: 스킬 {int(skill_contrib)} + 대쉬 {int(dash_contrib)} + 아이템 {int(item_contrib)} + 가드 {int(guard_contrib)}", True, COLOR_LIGHT_GRAY2)
            ...
            bonus_text = font_tiny.render(f"보너스: +{victory_bonus + stage_bonus} (승리/스테이지) | 페널티: -{mistake_penalty} (실수)", True, COLOR_LIGHT_GRAY2)
            ...
            draw_ability_radar_chart(stats, chart_center_x, chart_center_y, DEFAULT_SIZE)
            # 조작 안내
...
def show_result(won):
    ...
    if won:
        ...
        if not battery_obtained:
            available_items.append({"name": "battery", "color": COLOR_2, "type": "passive", "icon": battery_icon})
        ...
        if not master_obtained:
            available_items.append({"name": "master", "color": COLOR_GOLD, "type": "passive", "icon": master_icon})
        if not cooltime_obtained:
            available_items.append({"name": "cooltime", "color": COLOR_7, "type": "passive", "icon": cooltime_icon})
        if not chargebag_obtained:
            available_items.append({"name": "chargebag", "color": COLOR_3, "type": "passive", "icon": chargebag_icon})
        if not spikeboots_obtained:
        ...
        if not bulkup_obtained:
            available_items.append({"name": "bulkup", "color": COLOR_LIGHT_RED, "type": "passive", "icon": bulkup_icon})
        # 엑티브 아이템들 추가
        available_items.append({"name": "long_boost", "color": COLOR_LIGHT_RED, "type": "active", "icon": long_boost_icon})
        available_items.append({"name": "gauge_charge", "color": COLOR_3, "type": "active", "icon": gauge_charge_icon})
        available_items.append({"name": "aipill", "color": COLOR_LIGHT_YELLOW, "type": "active", "icon": aipill_icon})
        available_items.append({"name": "wall", "color": COLOR_DARK_GRAY2, "type": "active", "icon": wall_icon})
        # 화염병 아이콘 추가
        try:
            molotov_icon = pygame.image.load("items/molotov.png").convert_alpha()
            molotov_icon = pygame.transform.scale(molotov_icon, (32, 32))
        ...
        try:
            ...
            flare_icon = pygame.Surface((32, 32), pygame.SRCALPHA)
            draw.circle( (MAX_COLOR, MAX_COLOR, 200), (16, 16), 12)
        ...
        try:
            ...
            smoke_grenade_icon = pygame.Surface((32, 32), pygame.SRCALPHA)
            draw.circle( COLOR_MID_GRAY, (16, 16), 12)
        available_items.append({"name": "smoke_grenade", "color": COLOR_MID_GRAY, "type": "active", "icon": smoke_grenade_icon})
        # 레이저스코프 아이콘 추가
        try:
            predictor_icon = pygame.image.load("items/predictor.png").convert_alpha()
            predictor_icon = pygame.transform.scale(predictor_icon, (32, 32))
            ...
            predictor_icon = pygame.Surface((32, 32), pygame.SRCALPHA)
            draw.circle( (100, 200, MAX_COLOR), (16, 16), 12)
        available_items.append({"name": "predictor", "color": COLOR_8, "type": "active", "icon": predictor_icon})
        ...
        if not available_items:
            available_items = [{"name": "long_boost", "color": COLOR_8, "type": "active", "icon": long_boost_icon}]
...
def show_new_boss_selection_screen():
    ...
    while True:
        ...
        # 메인 제목
        title_text = font_title.render("🆕 NEW BOSS BATTLE", True, COLOR_1)
        title_rect = title_text.get_rect(center=(WIDTH//2, 80))
        SCREEN.blit(title_text, title_rect)
        if selection_step == 0:
            # 상단 보스 선택
            step_text = font_subtitle.render("1단계: 상단 보스 선택", True, COLOR_8)
            step_rect = step_text.get_rect(center=(WIDTH//2, 140))
            SCREEN.blit(step_text, step_rect)
            # 보스 선택지들
            for i, boss in enumerate(top_bosses):
                y = 200 + i * 80
                color = COLOR_LIGHT_YELLOW if i == top_selection else COLOR_LIGHT_GRAY2
                # 선택 표시
                if i == top_selection:
                    selector = font_subtitle.render("→", True, COLOR_LIGHT_YELLOW)
                ...
                SCREEN.blit(name_text, name_rect)
                # 설명
                desc_text = font_desc.render(boss['description'], True, COLOR_MID_GRAY)
            ...
            # 하단 보스 선택
            step_text = font_subtitle.render("2단계: 하단 보스 선택", True, COLOR_LIGHT_RED)
            step_rect = step_text.get_rect(center=(WIDTH//2, 140))
            SCREEN.blit(step_text, step_rect)
            # 선택된 상단 보스 표시
            top_boss_text = font_desc.render(f"상단: {top_bosses[top_selection]['name']}", True, COLOR_8)
            ...
            for i, boss in enumerate(bottom_bosses):
                y = 220 + i * 80
                color = COLOR_LIGHT_YELLOW if i == bottom_selection else COLOR_LIGHT_GRAY2
                # 선택 표시
...
def main(stage_num, new_boss_mode=False):
    ...
    if stage_num == 1:
        ...
        BOSS_COLOR = (100, MAX_COLOR, DEFAULT_SIZE)
    elif stage_num == 3:
        CURRENT_BG = STAGE3_BG
        BOSS_COLOR = (MAX_COLOR, MAX_COLOR, 0)
    elif stage_num == 4:
        CURRENT_BG = STAGE4_BG
        BOSS_COLOR = (MAX_COLOR, MAX_COLOR, MAX_COLOR)
    ...
    while running:
        ...
        ball_will_hit_paddle = (
            ...
            ball_to_paddle_distance > -10 and  # 약간의 여유 (0 → -DEFAULT_TIMER)
            ball_vel[1] > 0 and
        ...
        if perfect_timing_active:
            ...
            if (perfect_timing_frame_count <= perfect_timing_window and 
                ...
                if special_gauge >= 350 and keys[pygame.K_SPACE]:  # 파워스매시 발동 조건: 350 이상
                    ...
                    if not check_ball_player_collision() and ball_to_paddle_distance <= 30 and ball_vel[1] > 0:
                    ...
                    if not mega_smashing_active:
                        ...
                        for _ in range(15):
                            angle = random_angle()
                            speed = random.uniform(5, DEFAULT_TIMER)
                            power_smashing_particles.append({
                                'x': BALL.centerx,
                                'y': BALL.centery,
                                'vx': math.cos(angle) * speed,
        ...
        if is_player_serve and is_waiting_for_serve:
            if pygame.time.get_ticks() - waiting_start_time >= THREE_SECONDS_MS:  # 3초
                # 서브 실행 및 상태 업데이트
                serve_result = physics_manager.serve_ball(is_player_serve, current_stage)
                ball_vel = serve_result['ball_vel']
                ball_impact_boost = serve_result['ball_impact_boost']
        ...
        if drive_text_timer > 0:
            ...
            # 그림자 효과
            shadow_surf = drive_font.render("DRIVE!", True, COLOR_4)
            shadow_surf.set_alpha(alpha)
            shadow_rect = shadow_surf.get_rect(center=(text_x + 2, text_y + 2))
            SCREEN.blit(shadow_surf, shadow_rect)
            text_surf = drive_font.render("DRIVE!", True, COLOR_1)
...
def draw_pause_overlay():
    ...
    overlay.fill(COLOR_6)
    SCREEN.blit(overlay, (0, 0))
    # PAUSE 텍스트
    font_pause = pygame.font.Font("NanumSquareEB.ttf", 80)
    pause_text = font_pause.render("PAUSE", True, COLOR_1)
    # 텍스트에 그림자 효과 추가
    shadow_text = font_pause.render("PAUSE", True, COLOR_4)
    ...
    hint_text = font_hint.render("P키를 다시 눌러서 게임 재개", True, COLOR_LIGHT_GRAY2)
...
def show_pause_menu():
    ...
    while True:
        ...
        for i, (rect, text) in enumerate(zip(buttons, button_texts)):
            if i == selected:
                # 선택된 버튼
                draw.rect(COLOR_LIGHT_BLUE, rect)
                draw.rect(WHITE, rect, 3)
                text_color = BLACK
            else:
                # 선택되지 않은 버튼
                draw.rect(COLOR_4, rect)
                draw.rect(COLOR_MID_GRAY, rect, 2)
                text_color = WHITE
...
def draw_gradient_background(surface, rect, color1, color2, horizontal=False):
    ...
    for i in range(50):
        ...
        helper.rect(color, r)
def draw_modern_panel(surface, rect, title="", title_color=COLOR_GOLD):
    """현대적인 패널 그리기"""
    draw_gradient_background(surface, rect, (25, 25, 35), (45, 45, 55))
    for i in range(3):
        border_color = (100 + i*20, 150 + i*15, 200 + i*DEFAULT_TIMER)
        pygame.draw.rect(surface, border_color, 
                        (rect.x - i, rect.y - i, rect.width + 2*i, rect.height + 2*i), 2)
    # 내부 테두리
    draw.rect( (80, 80, DEFAULT_SIZE), rect, 2)
    # 상단 헤더 영역
    if title:
        header_rect = pygame.Rect(rect.x + DEFAULT_TIMER, rect.y + DEFAULT_TIMER, rect.width - 20, 50)
        draw_gradient_background(surface, header_rect, (60, 60, 80), (40, 40, 60))
        draw.rect( (120, 120, 140), header_rect, 1)
        # 타이틀 텍스트
...
def draw_section_container(surface, rect, title, icon=""):
    ...
    # 섹션 테두리
    draw.rect( (100, 150, 200, 150), rect, 2)
    # 섹션 제목
    font_section = pygame.font.Font("NanumSquareB.ttf", 18)
    title_text = f"{icon} {title}" if icon else title
    title_surface = font_section.render(title_text, True, (200, 220, MAX_COLOR))
    title_rect = title_surface.get_rect(left=rect.x + 15, top=rect.y + DEFAULT_TIMER)
    surface.blit(title_surface, title_rect)
    # 제목 밑줄
...
def show_player_info():
    ...
    if not player_analyzer:
        ...
        while True:
            ...
            SCREEN.blit(title_text, title_rect)
            msg_text = font_medium.render("플레이어 분석 시스템을 사용할 수 없습니다.", True, COLOR_LIGHT_GRAY2)
            msg_rect = msg_text.get_rect(center=(WIDTH//2, HEIGHT//2))
            SCREEN.blit(msg_text, msg_rect)
            esc_text = font_medium.render("ESC를 눌러 돌아가기", True, COLOR_MID_GRAY)
    ...
    while True:
        try:
            ...
            main_panel_rect = pygame.Rect(main_panel_x, 30, main_panel_width, main_panel_height)
            # 메인 패널 그리기
            draw_modern_panel(SCREEN, main_panel_rect, "🏆 플레이어 실력 분석", COLOR_GOLD)
            ...
            desc_surface = font_tiny.render(desc_text, True, COLOR_LIGHT_GRAY2)
            ...
            if achievements:
                ...
                for i, achievement in enumerate(achievements[-3:]):
                    if i < 3:  # 공간 제한
                        achievement_surface = font_tiny.render(achievement[:30] + "..." if len(achievement) > 30 else achievement, True, COLOR_GOLD)
            ...
            if tips:
                ...
                for i, tip in enumerate(tips[:2]):
                    ...
                    tip_surface = font_tiny.render(display_tip, True, (150, MAX_COLOR, 150))
            ...
            if show_feedback:
                ...
                try:
                    ...
                    # 피드백 패널 그리기
                    draw_modern_panel(SCREEN, feedback_panel_rect, "📝 상세 피드백 분석", COLOR_3)
                    # 피드백 내용 영역
                    ...
                    print(f"🔴 피드백 로드 오류: {e}")
                    error_text = font_small.render(f"피드백 로드 오류: {str(e)[:40]}", True, COLOR_LIGHT_RED)
                    error_rect = error_text.get_rect(center=(WIDTH//2, HEIGHT//2))
                    SCREEN.blit(error_text, error_rect)
            # 하단 조작 안내
            ...
            import traceback
            traceback.print_exc()
            error_text = font_medium.render("정보를 불러올 수 없습니다.", True, COLOR_LIGHT_RED)
            error_rect = error_text.get_rect(center=(WIDTH//2, HEIGHT//2))
            SCREEN.blit(error_text, error_rect)
            # 상세 오류 정보도 표시
...
def show_game_info():
    ...
    while True:
        ...
        if current_page == 0:  # 기본정보 페이지
            ...
            draw.rect(COLOR_LIGHT_BLUE, (info_panel_x, info_panel_y, info_panel_width, info_panel_height), 3)
            # 제목
            ...
            for i, info in enumerate(info_items):
                ...
                if i < 7:  # 게임 정보
                    ...
                    text_surface = font_small.render(info, True, COLOR_LIGHT_GRAY2)
            ...
            draw.rect(COLOR_LIGHT_BLUE, next_button_rect)
            ...
            draw.rect(COLOR_LIGHT_BLUE, (panel_x, panel_y, panel_width, panel_height), 3)
            # 제목
            title_text = font_large.render("아이템 관리", True, WHITE)
            ...
            if in_tab_selection and selected_category == 0:
                ...
                active_tab_color = (120, 180, MAX_COLOR)
                border_color = (MAX_COLOR, MAX_COLOR, MAX_COLOR)
                border_width = 4
            elif selected_category == 0:
                # 아이템 선택 모드에서 엑티브 탭이 활성인 경우 - 중간 하이라이트
                active_tab_color = (80, 120, 200)
                border_color = COLOR_LIGHT_GRAY2
                border_width = 2
            else:
                # 비활성 탭
                active_tab_color = (50, 50, 70)
                border_color = COLOR_DARK_GRAY2
                border_width = 1
            ...
            if in_tab_selection and selected_category == 1:
                ...
                passive_tab_color = (120, 180, MAX_COLOR)
                border_color = (MAX_COLOR, MAX_COLOR, MAX_COLOR)
                border_width = 4
            ...
            if selected_category == 0:  # 엑티브 아이템
                ...
                if not items_to_show:
                    no_item_text = font_medium.render("소지한 엑티브 아이템이 없습니다", True, COLOR_MID_GRAY)
                    no_item_rect = no_item_text.get_rect(center=(panel_x + panel_width // 2, item_list_y + DEFAULT_SIZE))
                    ...
                    for i, item in enumerate(items_to_show):
                        ...
                        if i == selected_index and selected_index >= 0:
                            # 선택된 아이콘 배경
                            highlight_rect = pygame.Rect(icon_x - 5, icon_y - 5, icon_size + DEFAULT_TIMER, icon_size + DEFAULT_TIMER)
                            draw.rect(COLOR_LIGHT_BLUE, highlight_rect)
                        ...
                        if item.get("icon"):
                            ...
                            draw.circle(COLOR_DARK_GRAY2, (icon_x + icon_size // 2, icon_y + icon_size // 2), icon_size // 2)
            else:  # 패시브 아이템
                items_to_show = passive_item_list if passive_item_list else []
                selected_index = selected_passive_item
                if not items_to_show:
                    no_item_text = font_medium.render("소지한 패시브 아이템이 없습니다", True, COLOR_MID_GRAY)
                    no_item_rect = no_item_text.get_rect(center=(panel_x + panel_width // 2, item_list_y + DEFAULT_SIZE))
                    SCREEN.blit(no_item_text, no_item_rect)
                else:
                    # 아이콘들을 가로로 나열
            ...
            for line_idx, line in enumerate(lines):
                if line_idx < 3:  # 최대 3줄까지만 표시
                    desc_text = font_small.render(line, True, COLOR_LIGHT_GRAY2)
                    desc_rect = desc_text.get_rect(left=desc_panel_x + DEFAULT_TIMER, top=desc_text_y + line_idx * line_spacing)
                    SCREEN.blit(desc_text, desc_rect)
        pygame.display.flip()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
...
def show_item_management_menu(item_list, selected_index, item_type):
    ...
    while True:
        ...
        if len(desc_lines) > 1:
            effect_text = font_medium.render(desc_lines[1], True, COLOR_LIGHT_GRAY2)
        ...
        for i, (rect, text) in enumerate(zip(buttons, button_texts)):
            if i == selected:
                # 선택된 버튼
                draw.rect(COLOR_LIGHT_RED if i == 0 else COLOR_LIGHT_BLUE, rect)
                draw.rect(WHITE, rect, 3)
                text_color = BLACK
            else:
                # 선택되지 않은 버튼
