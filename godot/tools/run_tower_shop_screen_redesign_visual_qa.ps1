...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Tower shop screen redesign Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/tower_shop_screen_redesign_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("tower_shop_screen_redesign_visual_qa_{0}_{1}.log" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff"))
    ...
    if (
        ...
        -or -not $outputText.Contains("tower_shop_screen_redesign_visual_qa: ok")
    ) {
        Write-Host "Tower shop screen redesign visual QA log preserved for triage: $logPath"
        if ($exitCode -ne 0) {
        ...
        throw "$qaPath did not emit its Vulkan capture terminal markers"
    }
    Remove-Item -LiteralPath $logPath -Force
    if (-not (Get-ChildItem -LiteralPath $logDir -Force)) {
        Remove-Item -LiteralPath $logDir -Force
    }
    Write-Host "Tower shop screen redesign Vulkan visual QA passed."
}
finally {
