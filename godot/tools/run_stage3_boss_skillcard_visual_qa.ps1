...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Stage 3 boss skillcard Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/stage3_boss_skillcard_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("stage3_boss_skillcard_visual_qa_{0}_{1}.log" -f `
        $PID,
    ...
    $okMarker = "stage3_boss_skillcard_visual_qa: ok"
    if ($exitCode -ne 0 -or $seriousErrors.Count -gt 0 -or -not $outputText.Contains($okMarker)) {
        Write-Host "Stage 3 skillcard visual QA log preserved for triage: $logPath"
        if ($exitCode -ne 0) {
    ...
    Write-Host "Stage 3 boss skillcard Vulkan visual QA passed at 2020x1246."
}
finally {
