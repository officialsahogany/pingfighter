...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Perk fusion Dowsing confirm readability Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tests/perk_fusion_confirm_readability_visual_qa.gd"
    $logDir = Join-Path $ProjectPath "logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir (
        "perk_fusion_confirm_readability_visual_qa_{0}_{1}.log" -f `
            $PID,
    ...
    $okMarker = "perk_fusion_confirm_readability_visual_qa: ok"
    if ($exitCode -ne 0 -or $seriousErrors.Count -gt 0 -or -not $outputText.Contains($okMarker)) {
        Write-Host "Confirm-readability visual QA log preserved for triage: $logPath"
    ...
    Write-Host "Perk fusion confirm readability Vulkan visual QA passed. Log: $logPath"
}
finally {
