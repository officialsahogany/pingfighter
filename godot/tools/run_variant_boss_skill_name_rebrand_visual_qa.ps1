...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Variant boss skill-name rebrand Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/variant_boss_skill_name_rebrand_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("variant_boss_skill_name_rebrand_visual_qa_{0}_{1}.log" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff"))
    ...
    if ($exitCode -ne 0 -or $seriousErrors.Count -gt 0 -or -not $outputText.Contains("variant_boss_skill_name_rebrand_visual_qa: ok")) {
        Write-Host "Variant boss skill-name rebrand visual QA log preserved for triage: $logPath"
    ...
    Write-Host "Variant boss skill-name rebrand Vulkan visual QA passed."
}
finally {
