...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Boss serve-wait traditional HUD Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/serve_wait_indicator_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir (
        "serve_wait_indicator_visual_qa_{0}_{1}.log" -f `
            $PID,
    ...
    $okMarker = "serve_wait_indicator_visual_qa: ok"
    if ($exitCode -ne 0 -or $seriousErrors.Count -gt 0 -or -not $outputText.Contains($okMarker)) {
        Write-Host "Serve-wait visual QA log preserved for triage: $logPath"
    ...
    Write-Host "Boss serve-wait Vulkan visual QA passed. Log: $logPath"
}
finally {
