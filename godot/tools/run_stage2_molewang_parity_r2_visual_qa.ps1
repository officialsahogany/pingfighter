...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Stage 2 Molewang parity r2 Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/stage2_molewang_parity_r2_visual_qa.gd"
    $logDir = Join-Path $ProjectPath "logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir (
        "stage2_molewang_parity_r2_visual_qa_{0}_{1}.log" -f `
            $PID,
    ...
    $okMarker = "stage2_molewang_parity_r2_visual_qa: ok"
    if ($exitCode -ne 0 -or $seriousErrors.Count -gt 0 -or -not $outputText.Contains($okMarker)) {
        Write-Host "Molewang parity r2 visual QA log preserved for triage: $logPath"
    ...
    Write-Host "Stage 2 Molewang parity r2 Vulkan visual QA passed. Log: $logPath"
}
finally {
