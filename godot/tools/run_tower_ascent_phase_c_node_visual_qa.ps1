...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Tower-ascent Phase-C node Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/tower_ascent_phase_c_node_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("tower_ascent_phase_c_node_visual_qa_{0}_{1}.log" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff"))
    ...
    Write-Host "Tower-ascent Phase-C node Vulkan visual QA passed."
}
finally {
