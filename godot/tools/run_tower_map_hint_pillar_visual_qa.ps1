...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Tower map-hint pillar Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/tower_map_hint_pillar_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir (
        "tower_map_hint_pillar_visual_qa_{0}_{1}.log" -f `
            $PID,
    ...
    $okMarker = "tower_map_hint_pillar_visual_qa: ok"
    if ($exitCode -ne 0 -or $seriousErrors.Count -gt 0 -or -not $outputText.Contains($okMarker)) {
        Write-Host "Tower map-hint pillar visual QA log preserved for triage: $logPath"
    ...
    Write-Host "Tower map-hint pillar Vulkan visual QA passed."
}
finally {
