...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Boss skillcard trigger marker Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/boss_skill_card_trigger_marker_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("boss_skill_card_trigger_marker_visual_qa_{0}_{1}.log" -f `
        $PID,
    ...
    $okMarker = "boss_skill_card_trigger_marker_visual_qa: ok"
    if ($exitCode -ne 0 -or $seriousErrors.Count -gt 0 -or -not $outputText.Contains($okMarker)) {
        Write-Host "Boss skillcard trigger marker visual QA log preserved for triage: $logPath"
        if ($exitCode -ne 0) {
    ...
    Write-Host "Boss skillcard trigger marker Vulkan visual QA passed: hit-wait and instant cards side by side."
}
finally {
