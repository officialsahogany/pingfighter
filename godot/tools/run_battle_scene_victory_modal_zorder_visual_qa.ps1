...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Victory reward external-modal z-order Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/battle_scene_victory_modal_zorder_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("battle_scene_victory_modal_zorder_visual_qa_{0}_{1}.log" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff"))
    ...
    Write-Host "Victory reward external-modal z-order Vulkan visual QA passed."
}
finally {
