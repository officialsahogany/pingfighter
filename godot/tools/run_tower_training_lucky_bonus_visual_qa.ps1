...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Tower training lucky bonus Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/tower_training_lucky_bonus_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("tower_training_lucky_bonus_visual_qa_{0}_{1}.log" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff"))
    ...
    if (
        ...
        -or -not $outputText.Contains("tower_training_lucky_bonus_visual_qa: captures=2") `
        -or -not $outputText.Contains("tower_training_lucky_bonus_visual_qa: badge_rows=4") `
        -or -not $outputText.Contains("tower_training_lucky_bonus_visual_qa: comparison=ok") `
        -or -not $outputText.Contains("tower_training_lucky_bonus_visual_qa: ok")
    ) {
        Write-Host "Tower training lucky bonus visual QA log preserved for triage: $logPath"
        if ($exitCode -ne 0) {
    ...
    Write-Host "Tower training lucky bonus Vulkan visual QA passed."
}
finally {
