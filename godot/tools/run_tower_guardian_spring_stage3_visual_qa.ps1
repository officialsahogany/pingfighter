...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Guardian spring S3 elite cards Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/tower_guardian_spring_stage3_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("guardian_spring_s3_visual_qa_{0}_{1}.log" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff"))
    ...
    if (
        ...
        -or -not $outputText.Contains("tower_guardian_spring_stage3_visual_qa: cards=3 floor=5 price=280") `
        -or -not $outputText.Contains("tower_guardian_spring_stage3_visual_qa: ok")
    ) {
        Write-Host "Guardian spring S3 visual QA log preserved for triage: $logPath"
    ...
    Write-Host "Guardian spring S3 elite-card Vulkan visual QA passed."
}
finally {
