...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Tower noncombat return selector Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/tower_noncombat_return_selector_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("tower_noncombat_return_selector_visual_qa_{0}_{1}.log" -f `
        $PID,
    ...
    if (
        ...
        Write-Host "Tower noncombat return selector QA log preserved for triage: $logPath"
        if ($exitCode -ne 0) {
        ...
        throw "$qaPath did not emit its capture and ok markers"
    }
    Remove-Item -LiteralPath $logPath -Force
    if (-not (Get-ChildItem -LiteralPath $logDir -Force)) {
        Remove-Item -LiteralPath $logDir -Force
    }
    Write-Host "Tower noncombat return selector Vulkan visual QA passed."
}
finally {
