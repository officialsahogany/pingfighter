...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Tower map drag and automatic-zoom overlap Vulkan QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/tower_map_drag_zoom_overlap_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir (
        "tower_map_drag_zoom_overlap_visual_qa_{0}_{1}.log" -f `
    ...
    $okMarker = "tower_map_drag_zoom_overlap_visual_qa: ok"
    if ($exitCode -ne 0 -or $seriousErrors.Count -gt 0 -or -not $outputText.Contains($okMarker)) {
        Write-Host "Tower map drag/zoom overlap QA log preserved: $logPath"
        if ($exitCode -ne 0) {
    ...
    Write-Host "Tower map drag and automatic-zoom overlap Vulkan QA passed."
}
finally {
