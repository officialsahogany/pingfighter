...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Tower map-camera tracking Vulkan and live traversal QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/tower_map_camera_tracking_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir (
        "tower_map_camera_tracking_visual_qa_{0}_{1}.log" -f `
    ...
    $okMarker = "tower_map_camera_tracking_visual_qa: ok"
    if ($exitCode -ne 0 -or $seriousErrors.Count -gt 0 -or -not $outputText.Contains($okMarker)) {
        Write-Host "Tower map-camera tracking QA log preserved: $logPath"
        if ($exitCode -ne 0) {
    ...
    Write-Host "Tower map-camera tracking Vulkan and live traversal QA passed."
}
finally {
