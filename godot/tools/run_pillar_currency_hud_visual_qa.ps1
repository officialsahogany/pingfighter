...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Pillar currency HUD Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/pillar_currency_hud_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("pillar_currency_hud_visual_qa_{0}_{1}.log" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff"))
    if ([string]::IsNullOrWhiteSpace($OutputDir)) {
        $OutputDir = Join-Path $ProjectPath ".godot\codex_artifacts\pillar_currency_hud"
    }
    ...
    if (
        ...
        -or -not $outputText.Contains("pillar_currency_hud_visual_qa: captures=3") `
        -or -not $outputText.Contains("pillar_currency_hud_visual_qa: ok")
    ) {
        Write-Host "Pillar currency visual QA log preserved for triage: $logPath"
        if ($exitCode -ne 0) {
    ...
    Write-Host "Pillar currency HUD Vulkan visual QA passed: $OutputDir"
}
finally {
