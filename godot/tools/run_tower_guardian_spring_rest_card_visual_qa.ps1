...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Tower guardian spring and rest card Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/tower_guardian_spring_rest_card_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("tower_node_modal_s4_visual_qa_{0}_{1}.log" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff"))
    ...
    if (
        ...
        -or -not $outputText.Contains("tower_guardian_spring_rest_card_visual_qa: captures=5 strips=2") `
        -or -not $outputText.Contains("tower_guardian_spring_rest_card_visual_qa: ok")
    ) {
        Write-Host "Tower node modal S4 visual QA log preserved for triage: $logPath"
        if ($exitCode -ne 0) {
        ...
        throw "$qaPath did not emit its capture and strip terminal markers"
    }
    Remove-Item -LiteralPath $logPath -Force
    if (-not (Get-ChildItem -LiteralPath $logDir -Force)) {
        Remove-Item -LiteralPath $logDir -Force
    }
    Write-Host "Tower guardian spring and rest card Vulkan visual QA passed."
}
finally {
