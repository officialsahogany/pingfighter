...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Ball contact squash/stretch Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/ball_contact_squash_stretch_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("ball_contact_squash_stretch_visual_qa_{0}_{1}.log" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff"))
    if ([string]::IsNullOrWhiteSpace($OutputDir)) {
        $OutputDir = Join-Path $ProjectPath ".godot\codex_artifacts\ball_contact_squash_stretch"
    }
    ...
    if (
        ...
        -or -not $outputText.Contains("ball_contact_squash_stretch_visual_qa: ok")
    ) {
        Write-Host "Ball contact squash/stretch visual QA log preserved for triage: $logPath"
        if ($exitCode -ne 0) {
    ...
    Write-Host "Ball contact squash/stretch Vulkan visual QA passed: $OutputDir"
}
finally {
