...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Gaksital Vision fan-throw Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/gaksital_vision_fan_throw_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("gaksital_vision_fan_throw_visual_qa_{0}_{1}.log" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff"))
    ...
    $okMarker = "gaksital_vision_fan_throw_visual_qa: ok"
    $sealMarker = "[GaksitalVisionFanThrowVisualSeal] renderer=Vulkan flying_fans=3 boss_stun_stars=3"
    if (
        $exitCode -ne 0 `
        -or $seriousErrors.Count -gt 0 `
        -or -not $outputText.Contains($okMarker) `
        -or -not $outputText.Contains($sealMarker)
    ) {
        Write-Host "Gaksital Vision visual QA log preserved for triage: $logPath"
        if ($exitCode -ne 0) {
        ...
        throw "$qaPath did not emit its terminal seal"
    }
    Remove-Item -LiteralPath $logPath -Force
    if (-not (Get-ChildItem -LiteralPath $logDir -Force)) {
        Remove-Item -LiteralPath $logDir -Force
    }
    Write-Host "Gaksital Vision fan-throw Vulkan visual QA passed."
}
finally {
