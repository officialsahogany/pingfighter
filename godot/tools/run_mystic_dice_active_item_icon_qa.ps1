...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Mystic Dice active-item icon Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/mystic_dice_active_item_icon_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("mystic_dice_active_item_icon_qa_{0}_{1}.log" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff"))
    ...
    if (
        ...
        -or -not $outputText.Contains("mystic_dice_active_item_icon_qa: evidence=") `
        -or -not $outputText.Contains("mystic_dice_active_item_icon_qa: ok")
    ) {
        Write-Host "Mystic Dice active-item icon visual QA log preserved for triage: $logPath"
        if ($exitCode -ne 0) {
        ...
        throw "$qaPath did not emit its evidence and terminal markers"
    }
    Remove-Item -LiteralPath $logPath -Force
    if (-not (Get-ChildItem -LiteralPath $logDir -Force)) {
        Remove-Item -LiteralPath $logDir -Force
    }
    Write-Host "Mystic Dice active-item icon Vulkan visual QA passed."
}
finally {
