...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Tower starting-card Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/tower_start_card_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("tower_start_card_visual_qa_{0}_{1}.log" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff"))
    ...
    if (
        ...
        -or -not $outputText.Contains("tower_start_card_visual_qa: meridian_expand_fusion=ok") `
    ...
    Write-Host "Tower starting-card Vulkan visual QA passed."
}
finally {
