...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Tower node modal scoreboard visibility Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $qaPath = "res://tools/tower_node_modal_scoreboard_visibility_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("tower_node_modal_scoreboard_visibility_visual_qa_{0}_{1}.log" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff"))
    ...
    if (
        ...
        -or -not $outputText.Contains("tower_node_modal_scoreboard_visibility_visual_qa: captures=4 strips=1") `
        -or -not $outputText.Contains("tower_node_modal_scoreboard_visibility_visual_qa: ok")
    ) {
        Write-Host "Tower node modal scoreboard visibility visual QA log preserved for triage: $logPath"
        if ($exitCode -ne 0) {
    ...
    Write-Host "Tower node modal scoreboard visibility Vulkan visual QA passed."
}
finally {
