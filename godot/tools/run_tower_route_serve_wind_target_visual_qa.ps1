...
try {
    ...
    $qaPath = "res://tools/tower_route_serve_wind_target_visual_qa.gd"
    $logDir = Join-Path $ProjectPath ".godot\codex_logs"
    New-Item -ItemType Directory -Force -Path $logDir | Out-Null
    $logPath = Join-Path $logDir ("tower_route_serve_wind_target_visual_qa_{0}_{1}.log" -f `
        $PID,
    ...
    $okMarker = "tower_route_serve_wind_target_visual_qa: ok"
    if ($exitCode -ne 0 -or $seriousErrors.Count -gt 0 -or -not $outputText.Contains($okMarker)) {
        Write-Host "Tower route wind visual QA log preserved for triage: $logPath"
        if ($exitCode -ne 0) {
