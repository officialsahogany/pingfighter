...
try {
    $validationPriorityContext = Assert-NoInteractiveGodotGame `
        ...
        -OperationName "Tower map edge-cull Vulkan visual QA" `
        -AllowDuringPlay
    ...
    $logPath = Join-Path $logDir (
        "tower_map_edge_cull_visual_qa_{0}_{1}.log" -f `
    ...
    $okMarker = "tower_map_edge_cull_visual_qa: ok"
    if ($exitCode -ne 0 -or $seriousErrors.Count -gt 0 -or -not $outputText.Contains($okMarker)) {
        Write-Host "Tower map edge-cull QA log preserved: $logPath"
        if ($exitCode -ne 0) {
            throw "$qaPath --edge-cull-only failed with exit code $exitCode"
        }
        if ($seriousErrors.Count -gt 0) {
            throw "$qaPath --edge-cull-only emitted a Godot error despite exit code 0"
        }
        throw "$qaPath --edge-cull-only did not emit its ok marker"
    }
    Remove-Item -LiteralPath $logPath -Force
    if (-not (Get-ChildItem -LiteralPath $logDir -Force)) {
        Remove-Item -LiteralPath $logDir -Force
    }
    Write-Host "Tower map edge-cull Vulkan visual QA passed."
}
finally {
