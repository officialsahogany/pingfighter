...
func _verify_expanded_paddle_uses_declared_width_key() -> void:
	...
	probe.rmb = true
	var result: Dictionary = state.advance(owner, near_pos, true, false, 0.0, LingpetMountState.is_mount_permitted("onimaru", []), false, false)
...
func _verify_held_rmb_is_edge_not_level() -> void:
	...
	probe.rmb = true
	state.advance(owner, near_pos, true, false, 0.0, LingpetMountState.is_mount_permitted("onimaru", []), false, false)
	var held: Dictionary = state.advance(owner, near_pos, true, false, 0.0, LingpetMountState.is_mount_permitted("onimaru", []), false, false)
...
func _verify_far_click_does_not_mount() -> void:
	...
	probe.rmb = true
	var result: Dictionary = state.advance(owner, far_pos, true, false, 0.0, LingpetMountState.is_mount_permitted("onimaru", []), false, false)
...
func _verify_position_override_keeps_lane_y() -> void:
	...
	probe.rmb = true
	state.advance(owner, pet_pos, true, false, 0.0, LingpetMountState.is_mount_permitted("onimaru", []), false, false)
...
func _verify_unsupported_pet_and_pet_switch_dismount() -> void:
	...
	probe.rmb = true
	var result: Dictionary = unsupported.advance(owner, Vector2(_player_center(owner), 660.0), true, false, 0.0, LingpetMountState.is_mount_permitted("onimaru", []), false, false)
	_expect(not bool(result.get("mounted", false)), "unsupported pet should never mount")

	var state := _make_state(probe)
	probe.rmb = false
	state.advance(owner, Vector2(_player_center(owner), 660.0), true, false, 0.0, LingpetMountState.is_mount_permitted("onimaru", []), false, false)
	probe.rmb = true
	state.advance(owner, Vector2(_player_center(owner), 660.0), true, false, 0.0, LingpetMountState.is_mount_permitted("onimaru", []), false, false)
...
func _verify_inactive_companion_forces_dismount() -> void:
	...
	state.advance(owner, Vector2(_player_center(owner), 660.0), true, false, 0.0, LingpetMountState.is_mount_permitted("onimaru", []), false, false)
	probe.rmb = false
	var result: Dictionary = state.advance(owner, Vector2(_player_center(owner), 660.0), false, false, 0.0, LingpetMountState.is_mount_permitted("onimaru", []), false, false)
...
func _verify_rider_lift_values() -> void:
	...
	state.advance(owner, Vector2(_player_center(owner), 660.0), true, false, 0.5, LingpetMountState.is_mount_permitted("onimaru", []), false, false)
