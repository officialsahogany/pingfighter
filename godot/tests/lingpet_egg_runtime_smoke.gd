...
func _verify_lingpet_catalog_random_hatch_scaffold() -> void:
	...
	_expect(all_catalog_pet_ids.has("draft_bat"), "catalog should keep the draft Bat lingpet concept as disabled metadata")
	_expect(not enabled_catalog_pet_ids.has("draft_bat"), "disabled draft Bat metadata should not appear in enabled pet ids")
	_expect(not candidates.has("draft_bat"), "disabled draft Bat metadata should not enter the random hatch pool")
	_expect(not LingpetCatalog.is_pet_enabled("draft_bat"), "disabled draft Bat metadata should not be treated as a live pet")
...
func _verify_companion_click_reaction() -> void:
	...
	_expect(runtime_source.find("COMPANION_CLICK_REACTION_PREWARM_KEYS") >= 0, "lingpet runtime should prewarm the current companion's click reaction sheet once the companion is active")
