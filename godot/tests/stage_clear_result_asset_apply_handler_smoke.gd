...
func _verify_apply_handler_contract() -> void:
	...
	scene._apply_scene_field_payload(payload)
...
func _verify_scene_delegates_asset_apply_handler() -> void:
	...
	_expect(load_textures_source.find("_apply_scene_field_payload(_get_field_payload_from_apply_result(") >= 0, "result scene should apply the texture payload through the guarded field applier")
