...
func _verify_apply_handler_contract() -> void:
	...
	scene._apply_scene_apply_result(StageClearResultAssetApplyHandler.load_texture_fields(scene, "commando", 1, "", ""))
...
func _verify_load_texture_fields_scene_apply() -> void:
	...
	scene._apply_scene_apply_result(StageClearResultAssetApplyHandler.load_texture_fields(scene, "smasher", 1, "", ""))
...
func _verify_scene_delegates_asset_apply_handler() -> void:
	...
	var load_textures_source: String = _slice_function(source, "func _load_textures", "func _load_audio")
	_expect(source.find("StageClearResultAssetApplyHandler.load_texture_fields") >= 0, "result scene should delegate texture field loading to the asset apply handler")
	...
	_expect(load_textures_source.find("_apply_scene_apply_result(StageClearResultAssetApplyHandler.load_texture_fields(") >= 0, "result scene should apply texture payloads through the unified scene apply-result helper")
