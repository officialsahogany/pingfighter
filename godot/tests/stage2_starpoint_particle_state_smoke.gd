...
func _verify_particle_update() -> void:
	...
	Stage2StarpointParticleState.update_particle(particle, 2.0)
...
func _verify_particle_array_compaction() -> void:
	...
	Stage2StarpointParticleState.update_particles(particles, 1.0)
...
func _verify_background_delegates_starpoint_particles() -> void:
	...
	_expect(
		source.find("Stage2StarpointParticleState.update_particles") >= 0,
		"Stage 2 background source should keep starpoint particle updates delegated"
	)
