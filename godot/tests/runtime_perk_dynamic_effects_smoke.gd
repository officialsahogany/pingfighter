...
func _verify_source_contract() -> void:
	...
	_expect(active_facade_body.find("_get_runtime_state_object(runtime_state, \"_effective_levels\")") >= 0, "dynamic active facade should own effective-level lookup")
	_expect(item_facade_body.find("_get_runtime_state_object(runtime_state, \"_effective_levels\")") >= 0, "dynamic item facade should own effective-level lookup")
	...
	_expect(viper_facade_body.find("_get_runtime_state_object(runtime_state, \"_effective_levels\")") >= 0, "dynamic Viper refresh facade should own effective-level lookup")
	_expect(item_refresh_facade_body.find("_get_runtime_state_object(runtime_state, \"_effective_levels\")") >= 0, "dynamic item refresh facade should own effective-level lookup")
	_expect(dirty_sync_facade_body.find("_get_runtime_state_object(runtime_state, \"_effective_levels\")") >= 0, "dynamic dirty-sync facade should own effective-level lookup")
