...
func _verify_scene_delegates_actor_draws() -> void:
	...
	_expect(source.find("StageClearResultActorPresenter.get_defeated_boss_draw_scene_apply_result") >= 0, "result scene should store Dalji click rect through actor presenter scene apply payloads")
	_expect(source.find("StageClearResultActorPresenter.get_player_victory_draw_scene_apply_result") >= 0, "result scene should store player victory click rect through actor presenter scene apply payloads")
