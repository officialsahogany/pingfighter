...
func _init() -> void:
	...
	_verify_click_affinity_visibility()
...
func _verify_hit_availability_priority() -> void:
	...
	_expect(
		not resolver.is_available_for_hit(true, true, true, true, true, true),
		"ring dash hidden should suppress hit availability before other override sources"
	)
	_expect(
		resolver.is_available_for_hit(true, false, false, false, false, false),
		"visible ring dash override should keep the companion available for hits"
	)
	_expect(
		resolver.is_available_for_hit(false, false, true, false, false, false),
		"active skill position override should keep the companion available for hits"
	)
	_expect(
		resolver.is_available_for_hit(false, false, false, true, false, false),
		"feed override should keep the companion available for hits"
	)
	_expect(
		resolver.is_available_for_hit(false, false, false, false, true, false),
		"starlight override should keep the companion available for hits"
	)
	_expect(
		not resolver.is_available_for_hit(false, false, false, false, false, false),
...
func _verify_draw_visibility_priority() -> void:
	var resolver := LingpetCompanionBodyPresenceResolver.new()
	_expect(
		not resolver.is_visible_for_draw(true, true, true, true, true, true),
		"ring dash hidden should suppress draw visibility before all visible sources"
	)
	_expect(
		resolver.is_visible_for_draw(false, false, true, false, false, false),
		"active skill position override should keep the companion visible"
	)
	_expect(
		resolver.is_visible_for_draw(false, false, false, true, false, false),
		"ring dash override should keep the companion visible when not hidden"
	)
	_expect(
		resolver.is_visible_for_draw(false, true, false, false, false, false),
		"motion visibility should keep the companion visible"
	)
	_expect(
		not resolver.is_visible_for_draw(false, false, false, false, false, false),
...
func _verify_draw_motion_ratio_selection() -> void:
	var resolver := LingpetCompanionBodyPresenceResolver.new()
	_expect_float(
		resolver.get_draw_motion_speed_ratio(true, false, false, false, true, "patrol", 0.9, 0.25, 0.12),
		0.25,
		"active skill override should use real override movement"
	)
	_expect_float(
		resolver.get_draw_motion_speed_ratio(false, true, false, false, true, "patrol", 0.9, 0.25, 0.12),
		0.0,
		"ring dash override should freeze the walk cadence"
	)
	_expect_float(
		resolver.get_draw_motion_speed_ratio(false, false, true, false, true, "patrol", 0.9, 0.25, 0.12),
		0.25,
		"feed override should use real override movement"
	)
	_expect_float(
		resolver.get_draw_motion_speed_ratio(false, false, false, true, true, "patrol", 0.9, 0.25, 0.12),
		0.25,
		"starlight override should use real horizontal movement"
	)
	_expect_float(
		resolver.get_draw_motion_speed_ratio(false, false, false, false, false, "patrol", 0.9, 0.25, 0.12),
		0.0,
		"hidden companion should freeze draw motion"
	)
	_expect_float(
		resolver.get_draw_motion_speed_ratio(false, false, false, false, true, "sortie_flight", 0.02, 0.0, 0.12),
		0.12,
		"sortie-flight companions should keep a minimum hover flap cadence"
	)
	_expect_float(
		resolver.get_draw_motion_speed_ratio(false, false, false, false, true, "patrol", 0.9, 0.25, 0.12),
		0.25,
		"ground patrol should use actual drawn movement instead of intended speed"
	)


func _verify_click_affinity_visibility() -> void:
	var resolver := LingpetCompanionBodyPresenceResolver.new()
	...
	_expect(resolver.can_grant_click_affinity(motion, ring_dash), "visible non-hidden companion should grant click affinity")

	motion.motion_visible = false
	_expect(not resolver.can_grant_click_affinity(motion, ring_dash), "motion-hidden companion should not grant click affinity")

	motion.motion_visible = true
	ring_dash.hidden = true
	_expect(not resolver.can_grant_click_affinity(motion, ring_dash), "ring-dash hidden companion should not grant click affinity")

	_expect(not resolver.can_grant_click_affinity(null, ring_dash), "null motion state should not grant click affinity")
	_expect(resolver.can_grant_click_affinity(motion, null), "null ring-dash state should not suppress visible click affinity")
...
func _verify_runtime_delegates_body_presence() -> void:
	...
	_expect(runtime_source.find("_companion_body_presence_resolver.can_grant_click_affinity") >= 0, "runtime click-affinity visibility gate should delegate to the resolver")
	...
	_expect(owner_source.find("func can_grant_click_affinity") >= 0, "presence resolver should own click-affinity visibility gating")
