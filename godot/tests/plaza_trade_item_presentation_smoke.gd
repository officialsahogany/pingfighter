...
func _init() -> void:
	...
	var fallback_color := Color(0.2, 0.3, 0.4, 1.0)
	_expect_eq(PlazaTradeItemPresentation.get_color({"quality_color": Color.RED}, fallback_color), Color.RED, "quality color priority")
	_expect_eq(PlazaTradeItemPresentation.get_color({"color": Color.BLUE}, fallback_color), Color.BLUE, "item color fallback")
	_expect_eq(PlazaTradeItemPresentation.get_color({"color": "invalid"}, fallback_color), fallback_color, "invalid color fallback")
	_expect(PlazaTradeItemPresentation.is_equipped({"equipped": true}), "equipped flag")
	_expect(PlazaTradeItemPresentation.is_equipped({"_equipped_slot": "accessory_0"}), "equipped slot fallback")
	_expect(not PlazaTradeItemPresentation.is_equipped({}), "unequipped item")
	_expect_eq(PlazaTradeItemPresentation.get_price_for_panel("player", {"shop_sell_price": 42}), 42, "player sell price")
	_expect_eq(PlazaTradeItemPresentation.get_price_for_panel("shop", {"shop_price": 120}), 120, "shop buy price")
	_expect_eq(PlazaTradeItemPresentation.get_price_for_panel("invalid", {"shop_price": 120}), 0, "invalid panel price")
