...
func _verify_dispatcher_audio_and_no_catalog_exposure() -> void:
	...
	_expect(str(LingpetCatalog.pick_skill_loadout("lumion").get("active_skill_id", "")) == "", "new Lumion hatch loadout should start with no active skill until affinity unlock resolve")
...
func _verify_source_wiring_surfaces() -> void:
	...
	_expect(egg_src.find("\"refire_chance_pct\"") >= 0, "egg runtime should pass Solar Bolt refire chance through launch_context")
