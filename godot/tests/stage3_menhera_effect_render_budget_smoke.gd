...
func _verify_render_budgets() -> void:
	...
	_expect(Stage3EffectRenderer.TAIL_DRAW_POINT_LIMIT_SEVERE_LOD <= 8, "tail whip should use the severe Viper curve draw cap")
	_expect(Stage3EffectRenderer.TAIL_HIT_RING_SEGMENTS <= 24, "tail-hit rings should avoid high segment counts")
	_expect(Stage3EffectRenderer.TAIL_HIT_RING_SEGMENTS_SEVERE_LOD <= 8, "tail-hit rings should use the severe Viper segment cap")
	_expect(Stage3EffectRenderer.TAIL_HIT_SLASH_SEGMENTS_SEVERE_LOD <= 6, "tail-hit slashes should use the severe Viper segment cap")
	_expect(Stage3EffectRenderer.TAIL_HIT_ARC_SEGMENTS_SEVERE_LOD <= 2, "tail-hit arcs should use the severe Viper segment cap")
	_expect(Stage3EffectRenderer.TAIL_HIT_RAY_COUNT <= 6, "tail-hit rays should keep draw calls bounded")
	_expect(Stage3EffectRenderer.TAIL_HIT_RAY_COUNT_SEVERE_LOD <= 2, "tail-hit rays should use the severe Viper draw cap")
	...
	_expect(Stage3EffectRenderer.MAX_RENDERED_PRISM_PARTICLES_SEVERE_LOD <= 7, "tail-hit prism particles should use the severe Viper LOD cap")
	...
	_expect(Stage3EffectRenderer.MAX_RENDERED_PSYCHOBALL_NEUTRALIZE_PARTICLES_SEVERE_LOD <= 10, "psychoball neutralize particles should use the severe Viper LOD cap")
	...
	_expect(Stage3EffectRenderer.MAX_RENDERED_CURSE_SMOKE_PARTICLES_SEVERE_LOD <= 18, "curse smoke should use the severe Viper LOD cap")
	...
	_expect(Stage3EffectRenderer.MAX_RENDERED_CURSE_EXPLOSION_PARTICLES_SEVERE_LOD <= 14, "curse explosion particles should use the severe Viper LOD cap")
	...
	_expect(Stage3EffectRenderer.MAX_RENDERED_KUROMI_EATING_PARTICLES_SEVERE_LOD <= 12, "Kuromi eating particles should use the severe Viper LOD cap")
	...
	_expect(Stage3EffectRenderer.MAX_RENDERED_STARPOINT_PARTICLES_SEVERE_LOD <= 14, "Stage 3 starpoint particles should use the severe Viper LOD cap")
