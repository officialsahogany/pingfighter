...
func _init() -> void:
	...
	_expect(str(catalog.get_data("meridian_expand").get("detail", "")) == "이번 런의 무공 최대 슬롯이 1칸 늘어납니다.", "catalog should preserve the approved meridian expansion detail")
	_expect(str(catalog.get_data("meridian_expand").get("rarity", "")) == "general", "meridian expansion must stay in the general pool")
	_expect("meridian_expand" in PerkFusionByproductCatalog.GENERAL_IDS, "meridian expansion must be declared in GENERAL_IDS")
	_expect("meridian_expand" not in PerkFusionByproductCatalog.RARE_IDS, "meridian expansion must never enter RARE_IDS")
	_expect(catalog.is_runtime_enabled("meridian_expand"), "meridian expansion must remain runtime-enabled")
