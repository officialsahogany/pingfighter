...
func _verify_tab_rows_and_benefit_colors() -> void:
	...
	_expect(_row_has(by_label, "대쉬 후딜", "-3%", CharacterInfoOverlayPerkPresenter.MYSTIC_DICE_STAT_BENEFIT_COLOR), "LIB raw -3 must invert to green")
	_expect(_row_has(by_label, "대쉬 쿨타임", "+3%", CharacterInfoOverlayPerkPresenter.MYSTIC_DICE_STAT_CURSE_COLOR), "LIB raw +3 must invert to red")
