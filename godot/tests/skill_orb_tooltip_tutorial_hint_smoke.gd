...
func _verify_runtime_wiring() -> void:
	...
	_expect(hint_source.find("_draw_guidance_pointer") >= 0, "skill tutorial should draw a guided cursor toward the orb")
	_expect(hint_source.find("POINTER_TRAIL_COUNT") >= 0, "skill tutorial cursor should include trail afterimages")
