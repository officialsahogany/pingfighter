...
func _verify_capture_teleport_release() -> void:
	...
	_expect(owner.ball_vel == swallowed_vel, "Rabi Ghost Summon should spit the ball with the last swallowed velocity")
	_expect(last_release_vel is Vector2 and last_release_vel == swallowed_vel, "Ghost Summon snapshot should expose the preserved swallow velocity")
