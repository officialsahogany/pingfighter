...
func _verify_dispatcher_and_catalog() -> void:
	...
	_expect(LingpetSkillDispatcher.has_supported_runtime("nekuring_ghost_summon"), "Nekuring Ghost Summon should route to a supported runtime")
	_expect(LingpetSkillDispatcher.is_ghost_summon("nekuring_ghost_summon"), "dispatcher should expose Nekuring's Ghost Summon helper through catalog metadata")
	...
	_expect(not nekuring_skill.is_empty(), "Nekuring catalog should expose Ghost Summon metadata")
	_expect(str(nekuring_skill.get("runtime_kind", "")) == "ghost_summon", "Nekuring Ghost Summon metadata should use the ghost_summon runtime kind")
	_expect(str(nekuring_skill.get("name", "")) == "해골소환", "Nekuring active skill should use the requested Korean Skeleton Summon name")
	_expect(is_equal_approx(float(nekuring_skill.get("cooldown", 0.0)), 40.0), "Nekuring Ghost Summon should use the requested 40-second cooldown")
