...
func _verify_random_building_layout_runtime() -> void:
	...
	_expect(is_equal_approx(float((status.get("world_size", Vector2.ZERO) as Vector2).x), 2400.0), "random layout should use the fixed Hwangyeok map-world width")
	...
	_expect(exit_zone.position.x > 2100.0 and exit_zone.end.x <= 2400.0, "Hwangyeok plaza exit should stay near the fixed map-world right edge")
