...
func _verify_japanese_quality_text(catalog: Object) -> void:
	LanguageSettings.set_language(LanguageSettings.LANGUAGE_JAPANESE)
	var lucky_coin: Dictionary = _sync_with_roll(catalog, "lucky_coin", {"double_spawn_pct": 15.0})
	var qualified_name := str(lucky_coin.get("qualified_display_name", ""))
	_expect(qualified_name.ends_with("ラッキーコイン"), "Japanese passive quality name should keep the localized item base name")
	_expect(qualified_name != "ラッキーコイン", "Japanese passive quality name should include a Japanese quality prefix")
	var option_text := PassiveItemQuality.format_roll_option_text({
		"label": "이동속도",
		"prefix": "+",
		"value": 12.0,
	...
	_expect(option_text == "移動速度 +12%", "Japanese roll option text should translate known labels")
	LanguageSettings.set_language(LanguageSettings.LANGUAGE_KOREAN)


func _verify_spanish_quality_text(catalog: Object) -> void:
	LanguageSettings.set_language(LanguageSettings.LANGUAGE_SPANISH)
	var lucky_coin: Dictionary = _sync_with_roll(catalog, "lucky_coin", {"double_spawn_pct": 15.0})
	var qualified_name := str(lucky_coin.get("qualified_display_name", ""))
	_expect(qualified_name.ends_with("Moneda de la suerte"), "Spanish passive quality name should keep the localized item base name")
	_expect(qualified_name != "Moneda de la suerte", "Spanish passive quality name should include a Spanish quality prefix")
	var option_text := PassiveItemQuality.format_roll_option_text({
		"label": "이동속도",
		"prefix": "+",
		"value": 12.0,
	...
	_expect(option_text == "Velocidad de movimiento +12%", "Spanish roll option text should translate known labels")
	LanguageSettings.set_language(LanguageSettings.LANGUAGE_KOREAN)


func _verify_portuguese_brazil_quality_text(catalog: Object) -> void:
	LanguageSettings.set_language(LanguageSettings.LANGUAGE_PORTUGUESE_BRAZIL)
	var lucky_coin: Dictionary = _sync_with_roll(catalog, "lucky_coin", {"double_spawn_pct": 15.0})
	var qualified_name := str(lucky_coin.get("qualified_display_name", ""))
	_expect(qualified_name.ends_with("Moeda da Sorte"), "Brazilian Portuguese passive quality name should keep the localized item base name")
	_expect(qualified_name != "Moeda da Sorte", "Brazilian Portuguese passive quality name should include a localized quality prefix")
	var option_text := PassiveItemQuality.format_roll_option_text({
		"label": "이동속도",
		"prefix": "+",
		"value": 12.0,
