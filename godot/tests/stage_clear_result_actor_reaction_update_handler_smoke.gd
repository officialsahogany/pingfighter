...
func _verify_scene_applies_actor_reaction_timer_update() -> void:
	...
	scene._apply_actor_reaction_timer_update({
		"dalji_base_timer": 2.0,
...
func _verify_scene_delegates_actor_reaction_updates() -> void:
	...
	var apply_source: String = _slice_function(source, "func _apply_actor_reaction_timer_update", "func handle_result_input")
	_expect(source.find("StageClearResultActorReactionUpdateHandler.update_actor_reaction_timers") >= 0, "result scene should delegate actor reaction timer updates")
	_expect(source.find("StageClearResultActorReactionUpdateHandler.get_actor_reaction_timer_context") >= 0, "result scene should delegate actor reaction timer context assembly")
	_expect(source.find("StageClearResultActorReactionUpdateHandler.get_actor_reaction_timer_scene_apply_result") >= 0, "result scene should delegate actor reaction timer scene field apply payloads")
	...
	_expect(source.find("_apply_actor_reaction_timer_update") >= 0, "result scene should apply actor reaction timer update results")
