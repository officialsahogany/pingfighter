...
func _verify_helper_source() -> void:
	...
	_expect(source.find("static func draw_ingame_starpoint_visual") >= 0, "starpoint draw helper should own in-game starpoint primitive drawing")
	_expect(source.find("static func draw_starpoint_sparkle_rays") >= 0, "starpoint draw helper should own starpoint sparkle rays")
	_expect(source.find("StageClearResultShapeHelper.draw_star_polygon") >= 0, "starpoint primitive drawing should delegate star polygons")
	...
	_expect(source.find("canvas.draw_line") >= 0, "starpoint sparkle rays should draw through the provided canvas")
	...
	StageClearResultStarpointDrawHelper.draw_starpoint_sparkle_rays(null, Vector2.ZERO, {})
...
func _verify_scene_no_longer_owns_starpoint_primitives() -> void:
	...
	_expect(source.find("func _draw_starpoint_sparkle_rays") < 0, "result scene should not keep starpoint ray drawing wrappers")
