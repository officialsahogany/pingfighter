...
func _verify_dispatch_actions() -> void:
	...
	_expect_action(helper.build_dispatch({"id": "instant_treasure_hunt"}, false, false), RuntimePerkChoiceDispatch.ACTION_INVALID, false, "retired Treasure Hunt should reject stale choices")
...
func _verify_source_contract() -> void:
	...
	_expect(dispatch_source.find("\"instant_treasure_hunt\": true") >= 0, "dispatch helper should keep a retired Treasure Hunt tombstone")
