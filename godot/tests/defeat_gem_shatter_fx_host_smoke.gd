...
func _verify_pipeline_status() -> void:
	...
	_expect(bool(status.get("quad_texture_ready", false)), "shatter host prewarm should create the white WritheEmber quad texture")
	_expect(bool(status.get("spark_texture_ready", false)), "shatter host prewarm should create the cyan sparkle texture")
	_expect(int(status.get("spark_amount", 0)) == 24, "shatter host should keep the bounded 24-spark particle budget")
...
func _verify_host_sync_and_cleanup() -> void:
	...
	_expect(int(active_status.get("particle_amount", 0)) == 24, "active host should keep its 24-particle cap")
