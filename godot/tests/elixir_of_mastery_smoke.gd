...
func _test_mythic_catalog_build() -> void:
	...
	_assert_true(str(item.get("description", "")).contains("Lv.3"), "mythic catalog: description follows the Mugong catalog max")
	_assert_true(not str(item.get("description", "")).contains("Lv.5"), "mythic catalog: stale Lv.5 description removed")
...
func _test_cinematic_draw_instantiation() -> void:
	...
	LanguageSettings.set_test_locale_override(LanguageSettings.LANGUAGE_KOREAN)
	_assert_eq(LanguageSettings.format_mugong_peak_reached(3), "극성 도달!", "draw: Korean max-rank banner")
	...
	_assert_true(cinematic_source.contains("format_mugong_peak_reached"), "draw: cinematic max-rank banner uses the shared formatter")
	_assert_true(not cinematic_source.contains("translate_text(\"Lv.5 달성!\")"), "draw: stale literal Lv.5 achievement key removed")
