...
func _verify_scene_delegates_box_draw() -> void:
	...
	_expect(source.find("StageClearResultBoxDrawHelper.draw_result_box_sheet_frame") >= 0, "result scene should delegate result-box sheet frame drawing")
	_expect(source.find("StageClearResultBoxDrawHelper.draw_result_box_fallback") >= 0, "result scene should delegate result-box fallback drawing")
