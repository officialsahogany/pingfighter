...
func _verify_catalog_walk_right_mapping() -> void:
	# 한미량 3.5등신 walk right f0 foot_l=(57,129), foot_r=(96,129).
	...
	_expect_vec(sockets.get("foot_l", Vector2.ZERO), Vector2(100.0 + 57.0 / 160.0 * 80.0, 200.0 + 129.0 / 160.0 * 80.0), "walk right f0 foot_l should map cell-local px into the dest rect")
	_expect_vec(sockets.get("foot_r", Vector2.ZERO), Vector2(100.0 + 96.0 / 160.0 * 80.0, 200.0 + 129.0 / 160.0 * 80.0), "walk right f0 foot_r should map cell-local px into the dest rect")


func _verify_catalog_left_mirror_swaps_ids() -> void:
	# Left sheets are mirrors of right: x flips AND _l/_r ids swap so foot_l
	# stays the screen-left socket. right f0 foot_l=57/foot_r=96.
	...
	_expect_vec(sockets.get("foot_l", Vector2.ZERO), Vector2(160.0 - 96.0, 129.0), "walk left foot_l should be the mirrored right-sheet foot_r")
	_expect_vec(sockets.get("foot_r", Vector2.ZERO), Vector2(160.0 - 57.0, 129.0), "walk left foot_r should be the mirrored right-sheet foot_l")
