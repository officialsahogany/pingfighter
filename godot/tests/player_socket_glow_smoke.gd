...
func _verify_catalog_walk_right_mapping() -> void:
	# 한미량 v1+원화구름 walk right f0 foot_l=(49,128), foot_r=(99,128).
	...
	_expect_vec(sockets.get("foot_l", Vector2.ZERO), Vector2(100.0 + 49.0 / 160.0 * 80.0, 200.0 + 128.0 / 160.0 * 80.0), "walk right f0 foot_l should map cell-local px into the dest rect")
...
func _verify_catalog_left_mirror_swaps_ids() -> void:
	...
	# stays the screen-left socket. right f0 foot_l=49/foot_r=99.
	...
	_expect_vec(sockets.get("foot_r", Vector2.ZERO), Vector2(160.0 - 49.0, 128.0), "walk left foot_r should be the mirrored right-sheet foot_l")
...
func _verify_catalog_idle_back_falls_to_any() -> void:
	...
	_expect_vec(sockets.get("foot_l", Vector2.ZERO), Vector2(58.0, 129.0), "idle direction 'back' should fall back to the authored 'any' data")
