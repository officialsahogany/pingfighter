...
func _verify_catalog_walk_right_mapping() -> void:
	# 한미량 v2(단청) walk right f0 authored foot_l=(57,127), foot_r=(105,127).
	...
	_expect_vec(sockets.get("foot_l", Vector2.ZERO), Vector2(100.0 + 57.0 / 160.0 * 80.0, 200.0 + 127.0 / 160.0 * 80.0), "walk right f0 foot_l should map cell-local px into the dest rect")
	_expect_vec(sockets.get("foot_r", Vector2.ZERO), Vector2(100.0 + 105.0 / 160.0 * 80.0, 200.0 + 127.0 / 160.0 * 80.0), "walk right f0 foot_r should map cell-local px into the dest rect")


func _verify_catalog_left_mirror_swaps_ids() -> void:
	# Left sheets are mirrors of right: x flips AND _l/_r ids swap so foot_l
	# stays the screen-left socket. 한미량 v2 right f0 foot_l=57/foot_r=105.
	...
	_expect_vec(sockets.get("foot_l", Vector2.ZERO), Vector2(160.0 - 105.0, 127.0), "walk left foot_l should be the mirrored right-sheet foot_r")
	_expect_vec(sockets.get("foot_r", Vector2.ZERO), Vector2(160.0 - 57.0, 127.0), "walk left foot_r should be the mirrored right-sheet foot_l")
...
func _verify_catalog_idle_back_falls_to_any() -> void:
	...
	_expect_vec(sockets.get("foot_l", Vector2.ZERO), Vector2(50.0, 129.0), "idle direction 'back' should fall back to the authored 'any' data")
