...
func _verify_runtime_delegates_overlay_host_resolver() -> void:
	...
	_expect(runtime_source.find("_acquisition_lifecycle.prewarm_registry_step") >= 0, "runtime prewarm path should delegate to the acquisition lifecycle owner")
	_expect(lifecycle_source.find("_asset_prewarm_state.prewarm_registry_step") >= 0, "acquisition lifecycle should delegate registry prewarm to the asset-prewarm owner")
	...
	_expect(lifecycle_source.find("_overlay_host_resolver.is_anim_ready") >= 0, "acquisition lifecycle reveal-readiness query should delegate")
