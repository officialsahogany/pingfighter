...
const CAPTURE_PATH := "res://.tmp/common_mugong_dash_seal_qa.png"
const SPECS := [
...
class IconProbe:
	...
	func _draw() -> void:
		...
		for index in SPECS.size():
			...
			if font != null:
				draw_string(font, Vector2(left, top - 23.0), str(spec.get("name", "")), HORIZONTAL_ALIGNMENT_LEFT, 130.0, 23, Color(0.94, 0.84, 0.63))
...
func _run() -> void:
	...
	for spec: Dictionary in SPECS:
		if not renderer.has_icon(str(spec.get("id", ""))):
			push_error("Dash Mugong seal should resolve before visual QA: %s" % str(spec.get("id", "")))
			quit(1)
			return
	...
	if probe.draw_results.size() != SPECS.size() * 2 or probe.draw_results.has(false):
		push_error("All dash Mugong seals should draw at acquisition and owned sizes")
		quit(1)
		return
	var image: Image = viewport.get_texture().get_image()
	if image.save_png(CAPTURE_PATH) != OK:
		push_error("Failed to save dash Mugong visual QA capture")
		quit(1)
		return
	probe.queue_free()
	viewport.queue_free()
	await process_frame
	print("common_mugong_dash_seal_visual_qa: ok")
