...
const EXPECTED_KO_DETAILS := [
	...
	"개광결로 모든 일반 성장형 무공의 증폭 가능한 수치 효과를 독립적으로 강화합니다. 횟수·슬롯·해금·활성 여부 같은 구조값은 그대로 유지됩니다.",
	"환보결로 사용한 아이템을 확률적으로 되돌려 보존합니다. 자체 회수되는 부메랑은 제외됩니다.",
	"천기보도가 승리 보상 픽에 절세무공 카드가 등장할 확률을 레벨당 150% 높입니다.",
]
const ICON_PATHS := {
...
func _test_compatibility_ids_and_values() -> void:
	...
	var contracts := {
		...
		"item_polish": [5, 1, "모든 일반 성장형 무공의 수치 능력치 5% 증폭", 5, "모든 일반 성장형 무공의 수치 능력치 25% 증폭"],
		"item_recycle": [5, 1, "아이템 유지 확률 7%", 5, "아이템 유지 확률 35%"],
		"downtown_treasure_map": [5, 1, "승리 보상 픽 절세무공 등장 확률 +150%", 5, "승리 보상 픽 절세무공 등장 확률 +750%"],
	}
