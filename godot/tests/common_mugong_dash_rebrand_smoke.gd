...
const EXPECTED := {
	"ko": {
		...
		"dash_jump": ["비천보", "비천보로 한 번의 활주 거리를 늘려 더 멀리 움직입니다."],
	...
	"en": {
		...
		"dash_jump": ["Sky-Soaring Step", "Sky-Soaring Step increases glide distance."],
	...
	"es": {
		...
		"dash_jump": ["Paso de Vuelo Celestial", "Paso de Vuelo Celestial aumenta la distancia de deslizamiento."],
	...
	"pt-BR": {
		...
		"dash_jump": ["Passo de Voo Celestial", "O Passo de Voo Celestial aumenta a distância do deslize."],
	...
	"ru": {
		...
		"dash_jump": ["Небесный Летящий Шаг", "Небесный Летящий Шаг увеличивает дальность скольжения."],
...
func _test_compatibility_ids_and_values() -> void:
	...
	var expected_contracts := {
		...
		"dash_jump": [5, 1, "활주 거리 7% 증가", 5, "활주 거리 35% 증가"],
