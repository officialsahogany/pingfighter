...
func _init() -> void:
	...
	_expect(is_equal_approx(ActiveItemThrowController.TEAR_GAS_MAX_RADIUS_X, 384.0), "tear gas runtime horizontal radius should be 384")
	_expect(is_equal_approx(ActiveItemThrowController.TEAR_GAS_EXPANSION_RATE_X, 10.72), "tear gas horizontal expansion should preserve fill timing")
	...
	_expect(is_equal_approx(ActiveItemThrowRenderer.TEAR_GAS_RADIUS_X, 384.0), "tear gas renderer horizontal fallback should match runtime")
	...
	var renderer_source: String = FileAccess.get_file_as_string("res://scripts/items/active_item_throw_renderer.gd")
	var tear_gas_zone_body: String = _function_body(renderer_source, "func _draw_tear_gas_zones")
