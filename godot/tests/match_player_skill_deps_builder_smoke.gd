...
func _init() -> void:
	...
	_expect(null_deps.get("skill_configs", []).size() == 5, "null registry should keep skill config array shape")
...
func _verify_skill_deps(deps: Dictionary, registry: FakeRegistry, source: String) -> void:
	...
	_expect(skill_configs.size() == 5, "%s should include five skill configs" % source)
	...
	_expect(skill_configs[3] == registry.instances["optimus_skill_config"], "%s should include Optimus skill config fourth" % source)
	_expect(skill_configs[4] == registry.instances["blacksmith_skill_config"], "%s should include Blacksmith skill config fifth" % source)
...
func _verify_scoped_smasher_deps(builder: Object, registry: FakeRegistry) -> void:
	...
	_expect(skill_configs.size() == 5, "scoped Smasher deps should preserve skill config array shape")
	...
	_expect(skill_configs[3] == null, "scoped Smasher deps should not include Optimus skill config")
	_expect(skill_configs[4] == null, "scoped Smasher deps should not include Blacksmith skill config")
	...
	_expect(not registry.requested_keys.has("optimus_skill_config"), "scoped Smasher deps should not wake Optimus skill config")
...
func _verify_scoped_viper_deps(builder: Object, registry: FakeRegistry) -> void:
	...
	_expect(skill_configs.size() == 5, "scoped Viper deps should preserve skill config array shape")
	...
	_expect(skill_configs[3] == null, "scoped Viper deps should not include Optimus skill config")
	_expect(skill_configs[4] == null, "scoped Viper deps should not include Blacksmith skill config")
	var skill_runtimes: Array = deps.get("skill_runtimes", [])
...
func _verify_scoped_commando_deps(builder: Object, registry: FakeRegistry, character_type: String = "commando") -> void:
	...
	_expect(skill_configs.size() == 5, "scoped Commando deps should preserve skill config array shape")
	...
	_expect(skill_configs[3] == null, "scoped Commando deps should not include Optimus skill config")
	_expect(skill_configs[4] == null, "scoped Commando deps should not include Blacksmith skill config")
	var skill_runtimes: Array = deps.get("skill_runtimes", [])
	...
	_expect(not registry.requested_keys.has("optimus_skill_config"), "scoped Commando deps should not wake Optimus skill config")
...
func _verify_scoped_optimus_deps(builder: Object, registry: FakeRegistry, character_type: String = "optimus") -> void:
	...
	_expect(skill_configs.size() == 5, "scoped Optimus deps should preserve skill config array shape")
	...
	_expect(skill_configs[3] == registry.instances["optimus_skill_config"], "scoped Optimus deps should include Optimus skill config")
	_expect(skill_configs[4] == null, "scoped Optimus deps should not include Blacksmith skill config")
	...
	_expect(registry.requested_keys.has("optimus_skill_config"), "scoped Optimus deps should wake Optimus skill config")
...
func _verify_scoped_blacksmith_deps(builder: Object, registry: FakeRegistry, character_type: String = "blacksmith") -> void:
	...
	_expect(skill_configs.size() == 5, "scoped Blacksmith deps should preserve skill config array shape")
	...
	_expect(skill_configs[3] == null, "scoped Blacksmith deps should not include Optimus skill config")
	_expect(skill_configs[4] == registry.instances["blacksmith_skill_config"], "scoped Blacksmith deps should include Blacksmith skill config")
