...
func _verify_runtime_minimum_hold_and_transition_feedback() -> void:
	...
	var stow_transition: Dictionary = stow_snapshot.get("guardian_transition", {}) as Dictionary
	_expect(str(stow_transition.get("mode", "")) == "stow", "stow should enter the dedicated reverse-stream transition")
	...
	_expect(runtime.try_toggle_guardian_stow(owner, registry), "repeat toggle during stow presentation should be consumed")
	_expect(runtime.is_guardian_stowed(), "repeat toggle must not reverse an active transition")
	runtime.update(0.47, owner, registry)
	_expect(runtime.try_toggle_guardian_stow(owner, registry), "healthy shared pool should begin resummon presentation")
	_expect(runtime.is_guardian_stowed(), "summon presentation must keep guardian unavailable until completion")
	var summon_transition: Dictionary = (runtime.get_snapshot().get("guardian_transition", {}) as Dictionary)
	_expect(str(summon_transition.get("mode", "")) == "summon", "resummon should enter the projectile transition")
	var pool_before_summon: float = runtime.get_duration_pool_current()
	runtime.update(0.25, owner, registry)
	_expect(runtime.is_guardian_stowed(), "guardian must stay unavailable in the middle of summon presentation")
	_expect(runtime.get_duration_pool_current() < pool_before_summon, "duration drain must start with summon presentation")
	_expect(runtime.try_toggle_guardian_stow(owner, registry), "toggle during summon presentation should be consumed")
	runtime.update(0.28, owner, registry)
	_expect(not runtime.is_guardian_stowed(), "healthy shared pool should activate after presentation completion")
	_expect(runtime.is_companion_active(), "completed resummon should publish active")
