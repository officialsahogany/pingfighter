...
func _verify_runtime_delegates_value_utils() -> void:
	...
	_expect(runtime._get_lingering_fire_flame_count() == 15, "lingering fire flame-count helper should preserve the deterministic flame count")
	_expect(runtime._get_array(fire_seed_effect.get("flames", [])).size() == runtime._get_lingering_fire_flame_count(), "lingering fire seed helper should seed deterministic fire flames")
	_expect(runtime._get_lingering_fire_effect_size({"width": 80.0, "height": 40.0}) == Vector2(80.0, 40.0), "lingering fire effect-size helper should preserve explicit dimensions")
	_expect(runtime._get_lingering_fire_effect_size({}) == Vector2(150.0, 60.0), "lingering fire effect-size helper should use default dimensions")
	_expect(runtime._get_lingering_fire_effect_size({"width": -4.0, "height": 0.0}) == Vector2(1.0, 1.0), "lingering fire effect-size helper should clamp dimensions to positive values")
	_expect(is_equal_approx(runtime._get_lingering_fire_effect_width({"width": 80.0}), 80.0), "lingering fire effect-width helper should preserve explicit widths")
	_expect(is_equal_approx(runtime._get_lingering_fire_effect_width({}), 150.0), "lingering fire effect-width helper should use the default width")
	_expect(is_equal_approx(runtime._get_lingering_fire_effect_height({"height": 40.0}), 40.0), "lingering fire effect-height helper should preserve explicit heights")
	_expect(is_equal_approx(runtime._get_lingering_fire_effect_height({}), 60.0), "lingering fire effect-height helper should use the default height")
	_expect(is_equal_approx(runtime._get_lingering_fire_effect_dimension({"width": -4.0}, "width", 150.0), 1.0), "lingering fire effect-dimension helper should clamp dimensions")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_cycle_angle(2), TAU * 2.0 / 15.0), "lingering fire flame cycle-angle helper should preserve circular spacing")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_angle(2), TAU * 2.0 / 15.0), "lingering fire flame angle helper should preserve spawn spacing")
	_expect(runtime._get_lingering_fire_flame_pattern_value(2, 7, 11) == 3, "lingering fire flame pattern helper should preserve modular spacing")
	_expect(runtime._get_lingering_fire_flame_ring_pattern_value(2) == 3, "lingering fire flame ring-pattern helper should preserve ring spacing")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_ring_factor(1), 0.7), "lingering fire flame ring-factor helper should preserve staggered ring factors")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_ring(0), 0.26), "lingering fire flame ring helper should preserve the first ring")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_ring(1), 0.736), "lingering fire flame ring helper should preserve staggered rings")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_spawn_radius_x(80.0, 0.26), 10.4), "lingering fire flame spawn-radius x helper should preserve ring-scaled x radii")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_spawn_radius_y(40.0, 0.26), 5.2), "lingering fire flame spawn-radius y helper should preserve ring-scaled y radii")
	_expect(runtime._get_lingering_fire_flame_spawn_radius(80.0, 40.0, 0.26).is_equal_approx(Vector2(10.4, 5.2)), "lingering fire flame spawn-radius helper should preserve ring-scaled radii")
	_expect(runtime._get_lingering_fire_flame_offset_from_radius(0.0, Vector2(10.4, 5.2)).is_equal_approx(Vector2(10.4, 0.0)), "lingering fire flame radius-offset helper should preserve angle projection")
	_expect(runtime._get_lingering_fire_flame_offset(0, 80.0, 40.0, 0.0).is_equal_approx(Vector2(10.4, 0.0)), "lingering fire flame offset helper should preserve the first flame offset")
	_expect(runtime._get_lingering_fire_flame_size_pattern_value(1) == 5, "lingering fire flame size-pattern helper should preserve size spacing")
	_expect(runtime._get_lingering_fire_flame_size_offset(1) == 5, "lingering fire flame size-offset helper should preserve staggered size offsets")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_size(0), 8.0), "lingering fire flame size helper should preserve the first size")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_size(1), 13.0), "lingering fire flame size helper should preserve staggered sizes")
	_expect(runtime._get_lingering_fire_flame_lifetime_pattern_value(5) == 15, "lingering fire flame lifetime-pattern helper should preserve lifetime spacing")
	_expect(runtime._get_lingering_fire_flame_lifetime_offset(5) == 15, "lingering fire flame lifetime-offset helper should preserve staggered lifetime offsets")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_lifetime(0), 22.0), "lingering fire flame lifetime helper should preserve the first lifetime")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_lifetime(5), 37.0), "lingering fire flame lifetime helper should preserve staggered lifetimes")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_phase_spacing(), 0.67), "lingering fire flame phase-spacing helper should preserve phase spacing")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_phase(2), 1.34), "lingering fire flame phase helper should preserve phase spacing")
	_expect(is_equal_approx(runtime._get_lingering_fire_flame_max_lifetime(), 40.0), "lingering fire flame max-lifetime helper should preserve max lifetime")
	var first_fire_flame: Dictionary = runtime._build_lingering_fire_flame(0, 80.0, 40.0)
	...
	_expect(runtime._get_array(fire_zone_frame_effect.get("flames", [])).size() == runtime._get_lingering_fire_flame_count(), "lingering frame helper should build fire-zone flames")
	...
	_expect(seeded_fire_flames.size() == runtime._get_lingering_fire_flame_count(), "fire flames frame helper should seed missing flame arrays")
	var invalid_fire_flames: Array = runtime._get_lingering_fire_flames_for_frame({"flames": "bad", "width": 80.0, "height": 40.0}, 2.0)
	_expect(invalid_fire_flames.size() == runtime._get_lingering_fire_flame_count(), "fire flames frame helper should seed invalid flame arrays")
