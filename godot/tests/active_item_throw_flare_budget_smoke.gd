...
const ActiveItemThrowFlareRenderer := preload("res://scripts/items/active_item_throw_flare_renderer.gd")
...
func _verify_flare_zone_draw_budget() -> void:
	_expect(ActiveItemThrowFlareRenderer.FLARE_FLASH_LAYERS <= 1, "flare flash should cap bright ring layers")
	_expect(ActiveItemThrowFlareRenderer.FLARE_GLOW_LAYERS <= 1, "flare glow should cap ambient ring layers")
	var source := FileAccess.get_file_as_string("res://scripts/items/active_item_throw_flare_renderer.gd")
	var body := _function_body(source, "func draw_flare_zones")
