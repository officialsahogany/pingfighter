...
const CAPTURE_PATH := "res://.tmp/cheongringwi_vision_reward_box_visual_qa.png"
...
class VisionBoxProbe:
	...
	func _draw() -> void:
		draw_rect(Rect2(Vector2.ZERO, Vector2(VIEW_SIZE)), Color(0.010, 0.030, 0.058), true)
		for y in range(40, VIEW_SIZE.y, 64):
			draw_line(Vector2(0.0, float(y)), Vector2(float(VIEW_SIZE.x), float(y)), Color(0.22, 0.66, 0.92, 0.07), 1.0)
		for x in range(72, VIEW_SIZE.x, 96):
			draw_line(Vector2(float(x), 0.0), Vector2(float(x), float(VIEW_SIZE.y)), Color(0.22, 0.66, 0.92, 0.05), 1.0)
		var samples := [
		...
		for sample_value: Variant in samples:
			...
			var box := {
				...
				"boss_vision_offer_id": CommonSkillCatalog.CHEONGRINGWI_VISION_DRAGON_TORRENT_UNLOCK_ID,
			}
...
func _init() -> void:
	ProjectResourceLoader.load_imported_texture(
		VictoryLootPhaseState.CHEONGRINGWI_VISION_BOX_SHEET_PATH,
		"Missing Cheongringwi Vision reward box sheet at %s",
		"Failed to load Cheongringwi Vision reward box sheet at %s"
	)
	...
	if not probe.draw_completed:
		push_error("Cheongringwi Vision reward box production render did not complete")
		quit(1)
		return
	var image: Image = viewport.get_texture().get_image()
	if image == null or image.is_empty() or image.save_png(CAPTURE_PATH) != OK:
		push_error("Cheongringwi Vision reward box visual QA capture failed")
		quit(1)
		return
	print("cheongringwi_vision_reward_box_visual_qa: ok")
