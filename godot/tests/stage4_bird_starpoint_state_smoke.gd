...
const Stage4BirdStarpointState := preload("res://scripts/stages/stage4/stage4_bird_starpoint_state.gd")
...
func _init() -> void:
	...
	print("stage4_bird_starpoint_state_smoke: ok")
	quit(0)


func _verify_owner_spawn_update_and_clear() -> void:
	var seeded_rng := RandomNumberGenerator.new()
	seeded_rng.seed = 41004
	var state := Stage4BirdStarpointState.new(seeded_rng)
	state.spawn_crow_drop(Vector2(320.0, 300.0), {}, _context(Vector2(-500.0, -500.0)))
	_expect(state.drops.size() == 1, "owner should append one crow drop")
	_expect(str(state.drops[0].get("source_type", "")) == "crow", "owner should retain the crow source tag")
	_expect(state.particles.size() == Stage4BirdStarpointState.PARTICLE_COUNT, "owner should append the Stage 4 spawn burst")
...
func _verify_source_ownership() -> void:
	var facade_source := FileAccess.get_file_as_string("res://scripts/stages/stage4/stage4_bird_event.gd")
	var owner_source := FileAccess.get_file_as_string("res://scripts/stages/stage4/stage4_bird_starpoint_state.gd")
	_expect(facade_source.contains("Stage4BirdStarpointState"), "bird facade should preload the starpoint owner")
	_expect(facade_source.contains("var starpoint_state: Object = Stage4BirdStarpointState.new(rng)"), "bird facade should retain one starpoint owner sharing its RNG")
	_expect(not facade_source.contains("var starpoint_drops: Array = []"), "bird facade should not retain a mirrored drop array")
	_expect(not facade_source.contains("var starpoint_particles: Array = []"), "bird facade should not retain a mirrored particle array")
...
func _make_drop(pos: Vector2) -> Dictionary:
	return {
		"pos": pos,
		"vel": Vector2.ZERO,
		"size": Stage4BirdStarpointState.DROP_SIZE,
		"rotation": 0.0,
		"rotation_speed": 0.0,
		"glow_intensity": 1.0,
		"glow_timer": 0.0,
		"life": Stage4BirdStarpointState.DROP_LIFETIME,
		"float_timer": 0.0,
	}
