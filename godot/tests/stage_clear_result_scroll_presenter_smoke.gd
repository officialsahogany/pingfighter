...
func _verify_scene_delegates_scroll_presenter() -> void:
	...
	_expect(draw_scene_handler_source.find("StageClearResultScrollSceneHandler.draw_scroll") >= 0, "draw scene handler should delegate scroll drawing through the scroll scene handler")
	_expect(source.find("StageClearResultScrollPresenter.") < 0, "result scene should not call the scroll presenter directly")
	_expect(scene_handler_source.find("StageClearResultScrollPresenter.draw_scroll") >= 0, "scroll scene handler should delegate scroll drawing to the presenter")
	_expect(scene_handler_source.find("StageClearResultScrollPresenter.get_scroll_draw_context") >= 0, "scroll scene handler should delegate scroll draw context assembly to the presenter")
	_expect(scene_handler_source.find("StageClearResultScrollPresenter.get_scroll_draw_apply_result") >= 0, "scroll scene handler should delegate scroll draw apply payloads to the presenter")
	_expect(source.find("func _get_scroll_draw_context") < 0, "result scene should not keep scroll draw context wrappers")
	_expect(scene_handler_source.find("static func get_scroll_draw_context") >= 0, "scroll scene handler should own scroll draw scene-context glue")
	_expect(scene_handler_source.find("static func apply_scroll_state_result") >= 0, "scroll scene handler should apply scroll state payloads in one helper")
	_expect(source.find("func _apply_scroll_state_result") < 0, "result scene should not keep scroll state apply fanout wrappers")
	_expect(source.find("func _draw_scroll") < 0, "result scene should not keep scroll draw fanout wrappers")
