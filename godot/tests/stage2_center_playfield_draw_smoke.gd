...
func _init() -> void:
	...
	_expect(Stage2PlayfieldRenderer.MAX_BUSH_TEXTURE_CLUSTERS_PER_BUSH <= 1, "Stage 2 bush clusters should keep a tighter texture draw budget")
	_expect(int(layout.get("bush_draw_cluster_count", 999)) <= 12, "Stage 2 bush anchors should not expand into an uncapped texture draw list")
	...
	_expect(Stage2PlayfieldRenderer.ELLIPSE_SEGMENTS <= 8, "Stage 2 decorative ellipse fills should stay within the draw budget")
	_expect(Stage2PlayfieldRenderer.ELLIPSE_OUTLINE_SEGMENTS <= 12, "Stage 2 decorative ellipse outlines should stay within the draw budget")
	_expect(Stage2PlayfieldRenderer.ELLIPSE_ARC_SEGMENTS <= 6, "Stage 2 decorative ellipse arcs should stay within the draw budget")
