...
func _init() -> void:
	...
	_expect(Stage2PlayfieldRenderer.FALLING_LEAF_RENDER_LIMIT <= 2, "Stage 2 falling leaves should keep a capped decorative draw budget")
	_expect(Stage2PlayfieldRenderer.FALLING_LEAF_RENDER_LIMIT_SEVERE_LOD <= 0, "Stage 2 severe LOD should disable decorative falling leaves")
	...
	_expect(Stage2PlayfieldRenderer.VINE_RENDER_STRIDE_SEVERE_LOD >= 2, "Stage 2 severe LOD should stride vine drawing")
	...
	_expect(Stage2PlayfieldRenderer.ELLIPSE_SEGMENTS >= 18, "Stage 2 center stadium ellipse fills should keep a round silhouette")
	_expect(Stage2PlayfieldRenderer.ELLIPSE_SEGMENTS <= 20, "Stage 2 center stadium ellipse fills should stay within a bounded draw budget")
	_expect(Stage2PlayfieldRenderer.ELLIPSE_OUTLINE_SEGMENTS >= 24, "Stage 2 center stadium outline should keep a round silhouette")
	_expect(Stage2PlayfieldRenderer.ELLIPSE_OUTLINE_SEGMENTS <= 24, "Stage 2 center stadium outline should stay within a bounded draw budget")
	_expect(Stage2PlayfieldRenderer.ELLIPSE_ARC_SEGMENTS >= 12, "Stage 2 center stadium electric arcs should avoid polygonal stepping")
	_expect(Stage2PlayfieldRenderer.ELLIPSE_ARC_SEGMENTS <= 12, "Stage 2 center stadium electric arcs should stay within a bounded draw budget")
