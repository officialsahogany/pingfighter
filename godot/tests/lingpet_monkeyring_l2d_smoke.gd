...
func _verify_catalog_entry() -> void:
	...
	_expect(effect_text.find("뒷모습 기준 제자리 숨쉬기") >= 0, "Monkeyring effect text should backfill the rear-view idle breathing SD contract")
	_expect(effect_text.find("마리보처럼 살짝 측면") >= 0, "Monkeyring effect text should backfill the Maribo-like slight side-read movement contract")
	_expect(effect_text.find("전용 포효 cast") >= 0, "Monkeyring effect text should backfill the dedicated rear Wild Roar cast contract")
