...
func _verify_death_cinematic_state_contract() -> void:
	...
	state.update(1.99)
	_expect(str(state.get_death_phase()) == "pre_explosion", "death should stay in pre-explosion before 4/9 (2.0s explosion fire)")
	_expect(float(state.get_death_overall_progress()) < 4.0 / 9.0, "death should not cross explosion before 4/9")
	_expect(not state.consume_death_explosion_edge(), "explosion edge should not fire before 4/9")
	var previous_progress: float = float(state.get_death_overall_progress())

	state.update(0.02)
	_expect(str(state.get_death_phase()) == "explosion", "death should enter explosion at 4/9 (2.0s = odindeath bang alignment)")
	_expect(float(state.get_death_overall_progress()) > previous_progress, "death progress should be monotonic")
	_expect(state.consume_death_explosion_edge(), "explosion edge should fire exactly once at 4/9")
	...
	state.update(0.98)
	_expect(str(state.get_death_phase()) == "explosion", "death should stay in explosion before 2/3 (3.0s disintegration)")
	_expect(float(state.get_death_overall_progress()) < 2.0 / 3.0, "death should not cross disintegration before 2/3")
	_expect(not state.consume_disintegration_edge(), "disintegration edge should not fire before 2/3")

	state.update(0.03)
	_expect(str(state.get_death_phase()) == "disintegrate", "death should enter disintegration at 2/3")
	_expect(state.consume_disintegration_edge(), "disintegration edge should fire exactly once at 2/3")
	_expect(not state.consume_disintegration_edge(), "disintegration edge should be one-shot")
	_expect(float(state.get_death_disintegrate_progress()) > 0.0, "disintegration progress should start after 2/3")
	...
	state.update(0.29)
	_expect(not state.consume_death_finalize_ready(), "death should not finalize before 4.5s")
