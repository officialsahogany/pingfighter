...
func _verify_ak47_hold_burst_ammo_duration_and_slowdown() -> void:
	...
	var first_velocity: Vector2 = _get_vector2(first_projectile.get("velocity", Vector2.ZERO), Vector2.ZERO)
	_expect(is_equal_approx(first_velocity.length(), CommandoFirearmRuntime.AK47_BULLET_SPEED), "AK-47 bullet speed should match the tuned runtime value")
...
func _verify_bowling_trap_guard_handoff(release_result: Dictionary, runtime: Object, deps: Dictionary) -> void:
	...
	_expect(is_equal_approx(float(_get_dict(status_calls[0]).get("duration_frames", 0.0)), CommandoFirearmRuntime.BOWLING_TRAP_GUARD_STUN_FRAMES), "bowling-trap guard stun should keep the 2.5s duration")
