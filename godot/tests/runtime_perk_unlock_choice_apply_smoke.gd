...
func _verify_state_source_contract() -> void:
	...
	_expect(facade_body.find("_get_runtime_state_dict(runtime_state, \"runtime_skill_levels\")") >= 0, "unlock facade should own runtime-level lookup")
	_expect(facade_body.find("_get_runtime_state_object(runtime_state, \"_character_context\")") >= 0, "unlock facade should own character-context lookup")
	_expect(facade_body.find("_get_runtime_state_object(runtime_state, \"_unlock_swap_flow\")") >= 0, "unlock facade should own unlock-swap lookup")
	_expect(facade_body.find("_get_runtime_state_object(runtime_state, \"_level_side_effects\")") >= 0, "unlock facade should own level-side-effect lookup")
