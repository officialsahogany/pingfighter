...
func _verify_socket_entry_dest_rect_right() -> void:
	...
	if commands.size() == 1:
		# 한미량 v2 walk right f0 head_top = (75.0, 28.0); offset (0,-15), size
		# 20x20 at 1:1 scale -> rect position (65.0, 3.0).
		var dest: Rect2 = (commands[0] as Dictionary).get("dest_rect", Rect2())
		_expect(dest.position.is_equal_approx(Vector2(65.0, 3.0)) and dest.size.is_equal_approx(Vector2(20.0, 20.0)), "accessory dest rect should center on head_top + offset (got %s)" % dest)
...
func _verify_socket_entry_dest_rect_left_mirror() -> void:
	...
	if commands.size() == 1:
		# Mirrored head_top x = 160 - 75.0 = 85.0 -> rect position (75.0, 3.0).
		var dest: Rect2 = (commands[0] as Dictionary).get("dest_rect", Rect2())
		_expect(dest.position.is_equal_approx(Vector2(75.0, 3.0)), "left-direction accessory should ride the mirrored head_top socket (got %s)" % dest)
