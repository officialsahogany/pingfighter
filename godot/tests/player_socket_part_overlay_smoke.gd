...
func _verify_socket_entry_dest_rect_right() -> void:
	...
	if commands.size() == 1:
		# 소프트 아이보리 walk right f0 head_top = (76.6, 28.0); offset (0,-15), size
		# 20x20 at 1:1 scale -> rect position (66.6, 3.0).
		var dest: Rect2 = (commands[0] as Dictionary).get("dest_rect", Rect2())
		_expect(dest.position.is_equal_approx(Vector2(66.6, 3.0)) and dest.size.is_equal_approx(Vector2(20.0, 20.0)), "accessory dest rect should center on head_top + offset (got %s)" % dest)
...
func _verify_socket_entry_dest_rect_left_mirror() -> void:
	...
	if commands.size() == 1:
		# Mirrored head_top x = 160 - 76.6 = 83.4 -> rect position (73.4, 3.0).
		var dest: Rect2 = (commands[0] as Dictionary).get("dest_rect", Rect2())
		_expect(dest.position.is_equal_approx(Vector2(73.4, 3.0)), "left-direction accessory should ride the mirrored head_top socket (got %s)" % dest)
