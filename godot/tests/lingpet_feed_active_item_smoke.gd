...
func _verify_feed_catalog_tiers_and_icon() -> void:
	...
	_expect_float(float(apple_data.get("feed_amount", 0.0)), 30.0, "apple feed should retain its legacy 30-point catalog payload")
	_expect_float(float(basic_data.get("feed_amount", 0.0)), 40.0, "basic feed should retain its legacy 40-point catalog payload")
	_expect_float(float(melon_data.get("feed_amount", 0.0)), 50.0, "melon feed should retain its legacy 50-point catalog payload")
	_expect_float(float(special_data.get("feed_amount", 0.0)), 100.0, "special feed should retain its legacy 100-point catalog payload")
	_expect(str(apple_data.get("description", "")).find("15초") >= 0, "apple feed copy should advertise 15 seconds")
	_expect(str(basic_data.get("description", "")).find("20초") >= 0, "basic feed copy should advertise 20 seconds")
	_expect(str(melon_data.get("description", "")).find("25초") >= 0, "melon feed copy should advertise 25 seconds")
	_expect(str(special_data.get("description", "")).find("50초") >= 0, "special feed copy should advertise 50 seconds")
	...
	_expect(str(special_data.get("icon_path", "")).ends_with("lingpet_special_feed_icon.png"), "special feed should use its dedicated accepted icon")
...
func _verify_basic_feed_active_item_restores_satiety() -> void:
	...
	_expect_float(float(last_result.get("duration_recovery_seconds", 0.0)), 20.0, "completed basic feed should preserve its mapped recovery seconds")
	_expect_float(float(last_result.get("granted_duration_seconds", 0.0)), 20.0, "completed basic feed should restore 20 duration seconds")
	_expect(lingpet_runtime.get_duration_pool_current() > 38.0 and lingpet_runtime.get_duration_pool_current() <= 40.0, "basic feed should restore shared duration after normal bowl-walk drain")
...
func _verify_melon_feed_wakes_ko_through_shared_effect() -> void:
	...
	_advance_feed_until_complete(lingpet_runtime, owner, registry)
	var duration_after_feed := lingpet_runtime.get_duration_pool_current()
	_expect(duration_after_feed > 25.0 and duration_after_feed <= lingpet_runtime.get_duration_pool_max(), "melon feed should restore shared duration while stowed (got %.2f)" % duration_after_feed)
	_expect(lingpet_runtime.is_guardian_stowed(), "melon feed should not auto-resummon a force-stowed guardian")
	_expect(lingpet_runtime.can_resummon_guardian(), "melon feed should unlock explicit resummon after crossing 10 seconds")
	var last_result: Dictionary = lingpet_runtime.get_last_affinity_result_for_tests()
	_expect_float(float(last_result.get("duration_recovery_seconds", 0.0)), 25.0, "melon feed should map 50 legacy points to 25 seconds")
	_expect_float(float(last_result.get("granted_duration_seconds", 0.0)), 25.0, "melon feed should grant the full 25 recovery seconds")
...
func _verify_full_satiety_and_battle_cap_preserve_item() -> void:
	...
	_expect_str(str(lingpet_runtime.get_last_affinity_result_for_tests().get("blocked_reason", "")), "duration_full", "full-duration blocked feed should report duration_full")
...
func _verify_lingpet_feed_source_contracts() -> void:
	...
	_expect(catalog_source.find("\"feed_amount\": 30.0") >= 0, "apple feed catalog should retain the 30-point compatibility payload")
	_expect(catalog_source.find("\"feed_amount\": 40.0") >= 0, "basic feed catalog should retain the 40-point compatibility payload")
	_expect(catalog_source.find("\"feed_amount\": 50.0") >= 0, "melon feed catalog should retain the 50-point compatibility payload")
	_expect(catalog_source.find("\"feed_amount\": 100.0") >= 0, "special feed catalog should retain the 100-point compatibility payload")
	_expect(catalog_source.find("지속시간을 15초 회복") >= 0 and catalog_source.find("지속시간을 20초 회복") >= 0 and catalog_source.find("지속시간을 25초 회복") >= 0 and catalog_source.find("지속시간을 50초 회복") >= 0, "feed catalog copy should expose all four transitional duration recoveries")
