...
func _verify_acquired_perk_cache_reuses_catalog_rows() -> void:
	...
	_expect(_function_body(source, "func _draw_text_centered_xy(").find("var text_size: Vector2 = _get_centered_text_size(font, text, size)") >= 0, "centered text draw should read cached centered text sizes")
