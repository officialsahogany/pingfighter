...
func _run() -> void:
	...
	var perk_select_stream: AudioStream = _verify_stream(GameAudio.UI_PERK_SELECT_SOUND_PATH, 0.500, 0.560, "UI perk select")
	...
	audio.ui_perk_select_sfx = _make_player(host, "UiPerkSelectSfx", perk_select_stream)
	...
	_expect(sfx_players.has(audio.ui_perk_select_sfx), "UI perk select player should be registered for SFX cleanup/bus routing")
	...
	_expect(audio.ui_perk_select_sfx.bus == GameAudio.SFX_BUS_NAME, "UI perk select should route through the central SFX bus")
	...
	audio.play_runtime_perk_select()
	_expect(audio.ui_perk_select_sfx.pitch_scale >= 0.98 and audio.ui_perk_select_sfx.pitch_scale <= 1.03, "perk select should apply small pitch jitter")

	for player in [audio.ui_move_sfx, audio.ui_confirm_sfx, audio.ui_back_sfx, audio.ui_perk_select_sfx]:
		if player != null:
			player.stop()
			player.stream = null
	audio.ui_move_sfx = null
	audio.ui_confirm_sfx = null
	audio.ui_back_sfx = null
	audio.ui_perk_select_sfx = null
	host.queue_free()
