...
class FakeLingpetRuntime:
	...
	func try_begin_companion_interact_reaction(
		_registry: Object = null
	) -> bool:
		interact_count += 1
		return interact_accept
...
func _init() -> void:
	...
	_verify_interact_trigger_latch_and_rearm()
...
func _verify_companion_click_coordinates_and_priority() -> void:
	...
	_expect(runtime.interact_count == 0, "mouse click must not enter key/trigger interaction")
...
func _verify_interact_trigger_latch_and_rearm() -> void:
	GamepadInput.clear_primary_action_trigger_suppression_for_tests()
	var fixture := _build_fixture()
	var holder: ModuleHolder = fixture["holder"]
	var registry: FakeRegistry = fixture["registry"]
	...
	var owner := FakeOwner.new()
	var router := BattleLingpetInteractionInputRouter.new()
	_expect(router.handle_companion_input(_trigger_event(0.9), owner, registry, Callable(holder, "get_module")), "first RT pull must interact")
	_expect(not router.handle_companion_input(_trigger_event(0.95), owner, registry, Callable(holder, "get_module")), "latched RT must not repeat")
	_expect(not router.handle_companion_input(_trigger_event(0.1), owner, registry, Callable(holder, "get_module")), "RT release only rearms")
	_expect(router.handle_companion_input(_trigger_event(0.9), owner, registry, Callable(holder, "get_module")), "rearmed RT must interact again")
	_expect(runtime.interact_count == 2, "RT latch must produce exactly two interactions across two pulls")
	_expect(owner.redraw_count == 2, "accepted RT interactions must redraw")
	_expect(GamepadInput.is_primary_action_trigger_suppressed_for_tests(), "accepted RT must suppress primary action until release")
	_clear_fixture(fixture)
...
func _verify_source_ownership() -> void:
	...
	_expect(router_source.contains("_lingpet_interact_trigger_latched"), "lingpet router must own RT latch state")
	...
	_expect(router_source.contains("try_begin_companion_interact_reaction(registry)"), "lingpet router must own E/RT interaction handoff")
	...
	_expect(not input_source.contains("_lingpet_interact_trigger_latched"), "scene input controller must not retain RT latch state")
