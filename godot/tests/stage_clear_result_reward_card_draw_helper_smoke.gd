...
func _verify_helper_source() -> void:
	...
	_expect(source.find("card_draw_callback.call") >= 0, "reward section-stack drawing should support callback-based card body drawing")
	_expect(source.find("draw_reward_card(canvas") >= 0, "reward section-stack drawing should draw helper-owned card bodies without a callback")
	_expect(source.find("static func draw_reward_card") >= 0, "reward card draw helper should own complete reward card drawing")
