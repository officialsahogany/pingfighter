...
func _verify_weapon_snapshot_round_trip() -> void:
	...
	_expect(controller.start_current_weapon_reload(), "commando pistol reload timer should start before save")
	...
	_expect(bool(pistol.get("reloading", false)), "pistol reload state should survive save/load")
	_expect(int(pistol.get("ammo_current", 0)) == 2, "pistol current magazine should survive save/load")
	_expect(int(pistol.get("magazines_current", 0)) == 1, "Beretta spare magazines should survive save/load")
	_expect(is_equal_approx(float(pistol.get("reload_timer_frames", 0.0)), 90.0), "pistol reload timer should survive save/load")
