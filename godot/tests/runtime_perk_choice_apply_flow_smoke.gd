...
func _verify_source_contract() -> void:
	...
	_expect(facade_body.find("RuntimePerkRuntimeStateAccess.get_object(runtime_state, \"_choice_dispatch\")") >= 0, "apply-flow facade should own dispatch helper lookup")
	_expect(facade_body.find("RuntimePerkRuntimeStateAccess.get_dict(runtime_state, \"runtime_skill_levels\")") >= 0, "apply-flow facade should own runtime level lookup")
