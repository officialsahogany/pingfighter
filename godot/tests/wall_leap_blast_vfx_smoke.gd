...
const FUSE_CAPTURE_PATH := "res://.godot/codex_logs/wall_leap_rev9_fuse_windowed.png"
	...
	if image != null and not image.is_empty():
		...
		for y in range(image.get_height()):
			for x in range(image.get_width()):
				...
				if color.r > 0.45 and color.g > 0.12 and color.r > color.b * 1.35:
					ember_pixels += 1
				if color.r > 0.16 and color.g > 0.065 and color.r > color.g * 1.35 and color.b < 0.10:
					cord_pixels += 1
		_expect(ember_pixels > 60, "burning wick must expose a readable hot ember instead of a sci-fi convergence ring")
		_expect(cord_pixels > 35, "half-burn FUSE must retain a visible physical cord")
	viewport.queue_free()
	await process_frame
...
func _verify_rendered_layers(wall_state: Object) -> void:
	...
	if image != null and not image.is_empty():
		...
		var background := Color(0.02, 0.03, 0.06)
		for y in range(image.get_height()):
			for x in range(image.get_width()):
				var color := image.get_pixel(x, y)
	...
	_expect(int(host_status.get("layer_count", 0)) == 3, "rendered blast host must own the three authored layers")
...
func _verify_source_contract() -> void:
	...
	_expect(host_source.contains(SMOKE_PATH) and host_source.contains(SHARDS_PATH) and host_source.contains(RING_PATH), "dedicated host must own all three rev9 texture paths")
