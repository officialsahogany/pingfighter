...
func _verify_screen_delegates_snapshot_builder() -> void:
	...
	_expect(screen_source.find("StageClearResultStageSnapshotBuilder.new()") >= 0, "result screen should own a stage snapshot builder instance")
	_expect(screen_source.find(".build_progress_snapshot(") >= 0, "result screen should delegate progress snapshots")
	_expect(screen_source.find(".build_stage_reward_snapshot(") >= 0, "result screen should delegate stage reward snapshots")
	...
	_expect(builder_source.find("func build_stage_reward_snapshot") >= 0, "stage snapshot builder should own stage reward assembly")
