...
func _verify_removed_runtime_active_projectile_bridges() -> void:
	...
	var state_source: String = FileAccess.get_file_as_string("res://scripts/characters/commando_firearm_suicide_drone_state.gd")
	_expect(runtime_source.contains("CommandoFirearmProjectileMotionState.advance_runtime_projectiles"), "runtime should delegate projectile update orchestration to motion owner")
	_expect(motion_source.contains("CommandoFirearmSuicideDroneState.resolve_runtime_collision_at_index"), "projectile motion owner should delegate suicide-drone collision reason resolution to state owner")
	_expect(runtime_source.contains("CommandoFirearmSuicideDroneState.update_runtime_active_input"), "runtime should delegate active suicide-drone input orchestration to state owner")
	_expect(state_source.contains("detonate_runtime_projectile_at_index"), "suicide-drone state owner should keep detonation orchestration")
