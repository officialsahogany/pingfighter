...
# ⚠️ **draw_size 는 셀 크기이지 캐릭터 크기가 아니다.** 좌석 계약이 엉덩이선 아래
# 공간을 고정하므로, 다리가 늘어진 포즈는 셀을 키워도 캐릭터가 그만큼 커지지 않는다
# (R1 실측: 셀 94 → 실제 라이더 56px, 셀 160 → 95px). 승인 기준은 draw_size 가 아니라
# **실제 라이더 높이 90~98px**(M 210 대비 ≈0.46)다.
#
# 스매셔 = **160** 확정(프로토타입). 나머지 4종은 각 후보 bbox 를 보고 같은 실제 높이
# 목표로 보정한다 — **160 을 무조건 복사하지 않는다**(아래 94 는 미보정 자리표시자).
#
# 저작 규격: 캐릭터 엉덩이선을 셀 하단에서 위로
#   운영     9.43359375px                      ( = 23 × 210/512 )
#   512 저작 9.43359375 × 512/draw_size        ( 스매셔 160 → 30.1875px, 좌석선 y 481.8125 )
# 에 둔다. ⚠️ 소스 오프셋은 draw_size 마다 달라진다 — 다른 값을 그대로 쓰면 어긋난다.
# 셀 하단 = M 안장 어셈블리 하단(금 프레임 밑단) = 라이더 rect 하단 = 화면 y 762.
#
# 이 비율은 **탑승 전용 presentation-scale 예외**이며 일반 캐릭터 크기 정본으로
# 전파 금지.
#
# 이 오프셋은 런타임 키가 아니라 시트 저작 값이며 매니페스트가 정본이다
# (assets/sprites/lingpet/baekrin_companion_mount_base_topdown_v1_manifest.json).
const RIDERS := {
	"smasher": {
		# 프로토타입 확정값(2026-08-14). 셀 160 에서 캐릭터가 90~98px 로 읽힌다.
		"path": "",
		"cols": 1,
		"rows": 1,
		"frame_count": 1,
