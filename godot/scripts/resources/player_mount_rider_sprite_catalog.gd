...
# 착석 draw_size = **94** (M(백린 빈안장) 운영 draw_size = **210** 기준, 2026-08-14 확정).
# 라이더/M 비율은 0.45 이고 이는 **탑승 전용 presentation-scale 예외**다 — 원화 비율
# 0.26 을 지키면 라이더가 55px 이 되어 플레이어 5종 가독성을 잃는다. 이 비율을 일반
# 캐릭터 아트의 크기 정본으로 전파하지 말 것.
#
# 저작 규격: 캐릭터 엉덩이선을 셀 하단이 아니라 **셀 하단에서 위로**
#   운영     9.43359375px   ( = 23 × 210/512 )
#   512 저작 51.38298px     ( = 9.43359375 × 512/94 )
# 에 둔다. ⚠️ N 원본이 512px 이면 **51.38298** 로 저작해야 한다 — 512 원본에 9.43 을
# 쓰면 5배 넘게 어긋난다. 셀 하단 = M 안장 어셈블리 하단(금 프레임 밑단) = 라이더
# rect 하단 = 화면 y 762.
#
