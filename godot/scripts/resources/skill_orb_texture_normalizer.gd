...
const HUD_CROP_ZOOM := 1.22
const HUD_INNER_FILL := 0.95
const CROP_OUTER_FRAME_SKILL_IDS := {
	"smasher_wheel": true,
}
...
static func normalize(skill_id: String, texture: Texture2D) -> Texture2D:
	if texture == null or not CROP_OUTER_FRAME_SKILL_IDS.has(skill_id):
		return texture
	...
	var normalized := _create_cropped_orb_texture(texture)
	if normalized == null:
		return texture
...
static func _create_cropped_orb_texture(texture: Texture2D) -> Texture2D:
	var image: Image = texture.get_image()
	if image == null or image.is_empty():
		return null

	image.convert(Image.FORMAT_RGBA8)
	var zoom_size: int = max(NORMALIZED_SIZE + 2, int(round(float(NORMALIZED_SIZE) * HUD_CROP_ZOOM)))
	...
	var inner_size: int = clampi(int(round(float(NORMALIZED_SIZE) * HUD_INNER_FILL)), 1, NORMALIZED_SIZE)
