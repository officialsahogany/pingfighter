...
const FIRE_SUPPORT_TEXTURE_LAYER_COUNT := 8
const FIRE_SUPPORT_SHOCKWAVE_RINGS := 4
const FIRE_SUPPORT_SMOKE_PUFFS := 7
const FIRE_SUPPORT_SPARKS := 10
...
static func _draw_fire_support_airstrike_layers(
	...
	var bloom_alpha: float = 0.28 * life + 0.22 * (1.0 - blast_progress)
	ImpactFlareTextureCache.draw_glow(canvas, center, radius * (1.08 + blast_progress * 0.35), color, bloom_alpha)
	ImpactFlareTextureCache.draw_burst(canvas, center, radius * (0.64 + blast_progress * 0.30), Color(1.0, 0.92, 0.52), 0.54 * max(life, 1.0 - blast_progress))
	...
	for ring_index in range(FIRE_SUPPORT_SHOCKWAVE_RINGS):
		...
		if ring_progress <= 0.0:
			continue
		var ring_radius: float = radius * (0.62 + ring_progress * (1.18 + float(ring_index) * 0.20))
		var ring_alpha: float = (0.30 - float(ring_index) * 0.045) * life * (1.0 - ring_progress * 0.35)
...
static func _draw_fire_support_smoke_column(
	...
	for index in range(FIRE_SUPPORT_SMOKE_PUFFS):
		...
		var smoke_radius: float = radius * (0.24 + ratio * 0.22) + elapsed * (1.2 + ratio * 0.7)
		var tone: float = 0.18 + ratio * 0.07
		var alpha: float = (0.16 + ratio * 0.07) * life
		canvas.draw_circle(smoke_pos, smoke_radius, Color(tone, tone * 0.92, tone * 0.78, alpha))
		canvas.draw_circle(smoke_pos + Vector2(-smoke_radius * 0.22, -smoke_radius * 0.18), smoke_radius * 0.42, Color(0.34, 0.30, 0.24, alpha * 0.42))
...
static func _draw_fire_support_foreground_sparks(
	...
	var secondary: Color = _get_color(zone.get("secondary", Color(1.0, 0.82, 0.25)), Color(1.0, 0.82, 0.25))
	var spark_window: float = clamp(1.0 - elapsed / 14.0, 0.0, 1.0)
	if spark_window <= 0.0:
		return
	for index in range(FIRE_SUPPORT_SPARKS):
		var angle: float = float(index) * TAU / float(FIRE_SUPPORT_SPARKS) + sin(float(index) * 1.93) * 0.20
		var dir := Vector2(cos(angle), sin(angle))
		var length: float = radius * (0.54 + float(index % 4) * 0.12) + elapsed * (5.2 + float(index % 3) * 1.4)
		var start: Vector2 = center + dir * radius * 0.16
		var end: Vector2 = center + dir * length
		var alpha: float = (0.74 - float(index % 3) * 0.08) * spark_window * life
		canvas.draw_line(start, end, Color(1.0, 0.92, 0.50, alpha), 2.4, true)
		if index % 5 == 0:
			ImpactFlareTextureCache.draw_sparkle(canvas, end, radius * 0.08, secondary, alpha * 0.44)
