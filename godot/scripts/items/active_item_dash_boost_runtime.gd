...
const DASH_BOOST_DURATION_FRAMES := 480.0
const DASH_BOOST_GLOW_ADVANCE_PER_FRAME := 0.15
const DASH_BOOST_COST_DISCOUNT := 1.00
const DASH_BOOST_COOLDOWN_REDUCTION := 0.99


func start_state(duration_multiplier: float = 1.0) -> Dictionary:
	var duration_frames: float = _scale_duration_frames(DASH_BOOST_DURATION_FRAMES, duration_multiplier)
	return {
		"active": true,
...
func update_state(
	...
	return {
		...
		"glow_phase": glow_phase + DASH_BOOST_GLOW_ADVANCE_PER_FRAME * fps_scale,
		"particle_accumulator_frames": particle_accumulator_frames,
		"clear_particles": false,
...
func apply_update(
	...
	if bool(state.get("advance_idle_particles", false)):
		target.set("dash_boost_particle_accumulator_frames", particles_helper.advance_idle_particles(
			particles,
...
func get_cost_multiplier(active: bool) -> float:
	if active:
		return 1.0 - DASH_BOOST_COST_DISCOUNT
	return 1.0


func get_cooldown_multiplier(active: bool) -> float:
	if active:
		return 1.0 - DASH_BOOST_COOLDOWN_REDUCTION
	return 1.0
