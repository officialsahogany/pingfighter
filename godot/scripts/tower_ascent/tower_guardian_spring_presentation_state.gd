...
const RITUAL_DURATION_SEC := 2.0
const RITUAL_ICON_RISE_END_SEC := 1.12
const RITUAL_ACQUISITION_START_SEC := 1.18
...
var _pending_palm_action: Dictionary = {}
var _palm_dispatch_ready := false
var _palm_dispatched := false
var _acquisition_started := false
var _acquisition_start_count := 0
var _palm_dispatch_count := 0
...
func close_scene(count_forced_cleanup: bool = true) -> void:
	if count_forced_cleanup and _phase == PHASE_RITUAL:
	...
	_pending_palm_action.clear()
	_palm_dispatch_ready = false
	_palm_dispatched = false
...
func is_ritual_active() -> bool:
	return _enabled and _phase == PHASE_RITUAL
...
func begin_palm_ritual(action: Dictionary) -> bool:
	if (
		not _enabled
		or _phase != PHASE_MENU
		or str(_action_payload(action).get("operation", "")) != "palm"
		or not bool(action.get("enabled", true))
	):
		return false
	_phase = PHASE_RITUAL
	_ritual_elapsed_sec = 0.0
	_pending_palm_action = action.duplicate(true)
	_palm_dispatch_ready = false
	_palm_dispatched = false
	_acquisition_started = false
	_statue_hovered = false
...
func advance(delta: float) -> bool:
	...
	_elapsed_sec += step
	if _phase != PHASE_RITUAL:
		return step > 0.0
	var previous_elapsed := _ritual_elapsed_sec
	_ritual_elapsed_sec = minf(RITUAL_DURATION_SEC, _ritual_elapsed_sec + step)
	if (
		not _acquisition_started
		and previous_elapsed < RITUAL_ACQUISITION_START_SEC
		and _ritual_elapsed_sec >= RITUAL_ACQUISITION_START_SEC
	):
		_acquisition_started = true
		_acquisition_start_count += 1
	if _ritual_elapsed_sec >= RITUAL_DURATION_SEC:
		if not _acquisition_started:
			_acquisition_started = true
			_acquisition_start_count += 1
		_palm_dispatch_ready = true
	return step > 0.0


func take_completed_palm_action() -> Dictionary:
	if not _palm_dispatch_ready or _palm_dispatched or _pending_palm_action.is_empty():
		return {}
	_palm_dispatched = true
	_palm_dispatch_count += 1
	_palm_dispatch_ready = false
	_phase = PHASE_MENU
	_ritual_elapsed_sec = 0.0
	var result := _pending_palm_action.duplicate(true)
	_pending_palm_action.clear()
	return result
...
func get_action_rects(actions: Array, view_size: Vector2 = BASE_VIEW_SIZE) -> Array[Rect2]:
	...
	if not _enabled or _phase in [PHASE_CLOSED, PHASE_STATUE, PHASE_RITUAL]:
		return result
...
func build_visual_model(actions: Array, view_size: Vector2 = BASE_VIEW_SIZE) -> Dictionary:
	...
	var ritual_progress := clampf(_ritual_elapsed_sec / RITUAL_DURATION_SEC, 0.0, 1.0)
	var icon_rise_progress := clampf(
		_ritual_elapsed_sec / RITUAL_ICON_RISE_END_SEC,
		0.0,
		1.0
	)
	var acquisition_progress := clampf(
		(_ritual_elapsed_sec - RITUAL_ACQUISITION_START_SEC)
		/ maxf(0.001, RITUAL_DURATION_SEC - RITUAL_ACQUISITION_START_SEC),
		0.0,
		1.0
	)
	return {
		...
		"ritual_duration_sec": RITUAL_DURATION_SEC,
		...
		"input_policy": "discard_all_no_skip",
	}
...
func get_debug_state() -> Dictionary:
	return {
		...
		"palm_dispatch_count": _palm_dispatch_count,
