...
func _build_process_node_summary() -> String:
	...
	_collect_process_nodes(root, result)
	return (
...
func _collect_process_nodes(node: Node, result: Dictionary) -> void:
	...
	if _is_stale_loading_host(node):
		...
		if ignored_labels.size() < GAP_TRACE_NODE_LIMIT:
	...
	if _script_defines_callback(node, "_process"):
		_record_callback_node(result, "process", node)
	if _script_defines_callback(node, "_physics_process"):
		_record_callback_node(result, "physics", node)
	if node.is_processing():
		_record_process_node(result, "process", node)
	if node.is_physics_processing():
		_record_process_node(result, "physics", node)
	for child in node.get_children():
		if child is Node:
			_collect_process_nodes(child as Node, result)
...
func _record_callback_node(result: Dictionary, kind: String, node: Node) -> void:
	...
	if active:
		...
		if inactive_labels.size() < GAP_TRACE_NODE_LIMIT:
	...
	if outside_labels.size() < GAP_TRACE_NODE_LIMIT:
		outside_labels.append(_format_node_label(node))
	result[outside_labels_key] = outside_labels


func _record_process_node(result: Dictionary, kind: String, node: Node) -> void:
	...
	if labels.size() < GAP_TRACE_NODE_LIMIT:
