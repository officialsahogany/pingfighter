...
static func draw_scroll_buttons(
	...
	var plaza_rect: Rect2 = button_layout.get("plaza_rect", Rect2())
	var exit_rect: Rect2 = button_layout.get("exit_rect", Rect2())
	var clickable: bool = scroll_phase == "visible"

	var next_hovered: bool = clickable and hovered_button == StageClearResultInteractionState.BUTTON_NEXT_STAGE
	...
	var plaza_hovered: bool = clickable and hovered_button == StageClearResultInteractionState.BUTTON_PLAZA
	...
	StageClearResultShapeHelper.draw_panel(canvas, plaza_rect, plaza_fill, plaza_border, max(1.5, 2.2 * scale), 14.0 * scale)
	StageClearResultTextLayoutHelper.draw_centered_text(
		canvas,
		font,
		plaza_label,
	...
	var exit_hovered: bool = clickable and hovered_button == StageClearResultInteractionState.BUTTON_EXIT
