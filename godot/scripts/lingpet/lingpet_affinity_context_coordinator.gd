...
func configure(
	...
	if context_loadout.is_empty() and loadout_state != null:
		context_loadout = loadout_state.get_loadout(normalized_pet_id)
	...
	var reward_seed := _get_or_create_reward_seed(normalized_pet_id)
...
func _get_or_create_reward_seed(pet_id: String) -> int:
