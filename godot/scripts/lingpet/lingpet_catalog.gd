...
const PETS := {
	...
	"milkring": {
		...
		"active_skill_pool": [
			{
				...
				"name": "우유생산",
				"description": "밀쿠가 3초 동안 우유병을 제조합니다. 우유병은 스킬 레벨에 따라 플레이어 패들과 이미지 크기를 12~20% 키우며, Lv.3부터는 생산마다 30% 확률로 우유병 대신 치즈가 나옵니다. 치즈는 크기 증가에 더해 게이지구슬도 함께 회복합니다.",
				"cooldown": 50.0,
		...
		"effect_text": "공을 직접 받아치면 게이지 +40 / 우유생산은 50/47/44/41/37초마다 우유병(크기 증가)을 제조하고 Lv.3부터는 30% 확률로 치즈(크기 증가+게이지 회복)가 대신 나옵니다. 우유발사는 30/27.5/25/22.5/20초마다 보스를 향해 우유 투사체를 발사하며, Lv.3부터는 30% 확률로 메가 우유 난사가 나갑니다.",
	...
	"volty": {
		...
		"effect_text": "공을 직접 받아치면 게이지 +40 / 폭탄 서프라이즈는 60~42초마다 공에 시한폭탄을 붙여 상대 쪽 폭발 시 강한 스턴+넉백, 플레이어 쪽 폭발 시 약한 스턴+넉백을 일으킵니다. 레벨에 따라 상대 폭발은 강해지고 자폭 피해는 줄며, Lv.2부터는 자폭 직전 일정 확률로 폭탄이 상대에게 날아가 대신 폭발하고 레벨이 오를수록 그 확률이 높아집니다.",
