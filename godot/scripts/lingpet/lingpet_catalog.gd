...
const PETS := {
	...
	"volty": {
		...
		"effect_text": "공을 직접 받아치면 게이지 +40 / 폭탄 서프라이즈는 60~42초마다 공에 시한폭탄을 붙여 상대 쪽 폭발 시 강한 스턴+넉백, 플레이어 쪽 폭발 시 약한 스턴+넉백을 일으킵니다. 레벨에 따라 상대 폭발은 강해지고 자폭 피해는 줄며, Lv.2부터는 자폭 직전 일정 확률로 폭탄이 상대에게 날아가 대신 폭발합니다.",
