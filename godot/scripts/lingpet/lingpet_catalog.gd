...
const PETS := {
	...
	"nekuring": {
		...
		"active_skill_pool": [
			...
			},
			{
				...
				"build_time": 3.0,
				"barrier_width": 120.0,
				"barrier_width_by_level": [120.0, 140.0, 160.0, 180.0, 200.0],
				"barrier_height": 12.0,
	...
	"orosha": {
		...
		"active_skill": [],
		"effect_text": "오로샤는 F7 디버그용 별자리 고리뱀 링펫입니다. 외주 투명 원화와 후면 소스를 보존해 획득/클릭/동행 정적 시트로 연결했으며, 전용 액티브 스킬과 부화 풀 편입은 아직 승인 전 단계입니다.",
		...
		"note": "Orosha is debug-only until dedicated constellation/serpent active-skill runtime, audio, and final hatch-pool QA are accepted. Orosha is a grounded rolling-hoop pet: companion movement now uses an AutoSprite-generated complete circular hoop redraw as a distance-based full runtime roll with a short visual deceleration coast on stop, avoiding the lumpy full rotation from the non-circular rear-view source art; the native left/right 8x6 / 48-frame AutoSprite ground-roll sheets remain as fallback assets. Acquisition uses the rigid v2 AutoSprite Live2D sheet that avoids face-only rotation plus a separate AutoSprite VFX layer, and click/dismiss/panel playback uses the rolling-hoop AutoSprite sheet. Keep it out of the random hatch pool until the active-skill runtime is accepted.",
	},
}
