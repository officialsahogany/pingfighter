...
const BELL_DECORATIVE_SIZE_RATIO := 28.0 / 57.5
# The decorative bell's alpha centroid sits on the visible orb's top boundary.
# The textured v3 dial expands token inner radius by 55/50, so -1.10 maps
# that inner radius back to exactly one full visible-content radius.
const BELL_DECORATIVE_Y_OFFSET_RATIO := -1.10
const BELL_VISUAL_CENTER_Y_RATIO := -0.067
const BELL_DECORATIVE_MODULATE := Color(1.0, 1.0, 1.0, 0.98)
...
# The one decorative bell stays outside the compact liquid cache. This keeps
# texture identity out of the cache key and prevents a stale baked ornament
# after an early-boot texture fallback.
func draw_decorative_bell_overlay(
	...
	canvas.draw_texture_rect(
		texture,
		_get_decorative_bell_rect(center, inner_radius),
		false,
		BELL_DECORATIVE_MODULATE
	)


func build_decorative_bell_draw_spec(
	center: Vector2,
	inner_radius: float,
	context: Dictionary
) -> Dictionary:
	...
	return {
		...
		"rect": _get_decorative_bell_rect(center, inner_radius),
		"modulate": BELL_DECORATIVE_MODULATE,
	}


func _get_decorative_bell_rect(center: Vector2, inner_radius: float) -> Rect2:
	var bell_size: float = max(10.0, inner_radius * BELL_DECORATIVE_SIZE_RATIO)
	var bell_center := center + Vector2(
		0.0,
		inner_radius * BELL_DECORATIVE_Y_OFFSET_RATIO + bell_size * BELL_VISUAL_CENTER_Y_RATIO
	)
	return Rect2(bell_center - Vector2.ONE * bell_size * 0.5, Vector2.ONE * bell_size)
