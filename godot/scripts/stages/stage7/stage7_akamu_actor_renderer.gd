...
# Stage 7 Akamu Rigo actor-pass orchestrator.
# Shared player and Commando renderers remain common owners; Stage 7 owns only
# its playfield effects and promoted boss sprite runtime.
...
var playfield_renderer: Object = Stage7AkamuPlayfieldRenderer.new()
var player_renderer: Object = Stage1PlayerActorRenderer.new()
var boss_renderer: Object = Stage7AkamuBossActorRenderer.new()
...
func _init() -> void:
	# 그림자분신은 보스 시트의 현재 프레임을 복제한다. 시트 소유자는 boss
	# child 하나뿐이므로 플레이필드 렌더러에 프레임 소스로 물려준다.
...
func prewarm_assets_step() -> bool:
	# The boot controller owns this parent as one Stage 7 step. Delegate the
	# repeated calls to the boss child so each call loads at most one of its
	# per-pose 1024x512 sheets instead of synchronously decoding the full set,
	# then bake the playfield VFX blit textures one per call. Hot-path lazy
	# bake is forbidden — draw() falls back to vector primitives until then.
