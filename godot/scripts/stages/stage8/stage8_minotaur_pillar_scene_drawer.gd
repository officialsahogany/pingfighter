...
# Stage 8 Minotaur pillar-scene orchestrator.
# Reuses the common pillar HUD and appends the Stage 8 boss-skill rail after
# the playfield, matching the Stage 7 module boundary.
...
const STAGE8_STATIC_HUD_LOD_SCALE := BattleRenderQuality.FPS_CAP_EFFECT_SCALE
...
func prewarm_assets_step(module_getter: Callable, selected_character_type: String = "smasher") -> bool:
	...
	match _prewarm_step_index:
		0:
			var stage_background: Object = _get_module(module_getter, "stage8_minotaur_pillar_background")
		...
		2:
			var skill_hud: Object = _get_module(module_getter, "stage8_minotaur_boss_skill_hud_renderer")
...
func draw(canvas: CanvasItem, context: Dictionary, registry: Object, states: Dictionary) -> void:
	...
	_perf_end(perf_logger, "stage8.pillar.background", sample_start)

	sample_start = _perf_begin(perf_logger)
	hud_scene_drawer.draw(
		canvas,
		_with_stage8_hud_lod_context(context, quality_scale),
		registry,
		states,
		view_size,
		game_offset,
	...
	_perf_end(perf_logger, "stage8.pillar.hud_scene", sample_start)
	_perf_end(perf_logger, "stage8.pillar.total", total_start)
...
func draw_pillar_hud_overlay(canvas: CanvasItem, context: Dictionary, registry: Object, states: Dictionary) -> void:
	...
	hud_scene_drawer.draw(
		canvas,
		_with_stage8_hud_lod_context(context, _get_pillar_quality_scale(context)),
		registry,
		states,
		view_size,
		game_offset,
	...
	_perf_end(perf_logger, "stage8.pillar.hud_overlay_scene", sample_start)
...
func draw_pillar_background_overlay(canvas: CanvasItem, context: Dictionary, _registry: Object, states: Dictionary) -> void:
	...
	_perf_end(perf_logger, "stage8.pillar.background_overlay", sample_start)
...
func draw_post_playfield_hud(canvas: CanvasItem, context: Dictionary, registry: Object) -> void:
	...
	_perf_end(perf_logger, "stage8.pillar.minotaur_boss_hud", sample_start)
...
func _draw_stage8_minotaur_boss_skill_hud(
	canvas: CanvasItem,
	context: Dictionary,
	registry: Object,
	view_size: Vector2,
	...
	var renderer := _get_stage8_boss_skill_renderer(registry)
	if renderer == null or not renderer.has_method("draw"):
		return
	var state: Object = registry.get_instance("stage8_minotaur_state")
	...
	LingpetRailCard.append_entry(
		hud_context, registry, "stage8_boss_skill_hud_skills", "stage8_boss_skill_hud_active"
	)
	renderer.draw(canvas, hud_context)


func _get_stage8_boss_skill_renderer(registry: Object) -> Object:
	var renderer_key := "stage8_minotaur_boss_skill_hud_renderer"
	var router: Object = registry.get_instance("stage_runtime_router")
	if router != null and router.has_method("get_module_key"):
		var routed_key := str(router.get_module_key(8, "boss_skill_hud_renderer"))
		if routed_key != "":
...
func _with_stage8_hud_lod_context(context: Dictionary, quality_scale: float) -> Dictionary:
	if quality_scale > STAGE8_STATIC_HUD_LOD_SCALE and not BattleRenderQuality.is_high_refresh_lod_active():
		return context
	var hud_context := context.duplicate()
	hud_context["pillar_hud_static_lod"] = true
	hud_context["stage8_pillar_hud_static_lod"] = true
	return hud_context
