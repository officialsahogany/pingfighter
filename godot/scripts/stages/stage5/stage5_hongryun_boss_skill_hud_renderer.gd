...
func _draw_skillcard_gauge(canvas: CanvasItem, rect: Rect2, skill_id: String, fill_ratio: float, fallback_color: Color) -> void:
	...
	if texture == null or source_rect.size.x <= 0.0 or source_rect.size.y <= 0.0:
		...
		return
	canvas.draw_texture_rect_region(texture, rect, source_rect, Color(0.22, 0.17, 0.15, 1.0), false, true)
	if clamped_fill <= 0.0:
		return
...
func _sort_entries(a: Dictionary, b: Dictionary) -> bool:
	...
	var progress_a: float = clampf(float(a.get("progress", 0.0)), 0.0, 1.0)
	var progress_b: float = clampf(float(b.get("progress", 0.0)), 0.0, 1.0)
...
func _entry_priority(skill: Dictionary) -> int:
	var status: String = str(skill.get("status", "charging"))
	if status == "casting" or status == "inferno_charge":
		return 0
	if bool(skill.get("ready", false)) or status == "ready":
		return 1
	if status == "locked":
