...
func serve_ball(config: Dictionary, deps: Dictionary, callbacks: Dictionary) -> void:
	...
	_notify_mythic_round_start(deps)
...
func _notify_mythic_round_start(deps: Dictionary) -> void:
	var mythic_item_runtime: Object = deps.get("mythic_item_runtime", null)
	if mythic_item_runtime == null or not mythic_item_runtime.has_method("on_round_start"):
		return
	mythic_item_runtime.on_round_start(deps.get("owner", null), deps.get("registry", null))
