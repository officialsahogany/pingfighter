...
var _layout := PerkFusionModalLayout.new()
...
func resolve(event: InputEvent, snapshot: Dictionary, view_size: Vector2) -> Dictionary:
			...
			if trigger_motion.axis == JOY_AXIS_TRIGGER_RIGHT:
				...
				if trigger_motion.axis_value >= GamepadInput.TRIGGER_DEADZONE:
					if _rt_confirm_latched:
						return {"consumed": true}
					_rt_confirm_latched = true
					return {"consumed": true, "confirm": true}
	...
	if event is InputEventMouseMotion:
		...
		var hovered_index := _layout.get_candidate_index_at(snapshot, motion.position, view_size)
		return {
			"consumed": true,
	...
	if event is InputEventMouseButton:
		...
		if mouse.button_index != MOUSE_BUTTON_LEFT or not mouse.pressed:
			return {"consumed": true}
		var candidate_index := _layout.get_candidate_index_at(snapshot, mouse.position, view_size)
		if candidate_index >= 0:
			return {
				"consumed": true,
				"select_index": candidate_index,
			}
		match _layout.get_action_at(snapshot, mouse.position, view_size):
			"confirm":
				return {"consumed": true, "confirm": true}
			"back":
				return {"consumed": true, "cancel": true}
