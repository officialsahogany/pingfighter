...
func _is_player_skill_locked() -> bool:
	...
	if (
		...
		return true
	if (
		_mythic_item_runtime.has_method("is_yachaman_skills_locked")
		and bool(_mythic_item_runtime.is_yachaman_skills_locked())
	):
		return true
	if (
		_mythic_item_runtime.has_method("is_yachaman_control_locked")
		and bool(_mythic_item_runtime.is_yachaman_control_locked())
	):
		return true
	return false
