...
func _build_pending_drops(deps: Dictionary) -> Array[Dictionary]:
	...
	var payload_count: int = _get_payload_count(deps)
	for payload_index in range(payload_count):
		var drop: Dictionary = _build_pending_drop(deps, reserved_weapons, payload_index)
...
func _build_pending_drop_delays(deps: Dictionary, payload_count: int, timing_pattern: String) -> Array[float]:
	...
	if configured_value is Array and not (configured_value as Array).is_empty():
		return _normalize_configured_drop_delays(configured_value as Array, payload_count)
	return _build_python_drop_delay_schedule(timing_pattern, payload_count)
...
func _build_python_drop_delay_schedule(timing_pattern: String, payload_count: int) -> Array[float]:
	var result: Array[float] = []
	var current_pattern: String = _normalize_drop_timing_pattern(timing_pattern)
...
func _get_drop_timing_pattern(deps: Dictionary) -> String:
	...
	if configured_pattern != "":
		return _normalize_drop_timing_pattern(configured_pattern)
...
func _build_pending_drop(deps: Dictionary, reserved_weapons: Dictionary = {}, payload_index: int = 0) -> Dictionary:
	var forced_drop: Dictionary = _get_forced_pending_drop(deps, payload_index)
	if not forced_drop.is_empty():
		_reserve_drop_weapon(forced_drop, reserved_weapons)
		return forced_drop

	var candidates: Array[Dictionary] = _build_weighted_drop_candidates(deps, reserved_weapons)
...
func _get_field_item_drop_candidates(deps: Dictionary) -> Array[Dictionary]:
	...
	for candidate_value in raw_candidates:
		if candidate_value is Dictionary:
			...
			if item_id == "":
				continue
			if not _is_field_item_candidate_available(item_id, deps):
				continue
			result.append({
				"item_id": item_id,
			...
			if item_name != "":
				if not _is_field_item_candidate_available(item_name, deps):
					continue
				result.append({
					"item_id": item_name,
					"weight": 1.0,
...
func _has_any_permanent_weapon(deps: Dictionary) -> bool:
	...
	return not _get_permanent_owned_from_deps(deps).is_empty()
