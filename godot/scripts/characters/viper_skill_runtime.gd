...
var core_flip_paddle_size := Vector2(155.0, 50.0)
...
func reset_round(deps: Dictionary = {}) -> void:
	...
	core_flip_left_press_frame = -999999
	core_flip_right_press_frame = -999999
...
func try_activate_before_movement(
	...
	_record_viper_command_inputs(input_snapshot, now_msec, _get_viper_skill_config(deps), deps)
	...
	if dual_glitch_state == "startup":
		...
		if dual_glitch_locked_player_x_valid:
			...
			dual_startup_pos.x = clamp(dual_glitch_locked_player_x, play_left, max(play_left, play_right - _get_paddle_size(config).x))
	...
	var core_flip_result: Dictionary = {}
	if (
		_is_core_flip_ready_window_active(now_msec)
		and _has_core_flip_input_buffer(now_msec, input_snapshot)
		and _can_start_core_flip(special_gauge, config, deps, now_msec)
	):
		core_flip_result = _start_core_flip(player_pos, special_gauge, config, deps, now_msec)
	if not core_flip_result.is_empty():
		return core_flip_result
	if _check_dual_glitch_command(now_msec) and _can_start_dual_glitch(special_gauge, config, deps, now_msec):
		return _start_dual_glitch(player_pos, special_gauge, config, deps, now_msec)
	if up_edge:
		var blade_combo_result: Dictionary = _try_start_blade_combo_from_motion(player_pos, special_gauge, config, deps, now_msec)
	...
	if chaos_state == "startup":
		...
		if chaos_locked_player_x_valid:
			...
			chaos_startup_pos.x = clamp(chaos_locked_player_x, chaos_play_left, max(chaos_play_left, chaos_play_right - _get_paddle_size(config).x))
		return {
			"handled": true,
			"activated": false,
	...
	if _check_chaos_command(now_msec) and _can_start_chaos_spear(special_gauge, config, deps, now_msec):
	...
	if up_edge and _can_start_air_blade(special_gauge, config, deps):
		return _start_blade_motion(player_pos, special_gauge, config, deps, false, now_msec)
	if not pressed_edge or not _can_shadow_step(special_gauge, input_snapshot, deps):
...
func update_effects(fps_scale: float, _current_msec: int, context: Dictionary, deps: Dictionary) -> Dictionary:
	_update_shadow_step_visuals(fps_scale, context, deps)
	_update_blade_windows(fps_scale, context, deps)
	...
	if chaos_state != "idle" and (_should_stop_viper_context_effect(context) or _is_round_waiting_for_serve(deps)):
		_reset_chaos_spear_runtime(true, deps)
		return {}
	if chaos_state == "idle":
		return {}
...
func apply_blade_rush_ball_motion(fps_scale: float, scene: Dictionary, context: Dictionary, deps: Dictionary) -> Dictionary:
	var result: Dictionary = {}
	if not blade_projectile_active and blade_followup_projectiles.is_empty():
		return result
	var motion_context: Dictionary = context.duplicate()
	motion_context.merge(scene, true)
...
func apply_emp_strike_ball_motion(_fps_scale: float, scene: Dictionary, context: Dictionary, deps: Dictionary) -> Dictionary:
	...
	if dive_active and dive_phase == 2 and not dive_ball_boosted:
		...
		var vertical_gap: float = dive_shockwave_pos.y - ball_pos.y
		if vertical_gap >= 0.0 and vertical_gap <= DIVE_SHOCKWAVE_HEIGHT:
			return _apply_dive_strike_impact(scene, context, deps)
	return _apply_dual_glitch_clone_dive_ball_motion(scene, context, deps)
...
func register_player_ball_contact(deps: Dictionary = {}, _context: Dictionary = {}) -> void:
	...
	if not bool(dash_snapshot.get("is_half", false)) and not core_flip_consumed and core_flip_last_dash_start_msec > -99999:
		...
		if (
			...
			and (dash_active or now_msec - core_flip_last_dash_start_msec <= CORE_FLIP_DASH_SUCCESS_WINDOW_MSEC)
		):
...
func _reset_core_flip_runtime(clear_window: bool = false) -> void:
	...
	core_flip_paddle_size = Vector2(155.0, 50.0)
...
func _start_core_flip(
	...
	if skill_state != null and skill_state.has_method("trigger_configured_cooldown"):
		skill_state.trigger_configured_cooldown(CORE_FLIP, now_msec, skill_config)
...
func _update_core_flip(
	...
	var result := {
		...
		"activated": false,
		"skill_name": CORE_FLIP,
		"player_pos": core_flip_visual_pos,
		"player_speed": 0.0,
		"special_gauge": special_gauge,
		"player_collision_cooldown": max(6.0, float(config.get("player_collision_cooldown", 0.0))),
	}
	match core_flip_attack_phase:
		...
		2:
			...
			if not core_flip_ball_hit and kick_center.distance_to(_get_ball_pos(config)) <= CORE_FLIP_HIT_RADIUS:
				...
				if _is_skill_equipped(_get_viper_skill_config(deps), DARK_BLADE):
		...
		3:
			...
			if t3 >= 1.0:
				next_pos = _center_to_player_pos(core_flip_origin_center, config)
				if dark_blade_window and _is_skill_equipped(_get_viper_skill_config(deps), DARK_BLADE):
...
func _apply_core_flip_hit(config: Dictionary, deps: Dictionary) -> Dictionary:
	...
	marshal_phantom_allowed = true
	dark_blade_window = _is_skill_equipped(_get_viper_skill_config(deps), DARK_BLADE)
	...
	for side_pref in preferred_sides:
		for y_off in target_y_offsets:
			...
			for idx in range(candidate_count):
				...
				var velocity: Vector2 = _core_flip_velocity_to_target(speed, safe_dir, ball_pos, width, candidate_x, candidate_y)
				...
				if bounce_y < 40.0 or bounce_y > ball_pos.y - 18.0:
					continue
				if _segment_intersects_rect(Vector2(wall_x, bounce_y), Vector2(candidate_x, candidate_y), boss_avoid_rect):
					continue
				return velocity
	var fallback_velocity: Vector2 = _core_flip_velocity_to_target(speed, safe_dir, ball_pos, width, fallback_target_x, fallback_target_y)
	if fallback_velocity.length() > 0.01:
		return fallback_velocity
	return Vector2(float(safe_dir) * 0.65, -0.76).normalized() * speed


func _core_flip_velocity_to_target(
	speed: float,
	kick_dir: int,
	ball_pos: Vector2,
	width: float,
	...
	return ViperSkillGeometry.core_flip_velocity_to_target(speed, kick_dir, ball_pos, width, target_x, target_y)


func _segment_intersects_rect(start_pos: Vector2, end_pos: Vector2, rect: Rect2) -> bool:
	return ViperSkillGeometry.segment_intersects_rect(start_pos, end_pos, rect)
...
func consume_kick_skill_knockback(ball_pos: Vector2, boss_pos: Vector2, boss_width: float, context: Dictionary, deps: Dictionary = {}) -> Dictionary:
	...
	if ai_state != null and ai_state.has_method("start_paddle_hit_knockback"):
		ai_state.start_paddle_hit_knockback(
			knockback_vel,
			KICK_GUARD_KNOCKBACK_FRAMES,
			KICK_GUARD_KNOCKBACK_DECAY,
			true
		)
	return {
...
func apply_shadow_step_paddle_hit(ball_vel: Vector2, context: Dictionary, deps: Dictionary = {}) -> Dictionary:
	if not shadow_kick_ready or shadow_hit_consumed:
		return {}
	if shadow_step_activation_msec > -99999:
	...
	var hit_size := Vector2(player_size.x + 40.0, player_size.y + 40.0)
	var scene := {
		"ball_pos": _get_ball_pos(context),
		"ball_vel": ball_vel,
...
func draw(
	...
	sample_start = _perf_begin(perf_logger)
	var center: Vector2 = chaos_target + shake_offset
	chaos_spear_effect_renderer.draw_chaos_absorb_pulses(canvas, center, chaos_absorb_pulses, shake_offset)
...
func _has_phantom_kick_chain_skill(skill_config: Object, deps: Dictionary) -> bool:
	return (
		...
		and _is_skill_equipped(skill_config, PHANTOM_KICK)
	)
...
func _record_viper_command_inputs(input_snapshot: Dictionary, now_msec: int, skill_config: Object, deps: Dictionary) -> void:
	...
	var can_record: bool = chaos_state == "idle" and _is_skill_equipped(skill_config, CHAOS_SPEAR)
...
func _push_dual_glitch_command(key_char: String, now_msec: int) -> void:
	var expected: Array[String] = ["a", "d", "a", "d"]
	_expire_dual_glitch_command(now_msec)
	var progress: int = dual_glitch_cmd_buffer.size()
	var expected_key: String = str(expected[progress]) if progress < expected.size() else str(expected[0])
	if key_char == expected_key:
		if progress == 0:
			dual_glitch_cmd_buffer = [{"key": key_char, "time": now_msec}]
		else:
			dual_glitch_cmd_buffer.append({"key": key_char, "time": now_msec})
		return
	dual_glitch_cmd_buffer = [{"key": key_char, "time": now_msec}] if key_char == "a" else []


func _check_dual_glitch_command(now_msec: int) -> bool:
	var expected: Array[String] = ["a", "d", "a", "d"]
	_expire_dual_glitch_command(now_msec)
	if dual_glitch_cmd_buffer.size() < expected.size():
		return false
	var first: Dictionary = dual_glitch_cmd_buffer[0]
	if now_msec - int(first.get("time", 0)) > DUAL_GLITCH_CMD_WINDOW_MSEC:
		dual_glitch_cmd_buffer.clear()
...
func _check_chaos_command(now_msec: int) -> bool:
	...
	if str(first.get("key", "")) != "a" or str(second.get("key", "")) != "w" or str(third.get("key", "")) != "d":
		return false
	var t1: int = int(first.get("time", 0))
	var t2: int = int(second.get("time", 0))
	var t3: int = int(third.get("time", 0))
	if t2 - t1 > CHAOS_CMD_WINDOW_MSEC:
		return false
	if t3 - t2 > CHAOS_CMD_WINDOW_MSEC:
		return false
	if now_msec - t3 > CHAOS_CMD_WINDOW_MSEC:
		return false
...
func _can_start_dual_glitch(
	...
	if _is_round_waiting_for_serve(deps):
		return false
	var skill_config: Object = _get_viper_skill_config(deps)
	if not _is_skill_equipped(skill_config, DUAL_GLITCH):
		return false
	if special_gauge < _get_skill_cost(skill_config, DUAL_GLITCH):
		return false
	return _is_configured_skill_ready(DUAL_GLITCH, deps, now_msec)
...
func _start_dual_glitch(
	...
	var cooldown_seconds := 45.0
	if skill_config != null and skill_config.has_method("get_cooldown_seconds"):
		cooldown_seconds = float(skill_config.get_cooldown_seconds(DUAL_GLITCH))
	if cooldown_seconds <= 0.0:
		cooldown_seconds = 45.0
	_trigger_viper_runtime_cooldown(
		DUAL_GLITCH,
		now_msec,
		skill_config,
		deps,
		_get_four_poisons_additive_cooldown_seconds(DUAL_GLITCH, skill_config, deps, cooldown_seconds)
	)
	var orb_hud_state: Object = deps.get("orb_hud_state", null)
	if orb_hud_state != null and orb_hud_state.has_method("trigger_gauge_spin"):
		orb_hud_state.trigger_gauge_spin(now_msec)
...
func _can_start_chaos_spear(
	...
	if jetpack_state != null and jetpack_state.has_method("is_airborne"):
		if bool(jetpack_state.is_airborne(2.0)):
			return false
	if _is_round_waiting_for_serve(deps):
		return false
	var skill_config: Object = _get_viper_skill_config(deps)
	if not _is_skill_equipped(skill_config, CHAOS_SPEAR):
		return false
	if special_gauge < _get_skill_cost(skill_config, CHAOS_SPEAR):
		return false
	return _is_configured_skill_ready(CHAOS_SPEAR, deps, now_msec)
...
func _start_chaos_spear(
	...
	var skill_state: Object = _get_viper_skill_state(deps)
	if skill_state != null and skill_state.has_method("trigger_configured_cooldown"):
		skill_state.trigger_configured_cooldown(CHAOS_SPEAR, now_msec, skill_config)
	var orb_hud_state: Object = deps.get("orb_hud_state", null)
	if orb_hud_state != null and orb_hud_state.has_method("trigger_gauge_spin"):
		orb_hud_state.trigger_gauge_spin(now_msec)
...
func _advance_chaos_spear_runtime(fps_scale: float, context: Dictionary, deps: Dictionary) -> void:
	...
	match chaos_state:
		"startup":
			...
			var chaos_startup_frames: float = max(1.0, CHAOS_STARTUP_FRAMES * max(0.0, 1.0 - prep_reduction_pct / 100.0))
			if chaos_phase_frames >= chaos_startup_frames:
				_enter_chaos_flying(deps)
		"flying":
			var travel_t: float = clamp(chaos_phase_frames / max(1.0, CHAOS_TRAVEL_FRAMES), 0.0, 1.0)
...
func _enter_chaos_blackhole(context: Dictionary) -> void:
	...
	chaos_phase_frames = 0.0
	chaos_current = chaos_target
	chaos_prev_ball_center = _get_vector2(context.get("ball_pos", Vector2.ZERO), Vector2.ZERO)
	chaos_prev_ball_valid = true
...
func _release_chaos_blackhole(early_hit: bool, context: Dictionary, deps: Dictionary) -> void:
	...
	if not early_hit:
		...
		var current_speed: float = current_vel.length()
		var release_speed: float = max(16.0, 8.0 * 2.0, current_speed * 1.6)
		var angle: float = randf_range(0.0, TAU)
		chaos_release_velocity = Vector2(cos(angle), sin(angle)) * release_speed
...
func _update_dual_glitch_runtime(fps_scale: float, context: Dictionary) -> void:
	...
	for clone_value in dual_glitch_clones:
		if not (clone_value is Dictionary):
			continue
		var clone: Dictionary = clone_value
		if _is_dual_glitch_clone_alive(clone) or _is_dual_glitch_clone_evaporating(clone):
			alive.append(clone)
...
func apply_dual_glitch_clone_ball_hit(context: Dictionary = {}, _deps: Dictionary = {}) -> Dictionary:
	if dual_glitch_state not in ["active", "fade"]:
		return {"hit": false}
...
func _can_hold_ignition_aura(
	...
	if _is_round_waiting_for_serve(deps):
		return false
	if bool(input_snapshot.get("down_pressed", false)) or bool(input_snapshot.get("left_pressed", false)) or bool(input_snapshot.get("right_pressed", false)):
		return false
	...
	if not _is_skill_equipped(skill_config, IGNITION_AURA):
		return false
	var configured_cost: float = _get_skill_cost(skill_config, IGNITION_AURA)
	var cost: float = configured_cost if configured_cost > 0.0 else IGNITION_GAUGE_COST
	if special_gauge < cost:
		return false
...
func _start_ignition_aura(
	...
	if skill_state != null:
		if skill_state.has_method("trigger_configured_cooldown"):
			...
			var cooldown_seconds := 80.0
			if skill_config != null and skill_config.has_method("get_cooldown_seconds"):
				var configured: float = float(skill_config.get_cooldown_seconds(IGNITION_AURA))
				if configured > 0.0:
					cooldown_seconds = configured
...
func _can_hold_dive_strike(
	...
	if dive_active or core_flip_attack_active or marshal_active or blade_motion_active or blade_projectile_active:
		return false
	...
	if not _is_skill_equipped(skill_config, DIVE_STRIKE):
		return false
	var configured_cost: float = _get_skill_cost(skill_config, DIVE_STRIKE)
	var cost: float = configured_cost if configured_cost > 0.0 else DIVE_GAUGE_COST
	if special_gauge < cost:
		return false
...
func _start_dive_strike(
	...
	var cost: float = configured_cost if configured_cost > 0.0 else DIVE_GAUGE_COST
	var next_gauge: float = max(0.0, special_gauge - cost)
	var cooldown_seconds := 70.0
	if skill_config != null and skill_config.has_method("get_cooldown_seconds"):
		cooldown_seconds = float(skill_config.get_cooldown_seconds(DIVE_STRIKE))
	_trigger_viper_runtime_cooldown(
		DIVE_STRIKE,
		now_msec,
		skill_config,
	...
	if orb_hud_state != null and orb_hud_state.has_method("trigger_gauge_spin"):
		orb_hud_state.trigger_gauge_spin(now_msec)
	_interrupt_viper_jetpack_thrust(deps)
...
func _update_dive_strike(
	...
	match dive_phase:
		...
		1:
			...
			_set_viper_jetpack_offset_y(deps, dive_player_pos.y - dive_floor_y)
			var center_x: float = dive_player_pos.x + _get_paddle_size(config).x * 0.5
			var foot_y: float = dive_player_pos.y + _get_paddle_size(config).y
...
func _apply_dual_glitch_clone_dive_ball_motion(scene: Dictionary, context: Dictionary, deps: Dictionary) -> Dictionary:
	if dual_glitch_clone_dive_entries.is_empty():
		return {}
	var ball_pos: Vector2 = _get_vector2(scene.get("ball_pos", context.get("ball_pos", Vector2.ZERO)), Vector2.ZERO)
	for index in range(dual_glitch_clone_dive_entries.size()):
		...
		if not bool(entry.get("activated", false)) or bool(entry.get("ball_boosted", false)):
			continue
		var shock_y: float = float(entry.get("y", dive_shockwave_pos.y))
		var vertical_gap: float = shock_y - ball_pos.y
		if vertical_gap < 0.0 or vertical_gap > DIVE_SHOCKWAVE_HEIGHT:
			continue
...
func _apply_dual_glitch_clone_dive_impact(entry: Dictionary, scene: Dictionary, context: Dictionary, deps: Dictionary) -> Dictionary:
	...
	return _mark_result_released_chaos_hit({
		"ball_vel": next_vel,
		"ball_impact_boost": max(1.0, float(scene.get("ball_impact_boost", 1.0))),
		"player_collision_cooldown": max(DIVE_PLAYER_COLLISION_COOLDOWN, float(scene.get("player_collision_cooldown", 0.0))),
...
func _get_viper_airborne_height(deps: Dictionary, config: Dictionary, player_pos: Vector2) -> float:
	return runtime_action_router.get_viper_airborne_height(deps, config, player_pos)
...
func _try_start_blade_combo_from_motion(
	...
	if not blade_motion_active or blade_motion_phase != 2:
		return {}
	if not blade_dark_mode:
		var combo_skill_config: Object = _get_viper_skill_config(deps)
		var nerve_and_dark_blade_equipped := _is_skill_equipped(combo_skill_config, NERVE_STRIKE) and _is_skill_equipped(combo_skill_config, DARK_BLADE)
		var nerve_strike_combo_window_active := false
		if not nerve_strike_combo_used and not nerve_strike_active:
			var nerve_strike_window_end_frame: float = NERVE_STRIKE_WINDOW_END_FRAMES
			if nerve_and_dark_blade_equipped:
				nerve_strike_window_end_frame = NERVE_STRIKE_DARK_BLADE_SPLIT_FRAMES
			nerve_strike_combo_window_active = blade_motion_total_frames >= NERVE_STRIKE_WINDOW_START_FRAMES and blade_motion_total_frames < nerve_strike_window_end_frame
		if nerve_strike_combo_window_active:
			...
			if nerve_and_dark_blade_equipped and blade_motion_total_frames < NERVE_STRIKE_DARK_BLADE_SPLIT_FRAMES:
				return {}
		var dark_blade_split_combo_window_active := false
		if nerve_and_dark_blade_equipped:
			dark_blade_split_combo_window_active = blade_motion_total_frames >= NERVE_STRIKE_DARK_BLADE_SPLIT_FRAMES and blade_motion_total_frames <= NERVE_STRIKE_WINDOW_END_FRAMES
		var can_start_dark_blade_combo := false
		if (
			(dark_blade_split_combo_window_active or blade_air_combo_window)
			and not _is_control_locked(deps)
			and bool(config.get("ball_active", true))
		):
			if _is_skill_equipped(combo_skill_config, DARK_BLADE):
				can_start_dark_blade_combo = (
					special_gauge >= _get_blade_skill_cost(combo_skill_config, deps, DARK_BLADE)
					and _is_configured_skill_ready(DARK_BLADE, deps)
				)
		if can_start_dark_blade_combo:
			_clear_blade_projectile()
			return _start_blade_motion(player_pos, special_gauge, config, deps, true, now_msec, true, true)
	elif blade_dark_mode and blade_dark_combo_window:
		var can_start_air_blade_combo := false
		if not _is_control_locked(deps) and bool(config.get("ball_active", true)):
			var skill_config: Object = _get_viper_skill_config(deps)
			if _is_skill_equipped(skill_config, BLADE_RUSH):
				can_start_air_blade_combo = special_gauge >= _get_blade_skill_cost(skill_config, deps, BLADE_RUSH)
...
func _can_start_nerve_strike_combo(special_gauge: float, config: Dictionary, deps: Dictionary, now_msec: int) -> bool:
	...
	var cost: float = 90.0 if configured_cost <= 0.0 else configured_cost
	if special_gauge < cost:
		return false
...
func _start_nerve_strike(
	...
	nerve_strike_floor_y = _get_player_floor_y(config)
...
func _update_nerve_strike_return(deps: Dictionary) -> Dictionary:
	...
	_force_viper_jetpack_land(deps)
	_reset_nerve_strike_runtime(false)
...
func _spawn_nerve_strike_slash_feedback(center: Vector2, deps: Dictionary) -> void:
	_trigger_feedback(deps, 0.16, 5.0)
	_spawn_fallback_hit_impact(
		center,
		Vector2(0.0, 24.0),
		deps,
...
func _spawn_dual_glitch_clone_nerve_slashes_for_current_cast(deps: Dictionary, config: Dictionary) -> void:
	...
	for origin_value in origins:
		...
		if side == 0:
			continue
		var delay_mult: float = 1.0 if side < 0 else 2.0
		var start := Vector2(float(origin.get("x", boss_center.x)), float(origin.get("y", boss_center.y)))
...
func _can_start_air_blade(special_gauge: float, config: Dictionary, deps: Dictionary) -> bool:
	if blade_motion_active or blade_projectile_active or marshal_active or core_flip_attack_active or chaos_state == "startup":
		return false
	if dark_blade_window:
		return false
	if not bool(config.get("ball_active", true)):
		return false
	if not _is_viper_airborne(deps):
		return false
	if _is_control_locked(deps):
		return false
	var skill_config: Object = _get_viper_skill_config(deps)
...
func _can_start_dark_blade_from_window(special_gauge: float, config: Dictionary, deps: Dictionary) -> bool:
	...
	if not _is_viper_airborne(deps) and not core_flip_dark_ready and not marshal_dark_ready:
		return false
	if _is_control_locked(deps):
		return false
	var skill_config: Object = _get_viper_skill_config(deps)
...
func _start_blade_motion(
	...
	if orb_hud_state != null and orb_hud_state.has_method("trigger_gauge_spin"):
		orb_hud_state.trigger_gauge_spin(now_msec)
	_trigger_feedback(deps, 0.10 if dark_mode else 0.08, 4.2 if dark_mode else 3.4)
	_interrupt_viper_jetpack_thrust(deps)
...
func _update_blade_motion(
	...
	var movement_result: Dictionary = _apply_blade_horizontal_control(delta, player_pos, float(config.get("player_speed", 0.0)), config, deps)
...
func _launch_blade_projectile(player_pos: Vector2, config: Dictionary, deps: Dictionary) -> void:
	...
	if impact_effects != null and impact_effects.has_method("create_energy_explosion"):
		impact_effects.create_energy_explosion(blade_projectile_pos, 0.72 if blade_dark_mode else 0.58, 0.88 if blade_dark_mode else 0.72)
	_spawn_dual_glitch_clone_blades_for_current_cast(deps, blade_dark_mode, size_mult, range_mult)


func _update_blade_windows(fps_scale: float, context: Dictionary, deps: Dictionary) -> void:
	...
	if dark_blade_window:
		...
		var core_flip_dark_ready := (core_flip_attack_active and core_flip_ball_hit) or core_flip_dark_blade_handoff_frames > 0.0
		var marshal_dark_ready := marshal_active
		if dark_blade_window_frames <= 0.0 or (not _is_viper_airborne(deps) and not bool(context.get("viper_jetpack_airborne", false)) and not core_flip_dark_ready and not marshal_dark_ready):
			dark_blade_window = false
			dark_blade_window_frames = 0.0
			core_flip_dark_blade_handoff_frames = 0.0
...
func _apply_blade_hit(
	...
	if dark_mode and allow_combo:
		var skill_config: Object = _get_viper_skill_config(deps)
		if _is_skill_equipped(skill_config, MARSHAL_KICK):
			if _context_has_enough_gauge(context, _get_marshal_skill_cost(skill_config, MARSHAL_KICK)):
	...
	if not _register_ball_hit_pulse(ball_pos, next_vel, deps, 1.0 if dark_mode else 0.86, pulse_kind):
		...
		_spawn_fallback_hit_impact(
			...
			deps,
			hit_color,
			1.6 if dark_mode else 1.25,
			1.05 if dark_mode else 0.82,
			1.15 if dark_mode else 0.95
		)
	if allow_followup:
		_maybe_spawn_blade_amp_followup_blade(context, deps, dark_mode)
	var result := {
		"ball_vel": next_vel,
...
func _spawn_dual_glitch_clone_blades_for_current_cast(
	...
	if origins.is_empty():
		return
	var width: float = BLADE_BASE_WIDTH * max(0.1, size_mult)
	var travel: float = BLADE_BASE_RANGE * max(0.1, range_mult)
	for origin_value in origins:
		if not (origin_value is Dictionary):
			continue
		var origin: Dictionary = origin_value
...
func _get_blade_skill_cost(skill_config: Object, deps: Dictionary, skill_name: String) -> float:
	return skill_scaling.get_blade_skill_cost(_get_skill_cost(skill_config, skill_name), _get_blade_amp_level(deps), skill_name, BLADE_RUSH, DARK_BLADE)


func _get_blade_amp_level(deps: Dictionary) -> int:
	return _get_runtime_skill_level(deps, "blade_amp")


func _limit_effective_velocity(velocity: Vector2, impact_boost: float, max_effective_speed: float) -> Vector2:
	return ViperSkillGeometry.limit_effective_velocity(velocity, impact_boost, max_effective_speed)
...
func _force_viper_jetpack_land(deps: Dictionary) -> void:
	runtime_action_router.force_viper_jetpack_land(deps)


func _interrupt_viper_jetpack_thrust(deps: Dictionary) -> void:
...
func _can_marshal_kick(special_gauge: float, input_snapshot: Dictionary, deps: Dictionary) -> bool:
	var skill_name: String = _get_marshal_skill_to_fire()
	if skill_name == "":
		return false
	if _is_control_locked(deps):
		return false
	...
	if not _is_skill_equipped(skill_config, skill_name):
		return false
	if special_gauge < _get_marshal_skill_cost(skill_config, skill_name):
		return false
...
func _start_marshal_kick(player_pos: Vector2, special_gauge: float, config: Dictionary, deps: Dictionary) -> Dictionary:
	...
	var skill_config: Object = _get_viper_skill_config(deps)
	var skill_name: String = _get_marshal_skill_to_fire()
	var is_double: bool = skill_name == PHANTOM_KICK
	var next_gauge: float = max(0.0, special_gauge - _get_marshal_skill_cost(skill_config, skill_name))
	var skill_state: Object = _get_viper_skill_state(deps)
	if skill_state != null and skill_state.has_method("trigger_configured_cooldown"):
		skill_state.trigger_configured_cooldown(skill_name, now_msec, skill_config)
	var orb_hud_state: Object = deps.get("orb_hud_state", null)
	if orb_hud_state != null and orb_hud_state.has_method("trigger_gauge_spin"):
		orb_hud_state.trigger_gauge_spin(now_msec)
	_trigger_feedback(deps, 0.12, 4.0)
	_cancel_dash_until_key_release(deps.get("dash_state", null))
	audio_router.play_marshal_backstep_sound(deps)
	...
	marshal_first_hit_pending = false
	marshal_first_hit_delay_frames = 0.0
	marshal_is_double = is_double
	phantom_aura_active = is_double
	phantom_kick_knockback_pending = false
	dmk_freeze_active = false
	dmk_freeze_frames = 0.0
	dmk_text_active = false
	...
	marshal_particles.clear()
	_set_marshal_web_line(_get_player_center(marshal_start_pos, config), _get_player_center(marshal_wall_pos, config))
	return {
		"handled": true,
		"activated": true,
		"skill_name": skill_name,
...
func _start_shadow_step(
	...
	var next_gauge: float = max(0.0, special_gauge - _get_skill_cost(skill_config, SHADOW_STEP))
	var skill_state: Object = _get_viper_skill_state(deps)
	if skill_state != null and skill_state.has_method("trigger_configured_cooldown"):
		skill_state.trigger_configured_cooldown(SHADOW_STEP, now_msec, skill_config)
	var orb_hud_state: Object = deps.get("orb_hud_state", null)
	if orb_hud_state != null and orb_hud_state.has_method("trigger_gauge_spin"):
		orb_hud_state.trigger_gauge_spin(now_msec)
	...
	_trigger_feedback(deps, 0.10, 5.0)
	_spawn_shadow_activation_feedback(origin_center, deps)
	...
	shadow_was_airborne = _is_viper_airborne(deps)
	shadow_marshal_delay_frames = 0.0
	shadow_curve_active = false
...
func _update_shadow_step_visuals(fps_scale: float, context: Dictionary, deps: Dictionary) -> void:
	...
	if shadow_marshal_delay_frames > 0.0:
		...
		if shadow_marshal_delay_frames <= 0.0:
			if core_flip_attack_active:
				...
				if not marshal_active and not core_flip_attack_active:
					var skill_config: Object = _get_viper_skill_config(deps)
					if _is_skill_equipped(skill_config, MARSHAL_KICK):
						if _context_has_enough_gauge(context, _get_marshal_skill_cost(skill_config, MARSHAL_KICK)):
							if _is_configured_skill_ready(MARSHAL_KICK, deps):
...
func _get_ball_rect(scene: Dictionary, context: Dictionary) -> Rect2:
	return ViperSkillGeometry.ball_rect(scene, context)


func _spawn_shadow_activation_feedback(pos: Vector2, deps: Dictionary, scale: float = 1.0) -> void:
	particle_drawer.spawn_shadow_activation_feedback(pos, deps.get("impact_effects", null), scale)
...
func _set_marshal_web_line(from_center: Vector2, to_center: Vector2) -> void:
	marshal_web_lines.clear()
	marshal_web_lines.append({"from": from_center, "to": to_center})


func _is_viper_airborne(deps: Dictionary) -> bool:
	return visibility_query.is_viper_airborne(deps)


func _can_shadow_step(special_gauge: float, input_snapshot: Dictionary, deps: Dictionary) -> bool:
	...
	if abs(float(input_snapshot.get("direction", 0.0))) > 0.01:
		return false
	if shadow_hologram_active or shadow_wave_active or shadow_marshal_delay_frames > 0.0 or marshal_ready or marshal_active or core_flip_attack_active:
		return false
	if _is_control_locked(deps):
		return false
	var skill_config: Object = _get_viper_skill_config(deps)
	...
	if special_gauge < _get_skill_cost(skill_config, SHADOW_STEP):
		return false
	return _is_configured_skill_ready(SHADOW_STEP, deps)
...
func _is_round_waiting_for_serve(deps: Dictionary) -> bool:
	return visibility_query.is_round_waiting_for_serve(deps)


func _should_stop_viper_context_effect(context: Dictionary) -> bool:
	return visibility_query.should_stop_viper_context_effect(context)


func _get_viper_skill_config(deps: Dictionary) -> Object:
	return visibility_query.get_viper_skill_config(deps)


func _get_viper_skill_state(deps: Dictionary) -> Object:
	return visibility_query.get_viper_skill_state(deps)


func _is_skill_equipped(skill_config: Object, skill_name: String) -> bool:
	return visibility_query.is_skill_equipped(skill_config, skill_name)


func _get_skill_cost(skill_config: Object, skill_name: String) -> float:
	return visibility_query.get_skill_cost(skill_config, skill_name)


func _get_dash_snapshot(dash_state: Object) -> Dictionary:
	return visibility_query.get_dash_snapshot(dash_state)


func _is_dash_motion_busy(deps: Dictionary) -> bool:
	return visibility_query.is_dash_motion_busy(deps)
...
func _clamp_player_pos(pos: Vector2, play_left: float, play_right: float, paddle_width: float) -> Vector2:
	return ViperSkillGeometry.clamp_player_pos(pos, play_left, play_right, paddle_width)
...
func _mark_kick_skill_knockback_pending(deps: Dictionary) -> void:
	kick_skill_knockback_pending_pct = 0
	var level: int = _get_kick_enhance_level(deps)
...
func _get_kick_enhance_level(deps: Dictionary) -> int:
	return _get_runtime_skill_level(deps, "kick_enhance")


func _award_skill_gold(deps: Dictionary, gold_award: int) -> Dictionary:
	return runtime_action_router.award_skill_gold(deps, gold_award)


func _trigger_feedback(deps: Dictionary, amount: float, intensity: float) -> void:
	runtime_action_router.trigger_feedback(deps, amount, intensity)


func _get_player_center(player_pos: Vector2, config: Dictionary) -> Vector2:
	return ViperSkillGeometry.player_center(player_pos, config)
...
func _get_ball_pos(config: Dictionary) -> Vector2:
	return ViperSkillGeometry.get_ball_pos(config)
...
func _get_or_create_emp_fx_host(canvas: CanvasItem) -> Node:
	return fx_host_controller.get_or_create_emp_fx_host(self, canvas)
...
func _get_or_create_chaos_fx_host(canvas: CanvasItem) -> Node:
	return fx_host_controller.get_or_create_chaos_fx_host(self, canvas)
...
func _is_valid_fx_host(host: Node) -> bool:
	return fx_host_controller.is_valid_fx_host(host)
