...
const MARSHAL_KICK_DOUBLE_SPEED_MULT := 2.8; const MARSHAL_KICK_DOUBLE_MIN_SPEED := 14.0; const MARSHAL_KICK_DOUBLE_FAST_MULT := 1.3; const MARSHAL_KICK_SHADOW_CHAIN_PREP_MULT := 0.8
...
const BLADE_COMBO_DELAY_FRAMES := 30.0; const BLADE_DASH_RELEASE_DELAY_FRAMES := 18.0; const DARK_BLADE_WINDOW_FRAMES := 180.0; const DARK_BLADE_DEFAULT_BALL_SIZE := 28.6; const BLADE_HIT_GOLD := 30
...
var shadow_hit_consumed := false; var shadow_was_airborne := false; var shadow_marshal_delay_frames := 0.0; var shadow_marshal_delay_from_shadow_step := false
...
var marshal_ready := false; var marshal_ready_frames := 0.0; var marshal_phantom_allowed := false; var marshal_ready_from_shadow_step_chain := false
...
var marshal_is_double := false; var marshal_from_shadow_step_chain := false; var phantom_kick_knockback_pending := false; var phantom_kick_speed_limit_disabled := false; var kick_skill_knockback_pending_pct := 0
...
func open_marshal_kick_window(from_shadow_step_chain: bool = false) -> void:
	if marshal_active:
		return
	marshal_ready = true; marshal_ready_frames = MARSHAL_KICK_READY_FRAMES; marshal_phantom_allowed = true; marshal_ready_from_shadow_step_chain = from_shadow_step_chain
	_clear_double_marshal_ready_window()
func _clear_marshal_ready_window() -> void: marshal_ready = false; marshal_ready_frames = 0.0; marshal_ready_from_shadow_step_chain = false
...
func _clear_phantom_kick_chain_window() -> void: marshal_phantom_allowed = false; marshal_from_shadow_step_chain = false; _clear_double_marshal_ready_window(); _clear_marshal_first_hit_pending()
...
func _start_chaos_spear(player_pos: Vector2, special_gauge: float, config: Dictionary, deps: Dictionary, now_msec: int) -> Dictionary: return ViperSkillChaosSpearRuntime.start(self, player_pos, special_gauge, config, deps, now_msec, {"skill_name": CHAOS_SPEAR})
...
func _get_marshal_prep_duration_frames(base_frames: float, deps: Dictionary, double_fast: bool = false) -> float: return ViperSkillMarshalKickRuntime.get_prep_duration_frames(self, base_frames, deps, double_fast, _get_marshal_kick_runtime_constants())
func _get_marshal_prep_duration_mult(deps: Dictionary) -> float: return ViperSkillMarshalKickRuntime.get_prep_duration_mult(self, deps, _get_marshal_kick_runtime_constants())
