...
# Dark Blade post-fire (phase 2) lateral steering is reduced 50%: the airborne jump
# (BLADE_DARK_JUMP_PEAK 320 > jetpack max 200) stacks the airborne bonus (x3.15) with the
# dark x3.0 multiplier, which made left/right movement read ~9x base right after the slash fires.
const BLADE_DARK_POST_FIRE_LATERAL_SCALE := 0.5
const DEBUG_EMP := false
const DIVE_HOLD_REQUIRED_MSEC := 300; const DIVE_PREP_FRAMES := 24.0; const DIVE_SPEED := 15.0; const DIVE_GAUGE_COST := 250.0; const DIVE_JETPACK_MAX_HEIGHT := 200.0
...
const DUAL_GLITCH_CMD_WINDOW_MSEC := 1200; const DUAL_GLITCH_MAX_KEY_GAP_MSEC := 260; const DUAL_GLITCH_STARTUP_FRAMES := 48.0; const DUAL_GLITCH_SPAWN_FRAMES := 22.8; const DUAL_GLITCH_ACTIVE_FRAMES := 900.0
...
# Air Blade follow-up windows after the projectile fires (W press time): Air Blade W+0.6.
# Sequential chain (2026-06-26): when BOTH Venom Edge + Dark Blade are equipped,
# Air Blade W resolves to Dark Blade across the whole window; Venom Edge is reached
# from Dark Blade's own phase-2 follow-up window after the combo delay.
# Window ownership is resolved in ViperSkillBladeMotionRuntime.try_phase2_followup_activation.
