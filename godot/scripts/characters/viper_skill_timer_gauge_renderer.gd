...
func draw_ignition_aura_timer_gauge(
	...
	if font != null:
		...
		var seconds_text: String = "%.1fs" % remaining_seconds if LanguageSettings.get_language() == LanguageSettings.LANGUAGE_ENGLISH else ("%.1f秒" % remaining_seconds if LanguageSettings.get_language() == LanguageSettings.LANGUAGE_CHINESE or LanguageSettings.get_language() == LanguageSettings.LANGUAGE_JAPANESE else "%.1f초" % remaining_seconds)
