...
func draw(
	...
	var jitter: Vector2 = Vector2(randf_range(-shake_intensity, shake_intensity), randf_range(-shake_intensity, shake_intensity))
...
func _draw_starburst_body(
	...
	var burst_points: int = min(28, 12 + combo * 2)
	...
	var burst_polygon: PackedVector2Array = _build_starburst(draw_center, burst_inner, burst_outer, burst_points, burst_rotation)
	canvas.draw_colored_polygon(
		burst_polygon,
		Color(base_color.r, base_color.g, base_color.b, (0.18 + min(0.10, float(combo) * 0.012)) * alpha)
	)
	_draw_closed_polyline(canvas, burst_polygon, Color(0.0, 0.0, 0.0, 0.40 * alpha), max(2.0, 2.0 + floor(float(combo) * 0.22)))
	_draw_closed_polyline(canvas, burst_polygon, Color(1.0, 1.0, 0.86, 0.62 * alpha), max(1.0, 1.0 + floor(float(combo) * 0.12)))
	if combo < 5:
		return
	...
	_draw_closed_polyline(canvas, inner_burst, Color(1.0, 0.95, 0.66, 0.42 * alpha), 1.5)

func _build_starburst(center: Vector2, inner_radius: float, outer_radius: float, point_count: int, rotation: float) -> PackedVector2Array:
...
func _draw_closed_polyline(canvas: CanvasItem, points: PackedVector2Array, color: Color, width: float) -> void:
	...
	canvas.draw_polyline(closed_points, color, width)
