...
ROUND_WIN = "round_win"    # 라운드(매치) 승리 시
ROUND_LOSE = "round_lose"  # 라운드(매치) 패배 시
...
class HeroDialogueManager:
    ...
    # ── 라운드 종료 대사 (30% 확률) ──
    ROUND_END_CHANCE = 0.3  # 30% 확률

    def get_round_end_line(self, hero_id: str, won: bool):
        """라운드 승/패 시 대사 반환 (30% 확률, None이면 대사 없음)"""
        if random.random() > self.ROUND_END_CHANCE:
            return None
        situation = ROUND_WIN if won else ROUND_LOSE
        lines = HERO_DIALOGUES.get(hero_id, {}).get(situation, [])
        if not lines:
            return None
        idx = random.randint(0, len(lines) - 1)
