...
class HeroPortraitRenderer:
    ...
    def _portrait_mugen(self, surf, w, h, color):
        ...
        band_hi = (255, 255, 255)
        aura = (120, 50, 180)
        # Background - layered purple aura
        self._soft_glow(surf, cx, int(h * 0.4), max(5, int(w * 0.4)), aura, 18)
        self._soft_glow(surf, cx + int(w * 0.1), int(h * 0.3), max(4, int(w * 0.25)), (160, 80, 220), 12)
        # Hair back mass
        ...
        # Nose - bridge + tip
        nose_y = int(h * 0.55)
        pygame.draw.line(surf, skin_sh, (cx, int(h * 0.47)), (cx - 1, nose_y), 1)
        pygame.draw.circle(surf, skin_sh, (cx, nose_y), max(1, b // 8))
        pygame.draw.circle(surf, (*skin_hi, 80), (cx, int(h * 0.50)), max(1, b // 10))
        # Mouth - thin serious
        mouth_y = int(h * 0.65)
        pygame.draw.line(surf, (180, 130, 120), (cx - int(w * 0.07), mouth_y),
    ...
    def _portrait_kraken(self, surf, w, h, color):
        ...
        eye_glow = (160, 220, 60)
        # Background - deep sea layered
        pygame.draw.rect(surf, (8, 15, 35, 140), (0, 0, w, h))
        self._soft_glow(surf, cx, int(h * 0.35), max(5, int(w * 0.35)), (20, 50, 100), 15)
        # Underwater caustic hints
        for i in range(3):
            cx2 = cx + int(_sin(i * 2.1) * w * 0.2)
            cy2 = int(h * 0.2 + i * h * 0.2)
            pygame.draw.ellipse(surf, (20, 60, 120, 8),
                                (cx2 - int(w * 0.15), cy2, int(w * 0.3), int(h * 0.08)))
        ...
        # Bioluminescent dots - layered glow
        import random
        ...
        for s in [-1, 1]:
            ...
            eh = max(5, int(h * 0.16))
            # Eye glow aura
            self._soft_glow(surf, ex, eye_y, max(4, int(ew * 0.8)), eye_glow, 20)
            # Eye socket shadow
        ...
        # Mouth slit with depth
        mouth_y = int(h * 0.58)
        pygame.draw.arc(surf, body_deep, (cx - int(w * 0.09), mouth_y, int(w * 0.18), int(h * 0.07)), 3.14, 6.28, 1)
        pygame.draw.arc(surf, body_dk, (cx - int(w * 0.07), mouth_y + 1, int(w * 0.14), int(h * 0.05)), 3.14, 6.28, 1)
        # Rim light on dome
    ...
    def _portrait_chronos(self, surf, w, h, color):
        ...
        for i in range(6):
            ...
            self._soft_glow(surf, ax, ay, max(3, int(b * 0.35 + i * b * 0.08)), aura, 12)
        ...
        nose_y = int(h * 0.54)
        pygame.draw.line(surf, skin_sh, (cx, int(h * 0.47)), (cx - 1, nose_y), 1)
        pygame.draw.circle(surf, (*skin_hi, 60), (cx, int(h * 0.50)), max(1, b // 10))
        # Mouth - mysterious slight smile with lip detail
        mouth_y = int(h * 0.64)
        pygame.draw.arc(surf, lip, (cx - int(w * 0.06), mouth_y - int(h * 0.02),
                                    int(w * 0.12), int(h * 0.05)), 3.3, 6.1, 1)
        # Upper lip line
        pygame.draw.line(surf, self._darken(lip, 20),
                         (cx - int(w * 0.04), mouth_y), (cx + int(w * 0.04), mouth_y), 1)
        # Chin shadow
        ...
        # Mystic rune hints on hood edge
        for i in range(3):
            rx = cx + int((i - 1) * w * 0.18)
            ry = int(h * 0.82)
            pygame.draw.circle(surf, (*aura, 35), (rx, ry), max(1, int(b * 0.05)))
    ...
    def _portrait_onimaru(self, surf, w, h, color):
        ...
        self._soft_glow(surf, cx, int(h * 0.5), max(6, int(w * 0.4)), (200, 60, 20), 20)
        self._soft_glow(surf, cx - int(w * 0.1), int(h * 0.3), max(4, int(w * 0.2)), (250, 100, 30), 10)
        ...
        # Facial markings - dark tribal lines
        for s in [-1, 1]:
            mx = cx + s * int(w * 0.16)
        ...
        pygame.draw.line(surf, skin_dk, (cx + 1, int(h * 0.47)), (cx + 2, nose_y), 1)
        for s in [-1, 1]:
            pygame.draw.circle(surf, skin_deep, (cx + s * max(1, int(w * 0.02)), nose_y), max(1, b // 10))
        # Mouth with fangs - wider, more menacing
        mouth_y = int(h * 0.65)
        mw = int(w * 0.16)
        # Mouth opening
        pygame.draw.line(surf, (100, 20, 15), (cx - mw, mouth_y), (cx + mw, mouth_y), max(1, int(b * 0.09)))
        # Mouth corners turned up (snarl)
        for s in [-1, 1]:
            pygame.draw.line(surf, (100, 20, 15), (cx + s * mw, mouth_y),
                             (cx + s * int(mw * 1.1), mouth_y - 2), 1)
        # Upper fangs - larger with highlights
        for s in [-1, 1]:
            fx = cx + s * int(mw * 0.55)
            fh = int(h * 0.07)
    ...
    def _portrait_maria(self, surf, w, h, color):
        ...
        # Soft pink background with sparkle atmosphere
        pygame.draw.rect(surf, (55, 28, 48, 60), (0, 0, w, h))
        self._soft_glow(surf, cx, int(h * 0.4), max(5, int(w * 0.35)), (200, 100, 160), 12)
        ...
        pygame.draw.circle(surf, ribbon, (rib_cx, rib_y + int(h * 0.02)), max(1, int(b * 0.05)))
        ...
        pygame.draw.circle(surf, (*skin_hi, 80), (cx, int(h * 0.535)), max(1, int(b * 0.03)))
        # Sweet lips - upper and lower
        mouth_y = int(h * 0.63)
        pygame.draw.ellipse(surf, lip, (cx - int(w * 0.04), mouth_y, int(w * 0.08), int(h * 0.035)))
        pygame.draw.ellipse(surf, lip_hi, (cx - int(w * 0.02), mouth_y, int(w * 0.04), int(h * 0.018)))
        # Lip line
        pygame.draw.line(surf, self._darken(lip, 20), (cx - int(w * 0.03), mouth_y + int(h * 0.015)),
                         (cx + int(w * 0.03), mouth_y + int(h * 0.015)), 1)
