...
class SpiderBossSprite:
    ...
    def _draw_character(self, surface, cx, cy, b, w, h):
        ...
        # 레이어 순서: 다리(뒤) → 복부 → 두흉부 → 눈 → 독아 → 체모 → 다리(앞)
        ...
        self._draw_hair(surface, gcx, gcy, b, p, squash_y)
    ...
    def _get_palette(self, flash=0.0):
        ...
        return {
            # 몸체
            "body_dark": _f((45, 30, 20)),
            "body": _f((70, 45, 30)),
            "body_light": _f((95, 65, 45)),
            "body_edge": _f((35, 22, 14)),
            # 복부
            "abdomen": _f((55, 35, 25)),
            "abdomen_dark": _f((35, 20, 15)),
            "abdomen_light": _f((80, 55, 40)),
            "abdomen_marking": _f((180, 60, 30)),
            "abdomen_marking2": _f((200, 80, 20)),
            # 눈
            "eye_main": _f((20, 20, 20)),
            "eye_secondary": _f((40, 10, 10)),
            "eye_shine": _f((255, 255, 255)),
            "eye_glow": _f((60, 180, 60)),
            # 독아
            "fang": _f((200, 180, 140)),
            "fang_tip": _f((120, 20, 20)),
            "fang_dark": _f((140, 120, 90)),
            # 다리
            "leg": _f((60, 40, 25)),
            "leg_light": _f((80, 58, 38)),
            "leg_dark": _f((40, 26, 16)),
            "leg_joint": _f((85, 60, 40)),
            # 체모
            "hair": _f((50, 35, 22)),
        }
    ...
    def _draw_abdomen(self, surface, cx, cy, b, p, squash_y, stretch_x):
        # 복부: 두흉부 뒤쪽 (아래쪽)에 큰 타원
        abd_cx = cx
        abd_cy = int(cy + 1.2 * b * squash_y)
        abd_w = int(3.2 * b * stretch_x)
        abd_h = int(2.8 * b * squash_y)
        ...
        pygame.draw.ellipse(hl_surf, (*p["abdomen_light"], 80),
                           (0, 0, hl_w, hl_h))
        surface.blit(hl_surf,
                    (abd_cx - hl_w // 2 - int(0.3 * b),
                     abd_cy - hl_h // 2 - int(0.4 * b)))

        # 빨간/주황 무늬 (모래시계 형태)
        mark_y = abd_cy - int(0.1 * b)
        mark_size = int(0.5 * b)
        # 위 삼각형
        pts_top = [
            (abd_cx - mark_size // 2, mark_y - mark_size),
            (abd_cx + mark_size // 2, mark_y - mark_size),
            (abd_cx, mark_y),
        ]
        pygame.draw.polygon(surface, p["abdomen_marking"], pts_top)
        # 아래 삼각형
        pts_bot = [
            (abd_cx - mark_size // 2, mark_y + mark_size),
            (abd_cx + mark_size // 2, mark_y + mark_size),
        ...
        pygame.draw.polygon(surface, p["abdomen_marking2"], pts_bot)

        # 복부 테두리
        pygame.draw.ellipse(surface, p["body_edge"],
                           (abd_cx - abd_w // 2,
                            abd_cy - abd_h // 2,
                            abd_w, abd_h), max(1, int(0.06 * b)))

    # ------- 두흉부 (cephalothorax) — 앞쪽 작은 몸체 -------
    def _draw_cephalothorax(self, surface, cx, cy, b, p, squash_y, stretch_x):
        ceph_cx = cx
        ceph_cy = int(cy - 0.8 * b * squash_y)
        ceph_w = int(2.0 * b * stretch_x)
        ceph_h = int(1.6 * b * squash_y)
    ...
    def _draw_eyes(self, surface, cx, cy, b, p, squash_y):
        ...
        # 큰 주눈 2개 (중앙 앞쪽)
        for side in [-1, 1]:
            ex = cx + side * int(0.32 * b)
            ey = ceph_cy - int(0.25 * b * squash_y)
            er = max(3, int(0.20 * b))
            # 눈 본체 (검정)
        ...
        for sx, sy in secondary_positions:
            ex = cx + int(sx * b)
            ey = ceph_cy + int(sy * b * squash_y) - int(0.15 * b)
            er = max(2, int(0.10 * b))
    ...
    def _draw_chelicerae(self, surface, cx, cy, b, p, squash_y):
        ceph_cy = int(cy - 0.8 * b * squash_y)
        fang_base_y = ceph_cy + int(0.45 * b * squash_y)
        ...
        if self.is_hit:
            prog = self.hit_timer / self.hit_duration
            if prog < 0.25:
                spread = 0.15 * b * (prog / 0.25)
                fang_flash = int(200 * (1.0 - prog / 0.25))
            else:
                spread = 0.15 * b * max(0, 1.0 - (prog - 0.25) / 0.3)

        for side in [-1, 1]:
            # 기저부 (두꺼운 부분)
            base_x = cx + side * int(0.20 * b + spread)
            base_y = fang_base_y
            # 끝 (뾰족한 부분, 아래로 + 바깥으로 곡선)
            tip_x = base_x + side * int(0.15 * b)
            tip_y = base_y + int(0.55 * b)
            # 중간 곡선점
            mid_x = base_x + side * int(0.22 * b)
            mid_y = base_y + int(0.30 * b)

            fang_w = max(2, int(0.12 * b))
            # 그림자
            ...
            pygame.draw.circle(surface, p["fang_tip"],
                              (tip_x, tip_y), max(2, int(0.06 * b)))

            # 히트 시 플래시
            if fang_flash > 0:
                fs = pygame.Surface((fang_w * 4, int(0.6 * b)), pygame.SRCALPHA)
                pygame.draw.line(fs, (255, 255, 200, fang_flash),
                                (fs.get_width() // 2, 0),
    ...
    def _draw_single_leg(self, surface, cx, cy, b, p, squash_y,
                          leg_idx, side, pair_idx):
        """단일 다리 렌더링 (3마디: coxa→femur→tibia)"""
        ...
        # 다리 시작점 (두흉부 측면)
        ceph_cy = int(cy - 0.8 * b * squash_y)
        pair_y_offset = (pair_idx - 1.5) * 0.3  # 쌍별 Y 오프셋
        hip_x = cx + side * int(0.85 * b)
        hip_y = int(ceph_cy + pair_y_offset * b * squash_y)

        # 다리 길이 (앞쪽이 더 길고 뒤로 갈수록 짧아짐 → 1쌍 제일 길다)
        leg_length_mult = [1.15, 1.15, 1.0, 1.0, 0.95, 0.95, 0.85, 0.85]
        leg_len = 2.0 * b * leg_length_mult[leg_idx]

        # Coxa (1마디 — 짧고 두꺼운)
        coxa_len = leg_len * 0.20
        ...
        femur_len = leg_len * 0.40
        femur_angle = angle + side * 0.5
        # 위로 올라갔다가 옆으로 뻗음 (거미 특유의 높은 무릎)
        knee_x = coxa_ex + int(_sin(femur_angle) * femur_len) * side
        knee_y = coxa_ey - int(abs(_cos(femur_angle)) * femur_len * 0.7 * squash_y)
        # 걷기 시 무릎 높이 변화
        if self.direction != 0:
            lift = _sin(self.step_phase + phase_offset) * 0.3 * b
            if lift > 0:
                knee_y -= int(lift)
        ...
        foot_x = knee_x + int(_sin(femur_angle * 0.5) * tibia_len * 0.4) * side
        foot_y = int(cy + 2.2 * b * squash_y)  # 바닥 근처
        # 걷기 시 발 위치 변화
        if self.direction != 0:
        ...
        pygame.draw.circle(surface, p["leg_dark"],
                          (foot_x, foot_y), max(1, int(0.04 * b)))

    # ------- 체모 (hair) -------
    def _draw_hair(self, surface, cx, cy, b, p, squash_y):
        """몸체 위에 짧은 체모를 그림"""
        ceph_cy = int(cy - 0.8 * b * squash_y)
        abd_cy = int(cy + 1.2 * b * squash_y)
        ...
        hair_positions = [
            # 두흉부 체모
            (cx - int(0.4 * b), ceph_cy - int(0.2 * b), 0.15, -0.5),
            (cx + int(0.4 * b), ceph_cy - int(0.1 * b), 0.15, -0.6),
            (cx - int(0.6 * b), ceph_cy + int(0.1 * b), 0.12, -0.3),
            (cx + int(0.6 * b), ceph_cy + int(0.15 * b), 0.12, -0.4),
            # 복부 체모
            (cx - int(0.8 * b), abd_cy - int(0.3 * b), 0.18, -0.4),
            (cx + int(0.8 * b), abd_cy - int(0.2 * b), 0.18, -0.5),
            (cx - int(0.3 * b), abd_cy - int(0.8 * b), 0.15, -0.7),
            (cx + int(0.3 * b), abd_cy - int(0.7 * b), 0.15, -0.6),
            (cx, abd_cy + int(0.5 * b), 0.14, 0.5),
            (cx - int(0.5 * b), abd_cy + int(0.4 * b), 0.13, 0.3),
            (cx + int(0.5 * b), abd_cy + int(0.3 * b), 0.13, 0.4),
        ]

        for hx, hy, length, angle in hair_positions:
            h_len = int(length * b)
            h_ex = hx + int(_sin(angle + _sin(self.time * 3 + hx * 0.1) * 0.1) * h_len)
